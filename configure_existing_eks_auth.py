...
def find_latest_file(pattern: str) -> str:
    ...
    for file_path in files:
        ...
        if match:
            ...
            try:
                ...
                print(
                    f"   📄 {file_path} - {formatted_time} UTC ({file_size_kb:.1f} KB)"
                )
    ...
    if not file_timestamps:
        print_colored(
            Colors.RED, f"❌ No valid files found with proper timestamp format!"
        )
        return None
...
def load_cluster_data():
    ...
    try:
        ...
        print_colored(
            Colors.GREEN, f"✅ Loaded {len(clusters)} clusters from {clusters_file}"
        )
...
def load_admin_credentials():
    ...
    if not os.path.exists(admin_config_file):
        print_colored(
            Colors.RED, f"❌ Admin config file {admin_config_file} not found!"
        )
        return None
    ...
    try:
        ...
        print_colored(
            Colors.GREEN, f"✅ Loaded admin credentials for {len(accounts)} accounts"
        )
...
def load_user_credentials():
    ...
    try:
        ...
        print_colored(
            Colors.GREEN,
            f"✅ Loaded user credentials for {total_users} users from {user_creds_file}",
        )
...
def display_clusters_menu(cluster_data):
    ...
    for i, cluster in enumerate(clusters, 1):
        ...
        print(
            f"      📊 Nodes: {cluster['default_nodes']} (max: {cluster['max_nodes']})"
        )
...
def get_user_credentials(user_creds, username, account_key):
    ...
    if account_key not in accounts:
        print_colored(
            Colors.RED, f"❌ Account {account_key} not found in user credentials"
        )
        return None
...
def fix_cluster_auth(cluster, admin_config, user_creds):
    ...
    if account_key not in admin_config.get("accounts", {}):
        print_colored(
            Colors.RED, f"❌ Admin credentials not found for account: {account_key}"
        )
        return False
    ...
    if not admin_access_key or not admin_secret_key:
        print_colored(
            Colors.RED, f"❌ Invalid admin credentials for account: {account_key}"
        )
        return False
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print_colored(
                Colors.RED, f"❌ Failed to update kubeconfig: {result.stderr}"
            )
            return False
        ...
        if result.returncode == 0:
            ...
            verify_result = subprocess.run(
                verify_cmd, env=env, capture_output=True, text=True, timeout=120
            )
            ...
            # Test user access
            success = test_user_access(
                cluster_name, region, username, user_access_key, user_secret_key
            )
            return success
...
def test_user_access(
    ...
    try:
        ...
        result = subprocess.run(
            update_cmd, env=user_env, capture_output=True, text=True, timeout=120
        )
        if result.returncode == 0:
            ...
            print_colored(
                Colors.RED,
                f"❌ Failed to update kubeconfig with user creds: {result.stderr}",
            )
            return False
        ...
        nodes_result = subprocess.run(
            nodes_cmd, env=user_env, capture_output=True, text=True, timeout=60
        )
        ...
        if nodes_result.returncode == 0:
            ...
            for i, node_line in enumerate(node_lines, 1):
                ...
                if len(node_parts) >= 2:
                    ...
                    print_colored(
                        Colors.CYAN, f"      {i}. {node_name} ({node_status})"
                    )
        else:
            print_colored(
                Colors.RED, f"   ❌ kubectl get nodes failed: {nodes_result.stderr}"
            )
            return False

        # Test kubectl get pods
        print_colored(
            Colors.CYAN, "   🔍 Testing 'kubectl get pods --all-namespaces'..."
        )
        pods_cmd = ["kubectl", "get", "pods", "--all-namespaces", "--no-headers"]
        pods_result = subprocess.run(
            pods_cmd, env=user_env, capture_output=True, text=True, timeout=60
        )
        ...
        if pods_result.returncode == 0:
            ...
            print_colored(
                Colors.GREEN, f"   ✅ Found {pod_count} pod(s) across all namespaces"
            )
            ...
            print_colored(
                Colors.RED, f"   ❌ kubectl get pods failed: {pods_result.stderr}"
            )
            return False
        ...
        info_result = subprocess.run(
            info_cmd, env=user_env, capture_output=True, text=True, timeout=60
        )
        ...
        if info_result.returncode == 0:
            ...
            print_colored(
                Colors.YELLOW, f"   ⚠️  kubectl cluster-info failed (non-critical)"
            )

        print_colored(
            Colors.GREEN, f"🎉 User access verification successful for {username}!"
        )
        return True
...
def main():
    """Main execution flow"""
    print_colored(
        Colors.GREEN, "🔧 EKS Cluster Authentication Fix Tool - Dynamic Version"
    )
    print("=" * 70)
    from datetime import datetime
    ...
    for i, cluster in enumerate(selected_clusters, 1):
        ...
        if success:
            ...
            print_colored(
                Colors.GREEN,
                f"✅ Successfully fixed authentication for {cluster['cluster_name']}",
            )
        else:
            failed_fixes += 1
            print_colored(
                Colors.RED,
                f"❌ Failed to fix authentication for {cluster['cluster_name']}",
            )
