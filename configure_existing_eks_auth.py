...
def find_latest_file(pattern: str) -> str:
    ...
    print_colored(Colors.BLUE, f"{Symbols.SCAN} Found {len(files)} files matching {pattern}:")
    ...
    for file_path in files:
        ...
        if match:
            ...
            print(f"   {Symbols.WARN}  No timestamp found in: {file_path}")
    
    if not file_timestamps:
        print_colored(Colors.RED, f"{Symbols.ERROR} No valid files found with proper timestamp format!")
        return None
    ...
    print_colored(Colors.GREEN, f"{Symbols.OK} Selected latest file: {latest_file}")
...
def load_cluster_data():
    ...
    try:
        ...
        print_colored(Colors.GREEN, f"{Symbols.OK} Loaded {len(clusters)} clusters from {clusters_file}")
        print(f"   {Symbols.DATE} Created on: {metadata.get('created_on', 'Unknown')}")
        print(f"   👤 Created by: {metadata.get('created_by', 'Unknown')}")
        
        return cluster_data
        
    except Exception as e:
        print_colored(Colors.RED, f"{Symbols.ERROR} Failed to load cluster data: {str(e)}")
        return None
...
def load_admin_credentials():
    ...
    if not os.path.exists(admin_config_file):
        print_colored(Colors.RED, f"{Symbols.ERROR} Admin config file {admin_config_file} not found!")
        return None
    ...
    try:
        ...
        print_colored(Colors.GREEN, f"{Symbols.OK} Loaded admin credentials for {len(accounts)} accounts")
        
        return admin_config
        
    except Exception as e:
        print_colored(Colors.RED, f"{Symbols.ERROR} Failed to load admin credentials: {str(e)}")
        return None
...
def load_user_credentials():
    ...
    try:
        ...
        print_colored(Colors.GREEN, f"{Symbols.OK} Loaded user credentials for {total_users} users from {user_creds_file}")
        
        return user_creds
        
    except Exception as e:
        print_colored(Colors.RED, f"{Symbols.ERROR} Failed to load user credentials: {str(e)}")
        return None
...
def display_clusters_menu(cluster_data):
    ...
    print_colored(Colors.BLUE, f"\n[BUILD]  Available EKS Clusters ({len(clusters)} total):")
    print("=" * 80)
    
    for i, cluster in enumerate(clusters, 1):
        auth_status = "[OK]" if cluster.get('auth_configured', False) else f"{Symbols.ERROR}"
        verify_status = f"{Symbols.OK}" if cluster.get('access_verified', False) else f"{Symbols.ERROR}"
        ...
        print(f"      {Symbols.REGION} Region: {cluster['region']}")
        print(f"      [COMPUTE] Instance: {cluster['instance_type']}")
        print(f"      {Symbols.STATS} Nodes: {cluster['default_nodes']} (max: {cluster['max_nodes']})")
    ...
    while True:
        ...
        try:
            ...
            if 1 <= cluster_num <= len(clusters):
                ...
                print(f"{Symbols.ERROR} Please enter a number between 1 and {len(clusters)}")
        except ValueError:
...
def get_user_credentials(user_creds, username, account_key):
    ...
    if account_key not in accounts:
        print_colored(Colors.RED, f"{Symbols.ERROR} Account {account_key} not found in user credentials")
        return None
    ...
    print_colored(Colors.RED, f"{Symbols.ERROR} User {username} not found in account {account_key}")
    return None
...
def fix_cluster_auth(cluster, admin_config, user_creds):
    ...
    print(f"{Symbols.ACCOUNT} Account: {account_key} ({account_id})")
    
    # Get admin credentials
    if account_key not in admin_config.get('accounts', {}):
        print_colored(Colors.RED, f"{Symbols.ERROR} Admin credentials not found for account: {account_key}")
        return False
    ...
    if not admin_access_key or not admin_secret_key:
        print_colored(Colors.RED, f"{Symbols.ERROR} Invalid admin credentials for account: {account_key}")
        return False
    
    print_colored(Colors.GREEN, f"{Symbols.OK} Retrieved admin credentials for {account_key}")
    ...
    if not user_access_key or not user_secret_key:
        print_colored(Colors.RED, f"{Symbols.ERROR} Invalid user credentials for {username}")
        return False
    
    print_colored(Colors.GREEN, f"{Symbols.OK} Retrieved user credentials for {username}")
    ...
    try:
        ...
        print_colored(Colors.GREEN, f"{Symbols.OK} Created ConfigMap file: {configmap_file}")
        ...
        if result.returncode == 0:
            print_colored(Colors.GREEN, f"{Symbols.OK} Updated kubeconfig with admin credentials")
        else:
            print_colored(Colors.RED, f"{Symbols.ERROR} Failed to update kubeconfig: {result.stderr}")
            return False
        ...
        if result.returncode == 0:
            print_colored(Colors.GREEN, "{Symbols.OK} Successfully applied aws-auth ConfigMap!")
            print("{Symbols.LIST} ConfigMap output:")
            print(result.stdout)
            ...
            if verify_result.returncode == 0:
                print_colored(Colors.CYAN, f"\n{Symbols.STATS} ConfigMap verification successful")
            ...
            print_colored(Colors.RED, f"{Symbols.ERROR} Failed to apply ConfigMap: {result.stderr}")
            return False
        ...
        print_colored(Colors.RED, f"{Symbols.ERROR} Error: {str(e)}")
        return False
        ...
        try:
            if os.path.exists(configmap_file):
                ...
                print_colored(Colors.CYAN, f"{Symbols.CLEANUP} Cleaned up {configmap_file}")
        except:
            pass
...
def test_user_access(cluster_name: str, region: str, username: str, user_access_key: str, user_secret_key: str):
    ...
    try:
        ...
        if result.returncode == 0:
            print_colored(Colors.GREEN, f"{Symbols.OK} Updated kubeconfig with user credentials")
        else:
            print_colored(Colors.RED, f"{Symbols.ERROR} Failed to update kubeconfig with user creds: {result.stderr}")
            return False
        ...
        if nodes_result.returncode == 0:
            ...
            print_colored(Colors.GREEN, f"   {Symbols.OK} Found {node_count} node(s)")
            ...
            print_colored(Colors.RED, f"   {Symbols.ERROR} kubectl get nodes failed: {nodes_result.stderr}")
            return False
        ...
        if pods_result.returncode == 0:
            ...
            print_colored(Colors.GREEN, f"   {Symbols.OK} Found {pod_count} pod(s) across all namespaces")
            ...
            print_colored(Colors.RED, f"   {Symbols.ERROR} kubectl get pods failed: {pods_result.stderr}")
            return False
        ...
        if info_result.returncode == 0:
            print_colored(Colors.GREEN, f"   {Symbols.OK} Cluster info retrieved successfully")
        else:
            print_colored(Colors.YELLOW, f"   {Symbols.WARN}  kubectl cluster-info failed (non-critical)")
        ...
        print_colored(Colors.RED, f"{Symbols.ERROR} Error testing user access: {str(e)}")
        return False
...
def main():
    ...
    print_colored(Colors.BLUE, f"\n{Symbols.START} Processing {len(selected_clusters)} cluster(s)...")
    ...
    for i, cluster in enumerate(selected_clusters, 1):
        ...
        if success:
            successful_fixes += 1
            print_colored(Colors.GREEN, f"{Symbols.OK} Successfully fixed authentication for {cluster['cluster_name']}")
        else:
            failed_fixes += 1
            print_colored(Colors.RED, f"{Symbols.ERROR} Failed to fix authentication for {cluster['cluster_name']}")
    
    # Summary
    print("\n" + "=" * 70)
    print_colored(Colors.GREEN, f"[PARTY] Authentication Fix Summary:")
    print_colored(Colors.GREEN, f"{Symbols.OK} Successful: {successful_fixes}")
    if failed_fixes > 0:
        print_colored(Colors.RED, f"{Symbols.ERROR} Failed: {failed_fixes}")
    
    print(f"{Symbols.STATS} Total processed: {len(selected_clusters)}")
    print("=" * 70)
