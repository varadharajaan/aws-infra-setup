...
class SpotInstanceAnalyzer:
    ...
    def _is_cache_valid(self, cache_file: Path) -> bool:
        ...
        try:
            ...
            return current_time_ist - cache_time_ist < timedelta(
    ...
    def get_service_quotas(
        self, instance_types: List[str], created_by="system"
    ) -> Dict[str, Dict]:
    ...
    def analyze_service_quotas(
        self,
        cred_info: CredentialInfo,
        instance_types: List[str],
        force_refresh: bool = False,
    ) -> Dict[str, ServiceQuotaInfo]:
        ...
        quotas_by_family = self.get_service_quotas(
            instance_types, getattr(cred_info, "username", "system")
        )
        usage_by_family = self.get_current_usage(
            instance_types, getattr(cred_info, "username", "system")
        )
        ...
        for family, quota_data in quotas_by_family.items():
            ...
            quota_value = quota_data.get(
                "QuotaValue", 32
            )  # Changed default from 10 to 32
            ...
            logger.info(
                f"{family}: {current_usage}/{quota_value} used, {available_capacity} available"
            )
        return quota_info

    def get_current_spot_price(
        self, ec2_client, instance_type: str, availability_zone: str
    ) -> float:
        try:
            ...
            logger.warning(
                f"Error getting spot price for {instance_type} in {availability_zone}: {e}"
            )
            return 0.0

    def get_price_history_average(
        self, ec2_client, instance_type: str, availability_zone: str, days: int = 7
    ) -> float:
        try:
            ...
            if response["SpotPriceHistory"]:
                prices = [
                    float(price["SpotPrice"]) for price in response["SpotPriceHistory"]
                ]
            ...
            logger.warning(
                f"Error getting price history for {instance_type} in {availability_zone}: {e}"
            )
            return 0.0

    def analyze_single_spot_instance(
        self, ec2_client, instance_type: str, region: str, availability_zone: str
    ) -> Optional[SpotAnalysis]:
        try:
            current_price = self.get_current_spot_price(
                ec2_client, instance_type, availability_zone
            )
            price_history_avg = self.get_price_history_average(
                ec2_client, instance_type, availability_zone
            )
            interruption_rate = self.estimate_interruption_rate(
                current_price, price_history_avg
            )
            score = self.calculate_spot_score(
                current_price, price_history_avg, interruption_rate
            )
            return SpotAnalysis(
            ...
            logger.warning(
                f"Error analyzing {instance_type} in {availability_zone}: {e}"
            )
            return None

    def analyze_spot_instances(
        self,
        cred_info: CredentialInfo,
        instance_types: List[str],
        force_refresh: bool = False,
            ...
            try:
                ...
                if "timestamp" in cache_data:
                    ...
                    if cache_age_hours > 1:
                        ...
                        use_cache = (
                            input("Do you want to use this cached data? (y/n): ")
                            .strip()
                            .lower()
                        )
                        if use_cache == "y":
                        ...
                        spot_analyses = [
                            SpotAnalysis(**item) for item in cache_data.get("data", [])
                        ]
                        return self._sort_spot_analyses(spot_analyses)
            except Exception as e:
        ...
        logger.info(
            f"Analyzing spot instances in {self.region} with enhanced data... This may take a few moments..."
        )
        ...
        try:
            ...
            quotas_by_family = self.get_real_time_service_quotas(
                ec2_client, service_quotas_client, instance_types
            )

            # Get spot placement scores for better AZ selection
            placement_scores = self.get_spot_placement_scores(
                ec2_client, instance_types, self.region
            )
            ...
            print(
                f"Analyzing {len(instance_types)} instance types across {len(availability_zones)} availability zones..."
            )
            ...
            for instance_type in instance_types:
                ...
                quota_available = quota_info.get(
                ...
                for az in availability_zones:
                    ...
                    az_placement_score = placement_scores.get(az, {}).get(
                        "PlacementScore", 50
                    )
                    ...
                    if completed % 5 == 0 or completed == total_analyses:
                        ...
                        print(
                            f"Progress: {progress:.1f}% ({completed}/{total_analyses})",
                            end="\r",
                        )
            ...
            # Save to cache
            self._save_to_cache(
                [asdict(analysis) for analysis in spot_analyses],
                cache_file,
    ...
    def _sort_spot_analyses(
        self, spot_analyses: List[SpotAnalysis]
    ) -> List[SpotAnalysis]:
        ...
        return sorted(
            spot_analyses,
            key=lambda x: (
                interruption_rate_map.get(
                    x.interruption_rate, 5
                ),  # First by interruption rate (low to high)
                -x.score,  # Then by score (high to low)
                -x.quota_available,  # Then by quota (high to low)
            ),
        )

    def calculate_enhanced_spot_score(
        self,
        current_price: float,
        avg_price: float,
        interruption_rate: str,
    ...
    def calculate_spot_score(
        self, current_price: float, avg_price: float, interruption_rate: str
    ) -> float:
        if avg_price == 0:
            return 0.0
    ...
    def _get_unsupported_azs(self, region: str) -> Set[str]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to load unsupported AZs from mapping file: {str(e)}"
            )
            return set()

    def get_spot_price_history_with_interruption(
        self, ec2_client, instance_type: str, availability_zone: str, days: int = 7
    ) -> Dict:
        ...
        try:
            ...
            price_history = sorted(
                [
                    (
                        datetime.fromisoformat(price["Timestamp"].isoformat()),
    ...
    def get_real_time_service_quotas(
        self, ec2_client, service_quotas_client, instance_types: List[str]
    ) -> Dict[str, Dict]:
        ...
        try:
            ...
            for quota in response.get("Quotas", []):
                ...
                for family in instance_families:
                    if family in quota_name and (
                        "running" in quota_name or "spot" in quota_name
                    ):
                        ...
                        quotas_by_family[family] = {
                            ...
                            "UtilizationPercentage": (
                                (usage / quota_value * 100) if quota_value > 0 else 0
                            ),
                        }
            ...
            for family in instance_families:
                if family not in quotas_by_family:
                    ...
                    quotas_by_family[family] = {
                        ...
                        "Unit": "Count",
                        "UtilizationPercentage": (
                            (usage / default_quota * 100) if default_quota > 0 else 0
                        ),
                        "IsDefault": True,
                    }
    ...
    def get_spot_placement_scores(
        self, ec2_client, instance_types: List[str], region: str
    ) -> Dict[str, Dict]:
        ...
        try:
            ...
            if not hasattr(self, "session") or self.session is None:
                if hasattr(self, "credentials") and self.credentials is not None:
                    ...
                    print(
                        "⚠️  No credentials or session available. Creating default session."
                    )
