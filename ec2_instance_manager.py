...
class EC2InstanceManager:
    ...
    def __init__(
        ...
        self.current_user = (
            current_user  # Add this line to define the current_user attribute
        )
        ...
        try:
            if not os.path.exists(self.ami_mapping_file):
                raise FileNotFoundError(
                    f"AMI mapping file '{self.ami_mapping_file}' not found"
                )
    ...
    def prepare_userdata_with_aws_config(
        self, base_userdata, access_key, secret_key, region
    ):
    ...
    def load_userdata_script(self):
        ...
        try:
            if not os.path.exists(self.userdata_file):
                raise FileNotFoundError(
                    f"User data script file '{self.userdata_file}' not found"
                )
            ...
            for encoding in encodings_to_try:
                try:
                    ...
                    print(
                        f"✅ Successfully read user data script using {encoding} encoding"
                    )
                    break
            ...
            if user_data_content is None:
                raise ValueError(
                    f"Could not read {self.userdata_file} with any supported encoding"
                )
            ...
            if lines and not lines[0].startswith("#!"):
                self.logger.warning(
                    "User data script doesn't start with a shebang (#!)"
                )
            ...
            user_data_content = "".join(
                char for char in user_data_content if ord(char) < 128 or char.isspace()
            )
    ...
    def select_instance_type(self, region: str) -> str:
        ...
        try:
            ...
            while True:
                try:
                    choice = input(
                        f"Select instance type (1-{len(display_types)}): "
                    ).strip()
                    ...
                    if 0 <= type_index < len(display_types):
                        ...
                        print(
                            f"❌ Invalid choice. Please enter a number between 1 and {len(display_types)}"
                        )
                except ValueError:
            ...
            while True:
                try:
                    ...
                    if 0 <= type_index < len(display_types):
                        selected = display_types[type_index].split(" ")[
                            0
                        ]  # Remove "(recommended)"
                        return selected
                    else:
                    ...
                    "TagSpecifications": [
                        {
                            ...
                            "Tags": [
                                {
                                    "Key": "Name",
                                    "Value": f"EC2-{cred_info.account_name}-{instance_config.region}-{suffix}",
                                },
    ...
    def create_security_group(
        self, ec2_client, cred_info: CredentialInfo, suffix=None
    ) -> str:
    ...
    def ensure_key_pair(self, region, credential=None):
        ...
        with self.keypair_lock:
            try:
                ...
                if "InvalidKeyPair.NotFound" in str(e):
                    ...
                    try:
                        ...
                        print(
                            f"✅ Imported public key as EC2 key pair '{key_name}' in region {region}"
                        )
                        return key_name
                        ...
                        self.log_operation(
                            "ERROR", f"Invalid public key format: {str(ve)}"
                        )
            ...
            if not os.path.exists(private_key_file):
                self.log_operation(
                    "WARNING",
                    f"Private key file '{private_key_file}' not found locally.",
                )
                print(
                    f"⚠️ Warning: Private key file '{private_key_file}' not found locally."
                )
            else:
    ...
    def create_ec2_instance(
        self, cred_info: CredentialInfo, instance_type: str = None
    ) -> Dict:
        ...
        try:
            ...
            if not supported_subnets:
                raise ValueError(
                    "No supported subnets found in default VPC after filtering unsupported AZs"
                )
            subnet_id = supported_subnets[0]["SubnetId"]

            enhanced_userdata = self.prepare_userdata_with_aws_config(
                self.userdata_script, cred_info.access_key, cred_info.secret_key, region
            )
            ...
            template_id = self.create_launch_template(
                ec2_client, cred_info, instance_config, security_group_id
            )
                ...
                TagSpecifications=[
                    {
                        ...
                        "Tags": [
                            {
                                "Key": "Name",
                                "Value": f"EC2-{cred_info.account_name}-{region}-{suffix}",
                            },
                            {"Key": "CreatedBy", "Value": "EC2-ASG-Automation"},
                            {"Key": "Account", "Value": cred_info.account_name},
            ...
            self.save_instance_details(
                cred_info, instance_config, instance_id, template_id
            )
