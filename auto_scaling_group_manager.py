...
class AutoScalingGroupManager:
    ...
    def generate_asg_report(self, cred_info, asg_details, instance_selections, execution_timestamp=None):
        ...
        print(f"{Symbols.STATS} ASG report saved to: {filename}")
        return report
    ...
    def create_ondemand_asg(self, asg_client, asg_config: ASGConfig, cred_info: CredentialInfo, schedule_scaling: bool = True) -> Dict:
        ...
        try:
            print(f"[BUILD] Creating On-Demand Auto Scaling Group...")
    ...
    def create_spot_asg(self, asg_client, asg_config: ASGConfig, cred_info: CredentialInfo, schedule_scaling: bool = True) -> Dict:
        ...
        try:
            ...
            print(f"{Symbols.OK} Spot ASG created successfully")
            # Attach scheduled scaling actions if requested
            schedule_scaling = True  # This should be passed as a parameter or class attribute
            if schedule_scaling:
                self.attach_scheduled_actions(asg_client, asg_config.name, asg_config.region)
                print(f"{Symbols.OK} Spot ASG created successfully with scheduled scaling")
            return response
            
        except Exception as e:
            print(f"{Symbols.ERROR} Error creating Spot ASG: {e}")
            raise
    ...
    def create_mixed_asg(self, asg_client, asg_config: ASGConfig, instance_selections: Dict, cred_info: CredentialInfo, schedule_scaling: bool = True) -> Dict:
        ...
        try:
            ...
            print(f"{Symbols.SCAN} Creating Mixed ASG ({on_demand_percentage}% On-Demand, {100-on_demand_percentage}% Spot)...")
            ...
            print(f"{Symbols.OK} Mixed ASG created successfully")
            # Attach scheduled scaling actions if requested
            schedule_scaling = True  # This should be passed as a parameter or class attribute
            if schedule_scaling:
                self.attach_scheduled_actions(asg_client, asg_config.name, asg_config.region)
            print(f"{Symbols.OK} Mixed ASG created successfully with scheduled scaling")
            return response
            
        except Exception as e:
            print(f"{Symbols.ERROR} Error creating Mixed ASG: {e}")
            raise
    ...
    def _get_subnets_for_azs(self, availability_zones: List[str], region: str, cred_info: CredentialInfo) -> List[str]:
        ...
        try:
            ...
            if not supported_azs:
                print(f"{Symbols.WARN} No supported availability zones after filtering unsupported AZs.")
                return []
            ...
            print(f"{Symbols.WARN} Error getting subnets: {e}")
    ...
    def prompt_asg_strategy(self) -> str:
        ...
        while True:
            ...
            if choice == '1':
                ...
                print(f"{Symbols.ERROR} Invalid choice. Please enter 1, 2, or 3.")
    ...
    def select_ondemand_instance_types(self, cred_info: CredentialInfo, 
        ...
        print(f"\n{Symbols.STATS} Service Quota Analysis:")
        print("-" * 50)
        ...
        for instance_type in allowed_types:
            ...
            if family in quota_info:
                ...
                if available_capacity > 0:
                    print(f"{Symbols.OK} {instance_type:15} | Available: {available_capacity:3} | Used: {current_usage:3}/{quota_limit}")
                else:
                    print(f"{Symbols.ERROR} {instance_type:15} | No capacity available ({current_usage}/{quota_limit})")
            else:
                ...
                print(f"{Symbols.WARN}  {instance_type:15} | Quota info unavailable (using default 32)")
        ...
        if not available_types:
            print(f"{Symbols.ERROR} No instance types available due to quota limits!")
    ...
    def select_spot_instance_types(self, cred_info: CredentialInfo, 
        ...
        print("\n" + "="*60)
        print("[UP] SPOT INSTANCE TYPE SELECTION")
        print("="*60)
    ...
    def select_mixed_instance_types(self, cred_info: CredentialInfo, 
        ...
        print("\n" + "="*60)
        print("[SCAN] MIXED STRATEGY INSTANCE TYPE SELECTION")
        print("="*60)
    ...
    def save_and_display_spot_summary(self, analyses: List[SpotAnalysis], cred_info: CredentialInfo):
        ...
        try:
            ...
            print(f"{Symbols.FOLDER} Spot analysis saved to: {filename}")
            ...
            print(f"{Symbols.WARN} Warning: Could not save spot analysis summary: {e}")
    ...
    def create_asg_with_strategy(self, cred_info: CredentialInfo, instance_selections: Dict[str, List[str]], 
        ...
        try:
            ...
            print(f"\n{Symbols.START} Creating Auto Scaling Group: {asg_config.name}")
            print(f"   [ROUNDPIN] Region: {region}")
            print(f"   {Symbols.STATS} Strategy: {strategy.upper()}")
            print(f"   [COMPUTE] Instance Types: {', '.join(asg_config.instance_types)}")
            print(f"   [UP] Capacity: Min={asg_config.min_size}, Desired={asg_config.desired_capacity}, Max={asg_config.max_size}")
            ...
            print(f"{Symbols.OK} Auto Scaling Group created successfully!")
            ...
            if scaling_policy_result['status'] == 'success':
                print(f"{Symbols.OK} Scaling policies and alarms attached to ASG: {asg_config.name}")
            else:
                print(f"{Symbols.WARN} Failed to attach scaling policies: {scaling_policy_result['error_message']}")
            ...
            print("\n" + "="*50)
            print(f"{Symbols.LIST} SUMMARY:")
            print(f"   {Symbols.KEY} Credential Type: {cred_info.credential_type}")
            print(f"   [ACCOUNT] Account: {cred_info.account_name}")
            print(f"   📧 Email: {cred_info.email}")
            print(f"   {Symbols.REGION} Region: {region}")
            print(f"   {Symbols.START} ASG Name: {asg_config.name}")
            print(f"   {Symbols.STATS} ASG Strategy: {strategy.upper()}")
            print(f"   {Symbols.FOLDER} Output saved to: aws/asg/{cred_info.account_name}/")
            print(f"   {Symbols.STATS} Report: aws/asg/{cred_info.account_name}/asg_report_{execution_timestamp}.json")
            print("="*50)
            ...
            print(f"{Symbols.ERROR} Error creating Auto Scaling Group: {e}")
            ...
            print(f"{Symbols.STATS} Failed ASG report saved to: {failed_filename}")
            raise
    ...
    def _get_vpc_from_subnet(self, subnet_id: str, region: str, cred_info: CredentialInfo) -> str:
        ...
        try:
            ...
            print(f"{Symbols.WARN} Warning: Could not get VPC ID for subnet {subnet_id}: {e}")
            return 'Unknown'
        ...
        while True:
            try:
                ...
                if desired_capacity < min_size:
                    print(f"{Symbols.ERROR} Desired capacity must be >= minimum capacity ({min_size})")
                    continue
                break
            except ValueError:
                print("[ERROR] Please enter a valid number")
        ...
        while True:
            try:
                ...
                if max_size < desired_capacity:
                    print(f"{Symbols.ERROR} Maximum capacity must be >= desired capacity ({desired_capacity})")
                    continue
                break
            except ValueError:
                print("[ERROR] Please enter a valid number")
    ...
    def save_asg_details(self, cred_info, asg_config, asg_response, scaling_policy_result=None):
        ...
        try:
            ...
            print(f"{Symbols.FOLDER} ASG details saved to: {filename}")
            
        except Exception as e:
            print(f"{Symbols.WARN} Warning: Could not save ASG details: {e}")
        ...
        try:
            ...
            try:
                ...
                if existing_actions:
                    ...
                    print(f"{Symbols.WARN} Found {len(existing_actions)} existing scheduled actions: {', '.join(action_names)}")
                    print("[INFO] These actions will be used for scheduled scaling")
                    return True
            except Exception as e:
                print(f"{Symbols.WARN} Warning: Could not check for existing scheduled actions: {str(e)}")
            ...
            print(f"{Symbols.OK} Created scale-up action: {scale_up_name} (starts at 11:00 AM IST on weekdays)")
            ...
            print(f"{Symbols.OK} Created scale-down action: {scale_down_name} (starts at 9:00 PM IST on weekdays)")
            print(f"[CALENDAR] Scaling actions will begin tomorrow and repeat on weekdays thereafter")
        
            print(f"{Symbols.OK} Scheduled scaling actions attached to ASG {asg_name}")
            ...
            print(f"{Symbols.ERROR} Failed to attach scheduled scaling actions: {str(e)}")
            return False
