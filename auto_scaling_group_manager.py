...
from spot_instance_analyzer import ServiceQuotaInfo, SpotAnalysis, SpotInstanceAnalyzer
...
class AutoScalingGroupManager:
    def __init__(
        self, current_user="varadharajaan", current_time="2025-06-13 05:13:24"
    ):
    ...
    def generate_asg_report(
        self, cred_info, asg_details, instance_selections, execution_timestamp=None
    ):
        """
    ...
    def create_asg_scaling_policies(self, asg_name, cred_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"Scaling policies and alarms created for ASG: {asg_name}"
            )
            return {
    ...
    def create_ondemand_asg(
        self,
        asg_client,
        asg_config: ASGConfig,
        cred_info: CredentialInfo,
        schedule_scaling: bool = True,
    ) -> Dict:
        ...
        try:
            ...
            subnets = self._get_subnets_for_azs(
                asg_config.availability_zones, asg_config.region, cred_info
            )
            ...
            if schedule_scaling:
                self.attach_scheduled_actions(
                    asg_client, asg_config.name, asg_config.region
                )
    ...
    def create_spot_asg(
        ...
        try:
            ...
            spot_allocation_strategy = (
                asg_config.spot_allocation_strategy or "capacity-optimized"
            )
            ...
            if spot_allocation_strategy == "lowest-price":
                instances_distribution["SpotInstancePools"] = min(
                    len(asg_config.instance_types), 20
                )
            ...
            # Attach scheduled scaling actions if requested
            schedule_scaling = (
                True  # This should be passed as a parameter or class attribute
            )
            if schedule_scaling:
                self.attach_scheduled_actions(
                    asg_client, asg_config.name, asg_config.region
    ...
    def create_mixed_asg(
        ...
        try:
            ...
            print(
                f"🔄 Creating Mixed ASG ({on_demand_percentage}% On-Demand, {100-on_demand_percentage}% Spot)..."
            )
    ...
    def _get_subnets_for_azs(
        self, availability_zones: List[str], region: str, cred_info: CredentialInfo
    ) -> List[str]:
        ...
        try:
            ...
            supported_azs = [
                az for az in availability_zones if az not in unsupported_azs
            ]
            if not supported_azs:
                print(
                    "⚠️ No supported availability zones after filtering unsupported AZs."
                )
                return []
    ...
    def select_ondemand_instance_types(
        self, cred_info: CredentialInfo, allowed_types: List[str]
    ) -> Dict[str, List[str]]:
        ...
        for instance_type in allowed_types:
            ...
            if family in quota_info:
                ...
                if available_capacity > 0:
                    print(
                        f"✅ {instance_type:15} | Available: {available_capacity:3} | Used: {current_usage:3}/{quota_limit}"
                    )
                else:
                    print(
                        f"❌ {instance_type:15} | No capacity available ({current_usage}/{quota_limit})"
                    )
            else:
                ...
                print(
                    f"⚠️  {instance_type:15} | Quota info unavailable (using default 32)"
                )

        # Sort by available capacity (highest first)
        sorted_instances = sorted(
            available_instance_data, key=lambda x: -x["available_capacity"]
        )
    ...
    def select_spot_instance_types(
        self,
        cred_info: CredentialInfo,
        ...
        if not force_refresh:
            refresh_choice = (
                input("Use cached spot data if available? (y/n): ").strip().lower()
            )
        ...
        spot_analyses = self.spot_analyzer.analyze_spot_instances(
            cred_info, allowed_types, force_refresh
        )
        ...
        enhanced_analyses = self.enhance_spot_analysis_with_quotas(
            spot_analyses, quota_info
        )
        ...
        if enhanced_analyses:
            print("\n" + "=" * 60)
            review_choice = (
                input("Review detailed JSON summary? (y/n): ").strip().lower()
            )
            if review_choice == "y":
        ...
        sorted_analyses = sorted(
            enhanced_analyses, key=lambda x: (x.score, x.quota_available), reverse=True
        )
    ...
    def select_mixed_instance_types(
        self, cred_info: CredentialInfo, allowed_types: List[str]
    ) -> Dict[str, List[str]]:
        ...
        ondemand_selection = self.select_ondemand_instance_types(
            cred_info, allowed_types
        )
        ...
        while True:
            try:
                percentage = input(
                    "Enter On-Demand percentage (0-100, default 50): "
                ).strip()
                if not percentage:
                    percentage = 50
                else:
    ...
    def multi_select_instance_types(
        self, available_types: List[str], strategy_name: str
    ) -> List[str]:
        ...
        while True:
            try:
                ...
                if selected_indices:
                    ...
                    print(
                        f"✅ Selected {strategy_name} types: {', '.join(selected_types)}"
                    )
    ...
    def enhance_spot_analysis_with_quotas(
        self, spot_analyses: List[SpotAnalysis], quota_info: Dict[str, ServiceQuotaInfo]
    ) -> List[SpotAnalysis]:
        ...
        for analysis in spot_analyses:
            ...
            if family in quota_info:
                ...
                quota_score = min(
                    100, (analysis.quota_available / 10) * 10
                )  # Scale quota to score
    ...
    def display_spot_analysis_results(self, analyses: List[SpotAnalysis]):
        ...
        sorted_analyses = sorted(
            analyses, key=lambda x: (x.score, x.quota_available), reverse=True
        )
        ...
        print(
            f"{'Type':<12} {'Zone':<15} {'Price':<8} {'Avg':<8} {'Interrupt':<10} {'Quota':<6} {'Score':<6}"
        )
        print("-" * 100)
        ...
        for analysis in sorted_analyses[:15]:
            print(
                f"{analysis.instance_type:<12} {analysis.availability_zone:<15} "
    ...
    def save_and_display_spot_summary(
        self, analyses: List[SpotAnalysis], cred_info: CredentialInfo
    ):
        ...
        try:
            ...
            summary = {
                ...
                "analysis_summary": {
                    ...
                    "medium_score_count": len(
                        [a for a in analyses if 40 <= a.score <= 70]
                    ),
            ...
            print("=" * 60)
            print(
                f"Total instances analyzed: {summary['analysis_summary']['total_analyzed']}"
            )
            print(
                f"High score (>70): {summary['analysis_summary']['high_score_count']}"
            )
            print(
                f"Medium score (40-70): {summary['analysis_summary']['medium_score_count']}"
            )
    ...
    def _get_vpc_from_subnet(
        self, subnet_id: str, region: str, cred_info: CredentialInfo
    ) -> str:
        ...
        try:
            ...
            if (
                subnet_response
                and "Subnets" in subnet_response
                and subnet_response["Subnets"]
            ):
    ...
    def build_asg_config(
        self,
        cred_info: CredentialInfo,
        instance_selections: Dict[str, list],
        launch_template_id: str,
        strategy: str,
    ) -> ASGConfig:
        """
        ...
        if strategy == "on-demand" and instance_selections.get("on-demand"):
            ...
            combined = instance_selections.get(
                "on-demand", []
        ...
        return ASGConfig(
            ...
            spot_allocation_strategy=(
                "capacity-optimized" if strategy in ["spot", "mixed"] else None
            ),
            on_demand_percentage=(
                instance_selections.get("on_demand_percentage", 50)
                if strategy == "mixed"
                else None
            ),
        )

    def save_asg_details(
        self, cred_info, asg_config, asg_response, scaling_policy_result=None
    ):
    ...
    def attach_scheduled_actions(self, asg_client, asg_name: str, region: str) -> bool:
        ...
        try:
            self.log_operation(
                "INFO", f"Attaching scheduled scaling actions to ASG {asg_name}"
            )
            ...
            try:
                ...
                if existing_actions:
                    action_names = [
                        action["ScheduledActionName"] for action in existing_actions
                    ]
                    print(
                        f"⚠️ Found {len(existing_actions)} existing scheduled actions: {', '.join(action_names)}"
                    )
                    print("ℹ️ These actions will be used for scheduled scaling")
                    return True
            except Exception as e:
                print(
                    f"⚠️ Warning: Could not check for existing scheduled actions: {str(e)}"
                )
            ...
            print(
                f"✅ Created scale-up action: {scale_up_name} (starts at 11:00 AM IST on weekdays)"
            )
            ...
            asg_client.put_scheduled_update_group_action(
                ...
                StartTime=unique_start_time
                + timedelta(minutes=1),  # Ensure different start time
            ...
            print(
                f"✅ Created scale-down action: {scale_down_name} (starts at 9:00 PM IST on weekdays)"
            )
            print(
                f"🗓️ Scaling actions will begin tomorrow and repeat on weekdays thereafter"
            )

            print(f"✅ Scheduled scaling actions attached to ASG {asg_name}")
            self.log_operation(
                "INFO",
                f"Successfully attached scheduled scaling actions to ASG {asg_name}",
            )
            return True
            ...
            self.log_operation(
                "ERROR",
                f"Failed to attach scheduled scaling actions to ASG {asg_name}: {str(e)}",
            )
