...
class EKSAutomation:
    ...
    def load_ami_configuration(self) -> Dict:
        ...
        try:
            if not os.path.exists(self.config_file):
                print(
                    f"⚠️  Configuration file {self.config_file} not found, using defaults"
                )
                return {
                    "eks_config": {"default_version": "1.27", "ami_type": "AL2_x86_64"}
                }
            ...
            return {"eks_config": {"default_version": "1.27", "ami_type": "AL2_x86_64"}}

    def get_eks_config(self) -> Dict:
        """Get EKS configuration from the loaded configuration"""
        # Get EKS config or use defaults
        return self.ami_config.get(
            "eks_config", {"default_version": "1.27", "ami_type": "AL2_x86_64"}
        )

    def convert_enhanced_credentials_to_legacy_format(
        self, multi_credentials: MultiUserCredentials
    ) -> List[Dict]:
        ...
        for cred in multi_credentials.users:
            user_obj = {
                "username": (
                    cred.username if cred.username else f"root-{cred.account_name}"
                ),
                "full_name": (
                    f"User ({cred.account_name})"
                    if cred.username
                    else f"Root User ({cred.account_name})"
                ),
    ...
    def run_automation(self):
        ...
        try:
            ...
            print(
                f"✅ {validated_credentials.total_users} valid credential(s) ready for EKS automation"
            )

            # Convert to legacy format for existing EKS logic
            all_selected_users = self.convert_enhanced_credentials_to_legacy_format(
                validated_credentials
            )
            ...
            install_addons = input(
                "Install essential EKS add-ons (EBS/EFS/VPC CNI)? (y/N): "
            ).strip().lower() in ["y", "yes"]
            enable_container_insights = input(
                "Enable Container Insights? (y/N): "
            ).strip().lower() in ["y", "yes"]
            ...
            confirmation = (
                input(
                    "\nProceed with cluster creation using these configurations? (Y/n): "
                )
                .strip()
                .lower()
            )
            ...
            self.display_cluster_creation_summary(
                credential_info, eks_version, selected_ami_type, nodegroup_configs
            )
            ...
            print(
                "Nodegroup configs assigned to all selected users:",
                all_selected_users[0]["nodegroup_configs"],
            )

            # Call process_multiple_user_selection with all selected users and automation options (EXISTING LOGIC)
            result = self.eks_manager.process_multiple_user_selection(
                all_selected_users, automation_options
            )
    ...
    def display_common_nodegroup_summary(self, nodegroup_configs: List[Dict]):
        ...
        for i, config in enumerate(nodegroup_configs, 1):
            ...
            print(
                f"   Instance Types: {self.format_instance_types_summary(config['instance_selections'])}"
            )
    ...
    def configure_multiple_nodegroups(self, credentials: CredentialInfo) -> List[Dict]:
        ...
        while True:
            try:
                num_nodegroups = input(
                    "How many nodegroups do you want to create? (1-5) [default: 2]: "
                ).strip()
        ...
        allowed_instance_types = self.ami_config.get(
        ...
        for i in range(num_nodegroups):
            ...
            nodegroup_name = input(
                f"Enter name for nodegroup {nodegroup_num} [default: {default_name}]: "
            ).strip()
            ...
            min_nodes, desired_nodes, max_nodes = (
                self.prompt_nodegroup_scaling_individual(nodegroup_name, strategy)
            )
            ...
            print(f"   Strategy: {strategy.upper()}")
            print(
                f"   Scaling: Min={min_nodes}, Desired={desired_nodes}, Max={max_nodes}"
            )
            print(
                f"   Instance Types: {self.format_instance_types_summary(instance_selections)}"
            )
    ...
    def display_cluster_creation_summary(
        self,
        credentials,
        eks_version: str,
        ami_type: str,
    ...
    def select_mixed_instance_types_for_nodegroup(
        self, credentials: CredentialInfo, allowed_types: List[str], nodegroup_name: str
    ) -> Dict[str, List[str]]:
    ...
    def select_ondemand_instance_types_for_nodegroup(
        self, credentials: CredentialInfo, allowed_types: List[str], nodegroup_name: str
    ) -> Dict[str, List[str]]:
        ...
        refresh_choice = (
            input("Use cached on-demand quota data if available? (y/n): ")
            .strip()
            .lower()
        )
        ...
        quota_info = self.analyze_service_quotas_with_cache(
            credentials, allowed_types, force_refresh
        )
        ...
        for instance_type in allowed_types:
            ...
            if family in quota_info:
                ...
                available = quota.get(
                    "available_capacity",
                    quota.get("quota_limit", 0) - quota.get("current_usage", 0),
                )
    ...
    def select_spot_instance_types_for_nodegroup(
        self, credentials: CredentialInfo, allowed_types: List[str], nodegroup_name: str
    ) -> Dict[str, List[str]]:
        ...
        print(
            f"{'#':<3} {'Type':<10} {'Zone':<15} {'Price':<8} {'Score':<6} {'Interrupt':<15}"
        )
        print("-" * 80)

        for i, analysis in enumerate(sorted_spots, 1):
            print(
                f"{i:<3} {analysis.instance_type:<10} {analysis.availability_zone:<15} "
    ...
    def prompt_nodegroup_scaling_individual(
        self, nodegroup_name: str, strategy: str
    ) -> Tuple[int, int, int]:
        ...
        use_defaults = (
            input(f"\nUse default scaling values for {nodegroup_name}? (y/n): ")
            .strip()
            .lower()
        )
        if use_defaults == "y":
        ...
        while True:
            try:
                min_nodes = int(
                    input(
                        f"Minimum nodes for {nodegroup_name} (default {default_min}): "
                    ).strip()
                    or default_min
                )
                if min_nodes < 0:
        ...
        while True:
            try:
                desired_nodes = int(
                    input(
                        f"Desired nodes for {nodegroup_name} (default {default_desired}): "
                    ).strip()
                    or default_desired
                )
        ...
        while True:
            try:
                max_nodes = int(
                    input(
                        f"Maximum nodes for {nodegroup_name} (default {default_max}): "
                    ).strip()
                    or default_max
                )
                if max_nodes < desired_nodes:
                    print(
                        f"❌ Maximum nodes must be >= desired nodes ({desired_nodes})"
                    )
                    continue
                break
            except ValueError:
    ...
    def analyze_service_quotas_with_cache(
        self,
        ...
        if not force_refresh and os.path.exists(cache_file):
            try:
                ...
                if cache_time and datetime.now() - cache_time < cache_duration:
                    print(
                        f"📁 Using cached quota data from {cache_time.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
        ...
        # Save to cache
        try:
            cache_data = {"timestamp": datetime.now(), "quota_info": quota_info}
    ...
    def fetch_service_quotas(
        self, credentials: CredentialInfo, instance_types: List[str]
    ) -> Dict:
        ...
        try:
            ...
            for instance in instances:
                ...
                if family in ["f", "g", "p", "x", "inf"]:
                    ...
                    family_usage["standard"] = (
                        family_usage.get("standard", 0) + vcpu_count
                    )
        ...
        for instance_family, quota_code in quota_code_mappings.items():
            try:
                ...
                print(
                    f"✅ {instance_family}: Limit={quota_limit}, Used={current_usage}, Available={available_capacity}"
                )
        ...
        for instance_type in instance_types:
            ...
            instance_quotas[family] = quota_results.get(
                quota_family,
                {
            ...
            instance_quotas[family]["available_capacity"] = max(
                0,
                instance_quotas[family]["quota_limit"]
        ...
        while True:
            try:
                ...
                if selection.lower() == "all":
                    ...
                    print(
                        f"✅ Selected all {len(selected_types)} {strategy_name} types"
                    )
