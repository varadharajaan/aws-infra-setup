...
class EnhancedInteractiveClusterManager:
    ...
    def enhanced_interactive_cluster_management(self) -> bool:
        ...
        try:
            ...
            self.print_colored(Colors.RED, f"❌ Enhanced interactive cluster management failed: {str(e)}")
    ...
    def analyze_cluster_credential_requirements(self, selected_clusters: List[Dict]) -> Dict:
        ...
        try:
            ...
            self.print_colored(Colors.CYAN, f"   📊 Root user clusters: {len(root_clusters)}")
            self.print_colored(Colors.CYAN, f"   📊 IAM user clusters: {len(iam_clusters)}")
            ...
            self.print_colored(Colors.RED, f"❌ Error analyzing credential requirements: {str(e)}")
    ...
    def scan_iam_credentials_files(self) -> List[Dict]:
        ...
        """
        try:
            self.print_colored(Colors.CYAN, "🔍 Scanning for IAM user credentials files...")
            ...
            if not files:
                self.print_colored(Colors.RED, "❌ No IAM credentials files found in aws/iam/")
                return []
            ...
            for file_path in files:
                try:
                    ...
                    self.print_colored(Colors.YELLOW, f"   ⚠️  Error parsing {file_path}: {str(e)}")

            # Sort by timestamp (newest first)
            credentials_files.sort(key=lambda x: f"{x['date_str']}{x['time_str']}", reverse=True)

            self.print_colored(Colors.GREEN, f"✅ Found {len(credentials_files)} IAM credentials files")
            return credentials_files

        except Exception as e:
            self.print_colored(Colors.RED, f"❌ Error scanning IAM credentials files: {str(e)}")
            return []
        ...
        try:
            ...
            if credential_requirements['needs_root_creds']:
                ...
                try:
                    ...
                    self.print_colored(Colors.GREEN, "✅ Loaded root account credentials from aws_accounts_config.json")

                except Exception as e:
                    self.print_colored(Colors.RED, f"❌ Failed to load root credentials: {str(e)}")
                    return None
            ...
            self.print_colored(Colors.RED, f"❌ Error in credential selection: {str(e)}")
            return None
            ...
            while True:
                try:
                    ...
                    if 1 <= choice_num <= len(iam_files):
                        ...
                        self.print_colored(Colors.RED, f"❌ Invalid choice. Please enter 1-{len(iam_files)}")
    ...
    def get_credentials_for_cluster(self, cluster_info: Dict, credentials: Dict) -> Tuple[str, str, str]:
        ...
        try:
            ...
            if not account_name:
                self.print_colored(Colors.RED, f"   ❌ Could not extract account from cluster name: {cluster_name}")
                return None, None, None

            if self.is_root_created_cluster(cluster_name):
                # Use root credentials
                if 'root_accounts' not in credentials:
                    self.print_colored(Colors.RED, f"   ❌ Root credentials not available for {cluster_name}")
                    return None, None, None

                account_config = credentials['root_accounts'].get(account_name)
                if not account_config:
                    self.print_colored(Colors.RED, f"   ❌ Root account {account_name} not found")
                    return None, None, None
                ...
                if 'iam_users' not in credentials:
                    self.print_colored(Colors.RED, f"   ❌ IAM credentials not available for {cluster_name}")
                    return None, None, None

                # Extract IAM username from cluster name
                iam_username = self.extract_iam_user_from_cluster_name(cluster_name)
                if not iam_username:
                    self.print_colored(Colors.RED, f"   ❌ Could not extract IAM username from: {cluster_name}")
                    return None, None, None
                ...
                self.print_colored(Colors.RED, f"   ❌ IAM user {iam_username} not found in credentials")
                return None, None, None

        except Exception as e:
            self.print_colored(Colors.RED, f"   ❌ Error getting credentials for cluster: {str(e)}")
            return None, None, None
    ...
    def delete_autoscalers_with_smart_credentials(self, selected_clusters: List[Dict], credentials: Dict) -> bool:
        ...
        try:
            ...
            for i, cluster_info in enumerate(selected_clusters, 1):
                ...
                self.print_colored(Colors.CYAN, f"   📍 Type: {cluster_type}, Region: {region}")
                ...
                try:
                    ...
                    if not access_key or not secret_key:
                        self.print_colored(Colors.RED, f"   ❌ Missing credentials for {cluster_name}")
                    ...
                    if success:
                        ...
                        self.print_colored(Colors.GREEN, f"   ✅ Successfully deleted autoscaler from {cluster_name}")
                    else:
                        failed_deletions.append(cluster_name)
                        self.print_colored(Colors.RED, f"   ❌ Failed to delete autoscaler from {cluster_name}")

                except Exception as e:
                    self.print_colored(Colors.RED, f"   ❌ Error processing {cluster_name}: {str(e)}")
                    failed_deletions.append(cluster_name)
            ...
            self.print_colored(Colors.RED, f"❌ Error during smart autoscaler deletion: {str(e)}")
            return False
    ...
    def configure_auth_with_smart_credentials(self, selected_clusters: List[Dict], credentials: Dict) -> bool:
        ...
        try:
            ...
            for i, cluster_info in enumerate(selected_clusters, 1):
                ...
                try:
                    ...
                    if success:
                        ...
                        self.print_colored(Colors.GREEN, f"   ✅ Successfully configured auth for {cluster_name}")
                    else:
                        failed_configs.append(cluster_name)
                        self.print_colored(Colors.RED, f"   ❌ Failed to configure auth for {cluster_name}")
            ...
            self.print_colored(Colors.RED, f"❌ Error during ConfigMap-only auth configuration: {str(e)}")
            return False
    ...
    def configure_aws_auth_configmap_simplified(self, cluster_name: str, region: str, account_id: str, user_data: Dict,
        ...
        try:
            ...
            self.print_colored(Colors.CYAN, f"   🔐 Configuring aws-auth ConfigMap (ConfigMap only)...")
    ...
    def apply_configmap_only(self, cluster_name: str, region: str, account_id: str, users_to_add: List[Dict],
        ...
        """
        try:
            self.print_colored(Colors.CYAN, "   📋 Creating/updating aws-auth ConfigMap (ConfigMap only mode)...")

            # Check if kubectl is available
            kubectl_available = shutil.which('kubectl') is not None

            if not kubectl_available:
                self.print_colored(Colors.RED, f"   ❌ kubectl not found. ConfigMap setup failed.")
                return False
            ...
            try:
                ...
                self.print_colored(Colors.RED, f"   ❌ Failed to create ConfigMap file: {str(e)}")
                return False
            ...
            try:
                ...
                self.print_colored(Colors.CYAN, f"   📋 Applying ConfigMap (standard apply)...")
                ...
                if apply_result.returncode == 0:
                    ...
                    self.print_colored(Colors.YELLOW, f"   ⚠️  Standard apply failed: {apply_result.stderr.strip()}")
                    ...
                    if replace_result.returncode == 0:
                        self.print_colored(Colors.GREEN, f"   ✅ ConfigMap replaced successfully")
                        success = True
                        ...
                        self.print_colored(Colors.CYAN, f"   📋 Trying delete and recreate...")
                        ...
                        if delete_result.returncode == 0:
                            self.print_colored(Colors.CYAN, f"   🗑️  Existing ConfigMap deleted")
                        ...
                        if create_result.returncode == 0:
                            self.print_colored(Colors.GREEN, f"   ✅ ConfigMap recreated successfully")
                            success = True
                ...
                if success:
                    self.print_colored(Colors.CYAN, f"   🔍 Verifying ConfigMap (applied via {applied_method})...")
                    ...
                    try:
                        ...
                        if verify_result1.returncode == 0:
                            self.print_colored(Colors.GREEN, f"   ✅ ConfigMap exists and is accessible")
                    ...
                    if not verification_passed:
                        try:
                            ...
                            if verify_result2.returncode == 0 and 'aws-auth' in verify_result2.stdout:
                                self.print_colored(Colors.GREEN, f"   ✅ ConfigMap found in namespace listing")
                                verification_passed = True
                            else:
                        ...
                        try:
                            ...
                            if content_result.returncode == 0 and content_result.stdout.strip():
                                ...
                                try:
                                    ...
                                    self.print_colored(Colors.CYAN, f"   📋 Configured user mappings:")
                                    ...
                                    self.print_colored(Colors.GREEN, f"   ✅ ConfigMap contains user mappings")
                    ...
                    if verification_passed:
                        self.print_colored(Colors.GREEN, f"   ✅ ConfigMap verification completed successfully")
                    else:
                        self.print_colored(Colors.YELLOW,
                    ...
                    self.print_colored(Colors.RED, f"   ❌ All ConfigMap application strategies failed")
                    return False
            ...
            self.print_colored(Colors.RED, f"   ❌ ConfigMap application failed: {str(e)}")
            return False
    ...
    def configure_aws_auth_configmap_enhanced(self, cluster_name: str, region: str, account_id: str, user_data: Dict,
        ...
        try:
            ...
            if auth_mode in ['API', 'API_AND_CONFIG_MAP']:
                self.print_colored(Colors.CYAN, "   🔑 Creating access entries for API-based authentication...")
                ...
                for principal_arn in principals_to_add:
                    try:
                        ...
                        self.print_colored(Colors.GREEN, f"   ✅ Created access entry for {principal_type}")
                        success_count += 1
                ...
                if success_count > 0:
                    if is_root_cluster:
                        self.print_colored(Colors.GREEN, f"   ✅ Root user configured for cluster access via API")
                    else:
                        username = user_data.get('username', 'unknown')
                        self.print_colored(Colors.GREEN,
    ...
    def apply_configmap_with_kubectl(self, cluster_name: str, region: str, account_id: str, users_to_add: List[Dict],
        ...
        try:
            ...
            if not kubectl_available:
                self.print_colored(Colors.YELLOW, f"   ⚠️  kubectl not found. ConfigMap setup skipped.")
                return True
            ...
            try:
                ...
                self.print_colored(Colors.CYAN, f"   🔍 Testing cluster connectivity...")
                ...
                if test_result.returncode != 0:
                    self.print_colored(Colors.YELLOW, f"   ⚠️  Cluster connectivity test failed: {test_result.stderr}")
                    ...
                    try:
                        ...
                        if caller_arn:
                            ...
                            try:
                                ...
                                self.print_colored(Colors.GREEN, f"   ✅ Temporary admin access entry created")
                                ...
                                if test_result2.returncode == 0:
                                    self.print_colored(Colors.GREEN, f"   ✅ Cluster connectivity established")
                                else:
                                    self.print_colored(Colors.YELLOW,
                        ...
                        self.print_colored(Colors.YELLOW, f"   ⚠️  Authentication setup failed: {str(auth_e)}")
                else:
                    self.print_colored(Colors.GREEN, f"   ✅ Cluster connectivity confirmed")
                ...
                for i, strategy in enumerate(apply_strategies, 1):
                    ...
                    if result.returncode == 0:
                        ...
                        self.print_colored(Colors.YELLOW, f"   ⚠️  {strategy['name']} failed: {result.stderr.strip()}")
                ...
                if not success:
                    ...
                    try:
                        ...
                        if delete_result.returncode == 0:
                            ...
                            self.print_colored(Colors.YELLOW, f"   ⚠️  Delete result: {delete_result.stderr}")
                        ...
                        if create_result.returncode == 0:
                            ...
                            self.print_colored(Colors.RED, f"   ❌ ConfigMap recreation failed: {create_result.stderr}")
                            ...
                            if server_side_result.returncode == 0:
                                self.print_colored(Colors.GREEN, f"   ✅ ConfigMap applied using server-side apply")
                                success = True
                            else:
                                self.print_colored(Colors.RED,
                        ...
                        self.print_colored(Colors.RED, f"   ❌ Error during recreate strategy: {str(recreate_e)}")
                ...
                if success:
                    try:
                        ...
                        if verify_result.returncode == 0 and 'aws-auth' in verify_result.stdout:
                            self.print_colored(Colors.GREEN, f"   ✅ ConfigMap verification successful")
                        ...
                        self.print_colored(Colors.YELLOW, f"   ⚠️  ConfigMap verification error: {str(verify_e)}")
    ...
    def scan_cluster_files(self) -> List[Dict]:
        ...
        try:
            ...
            for base_dir in base_dirs:
                ...
                self.print_colored(Colors.CYAN, f"   📁 {account_id}: Found {len(files)} cluster files")
            ...
            self.print_colored(Colors.GREEN, f"✅ Total cluster files found: {len(cluster_files)}")
    ...
    def group_clusters_by_date(self, cluster_files: List[Dict]) -> Dict:
        ...
        try:
            ...
            self.print_colored(Colors.RED, f"❌ Error grouping clusters by date: {str(e)}")
            return {}
            ...
            while True:
                try:
                    ...
                    if 1 <= choice_num <= len(dates):
                        ...
                        self.print_colored(Colors.GREEN, f"✅ Selected date: {selected_date}")
                        return selected_date
                    else:
                        self.print_colored(Colors.RED, f"❌ Invalid choice. Please enter 1-{len(dates)}")
            ...
            while True:
                try:
                    ...
                    if selection.lower() == 'all':
                        ...
                        self.print_colored(Colors.GREEN, f"✅ Selected all {len(selected_clusters)} clusters")
                        return selected_clusters

                    selected_indices = self.parse_cluster_selection(selection, len(date_clusters))

                    if not selected_indices:
                        self.print_colored(Colors.RED, "❌ Invalid selection. Please try again.")
                        continue
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if choice == "1":
                        self.print_colored(Colors.GREEN, f"✅ Selected: Delete Cluster Autoscaler")
                        return "delete_autoscaler"
                    elif choice == "2":
                        self.print_colored(Colors.GREEN, f"✅ Selected: Configure User Authentication")
                        return "configure_auth"
                    else:
                        self.print_colored(Colors.RED, f"❌ Invalid choice. Please enter 1 or 2")
    ...
    def delete_cluster_autoscaler(self, cluster_name: str, region: str, access_key: str, secret_key: str) -> bool:
        ...
        try:
            ...
            if not kubectl_available:
                self.print_colored(Colors.RED, f"   ❌ kubectl not found. Cannot delete autoscaler")
                return False
            ...
            self.print_colored(Colors.CYAN, f"   🗑️  Deleting cluster autoscaler components...")
                ...
                try:
                    ...
                    if result.returncode == 0:
                        if "deleted" in result.stdout or "not found" in result.stdout:
                            ...
                            deletion_results.append(f"   ✅ {component} (already deleted)")
                    else:
                        deletion_results.append(f"   ⚠️  {component}: {result.stderr.strip()}")
            ...
            if success_count == total_count:
                self.print_colored(Colors.GREEN, f"   ✅ All {total_count} components deleted successfully")
            else:
                self.print_colored(Colors.YELLOW, f"   ⚠️  {success_count}/{total_count} components deleted")
            ...
            if verify_result.returncode == 0:
                ...
                if not remaining_pods:
                    self.print_colored(Colors.GREEN, f"   ✅ No autoscaler pods found - deletion successful")
                    return True
                else:
                    self.print_colored(Colors.YELLOW, f"   ⚠️  {len(remaining_pods)} autoscaler pods still found")
                    return False
            else:
                self.print_colored(Colors.GREEN, f"   ✅ Deletion commands completed (verification skipped)")
                return True
