...
class EnhancedInteractiveClusterManager:
    ...
    def setup_logging(self):
        ...
        log_file = os.path.join(
            log_dir, f"enhanced_cluster_management_{self.execution_timestamp}.log"
        )
    ...
    def enhanced_interactive_cluster_management(self) -> bool:
        ...
        try:
            ...
            self.print_colored(
                Colors.YELLOW, f"Current Date and Time (UTC): {self.current_time}"
            )
            ...
            if not cluster_files:
                self.print_colored(Colors.RED, "❌ No cluster files found")
                return False
            ...
            selected_clusters = self.select_clusters_interactive(
                date_clusters, selected_date
            )
            if not selected_clusters:
                return False

            # Analyze cluster types and determine credential requirements
            credential_requirements = self.analyze_cluster_credential_requirements(
                selected_clusters
            )

            # Select appropriate credentials
            selected_credentials = self.select_credentials_interactive(
            ...
            if operation == "delete_autoscaler":
                return self.delete_autoscalers_with_smart_credentials(
                    selected_clusters, selected_credentials
                )
            elif operation == "configure_auth":
                return self.configure_auth_with_smart_credentials(
                    selected_clusters, selected_credentials
                )
            else:
                self.print_colored(Colors.RED, "❌ Unknown operation selected")
                return False

        except Exception as e:
            self.print_colored(
                Colors.RED,
                f"❌ Enhanced interactive cluster management failed: {str(e)}",
            )
            self.log_operation(
                "ERROR", f"Enhanced interactive cluster management failed: {str(e)}"
            )
            return False

    def analyze_cluster_credential_requirements(
        self, selected_clusters: List[Dict]
    ) -> Dict:
        """
        ...
        try:
            self.print_colored(
                Colors.CYAN, "\n🔍 Analyzing cluster credential requirements..."
            )
            ...
            self.print_colored(
                Colors.CYAN, f"   📊 Root user clusters: {len(root_clusters)}"
            )
            self.print_colored(
                Colors.CYAN, f"   📊 IAM user clusters: {len(iam_clusters)}"
            )
            ...
            if root_clusters:
                ...
                for cluster in root_clusters:
                    self.print_colored(
                        Colors.WHITE, f"      - {cluster['cluster_name']}"
                    )
            ...
            self.print_colored(
                Colors.RED, f"❌ Error analyzing credential requirements: {str(e)}"
            )
            return {
                "root_clusters": [],
                "iam_clusters": [],
                "needs_root_creds": False,
                "needs_iam_creds": False,
            }
    ...
    def extract_account_from_cluster_name(self, cluster_name: str) -> str:
        ...
        try:
            ...
            self.log_operation(
                "ERROR",
                f"Error extracting account from cluster name {cluster_name}: {str(e)}",
            )
            return ""
    ...
    def extract_iam_user_from_cluster_name(self, cluster_name: str) -> str:
        ...
        try:
            ...
            self.log_operation(
                "ERROR",
                f"Error extracting IAM user from cluster name {cluster_name}: {str(e)}",
            )
            return ""
    ...
    def scan_iam_credentials_files(self) -> List[Dict]:
        ...
        try:
            self.print_colored(
                Colors.CYAN, "🔍 Scanning for IAM user credentials files..."
            )
            ...
            if not files:
                self.print_colored(
                    Colors.RED, "❌ No IAM credentials files found in aws/iam/"
                )
                return []
            ...
            for file_path in files:
                try:
                        ...
                        credentials_files.append(
                            {
                                ...
                                "date_str": date_str,
                                "time_str": time_str,
                                "created_date": cred_data.get(
                                    "created_date", "Unknown"
                                ),
                                "created_time": cred_data.get(
                    ...
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️  Error parsing {file_path}: {str(e)}"
                    )

            # Sort by timestamp (newest first)
            credentials_files.sort(
                key=lambda x: f"{x['date_str']}{x['time_str']}", reverse=True
            )

            self.print_colored(
                Colors.GREEN, f"✅ Found {len(credentials_files)} IAM credentials files"
            )
            return credentials_files

        except Exception as e:
            self.print_colored(
                Colors.RED, f"❌ Error scanning IAM credentials files: {str(e)}"
            )
            return []
        ...
        try:
            ...
            if credential_requirements["needs_root_creds"]:
                ...
                try:
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        "✅ Loaded root account credentials from aws_accounts_config.json",
                    )

                except Exception as e:
                    self.print_colored(
                        Colors.RED, f"❌ Failed to load root credentials: {str(e)}"
                    )
                    return None
            ...
            if credential_requirements["needs_iam_creds"]:
                ...
                if not iam_files:
                    self.print_colored(Colors.RED, "❌ No IAM credentials files found")
                    return None

                selected_iam_file = self.select_iam_credentials_file_interactive(
                    iam_files
                )
            ...
            self.print_colored(
                Colors.RED, f"❌ Error in credential selection: {str(e)}"
            )
            return None
    ...
    def select_iam_credentials_file_interactive(self, iam_files: List[Dict]) -> Dict:
        ...
        try:
            self.print_colored(
                Colors.YELLOW,
                f"\n📂 Available IAM credentials files (sorted by timestamp):",
            )
            ...
            while True:
                try:
                    choice = input(
                        f"Select IAM credentials file (1-{len(iam_files)}) [default: 1 (latest)] or 'q' to quit: "
                    ).strip()
                    ...
                    if 1 <= choice_num <= len(iam_files):
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"✅ Selected: {selected_file['filename']} ({selected_file['timestamp']})",
                        )
                        return selected_file
                    else:
                        self.print_colored(
                            Colors.RED,
                            f"❌ Invalid choice. Please enter 1-{len(iam_files)}",
                        )

                except ValueError:
                    self.print_colored(
                        Colors.RED, "❌ Invalid input. Please enter a number"
                    )

        except Exception as e:
            self.print_colored(Colors.RED, f"❌ Error in IAM file selection: {str(e)}")
            return None

    def get_credentials_for_cluster(
        self, cluster_info: Dict, credentials: Dict
    ) -> Tuple[str, str, str]:
        """
        ...
        try:
            ...
            if not account_name:
                self.print_colored(
                    Colors.RED,
                    f"   ❌ Could not extract account from cluster name: {cluster_name}",
                )
            ...
            if self.is_root_created_cluster(cluster_name):
                ...
                if "root_accounts" not in credentials:
                    self.print_colored(
                        Colors.RED,
                        f"   ❌ Root credentials not available for {cluster_name}",
                    )
                    return None, None, None
                ...
                if not account_config:
                    self.print_colored(
                        Colors.RED, f"   ❌ Root account {account_name} not found"
                    )
                    return None, None, None
                ...
                if "iam_users" not in credentials:
                    self.print_colored(
                        Colors.RED,
                        f"   ❌ IAM credentials not available for {cluster_name}",
                    )
                    return None, None, None
                ...
                if not iam_username:
                    self.print_colored(
                        Colors.RED,
                        f"   ❌ Could not extract IAM username from: {cluster_name}",
                    )
                    return None, None, None
                ...
                self.print_colored(
                    Colors.RED,
                    f"   ❌ IAM user {iam_username} not found in credentials",
                )
                return None, None, None

        except Exception as e:
            self.print_colored(
                Colors.RED, f"   ❌ Error getting credentials for cluster: {str(e)}"
            )
            return None, None, None

    def delete_autoscalers_with_smart_credentials(
        self, selected_clusters: List[Dict], credentials: Dict
    ) -> bool:
        """
        ...
        try:
            self.print_colored(
                Colors.YELLOW,
                f"\n🗑️  Starting smart autoscaler deletion for {len(selected_clusters)} clusters",
            )

            # Final confirmation
            confirm = (
                input(
                    f"\nConfirm deletion of autoscaler from {len(selected_clusters)} clusters? (y/N): "
                )
                .strip()
                .lower()
            )
            ...
            for i, cluster_info in enumerate(selected_clusters, 1):
                ...
                self.print_colored(
                    Colors.CYAN, f"   📍 Type: {cluster_type}, Region: {region}"
                )

                try:
                    # Get appropriate credentials
                    access_key, secret_key, account_id = (
                        self.get_credentials_for_cluster(cluster_info, credentials)
                    )

                    if not access_key or not secret_key:
                        self.print_colored(
                            Colors.RED, f"   ❌ Missing credentials for {cluster_name}"
                        )
                        failed_deletions.append(cluster_name)
                        continue

                    # Delete cluster autoscaler
                    success = self.delete_cluster_autoscaler(
                    ...
                    if success:
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Successfully deleted autoscaler from {cluster_name}",
                        )
                    else:
                        failed_deletions.append(cluster_name)
                        self.print_colored(
                            Colors.RED,
                            f"   ❌ Failed to delete autoscaler from {cluster_name}",
                        )

                except Exception as e:
                    self.print_colored(
                        Colors.RED, f"   ❌ Error processing {cluster_name}: {str(e)}"
                    )
                    failed_deletions.append(cluster_name)

            # Print final summary
            self.print_summary(
                "SMART AUTOSCALER DELETION",
                successful_deletions,
                failed_deletions,
            ...
            self.print_colored(
                Colors.RED, f"❌ Error during smart autoscaler deletion: {str(e)}"
            )
            return False

    def configure_auth_with_smart_credentials(
        self, selected_clusters: List[Dict], credentials: Dict
    ) -> bool:
        """
        ...
        try:
            self.print_colored(
                Colors.YELLOW,
                f"\n🔑 Starting ConfigMap-only authentication configuration for {len(selected_clusters)} clusters",
            )
            self.print_colored(
                Colors.YELLOW, f"Mode: ConfigMap updates only (no access entries)"
            )

            # Final confirmation
            confirm = (
                input(
                    f"\nConfirm user authentication configuration for {len(selected_clusters)} clusters? (y/N): "
                )
                .strip()
                .lower()
            )
            ...
            for i, cluster_info in enumerate(selected_clusters, 1):
                ...
                try:
                    ...
                    user_data = self.extract_user_data_from_cluster(
                    ...
                    if success:
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Successfully configured auth for {cluster_name}",
                        )
                    else:
                        failed_configs.append(cluster_name)
                        self.print_colored(
                            Colors.RED,
                            f"   ❌ Failed to configure auth for {cluster_name}",
                        )
            ...
            # Print final summary
            self.print_summary(
                "CONFIGMAP-ONLY USER AUTHENTICATION",
                successful_configs,
                failed_configs,
                len(selected_clusters),
            )
            ...
            self.print_colored(
                Colors.RED,
                f"❌ Error during ConfigMap-only auth configuration: {str(e)}",
            )
            return False

    def extract_user_data_from_cluster(
        self, cluster_data: Dict, cluster_name: str
    ) -> Dict:
        """
        ...
        try:
            ...
            if username == "unknown" and not self.is_root_created_cluster(cluster_name):
                ...
                extracted_username = self.extract_iam_user_from_cluster_name(
                    cluster_name
                )
    ...
    def configure_aws_auth_configmap_simplified(
        self,
        cluster_name: str,
        region: str,
        account_id: str,
        ...
        try:
            ...
            self.print_colored(
                Colors.CYAN, f"   🔐 Configuring aws-auth ConfigMap (ConfigMap only)..."
            )
            ...
            if is_root_cluster:
                ...
                self.print_colored(
                    Colors.CYAN,
                    f"   👑 Root-created cluster - configuring root access only",
                )
            ...
            return self.apply_configmap_only(
                cluster_name,
                region,
                account_id,
                users_to_add,
    ...
    def apply_configmap_only(
        ...
        try:
            self.print_colored(
                Colors.CYAN,
                "   📋 Creating/updating aws-auth ConfigMap (ConfigMap only mode)...",
            )
            ...
            if not kubectl_available:
                self.print_colored(
                    Colors.RED, f"   ❌ kubectl not found. ConfigMap setup failed."
                )
                return False
            ...
            try:
                ...
                self.print_colored(
                    Colors.RED, f"   ❌ Failed to create ConfigMap file: {str(e)}"
                )
                return False
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ Kubeconfig updated successfully"
                )
                ...
                self.print_colored(
                    Colors.CYAN, f"   📋 Applying ConfigMap (standard apply)..."
                )
                apply_cmd = [
                    "kubectl",
                    "apply",
                ...
                apply_result = subprocess.run(
                    apply_cmd, env=env, capture_output=True, text=True, timeout=300
                )
                ...
                if apply_result.returncode == 0:
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  Standard apply failed: {apply_result.stderr.strip()}",
                    )

                    # Strategy 2: Force replace
                    self.print_colored(Colors.CYAN, f"   📋 Trying force replace...")
                    replace_cmd = [
                        "kubectl",
                        "replace",
                        "-f",
                    ...
                    if replace_result.returncode == 0:
                        self.print_colored(
                            Colors.GREEN, f"   ✅ ConfigMap replaced successfully"
                        )
                        success = True
                        ...
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️  Force replace failed: {replace_result.stderr.strip()}",
                        )

                        # Strategy 3: Delete and recreate
                        self.print_colored(
                            Colors.CYAN, f"   📋 Trying delete and recreate..."
                        )
                        ...
                        if delete_result.returncode == 0:
                            self.print_colored(
                                Colors.CYAN, f"   🗑️  Existing ConfigMap deleted"
                            )
                        ...
                        if create_result.returncode == 0:
                            self.print_colored(
                                Colors.GREEN, f"   ✅ ConfigMap recreated successfully"
                            )
                            success = True
                            ...
                            self.print_colored(
                                Colors.RED,
                                f"   ❌ All ConfigMap strategies failed: {create_result.stderr}",
                            )
                ...
                if success:
                    self.print_colored(
                        Colors.CYAN,
                        f"   🔍 Verifying ConfigMap (applied via {applied_method})...",
                    )
                    ...
                    try:
                        ...
                        if verify_result1.returncode == 0:
                            self.print_colored(
                                Colors.GREEN,
                                f"   ✅ ConfigMap exists and is accessible",
                            )
                            verification_passed = True
                        else:
                            self.log_operation(
                                "DEBUG",
                                f"Method 1 verification failed: {verify_result1.stderr}",
                            )
                    except Exception as e1:
                        self.log_operation(
                            "DEBUG", f"Method 1 verification error: {str(e1)}"
                        )
                    ...
                    if not verification_passed:
                        try:
                            ...
                            if (
                                verify_result2.returncode == 0
                                and "aws-auth" in verify_result2.stdout
                            ):
                                self.print_colored(
                                    Colors.GREEN,
                                    f"   ✅ ConfigMap found in namespace listing",
                                )
                                verification_passed = True
                            else:
                                self.log_operation(
                                    "DEBUG",
                                    f"Method 2 verification failed or aws-auth not found",
                                )
                        except Exception as e2:
                            self.log_operation(
                                "DEBUG", f"Method 2 verification error: {str(e2)}"
                            )
                    ...
                    if verification_passed:
                        try:
                            ...
                            if (
                                content_result.returncode == 0
                                and content_result.stdout.strip()
                            ):
                                ...
                                try:
                                    ...
                                    self.print_colored(
                                        Colors.GREEN,
                                        f"   ✅ ConfigMap contains {user_count} user mappings",
                                    )

                                    # Show the users for confirmation
                                    self.print_colored(
                                        Colors.CYAN, f"   📋 Configured user mappings:"
                                    )
                                    for user in users_list[:3]:  # Show first 3 users
                                        ...
                                        if len(userarn) > 60:
                                            userarn = (
                                                userarn[:30] + "..." + userarn[-30:]
                                            )
                                        self.print_colored(
                                            Colors.WHITE, f"      - {username}"
                                        )
                                    ...
                                    self.print_colored(
                                        Colors.GREEN,
                                        f"   ✅ ConfigMap contains user mappings",
                                    )
                                    self.log_operation(
                                        "DEBUG",
                                        f"User mapping parse error: {str(parse_e)}",
                                    )
                            else:
                                self.print_colored(
                                    Colors.YELLOW,
                                    f"   ⚠️  ConfigMap exists but content verification failed",
                                )

                        except Exception as content_e:
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️  Could not verify ConfigMap content: {str(content_e)}",
                            )
                    ...
                    if verification_passed:
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ ConfigMap verification completed successfully",
                        )
                    else:
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️  ConfigMap was applied but verification had issues (this is often normal)",
                        )
                        self.print_colored(
                            Colors.CYAN,
                            f"   💡 You can manually verify with: kubectl get configmap aws-auth -n kube-system",
                        )
                    ...
                    if is_root_cluster:
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Root user configured for cluster access (ConfigMap only)",
                        )
                    else:
                        username = user_data.get("username", "unknown")
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ User [{username}] and root user configured for cluster access (ConfigMap only)",
                        )

                    # Show useful commands
                    self.print_colored(Colors.CYAN, f"   💡 Test access with:")
                    self.print_colored(Colors.WHITE, f"      kubectl get nodes")
                    self.print_colored(
                        Colors.WHITE, f"      kubectl get pods --all-namespaces"
                    )

                    return True
                else:
                    self.print_colored(
                        Colors.RED, f"   ❌ All ConfigMap application strategies failed"
                    )
                    return False

            except subprocess.TimeoutExpired:
                self.print_colored(Colors.RED, f"   ❌ kubectl/aws command timed out")
                return False
            except Exception as e:
                self.print_colored(
            ...
            self.print_colored(
                Colors.RED, f"   ❌ ConfigMap application failed: {str(e)}"
            )
            return False

    def configure_aws_auth_configmap_enhanced(
        self,
        cluster_name: str,
        region: str,
        account_id: str,
        ...
        try:
            ...
            self.print_colored(Colors.CYAN, f"   🔐 Configuring aws-auth ConfigMap...")
            ...
            if auth_mode in ["API", "API_AND_CONFIG_MAP"]:
                self.print_colored(
                    Colors.CYAN,
                    "   🔑 Creating access entries for API-based authentication...",
                )
                ...
                for principal_arn in principals_to_add:
                    try:
                        ...
                        principal_type = (
                            ...
                            else f"IAM user ({user_data.get('username', 'unknown')})"
                        )
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Created access entry for {principal_type}",
                        )
                        success_count += 1
                        ...
                        if "already exists" in str(e).lower():
                            self.print_colored(
                                Colors.CYAN,
                                f"   ℹ️  Access entry already exists for {principal_type}",
                            )
                            success_count += 1
                        else:
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️  Failed to create access entry for {principal_type}: {str(e)}",
                            )
                ...
                if success_count > 0:
                    if is_root_cluster:
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Root user configured for cluster access via API",
                        )
                    else:
                        username = user_data.get("username", "unknown")
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ User [{username}] and root user configured for cluster access via API",
                        )
            ...
            if auth_mode in ["CONFIG_MAP", "API_AND_CONFIG_MAP"]:
                return self.apply_configmap_with_kubectl(
                    cluster_name,
                    region,
                    account_id,
                    users_to_add,
    ...
    def apply_configmap_with_kubectl(
        self,
        cluster_name: str,
        region: str,
        account_id: str,
        ...
        try:
            ...
            if not kubectl_available:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️  kubectl not found. ConfigMap setup skipped."
                )
                return True
            ...
            try:
                ...
                self.print_colored(
                    Colors.CYAN, f"   🔍 Testing cluster connectivity..."
                )
                ...
                if test_result.returncode != 0:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  Cluster connectivity test failed: {test_result.stderr}",
                    )
                    self.print_colored(
                        Colors.CYAN,
                        f"   🔄 Attempting to authenticate with current credentials...",
                    )
                    ...
                    try:
                        ...
                        if caller_arn:
                            self.print_colored(
                                Colors.CYAN,
                                f"   🔐 Creating temporary access entry for admin: {caller_arn}",
                            )
                            ...
                            try:
                                ...
                                self.print_colored(
                                    Colors.GREEN,
                                    f"   ✅ Temporary admin access entry created",
                                )
                                ...
                                if test_result2.returncode == 0:
                                    self.print_colored(
                                        Colors.GREEN,
                                        f"   ✅ Cluster connectivity established",
                                    )
                                else:
                                    self.print_colored(
                                        Colors.YELLOW,
                                        f"   ⚠️  Still having connectivity issues, proceeding anyway...",
                                    )
                                ...
                                if "already exists" in str(access_e).lower():
                                    self.print_colored(
                                        Colors.CYAN,
                                        f"   ℹ️  Admin access entry already exists",
                                    )
                                else:
                                    self.print_colored(
                                        Colors.YELLOW,
                                        f"   ⚠️  Could not create admin access entry: {str(access_e)}",
                                    )

                    except Exception as auth_e:
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️  Authentication setup failed: {str(auth_e)}",
                        )
                else:
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Cluster connectivity confirmed"
                    )
                ...
                for i, strategy in enumerate(apply_strategies, 1):
                    self.print_colored(
                        Colors.CYAN,
                        f"   📋 Applying ConfigMap using strategy {i}: {strategy['name']}...",
                    )
                    ...
                    if result.returncode == 0:
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ ConfigMap applied successfully using {strategy['name']}",
                        )
                        success = True
                        break
                    else:
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️  {strategy['name']} failed: {result.stderr.strip()}",
                        )
                ...
                if not success:
                    self.print_colored(
                        Colors.CYAN, f"   🔄 Trying delete and recreate strategy..."
                    )
                    ...
                    try:
                        ...
                        if delete_result.returncode == 0:
                            ...
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️  Delete result: {delete_result.stderr}",
                            )
                        ...
                        if create_result.returncode == 0:
                            ...
                            self.print_colored(
                                Colors.RED,
                                f"   ❌ ConfigMap recreation failed: {create_result.stderr}",
                            )

                            # Last resort: try server-side apply (kubectl 1.18+)
                            self.print_colored(
                                Colors.CYAN,
                                f"   🔄 Trying server-side apply as last resort...",
                            )
                            ...
                            server_side_result = subprocess.run(
                                server_side_cmd,
                                env=env,
                                capture_output=True,
                                text=True,
                            ...
                            if server_side_result.returncode == 0:
                                self.print_colored(
                                    Colors.GREEN,
                                    f"   ✅ ConfigMap applied using server-side apply",
                                )
                                success = True
                            else:
                                self.print_colored(
                                    Colors.RED,
                                    f"   ❌ Server-side apply also failed: {server_side_result.stderr}",
                                )

                    except Exception as recreate_e:
                        self.print_colored(
                            Colors.RED,
                            f"   ❌ Error during recreate strategy: {str(recreate_e)}",
                        )

                # Verify ConfigMap was applied (but don't fail if verification fails)
                if success:
                    try:
                        self.print_colored(Colors.CYAN, f"   🔍 Verifying ConfigMap...")
                        verify_cmd = [
                            "kubectl",
                            "get",
                            "configmap",
                        ...
                        if (
                            ...
                            and "aws-auth" in verify_result.stdout
                        ):
                            self.print_colored(
                                Colors.GREEN, f"   ✅ ConfigMap verification successful"
                            )
                            ...
                            if content_result.returncode == 0:
                                ...
                                try:
                                    ...
                                    configmap_content = yaml_parser.safe_load(
                                        content_result.stdout
                                    )
                                    map_users = configmap_content.get("data", {}).get(
                                        "mapUsers", ""
                                    )
                                    if map_users:
                                        ...
                                        self.print_colored(
                                            Colors.YELLOW,
                                            f"   ⚠️  ConfigMap exists but no user mappings found",
                                        )
                                except Exception as parse_e:
                                    self.print_colored(
                                        Colors.YELLOW,
                                        f"   ⚠️  Could not parse ConfigMap content: {str(parse_e)}",
                                    )

                        else:
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️  ConfigMap verification failed, but apply reported success",
                            )

                    except Exception as verify_e:
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️  ConfigMap verification error: {str(verify_e)}",
                        )
                ...
                if success:
                    if is_root_cluster:
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Root user configured for cluster access",
                        )
                    else:
                        username = user_data.get("username", "unknown")
                        self.print_colored(
    ...
    def scan_cluster_files(self) -> List[Dict]:
        ...
        try:
            ...
            self.print_colored(Colors.CYAN, "🔍 Scanning for cluster files...")
            ...
            for base_dir in base_dirs:
                ...
                self.print_colored(
                    Colors.CYAN, f"   📁 {account_id}: Found {len(files)} cluster files"
                )
            ...
            self.print_colored(
                Colors.GREEN, f"✅ Total cluster files found: {len(cluster_files)}"
            )
            return cluster_files

        except Exception as e:
            self.print_colored(Colors.RED, f"❌ Error scanning cluster files: {str(e)}")
            return []
    ...
    def parse_cluster_file_info(self, file_path: str, account_id: str) -> Dict:
        ...
        try:
            ...
            cluster_name = cluster_data.get("cluster_info", {}).get(
            ...
            account_name = cluster_data.get("account_info", {}).get(
            ...
            self.log_operation(
                "ERROR", f"Error parsing cluster file {file_path}: {str(e)}"
            )
            return None
    ...
    def group_clusters_by_date(self, cluster_files: List[Dict]) -> Dict:
        ...
        try:
            ...
            self.print_colored(
                Colors.RED, f"❌ Error grouping clusters by date: {str(e)}"
            )
            return {}
    ...
    def select_date_interactive(self, clusters_by_date: Dict) -> str:
        ...
        try:
            ...
            for i, date in enumerate(dates, 1):
                ...
                accounts = set(
                    cluster["account_id"] for cluster in clusters_by_date[date]
                )
                account_summary = ", ".join(sorted(accounts))

                self.print_colored(
                    Colors.CYAN,
                    f"   {i}. {date} - {cluster_count} clusters ({account_summary})",
                )
            ...
            while True:
                try:
                    choice = input(
                        f"Select date (1-{len(dates)}) or 'q' to quit: "
                    ).strip()
                    ...
                    if 1 <= choice_num <= len(dates):
                        ...
                        self.print_colored(
                            Colors.GREEN, f"✅ Selected date: {selected_date}"
                        )
                        return selected_date
                    else:
                        self.print_colored(
                            Colors.RED,
                            f"❌ Invalid choice. Please enter 1-{len(dates)}",
                        )
            ...
            self.print_colored(Colors.RED, f"❌ Error in date selection: {str(e)}")
            return None

    def select_clusters_interactive(
        self, date_clusters: List[Dict], selected_date: str
    ) -> List[Dict]:
        """Interactive cluster selection for a specific date"""
        try:
            self.print_colored(
                Colors.YELLOW, f"\n🎯 Clusters available for {selected_date}:"
            )
            self.print_colored(Colors.YELLOW, f"=" * 80)
            ...
            for i, cluster in enumerate(date_clusters, 1):
                ...
                self.print_colored(
                    Colors.CYAN, f"   {i:2d}. {cluster_name} {cluster_type}"
                )
                self.print_colored(
                    Colors.WHITE,
            ...
            while True:
                try:
                    ...
                    if selection.lower() == "all":
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"✅ Selected all {len(selected_clusters)} clusters",
                        )
                        return selected_clusters

                    selected_indices = self.parse_cluster_selection(
                        selection, len(date_clusters)
                    )

                    if not selected_indices:
                        self.print_colored(
                            Colors.RED, "❌ Invalid selection. Please try again."
                        )
                        continue
                    ...
                    self.print_colored(
                        Colors.GREEN, f"✅ Selected {len(selected_clusters)} clusters:"
                    )
                    for cluster in selected_clusters:
                        cluster_type = (
                            "👑 Root"
                            if self.is_root_created_cluster(cluster["cluster_name"])
                            else "👤 IAM"
                        )
                        self.print_colored(
                            Colors.GREEN,
            ...
            self.print_colored(Colors.RED, f"❌ Error in cluster selection: {str(e)}")
            return None
    ...
    def parse_cluster_selection(self, selection: str, max_count: int) -> List[int]:
        ...
        try:
            ...
            for part in parts:
                if "-" in part:
                    try:
                        ...
                        if (
                            1 <= start_idx <= max_count
                            and 1 <= end_idx <= max_count
            ...
            self.log_operation(
                "ERROR", f"Error parsing selection '{selection}': {str(e)}"
            )
            return None
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if choice == "1":
                        self.print_colored(
                            Colors.GREEN, f"✅ Selected: Delete Cluster Autoscaler"
                        )
                        return "delete_autoscaler"
                    elif choice == "2":
                        self.print_colored(
                            Colors.GREEN, f"✅ Selected: Configure User Authentication"
                        )
                        return "configure_auth"
                    else:
                        self.print_colored(
                            Colors.RED, f"❌ Invalid choice. Please enter 1 or 2"
                        )
            ...
            self.print_colored(Colors.RED, f"❌ Error in operation selection: {str(e)}")
            return None

    def delete_cluster_autoscaler(
        ...
        try:
            self.log_operation(
                "INFO", f"Deleting cluster autoscaler from {cluster_name}"
            )
            ...
            if not kubectl_available:
                self.print_colored(
                    Colors.RED, f"   ❌ kubectl not found. Cannot delete autoscaler"
                )
                return False
            ...
            self.print_colored(
                Colors.CYAN, f"   🔄 Updating kubeconfig for {cluster_name}..."
            )
            update_cmd = [
                "aws",
                "eks",
            ...
            self.print_colored(
                Colors.CYAN, f"   🗑️  Deleting cluster autoscaler components..."
            )
            ...
            delete_commands = [
                ...
                [
                    ...
                    "delete",
                    "secret",
                    "cluster-autoscaler-aws-credentials",
                    "-n",
                    "kube-system",
                    "--ignore-not-found",
                ],
                ...
                try:
                    ...
                    if result.returncode == 0:
                        if "deleted" in result.stdout or "not found" in result.stdout:
                            deletion_results.append(f"   ✅ {component}")
                        else:
                            deletion_results.append(
                                f"   ✅ {component} (already deleted)"
                            )
                    else:
                        deletion_results.append(
                            f"   ⚠️  {component}: {result.stderr.strip()}"
                        )

                except subprocess.TimeoutExpired:
                    deletion_results.append(f"   ❌ {component}: Timeout")
                except Exception as e:
                    deletion_results.append(f"   ❌ {component}: {str(e)}")

            success_count = len([r for r in deletion_results if "✅" in r])
            total_count = len(deletion_results)

            if success_count == total_count:
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ All {total_count} components deleted successfully",
                )
            else:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️  {success_count}/{total_count} components deleted",
                )

            self.print_colored(Colors.CYAN, f"   🔍 Verifying autoscaler deletion...")
            ...
            if verify_result.returncode == 0:
                ...
                if not remaining_pods:
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ No autoscaler pods found - deletion successful",
                    )
                    return True
                else:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  {len(remaining_pods)} autoscaler pods still found",
                    )
                    return False
            else:
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Deletion commands completed (verification skipped)",
                )
                return True

        except Exception as e:
            self.print_colored(Colors.RED, f"   ❌ Error deleting autoscaler: {str(e)}")
            self.log_operation(
                "ERROR",
                f"Failed to delete cluster autoscaler from {cluster_name}: {str(e)}",
            )
            return False

    def print_summary(
        self, operation_name: str, successful: List[str], failed: List[str], total: int
    ):
        """Print operation summary"""
        self.print_colored(Colors.YELLOW, f"\n📊 {operation_name} SUMMARY")
        self.print_colored(Colors.YELLOW, f"=" * 60)
        self.print_colored(Colors.GREEN, f"✅ Successful: {len(successful)}/{total}")
        self.print_colored(Colors.RED, f"❌ Failed: {len(failed)}/{total}")

        if successful:
            self.print_colored(Colors.GREEN, f"\n✅ Successfully processed clusters:")
...
def main():
    ...
    if success:
        print("\n🎉 Enhanced interactive cluster management completed successfully")
    else:
        print("\n❌ Enhanced interactive cluster management failed or was cancelled")
