...
class IAMUserManager:
    ...
    def load_user_mapping(self):
        ...
        try:
            if not os.path.exists(self.mapping_file):
                self.logger.warning(
                    f"User mapping file '{self.mapping_file}' not found"
                )
                self.user_mappings = {}
                return
    ...
    def analyze_account_coverage(self):
        ...
        self.logger.info(
            f"Accounts with user mappings: {sorted(accounts_with_mappings)}"
        )
        if accounts_without_mappings:
            self.logger.warning(
                f"Accounts without user mappings: {sorted(accounts_without_mappings)}"
            )
    ...
    def create_iam_client(self, account_name):
        ...
        try:
            ...
            # Test the connection
            iam_client.get_user()
            self.logger.log_account_action(
                account_name,
                "CONNECT",
                "SUCCESS",
    ...
    def create_or_get_group(self, iam_client, account_name):
        ...
        try:
            ...
            if not self.check_group_has_policy(
                iam_client, group_name, admin_policy_arn
            ):
                iam_client.attach_group_policy(
                    GroupName=group_name, PolicyArn=admin_policy_arn
                )
                self.logger.log_account_action(
                    account_name,
                    "ATTACH_POLICY",
            ...
            for page in paginator.paginate(GroupName=group_name):
                for policy in page["AttachedPolicies"]:
                    if policy["PolicyArn"] == policy_arn:
                        self.logger.debug(
                            f"Group {group_name} already has policy {policy_arn}"
                        )
                        return True
    ...
    def create_single_user(
        self, iam_client, username, region, account_config, group_name
    ):
        ...
        try:
            ...
            self.logger.log_user_action(
                username, "ADD_TO_GROUP", "SUCCESS", f"Added to group {group_name}"
            )
            ...
            self.logger.log_user_action(
                username, "CREATE_RESTRICTION_POLICY", "SUCCESS", f"Region: {region}"
            )
            ...
            self.logger.log_user_action(
                username, "CREATE_ACCESS_KEY", "SUCCESS", f"Key ID: {access_key}"
            )
    ...
    def verify_user_permissions(self, iam_client, username, region):
        ...
        try:
            ...
            self.logger.debug(
                f"User {username} belongs to {len(groups_response['Groups'])} groups"
            )

            for group in groups_response["Groups"]:
                self.logger.debug(
                    f"User {username} is member of group: {group['GroupName']}"
                )
            ...
            try:
                    ...
                    if statement.get("Sid") == "DenyIfNotInRegion":
                        ...
                        if isinstance(policy_region, list):
                            ...
                            if region in policy_region:
                                self.logger.debug(
                                    f"User {username} has correct region restriction: {region}"
                                )
                            else:
                                self.logger.warning(
                            ...
                            self.logger.warning(
                                f"User {username} has no region restriction policy found"
                            )
                        else:
                            self.logger.warning(
                ...
                if e.response["Error"]["Code"] == "NoSuchEntity":
                    self.logger.warning(
                        f"User {username} does not have a region restriction policy"
                    )
                else:
                    self.logger.error(
                        f"Error checking region policy for {username}: {e}"
                    )
            ...
            self.logger.log_user_action(
                ...
                "VERIFY_PERMISSIONS",
                "SUCCESS",
                f"User has expected group memberships for region {region}",
            )
    ...
    def ensure_user_in_group(self, iam_client, username, group_name, account_name):
        ...
        try:
            ...
            groups_response = iam_client.list_groups_for_user(UserName=username)
            is_in_group = any(
                g["GroupName"] == group_name for g in groups_response["Groups"]
            )

            if not is_in_group:
                self.logger.info(
                    f"Adding existing user {username} to group {group_name}"
                )
            ...
            self.logger.error(
                f"Failed to add user {username} to group {group_name}: {e}"
            )
            raise
    ...
    def create_users_in_account(self, account_name):
        ...
        self.logger.info(
            f"Found {len(users_regions)} mapped users for account {account_name}"
        )
        ...
        try:
            ...
            self.logger.error(
                f"Failed to connect to {account_name} or create group: {e}"
            )
            return [], [], []
        ...
        for username, region in users_regions.items():
            ...
            try:
                if self.check_user_exists(iam_client, username):
                    self.logger.log_user_action(
                        username, "SKIP", "ALREADY_EXISTS", user_info["full_name"]
                    )

                    # Try to add existing user to group if they're not already a member
                    try:
                        self.ensure_user_in_group(
                            iam_client, username, group_name, account_name
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to add existing user {username} to group: {e}"
                        )
        ...
        users_to_create = {
            k: v
            for k, v in users_regions.items()
            if k not in [u["username"] for u in skipped_users] and k not in failed_users
        }
        ...
        for username, region in users_to_create.items():
            ...
            self.logger.info(
                f"Creating user: {username} → {user_info['full_name']} (Region: {region})"
            )

            try:
                user_data = self.create_single_user(
                    iam_client, username, region, account_config, group_name
                )
    ...
    def display_account_menu(self):
        ...
        for i, (account_name, config) in enumerate(self.aws_accounts.items(), 1):
            ...
            if user_count > 0:
                print(
                    f"  {i}. {account_name} ({config['account_id']}) - {config['email']} [{user_count} users mapped]"
                )
                accounts_with_mappings.append(account_name)
            else:
                print(
                    f"  {i}. {account_name} ({config['account_id']}) - {config['email']} [⚠️  NO USERS MAPPED]"
                )
                accounts_without_mappings.append(account_name)

        print(
            f"  {len(self.aws_accounts) + 1}. All accounts with mappings ({len(accounts_with_mappings)} accounts)"
        )
        ...
        if accounts_without_mappings:
            ...
            for account in accounts_without_mappings:
                print(f"     - {account}")
            print(
                "   These accounts will be skipped unless you explicitly select them."
            )
            ...
            try:
                ...
                if choice == str(len(self.aws_accounts) + 1):
                    ...
                    if accounts_without_mappings:
                        confirm = (
                            input(
                                f"⚠️  This includes {len(accounts_without_mappings)} accounts with no mappings. Continue? (y/N): "
                            )
                            .lower()
                            .strip()
                        )
                ...
                invalid_selections = [
                    num
                    for num in selected_accounts
                    if num < 1 or num > len(self.aws_accounts)
                ]
                ...
                selected_account_names = [
                    account_names[num - 1] for num in selected_accounts
                ]
                ...
                unmapped_in_selection = [
                    acc
                    for acc in selected_account_names
                    if acc in accounts_without_mappings
                ]
    ...
    def save_credentials_to_file(self, all_created_users):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Excel file created with correct column order: {excel_path}"
                )
                print(f"📊 Excel file created: {excel_path}")
                print(
                    f"📋 Columns: firstname, lastname, mail id, username, password, loginurl, homeregion, accesskey, secretkey"
                )
    ...
    def run(self):
        ...
        for account_name in accounts_to_process:
            created_users, skipped_users, failed_users = self.create_users_in_account(
                account_name
            )
        ...
        # Log final summary
        total_processed = (
            len(all_created_users) + len(all_skipped_users) + len(all_failed_users)
        )
        self.logger.log_summary(
            total_processed,
            len(all_created_users),
            len(all_failed_users),
            len(all_skipped_users),
        )
