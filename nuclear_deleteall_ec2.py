...
class UltraEC2CleanupManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{log_dir}/nuclear_ec2_cleanup_log_{self.execution_timestamp}.log"
            )
    ...
    def get_all_instances_in_region(self, ec2_client, region, account_name):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🔍 Scanning for instances in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "INFO",
                f"📦 Found {len(instances)} instances in {region} ({account_name})",
            )
            ...
            self.log_operation(
                "ERROR", f"Error getting instances in {region} ({account_name}): {e}"
            )
            return []
    ...
    def get_all_security_groups_in_region(self, ec2_client, region, account_name):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🔍 Scanning for security groups in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "INFO",
                f"🛡️  Found {len(security_groups)} security groups in {region} ({account_name})",
            )
    ...
    def correlate_instances_and_security_groups(self, instances, security_groups):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"   📎 Attached to instances: {len(attached_sgs)}"
            )
            ...
            self.log_operation(
                "ERROR", f"Error correlating instances and security groups: {e}"
            )
            return [], []
    ...
    def terminate_instance(self, ec2_client, instance_info):
        ...
        try:
            ...
            if current_state in ["terminated", "terminating"]:
                self.log_operation(
                    "INFO", f"Instance {instance_id} already {current_state}"
                )
            ...
            self.log_operation(
                "INFO",
                f"🗑️  Terminating instance {instance_id} in {region} ({account_name})",
            )
            ...
            self.log_operation(
                "INFO",
                f"✅ Instance {instance_id} termination initiated: {previous_state} → {current_state}",
            )
            ...
            self.log_operation(
                "ERROR", f"Failed to terminate instance {instance_id}: {e}"
            )
        ...
        try:
            ...
            try:
                ...
                if e.response["Error"]["Code"] == "InvalidGroupId.NotFound":
                    self.log_operation(
                        "INFO",
                        f"Security group {sg_id} does not exist, skipping rule clearing",
                    )
                    return True
                else:
                    raise
            ...
            if ingress_rules:
                self.log_operation(
                    "INFO",
                    f"Removing {len(ingress_rules)} ingress rules from {sg_id} ({sg_name})",
                )
                ...
                for rule_index, rule in enumerate(ingress_rules):
                    try:
                        ...
                        if sg_references:
                            ref_groups = [
                                ref.get("GroupId", "unknown") for ref in sg_references
                            ]
                            self.log_operation(
                                "INFO",
                                f"  Rule {rule_index + 1}: {protocol}:{from_port}-{to_port} references SGs: {ref_groups}",
                            )
                        else:
                            self.log_operation(
                                "INFO",
                                f"  Rule {rule_index + 1}: {protocol}:{from_port}-{to_port} from IPs: {[ip.get('CidrIp', 'unknown') for ip in ip_ranges]}",
                            )
                        ...
                        self.log_operation(
                            "INFO",
                            f"  ✅ Successfully removed ingress rule {rule_index + 1}",
                        )
                        ...
                        if error_code == "InvalidGroupId.NotFound":
                            self.log_operation(
                                "INFO", f"  Security group {sg_id} no longer exists"
                            )
                            return True
                        elif error_code == "InvalidPermission.NotFound":
                            self.log_operation(
                                "INFO",
                                f"  Ingress rule {rule_index + 1} already removed",
                            )
                            rules_cleared += 1
                        else:
                            self.log_operation(
                                "ERROR",
                                f"  ❌ Failed to remove ingress rule {rule_index + 1}: {e}",
                            )
                            rules_failed += 1
                    except Exception as e:
                        self.log_operation(
                            "ERROR",
                            f"  ❌ Unexpected error removing ingress rule {rule_index + 1}: {e}",
                        )
                        rules_failed += 1
            ...
            if egress_rules:
                ...
                for rule in egress_rules:
                    ...
                    is_default = (
                        ...
                        and len(rule.get("IpRanges", [])) == 1
                        and rule.get("IpRanges", [{}])[0].get("CidrIp") == "0.0.0.0/0"
                ...
                if non_default_egress:
                    ...
                    for rule_index, rule in enumerate(non_default_egress):
                        try:
                            ...
                            if sg_references:
                                ref_groups = [
                                    ref.get("GroupId", "unknown")
                                    for ref in sg_references
                                ]
                                self.log_operation(
                                    "INFO",
                                    f"  Egress Rule {rule_index + 1}: {protocol}:{from_port}-{to_port} references SGs: {ref_groups}",
                                )
                            else:
                                self.log_operation(
                                    "INFO",
                                    f"  Egress Rule {rule_index + 1}: {protocol}:{from_port}-{to_port} to IPs: {[ip.get('CidrIp', 'unknown') for ip in ip_ranges]}",
                                )
                            ...
                            self.log_operation(
                                "INFO",
                                f"  ✅ Successfully removed egress rule {rule_index + 1}",
                            )
                            ...
                            if error_code == "InvalidGroupId.NotFound":
                                ...
                                self.log_operation(
                                    "INFO",
                                    f"  Egress rule {rule_index + 1} already removed",
                                )
                                rules_cleared += 1
                            else:
                                self.log_operation(
                                    "ERROR",
                                    f"  ❌ Failed to remove egress rule {rule_index + 1}: {e}",
                                )
                                rules_failed += 1
                        except Exception as e:
                            self.log_operation(
                    ...
                    self.log_operation(
                        "INFO", f"No non-default egress rules to remove from {sg_id}"
                    )
            ...
            if total_rules == 0:
                ...
                self.log_operation(
                    "INFO",
                    f"Rule clearing summary for {sg_id}: {rules_cleared} cleared, {rules_failed} failed",
                )

            # Wait briefly for rule changes to propagate
            if rules_cleared > 0:
                self.log_operation(
                    "INFO", f"Waiting 10 seconds for rule changes to propagate..."
                )
            ...
            self.log_operation(
                "ERROR",
                f"Unexpected error clearing rules for security group {sg_id}: {e}",
            )
            return False
    ...
    def delete_security_group(self, ec2_client, sg_info, force_delete=False):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🗑️  Deleting security group {sg_id} ({sg_name}) in {region} ({account_name})",
            )
            ...
            if sg_info["is_attached"] and force_delete:
                self.log_operation(
                    "INFO",
                    f"Security group {sg_id} is attached to instances, waiting for termination...",
                )
                time.sleep(30)  # Wait for instance termination

            # Step 1: Clear all security group rules first
            self.log_operation(
                "INFO", f"Step 1: Clearing security group rules for {sg_id}"
            )
            ...
            self.log_operation(
                "INFO", f"Step 2: Attempting to delete security group {sg_id}"
            )
            ec2_client.delete_security_group(GroupId=sg_id)

            self.log_operation(
                "INFO", f"✅ Successfully deleted security group {sg_id} ({sg_name})"
            )
            ...
            self.log_operation(
                "ERROR", f"Unexpected error deleting security group {sg_id}: {e}"
            )
            self.cleanup_results["failed_deletions"].append(
                {
                    "resource_type": "security_group",
    ...
    def cleanup_account_region(self, account_name, account_data, region):
        ...
        try:
            ...
            try:
                # Get all instances
                instances = self.get_all_instances_in_region(
                    ec2_client, region, account_name
                )

                # Get all security groups
                security_groups = self.get_all_security_groups_in_region(
                    ec2_client, region, account_name
                )

                # Correlate instances and security groups
                attached_sgs, unattached_sgs = (
                    self.correlate_instances_and_security_groups(
                ...
                self.log_operation(
                    "ERROR",
                    f"Error during resource discovery in {account_name} ({region}): {discovery_error}",
                )
            ...
            self.log_operation(
                "INFO", f"   🛡️  Total Security Groups: {len(security_groups)}"
            )
            ...
            if not instances and not security_groups:
                self.log_operation(
                    "INFO", f"No resources found in {account_name} ({region})"
                )
                return True
            ...
            if instances:
                self.log_operation(
                    "INFO",
                    f"🗑️  Terminating {len(instances)} instances in {account_name} ({region})",
                )
                ...
                for instance in instances:
                    try:
                        ...
                        self.log_operation(
                            "ERROR",
                            f"Error terminating instance {instance['instance_id']}: {e}",
                        )
                ...
                if attached_sgs:
                    self.log_operation(
                        "INFO",
                        f"⏳ Waiting 60 seconds for instances to start terminating...",
                    )
                    time.sleep(60)
            ...
            if unattached_sgs:
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting {len(unattached_sgs)} unattached security groups in {account_name} ({region})",
                )
                ...
                for sg in unattached_sgs:
                    try:
                        ...
                        self.log_operation(
                            "ERROR",
                            f"Error deleting unattached security group {sg['group_id']}: {e}",
                        )
            ...
            if attached_sgs:
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting {len(attached_sgs)} attached security groups in {account_name} ({region})",
                )
                ...
                for retry in range(max_retries):
                    self.log_operation(
                        "INFO",
                        f"🔄 Security group deletion attempt {retry + 1}/{max_retries}",
                    )
                    ...
                    for sg in remaining_sgs:
                        try:
                            success = self.delete_security_group(
                                ec2_client, sg, force_delete=True
                            )
                            if success:
                                sgs_deleted_this_round += 1
                                self.log_operation(
                                    "INFO",
                                    f"✅ Deleted {sg['group_id']} in attempt {retry + 1}",
                                )
                            else:
                                still_remaining.append(sg)
                                self.log_operation(
                                    "WARNING",
                                    f"⏳ {sg['group_id']} still has dependencies, will retry",
                                )
                        except Exception as e:
                            self.log_operation(
                                "ERROR",
                                f"Error deleting attached security group {sg['group_id']}: {e}",
                            )
                            still_remaining.append(sg)
                    ...
                    if not remaining_sgs:
                        self.log_operation(
                            "INFO",
                            f"✅ All attached security groups deleted in {account_name} ({region})",
                        )
                        break
                    ...
                    if (
                        ...
                        self.log_operation(
                            "INFO",
                            f"🧹 No progress made, re-clearing rules for remaining {len(remaining_sgs)} security groups",
                        )
                        for sg in remaining_sgs:
                            try:
                                self.clear_security_group_rules(
                                    ec2_client, sg["group_id"]
                                )
                            except Exception as e:
                                self.log_operation(
                                    "ERROR",
                                    f"Error re-clearing rules for {sg['group_id']}: {e}",
                                )

                    if retry < max_retries - 1:
                        self.log_operation(
                            "INFO",
                            f"⏳ Waiting {retry_delay}s before retry {retry + 2}/{max_retries}",
                        )
                ...
                if remaining_sgs:
                    self.log_operation(
                        "WARNING",
                        f"⚠️  {len(remaining_sgs)} security groups could not be deleted after {max_retries} retries",
                    )
                    self.log_operation(
                        "WARNING",
                        f"Remaining security groups: {[sg['group_id'] for sg in remaining_sgs]}",
                    )
                    ...
                    for sg in remaining_sgs:
                        try:
                            response = ec2_client.describe_security_groups(
                                GroupIds=[sg["group_id"]]
                            )
                            sg_details = response["SecurityGroups"][0]
                            ingress_count = len(sg_details.get("IpPermissions", []))
                            egress_count = len(
                                sg_details.get("IpPermissionsEgress", [])
                            )
                            self.log_operation(
                                "WARNING",
                                f"  {sg['group_id']} still has {ingress_count} ingress, {egress_count} egress rules",
                            )
                        except Exception as e:
                            self.log_operation(
                                "ERROR",
                                f"  Could not get details for {sg['group_id']}: {e}",
                            )
    ...
    def run_parallel_cleanup(self, max_workers=5):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🚀 Starting parallel cleanup across {len(accounts)} accounts and {len(regions)} regions",
            )
            self.log_operation(
                "INFO", f"📋 Total tasks: {len(tasks)} (max workers: {max_workers})"
            )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_task = {
                    executor.submit(
                        self.cleanup_account_region, account_name, account_data, region
                    ): (account_name, region)
    ...
    def save_cleanup_report(self):
        ...
        try:
            report_filename = (
                f"ultra_ec2_cleanup_report_{self.execution_timestamp}.json"
            )
            ...
            report_data = {
                ...
                "detailed_results": {
                    ...
                    "deleted_security_groups": self.cleanup_results[
                        "deleted_security_groups"
                    ],
    ...
    def run(self):
        ...
        try:
            ...
            if cleanup_option == "1":
                ...
                print(
                    f"    across {account_count} accounts in {region_count} regions ({total_operations} operations)"
                )
                ...
                self.log_operation(
                    "INFO",
                    f"🚨 CLEANUP INITIATED - {account_count} accounts, {region_count} regions",
                )

                start_time = time.time()

                # Run parallel cleanup
                successful_tasks, failed_tasks = self.run_parallel_cleanup(
                    max_workers=10
                )
    ...
    def selective_cleanup(self):
        ...
        try:
            ...
            if selection in ["cancel", "quit"]:
                self.log_operation(
                    "INFO", "Operation cancelled during account selection"
                )
            ...
            for account in selected_accounts:
                ...
                if not region_selection or region_selection == "all":
                    ...
                    print(
                        f"✅ Selected all {len(available_regions)} regions for {account_name}"
                    )
                else:
                    try:
                        # Parse selection
                        indices = self.parse_selection(
                            region_selection, len(available_regions)
                        )
                        selected_regions = [available_regions[i - 1] for i in indices]
                        print(
                            f"✅ Selected {len(selected_regions)} regions for {account_name}"
                        )
                    except Exception as e:
                        self.log_operation(
                            "ERROR", f"Invalid region selection for {account_name}: {e}"
                        )
                        print(
                            f"❌ Invalid selection: {e}. Skipping account {account_name}."
                        )
                        continue
            ...
            if not accounts_regions_to_process:
                self.log_operation(
                    "INFO", "No accounts/regions selected for processing"
                )
            ...
            for account, region in accounts_regions_to_process:
                ...
                try:
                    ...
                    ec2_client = self.create_ec2_client(
                        account["access_key"], account["secret_key"], region
                    )
                    ...
                    print(
                        f"   ✅ Found {len(instances)} instances and {len(security_groups)} security groups"
                    )

                except Exception as e:
                    self.log_operation(
                        "ERROR", f"Error scanning {account_name} in {region}: {e}"
                    )
                    print(f"   ❌ Error: {e}")
            ...
            for resource_key, resources in all_discovered_resources.items():
                ...
                if instances:
                    print("\nAvailable instances:")
                    print(
                        f"{'#':<4} {'Instance ID':<20} {'Name':<30} {'Type':<12} {'State':<12} {'IP Address':<15}"
                    )
                    print("-" * 90)
                    ...
                    for i, instance in enumerate(instances, 1):
                        ...
                        ip = instance.get("public_ip") or instance.get(
                            "private_ip", "N/A"
                        )

                        print(
                            f"{i:<4} {instance_id:<20} {instance_name:<30} {instance_type:<12} {state:<12} {ip:<15}"
                        )
                    ...
                    if instance_selection != "skip":
                        ...
                        if not instance_selection or instance_selection == "all":
                            ...
                            print(
                                f"✅ Selected all {len(instances)} instances for deletion"
                            )
                        else:
                            try:
                                # Parse selection
                                indices = self.parse_selection(
                                    instance_selection, len(instances)
                                )
                                selected_instances = [instances[i - 1] for i in indices]
                                print(
                                    f"✅ Selected {len(selected_instances)} instances for deletion"
                                )
                            except Exception as e:
                                self.log_operation(
                                    "ERROR", f"Invalid instance selection: {e}"
                                )
                                print(
                                    f"❌ Invalid selection: {e}. No instances selected."
                                )
                                selected_instances = []

                        if selected_instances:
                            selected_instances_for_deletion[resource_key] = (
                                selected_instances
                            )
                else:
                ...
                if security_groups:
                    ...
                    if unattached_sgs:
                        ...
                        print(
                            f"{'#':<4} {'Group ID':<20} {'Group Name':<30} {'Description'}"
                        )
                        print("-" * 80)
                        ...
                        sg_selection = (
                            input("\n🔢 Select unattached security groups to delete: ")
                            .strip()
                            .lower()
                        )
                        ...
                        if sg_selection != "skip":
                            ...
                            if not sg_selection or sg_selection == "all":
                                ...
                                print(
                                    f"✅ Selected all {len(unattached_sgs)} unattached security groups for deletion"
                                )
                            else:
                                try:
                                    # Parse selection
                                    indices = self.parse_selection(
                                        sg_selection, len(unattached_sgs)
                                    )
                                    selected_sgs = [
                                        unattached_sgs[i - 1] for i in indices
                                    ]
                                    print(
                                        f"✅ Selected {len(selected_sgs)} unattached security groups for deletion"
                                    )
                                except Exception as e:
                                    self.log_operation(
                                        "ERROR",
                                        f"Invalid security group selection: {e}",
                                    )
                                    print(
                                        f"❌ Invalid selection: {e}. No security groups selected."
                                    )
                                    selected_sgs = []
                            ...
                            if selected_sgs:
                                ...
                                selected_sgs_for_deletion[resource_key].extend(
                                    selected_sgs
                                )
                    else:
                        print(
                            "No unattached security groups found in this account/region."
                        )
                    ...
                    if attached_sgs:
                        ...
                        print(
                            f"{'#':<4} {'Group ID':<20} {'Group Name':<30} {'Attached To'}"
                        )
                        print("-" * 80)
                        ...
                        for i, sg in enumerate(attached_sgs, 1):
                            ...
                            if len(sg["attached_instances"]) > 2:
                                attached_to += (
                                    f" and {len(sg['attached_instances']) - 2} more"
                                )
                        ...
                        if (
                            resource_key in selected_instances_for_deletion
                            and selected_instances_for_deletion[resource_key]
                        ):
                            print("\nSelection Options:")
                            print("  • Single groups: 1,3,5")
                            print("  • Ranges: 1-3")
                            ...
                            attached_sg_selection = (
                                input(
                                    "\n🔢 Select attached security groups to delete: "
                                )
                                .strip()
                                .lower()
                            )
                            ...
                            if attached_sg_selection != "skip":
                                ...
                                if (
                                    not attached_sg_selection
                                    or attached_sg_selection == "all"
                                ):
                                    selected_attached_sgs = attached_sgs
                                    print(
                                        f"✅ Selected all {len(attached_sgs)} attached security groups for deletion"
                                    )
                                else:
                                    try:
                                        # Parse selection
                                        indices = self.parse_selection(
                                            attached_sg_selection, len(attached_sgs)
                                        )
                                        selected_attached_sgs = [
                                            attached_sgs[i - 1] for i in indices
                                        ]
                                        print(
                                            f"✅ Selected {len(selected_attached_sgs)} attached security groups for deletion"
                                        )
                                    except Exception as e:
                                        self.log_operation(
                                            "ERROR",
                                        ...
                                        print(
                                            f"❌ Invalid selection: {e}. No attached security groups selected."
                                        )
                                        selected_attached_sgs = []
                            ...
                            print(
                                "⚠️ Note: Attached security groups can only be selected if their instances are also selected for deletion."
                            )
                    else:
                        print(
                            "No attached security groups found in this account/region."
                        )
                else:
            ...
            total_instances = sum(
                len(instances) for instances in selected_instances_for_deletion.values()
            )
            ...
            print(
                f"across {len(selected_instances_for_deletion) + len(selected_sgs_for_deletion)} account/region combinations."
            )
            ...
            if confirmation != "yes":
                self.log_operation(
                    "INFO", "Selective deletion cancelled at confirmation"
                )
            ...
            for resource_key, instances in selected_instances_for_deletion.items():
                ...
                if instances and resource_key in selected_sgs_for_deletion:
                    print(
                        f"⏳ Waiting 30 seconds for instances to start terminating..."
                    )
                    time.sleep(30)
            ...
            for resource_key, security_groups in selected_sgs_for_deletion.items():
                ...
                if unattached:
                    ...
                    for sg in unattached:
                        try:
                            ...
                            self.log_operation(
                                "ERROR",
                                f"Error deleting security group {sg['group_id']}: {e}",
                            )
                ...
                if attached:
                    ...
                    for retry in range(max_retries):
                        ...
                        if retry < max_retries - 1:
                            print(
                                f"⏳ Waiting for instance termination to complete before retry {retry + 2}/{max_retries}..."
                            )
                            time.sleep(30)
            ...
            print(
                f"💻 Instances terminated: {len(self.cleanup_results['deleted_instances'])}"
            )
            print(
                f"🛡️  Security groups deleted: {len(self.cleanup_results['deleted_security_groups'])}"
            )
            print(
            ...
            if "-" in part:
                ...
                try:
                    ...
                    if start < 1 or end > max_count:
                        raise ValueError(
                            f"Range {part} is out of bounds (1-{max_count})"
                        )
                ...
                try:
                    ...
                    if num < 1 or num > max_count:
                        raise ValueError(
                            f"Selection {num} is out of bounds (1-{max_count})"
                        )
    ...
    def display_cleanup_results(self, successful_tasks, failed_tasks, total_time):
        ...
        self.log_operation(
            "INFO",
            f"Instances deleted: {len(self.cleanup_results['deleted_instances'])}",
        )
        self.log_operation(
            "INFO",
        ...
        if (
            self.cleanup_results["deleted_instances"]
            or self.cleanup_results["deleted_security_groups"]
        ):
