...
class AWSCredentialManager:
    ...
    def _check_config_files(self):
        ...
        if not os.path.exists(self.aws_accounts_config_file):
            self.print_colored(Colors.YELLOW, f"⚠️  Warning: aws_accounts_config.json not found in {self.config_dir}")
        elif not os.access(self.aws_accounts_config_file, os.R_OK):
            self.print_colored(Colors.YELLOW, f"⚠️  Warning: Cannot read aws_accounts_config.json")
        else:
        ...
        if not os.path.exists(self.iam_dir):
            self.print_colored(Colors.YELLOW, f"⚠️  Warning: aws/iam directory not found in {self.config_dir}")
        elif not os.access(self.iam_dir, os.R_OK):
            self.print_colored(Colors.YELLOW, f"⚠️  Warning: Cannot read aws/iam directory")
        else:
            self.print_colored(Colors.GREEN, f"✅ Found aws/iam directory")
            iam_files = glob.glob(self.iam_users_pattern)
            if not iam_files:
                self.print_colored(Colors.YELLOW, f"⚠️  Warning: No iam_users_credentials_*.json files found")
            else:
                self.print_colored(Colors.GREEN, f"✅ Found {len(iam_files)} IAM credential files")
    ...
    def load_root_accounts_config(self) -> Optional[Dict]:
        ...
        try:
            if not os.path.exists(self.aws_accounts_config_file):
                self.print_colored(Colors.RED, f"❌ AWS accounts config file not found: {self.aws_accounts_config_file}")
                return None

            if not os.access(self.aws_accounts_config_file, os.R_OK):
                self.print_colored(Colors.RED, f"❌ Cannot read AWS accounts config file")
                return None
            ...
            if 'accounts' not in config:
                self.print_colored(Colors.RED, "❌ Invalid config file: missing 'accounts' section")
                return None

            accounts_count = len(config['accounts'])
            self.print_colored(Colors.GREEN, f"✅ Loaded {accounts_count} root accounts")
            return config

        except json.JSONDecodeError as e:
            self.print_colored(Colors.RED, f"❌ Error parsing root accounts config: {e}")
            return None
        except Exception as e:
            self.print_colored(Colors.RED, f"❌ Error loading root accounts config: {e}")
            return None
            ...
            try:
                ...
                if allow_multiple:
                    ...
                    if 1 <= choice_num <= len(account_keys):
                        ...
                        self.print_colored(Colors.RED, f"❌ Invalid choice. Please enter 1-{len(account_keys)}")
                        continue
                ...
                self.print_colored(Colors.GREEN, f"✅ Selected {len(selected_accounts)} account(s)")
    ...
    def load_iam_users_from_file(self, file_path: str) -> Optional[Dict]:
        ...
        try:
            if not os.access(file_path, os.R_OK):
                self.print_colored(Colors.RED, f"❌ Cannot read IAM users file: {file_path}")
                return None
            ...
            if 'accounts' not in data:
                self.print_colored(Colors.RED, "❌ Invalid IAM users file: missing 'accounts' section")
                return None

            total_users = sum(len(account_data.get('users', [])) for account_data in data['accounts'].values())
            accounts_count = len(data['accounts'])

            self.print_colored(Colors.GREEN, f"✅ Loaded {total_users} users from {accounts_count} accounts")
            return data
        ...
        while True:
            try:
                ...
                if choice.lower() == 'q':
                    ...
                    self.print_colored(Colors.GREEN, f"✅ Selected all {len(all_users)} users")
                    return all_users
                else:
                    selected_indices = self._parse_selection(choice, len(all_users))
                    if selected_indices:
                        selected_users = [all_users[i - 1] for i in selected_indices]
                        self.print_colored(Colors.GREEN, f"✅ Selected {len(selected_users)} users")
                        return selected_users
                    else:
                        self.print_colored(Colors.RED, "❌ Invalid selection format. Examples: 1, 1,3,5, 1-5, all")
