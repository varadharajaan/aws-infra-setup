...
patterns_to_folders = [
    ...
    # ELB related
    (re.compile(r'^(elb_cleanup_log|elb_deletion_report).*\.(json|txt)?$', re.IGNORECASE), "elb"),
    # IAM related
    (re.compile(r'^(iam_cleanup_report|iam_deletion_report|user_instructions_).*\.(json|txt)?$', re.IGNORECASE), "iam"),
    # Ultra EC2 cleanup
