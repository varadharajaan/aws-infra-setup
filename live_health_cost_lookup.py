...
class LiveCostCalculator:
    ...
    def setup_logging(self):
        ...
        self.log_filename = (
            f"{self.log_dir}/live_cost_calculator_{self.execution_timestamp}.log"
        )
    ...
    def load_aws_accounts_config(self):
        ...
        try:
            ...
            self.logger.info(
                f"✅ AWS accounts configuration loaded from: {self.config_file}"
            )
            ...
            for account_name, account_data in self.aws_config["accounts"].items():
                if (
                    ...
                    self.logger.warning(
                        f"Skipping account with invalid credentials: {account_name}"
                    )
            ...
            # Get default regions
            self.default_regions = self.aws_config.get("user_settings", {}).get(
                "user_regions",
    ...
    def create_cloudwatch_client(self, access_key, secret_key, region):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create CloudWatch client for region {region}: {e}"
            )
            return None
    ...
    def load_ec2_pricing_data(self, region_code):
        ...
        if self.pricing_cache_timestamp and (
            datetime.now() - self.pricing_cache_timestamp
        ).total_seconds() < (self.pricing_cache_valid_hours * 3600):
        ...
        try:
            ...
            region_map.get(region_code, region_code)
            ...
            self.logger.info(
                f"Cached EC2 pricing data for {region_code} ({len(pricing_map)} instance types)"
            )
            return pricing_map
    ...
    def get_ec2_instance_health(self, ec2_client, cloudwatch_client, instance_id):
        ...
        health_data = {
            "status": "Unknown",
            "status_checks": {"system": "Unknown", "instance": "Unknown"},
        ...
        try:
            ...
            if response["InstanceStatuses"]:
                ...
                if "SystemStatus" in status_info:
                    health_data["status_checks"]["system"] = status_info[
                        "SystemStatus"
                    ]["Status"]

                if "InstanceStatus" in status_info:
                    health_data["status_checks"]["instance"] = status_info[
                        "InstanceStatus"
                    ]["Status"]
            ...
            try:
                ...
                if cpu_response["Datapoints"]:
                    ...
                    health_data["cpu_utilization"] = round(
                        latest_datapoint["Average"], 2
                    )
            except Exception as e:
                self.logger.warning(
                    f"Could not get CPU utilization for instance {instance_id}: {e}"
                )
            ...
            try:
                ...
                if memory_response["Datapoints"]:
                    ...
                    health_data["memory_utilization"] = round(
                        latest_datapoint["Average"], 2
                    )
            except Exception:
            ...
            self.logger.warning(
                f"Error getting health data for instance {instance_id}: {e}"
            )
    ...
    def calculate_ec2_costs(
        self, account_selection, region_selection, ec2_selection=None
    ):
        """Calculate costs for EC2 instances"""
        self.logger.info(
            f"Calculating EC2 costs for account {account_selection} in regions: {region_selection}"
        )
        ...
        if not account_name:
            self.logger.error(
                f"Account ID {account_selection} not found in configuration"
            )
            return results
        ...
        for region in region_selection:
            ...
            region_results = {"instances": [], "total_cost": 0.0, "instance_count": 0}
            ...
            cloudwatch_client = self.create_cloudwatch_client(
                access_key, secret_key, region
            )
            if not cloudwatch_client:
                self.logger.warning(
                    f"Could not create CloudWatch client for region {region}"
                )
            ...
            try:
                if ec2_selection and ec2_selection != "all":
                    ...
                    instance_ids = (
                        ec2_selection
                        if isinstance(ec2_selection, list)
                        else [ec2_selection]
                    )
                ...
                for reservation in response.get("Reservations", []):
                    for instance in reservation.get("Instances", []):
                        ...
                        if state != "running":
                            self.logger.info(
                                f"Skipping instance {instance_id} with state: {state}"
                            )
                            continue
                        ...
                        uptime_hours = (
                            self.current_time - launch_time.replace(tzinfo=None)
                        ).total_seconds() / 3600
                        ...
                        # Get instance health
                        health_data = self.get_ec2_instance_health(
        ...
        self.logger.info(
            f"Total EC2 cost: ${results['total_cost']:.2f} for {results['instance_count']} instances"
        )

        return results

    def calculate_historical_costs(
        self, account_selection, region_selection, hours_back=9
    ):
        ...
        historical_results = {
            ...
            "start_time": (self.current_time - timedelta(hours=hours_back)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        ...
        for region in region_selection:
            ...
            region_results["hourly_costs"] = self.generate_hourly_breakdown(
                region_results["ec2_instances"],
                region_results["eks_clusters"],
                hours_back,
            )
    ...
    def calculate_forecast_costs(
        self, account_selection, region_selection, hours_ahead=9
    ):
        ...
        forecast_results = {
            ...
            "end_time": (self.current_time + timedelta(hours=hours_ahead)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "regions": {},
        ...
        for region in region_selection:
            ...
            region_results["hourly_forecast"] = self.generate_hourly_forecast(
                region_results["ec2_forecast"],
                region_results["eks_forecast"],
                hours_ahead,
            )

            forecast_results["regions"][region] = region_results
            forecast_results["total_forecast_cost"] += region_results[
                "total_forecast_cost"
            ]
    ...
    def analyze_nodegroup_historical_cost(
        self,
        eks_client,
        ec2_client,
        cluster_name,
        ...
        try:
            ...
            if (
                "resources" in nodegroup_info
                and "autoScalingGroups" in nodegroup_info["resources"]
            ):
                for asg in nodegroup_info["resources"]["autoScalingGroups"]:
                    ...
                    if asg_name:
                        try:
                            ...
                            account_name = next(
                                iter(self.aws_config["accounts"].keys())
                            )
                                ...
                                for instance in asg_detail.get("Instances", []):
                                    ...
                                    try:
                                        ...
                                        for reservation in ec2_response.get(
                                            ...
                                            for ec2_instance in reservation.get(
                                                "Instances", []
                                            ):
                                                instance_type = ec2_instance[
                                                    "InstanceType"
                                                ]
                                                hourly_rate = ec2_pricing.get(
                                                    instance_type, 0.0
                                                )
                                                instance_cost = (
                                                    running_hours * hourly_rate
                                                )
                            ...
                            self.logger.warning(
                                f"Could not get ASG details for {asg_name}: {e}"
                            )
            ...
            if not instance_costs:
                ...
                if desired_size > 0 and ng_instance_types:
                    ...
                    hourly_rate = ec2_pricing.get(
                        primary_instance_type, 0.096
                    )  # Default to m5.large rate
        ...
        try:
            ...
            if desired_size > 0 and ng_instance_types:
                for instance_type in ng_instance_types:
                    ...
                    instances_of_this_type = max(
                        1, desired_size // len(ng_instance_types)
                    )
                    if ng_instance_types.index(instance_type) < (
                        desired_size % len(ng_instance_types)
                    ):
                        instances_of_this_type += 1

                    type_forecast_cost = (
                        instances_of_this_type * hourly_rate * hours_ahead
                    )
    ...
    def get_recent_cpu_trend(self, cloudwatch_client, instance_id):
        ...
        return {"values": [], "average": 0, "min": 0, "max": 0, "trend": "unknown"}
    ...
    def generate_overall_hourly_breakdown(self, regions_data, hours):
        ...
        for hour in range(hours):
            ...
            for region, region_data in regions_data.items():
                region_cost = {"ec2": 0.0, "eks": 0.0, "total": 0.0}
                ...
                for cluster in region_data.get("eks_clusters", []):
                    if cluster.get("running_hours", 0) > hour:
                        ...
                        if cluster.get("running_hours", 0) > 0:
                            worker_hourly = worker_cost / cluster.get(
                                "running_hours", 1
                            )
            ...
            for region, region_data in regions_data.items():
                ...
                for cluster in region_data.get("eks_forecast", []):
                    ...
                    if cluster.get("forecast_hours", 0) > 0:
                        worker_hourly = worker_forecast / cluster.get(
                            "forecast_hours", 1
                        )
                        region_cost["eks"] += worker_hourly
                        hour_data["eks_cost"] += worker_hourly
    ...
    def generate_hourly_forecast(self, ec2_forecast, eks_forecast, hours):
        ...
        for hour in range(hours):
            ...
            for cluster in eks_forecast:
                ...
                # Worker nodes
                if cluster.get("forecast_hours", 0) > 0:
                    worker_hourly = cluster.get("nodegroups_forecast", 0) / cluster.get(
                        "forecast_hours", 1
                    )
                    hour_data["eks_cost"] += worker_hourly
    ...
    def analyze_ec2_historical_costs(
        self, ec2_client, cloudwatch_client, ec2_pricing, hours_back
    ):
        """Analyze EC2 historical costs"""
    ...
    def analyze_eks_historical_costs(
        self,
        eks_client,
        ec2_client,
        cloudwatch_client,
    ...
    def generate_ec2_forecast(
        self, ec2_client, cloudwatch_client, ec2_pricing, hours_ahead
    ):
        ...
        try:
            ...
            for reservation in response.get("Reservations", []):
                for instance in reservation.get("Instances", []):
                    ...
                    cpu_trend = self.get_recent_cpu_trend(
    ...
    def generate_eks_forecast(
        self, eks_client, ec2_client, ec2_pricing, eks_pricing, hours_ahead
    ):
    ...
    def get_cpu_utilization_trend(
        self, cloudwatch_client, instance_id, start_time, end_time
    ):
        ...
        for hour in range(hours):
            ...
            for cluster in eks_clusters:
                if cluster["running_hours"] > hour:
                    hour_data["eks_cost"] += (
                        cluster["control_plane_cost"] + cluster["nodegroups_cost"]
                    ) / cluster["running_hours"]
            ...
            for reservation in response.get("Reservations", []):
                for instance in reservation.get("Instances", []):
                    ...
                    for tag in instance.get("Tags", []):
                        if (
                            tag["Key"] == "Name"
                            or tag["Key"] == "kubernetes.io/hostname"
                        ):
                            node_name = tag["Value"]
                            break
    ...
    def calculate_eks_costs(
        self, account_selection, region_selection, eks_selection=None
    ):
        ...
        self.logger.info(
            f"Calculating EKS costs for account {account_selection} in regions: {region_selection}"
        )
        ...
        for region in region_selection:
            ...
            region_results = {"clusters": [], "total_cost": 0.0, "cluster_count": 0}
            ...
            control_plane_hourly = eks_pricing.get(region, {}).get(
                "control_plane", 0.10
            )  # Default to $0.10/hour
            ...
            try:
                ...
                if eks_selection and eks_selection != "all":
                    ...
                    cluster_names = (
                        eks_selection
                        if isinstance(eks_selection, list)
                        else [eks_selection]
                    )
                    for cluster_name in cluster_names:
                        try:
                            ...
                            self.logger.error(
                                f"Error getting cluster {cluster_name}: {e}"
                            )
                else:
                    # Get all clusters
                    try:
                        ...
                        self.logger.info(
                            f"Found {len(cluster_names)} clusters in region {region}: {cluster_names}"
                        )
                        ...
                        for cluster_name in cluster_names:
                            try:
                                ...
                                clusters_to_process.append(cluster_info["cluster"])
                                self.logger.info(
                                    f"Successfully described cluster: {cluster_name}"
                                )
                            except Exception as e:
                                self.logger.error(
                                    f"Error describing cluster {cluster_name}: {e}"
                                )
                    except Exception as e:
                        self.logger.error(
                            f"Error listing clusters in region {region}: {e}"
                        )

                self.logger.info(
                    f"Processing {len(clusters_to_process)} clusters in region {region}"
                )
                ...
                for cluster in clusters_to_process:
                    ...
                    if (
                        hasattr(cluster_created_at, "tzinfo")
                        and cluster_created_at.tzinfo
                    ):
                    ...
                    if cluster_status != "ACTIVE":
                        self.logger.warning(
                            f"Skipping cluster {cluster_name} with status: {cluster_status}"
                        )
                        continue
                    ...
                    if cluster_created_at <= current_time:
                        uptime_hours = (
                            current_time - cluster_created_at
                        ).total_seconds() / 3600
                    else:
                        ...
                        self.logger.warning(
                            f"Cluster {cluster_name} has future creation time, using minimum uptime"
                        )
                    ...
                    try:
                        ...
                        for ng_name in nodegroup_names:
                            try:
                                ...
                                ng_instance_types = ng_info.get(
                                    "instanceTypes", ["m5.large"]
                                )
                                ng_desired_size = ng_scaling_config.get(
                                    "desiredSize", 1
                                )
                                ...
                                ng_cost = 0.0
                                for instance_type in ng_instance_types:
                                    instance_hourly_rate = ec2_pricing.get(
                                        instance_type, 0.096
                                    )
                                    # Distribute instances across types
                                    instances_of_type = max(
                                        1, ng_desired_size // len(ng_instance_types)
                                    )
                                    ng_cost += (
                                        instances_of_type
                                ...
                                self.logger.error(
                                    f"Error processing nodegroup {ng_name}: {e}"
                                )

                    except Exception as e:
                        self.logger.error(
                            f"Error listing nodegroups for cluster {cluster_name}: {e}"
                        )
                        nodegroups_info = []
                        total_nodegroups_cost = 0.0
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create EKS directory {account_output_dir}: {e}"
            )
            # Fallback to parent directory
        ...
        try:
            ...
            print(
                f"✅ EKS results saved: {results['cluster_count']} clusters, ${results['total_cost']:.2f}"
            )
        except Exception as e:
    ...
    def display_ec2_cost_summary(self, cost_data):
        ...
        print("\n" + "=" * 80)
        print(
            f"💰 EC2 COST SUMMARY FOR ACCOUNT: {cost_data['account_id']} ({cost_data['account_name']})"
        )
        print("=" * 80)
        ...
        for region, region_data in cost_data["regions"].items():
            if region_data["instance_count"] > 0:
                ...
                print(
                    f"   {'Instance ID':<20} {'Name':<30} {'Type':<15} {'Hours':<10} {'Cost':<10} {'CPU %':<10}"
                )
                print("   " + "-" * 90)
                ...
                for instance in instances[:5]:
                    ...
                    print(
                        f"   {instance['instance_id']:<20} {instance['instance_name'][:28]:<30} "
    ...
    def display_eks_cost_summary(self, cost_data):
        ...
        print("\n" + "=" * 80)
        print(
            f"💰 EKS COST SUMMARY FOR ACCOUNT: {cost_data['account_id']} ({cost_data['account_name']})"
        )
        print("=" * 80)
        ...
        for region, region_data in cost_data["regions"].items():
            if region_data["cluster_count"] > 0:
                ...
                # Display clusters
                clusters = sorted(
                    region_data["clusters"], key=lambda x: x["total_cost"], reverse=True
                )
                ...
                for cluster in clusters:
                    ...
                    print(
                        f"      Control Plane Cost: ${cluster['control_plane']['cost']:.2f}"
                    )
                    ...
                    if cluster["nodegroups"]:
                        ...
                        for ng in cluster["nodegroups"]:
                            print(
                                f"         - {ng['nodegroup_name']}: ${ng['cost']:.2f}"
                            )
                            ...
                            if ng["nodes"]:
                                ...
                                for node in ng["nodes"]:
                                    if "instance_id" in node:
                                        print(
                                            f"               {node['instance_id']} ({node['instance_type']}): ${node['cost']:.2f}"
                                        )
                                    else:
                                        print(
                                            f"               {node['instance_type']} x {node['count']}: ${node['estimated_cost']:.2f}"
                                        )
                ...
                try:
                    ...
                    if cluster["status"] == "ACTIVE":
                        active_clusters.append(
                            {
                                ...
                                "created_at": cluster["createdAt"].strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                ),
                            }
                        )
                        ...
                        try:
                            ...
                            active_clusters[-1]["nodegroups_count"] = len(
                                nodegroups_response.get("nodegroups", [])
                            )
                        except Exception:
    ...
    def display_active_resources(
        self, account_id, account_name, regions, access_key, secret_key
    ):
        ...
        for region in regions:
            ...
            if instances:
                ec2_regions_with_instances.append(
                    region
                )  # Add region if instances found
                for instance in instances:
                    ...
                    try:
                        ...
                        launch_time = datetime.strptime(
                            instance["launch_time"], "%Y-%m-%d %H:%M:%S"
                        )
                        uptime_hours = (
                            (current_time - launch_time).total_seconds() / 3600
                        ) - 5.30
                        ...
                        instance["estimated_hourly_cost"] = instance_type_costs.get(
                            instance["instance_type"], 0.05
                        )
        ...
        if all_instances:
            ...
            sorted_instances = sorted(
                all_instances,
                key=lambda x: x.get("estimated_hourly_cost", 0),
                reverse=True,
            )
            ...
            for instance in sorted_instances:
                ...
                print(
                    f"   {instance['instance_id']:<20} {instance['instance_name'][:23]:<25} "
            ...
            print("-" * 120)
            print(
                f"   📊 Summary: {len(all_instances)} instances across {len(ec2_regions_with_instances)} regions"
            )
            print(
                f"   💰 Estimated Total Hourly Cost: ${sum(i.get('estimated_hourly_cost', 0) for i in all_instances):.4f}"
            )
            print(
                f"   🌍 Regions with instances: {', '.join(ec2_regions_with_instances)}"
            )
                ...
                for cluster in clusters:
                    ...
                    try:
                        ...
                        created_time = datetime.strptime(
                            cluster["created_at"], "%Y-%m-%d %H:%M:%S"
                        )
                        uptime_hours = (
                            current_time - created_time
                        ).total_seconds() / 3600 - 5.30
                        ...
                        estimated_worker_cost = (
                            nodegroup_count * 0.096
                        )  # Assume m5.large equivalent
                        cluster["estimated_worker_hourly_cost"] = estimated_worker_cost
                        cluster["total_estimated_hourly_cost"] = (
                            0.10 + estimated_worker_cost
                        )
        ...
        if all_clusters:
            ...
            sorted_clusters = sorted(
                all_clusters,
                key=lambda x: x.get("total_estimated_hourly_cost", 0),
                reverse=True,
            )
            ...
            for cluster in sorted_clusters:
                ...
                print(
                    f"   {cluster['cluster_name'][:23]:<25} {cluster['version']:<10} "
            ...
            print("-" * 130)
            print(
                f"   📊 Summary: {len(all_clusters)} clusters across {len(eks_regions_with_clusters)} regions"
            )
            print(f"   💰 Estimated Total Hourly Cost: ${total_eks_hourly_cost:.4f}")
            print(
                f"   🌍 Regions with clusters: {', '.join(eks_regions_with_clusters)}"
            )

            # Show cluster breakdown
            total_control_plane_cost = len(all_clusters) * 0.10
            total_worker_cost = sum(
                c.get("estimated_worker_hourly_cost", 0) for c in all_clusters
            )
            print(
                f"   🎛️  Control Plane Cost: ${total_control_plane_cost:.2f}/hr ({len(all_clusters)} × $0.10)"
            )
        ...
        if has_active_resources:
            ...
            print(
                f"   🌍 Regions with resources: {len(set(ec2_regions_with_instances + eks_regions_with_clusters))}"
            )
            ...
            if total_hourly_cost > 0:
                ...
                print(f"\n💹 Cost Breakdown:")
                print(
                    f"   🖥️  EC2: ${total_estimated_hourly_cost:.4f}/hr ({ec2_percentage:.1f}%)"
                )
                print(
                    f"   🚢 EKS: ${total_eks_hourly_cost:.4f}/hr ({eks_percentage:.1f}%)"
                )
            ...
            if len(all_instances) > 10:
                ...
                print(
                    f"   💡 Consider using auto-scaling or spot instances for cost optimization"
                )
        ...
        scan_summary.update(
            {
                ...
                "regions_with_resources": list(
                    set(ec2_regions_with_instances + eks_regions_with_clusters)
                ),
            }
        )
    ...
    def run(self):
        ...
        for i, (account_name, account_data) in enumerate(
            self.aws_config["accounts"].items(), 1
        ):
                ...
                for part in parts:
                    if "-" in part:
                        ...
                        try:
                            ...
                            if not found:
                                print(
                                    f"Warning: Account ID '{part}' not found, skipping."
                                )
        ...
        for account_id in selected_account_ids:
            ...
            resources = self.display_active_resources(
                account_id, account_name, selected_regions, access_key, secret_key
            )
        ...
        if calculate_eks:
            ...
            if eks_option == "2":
                eks_names = input(
                    "\nEnter EKS cluster names (comma-separated): "
                ).strip()
                if eks_names:
        ...
        # Calculate totals
        total_ec2_cost = (
            sum(result["total_cost"] for result in all_ec2_results.values())
            if all_ec2_results
            else 0
        )
        total_ec2_instances = (
            sum(result["instance_count"] for result in all_ec2_results.values())
            if all_ec2_results
            else 0
        )

        total_eks_cost = (
            sum(result["total_cost"] for result in all_eks_results.values())
            if all_eks_results
            else 0
        )
        total_eks_clusters = (
            sum(result["cluster_count"] for result in all_eks_results.values())
            if all_eks_results
            else 0
        )
        ...
        total_historical_cost = (
            sum(
                ...
                for result in all_historical_results.values()
            )
        ...
        total_forecast_cost = (
            sum(
                result["total_forecast_cost"]
                for result in all_forecast_results.values()
            )
        ...
        for account_id in selected_account_ids:
            ...
            account_historical = all_historical_results.get(account_id, {}).get(
                "total_historical_cost", 0
            )
            account_forecast = all_forecast_results.get(account_id, {}).get(
        ...
        aggregated_output_file = (
            f"aws/live-cost/cost_summary_{self.execution_timestamp}.json"
        )
    ...
    def generate_data_tables_html(self, aggregated_results):
            ...
            for account_id, eks_data in aggregated_results["eks"]["accounts"].items():
                ...
                for region, region_data in eks_data.get("regions", {}).items():
                    for cluster in region_data.get("clusters", []):
                        worker_cost = (
                            cluster["total_cost"] - cluster["control_plane"]["cost"]
                        )
                        ...
                        version = cluster["version"]
                        version_class = (
                            "status-active" if version >= "1.25" else "status-warning"
                        )
        ...
        if aggregated_results.get("historical_analysis", {}).get("accounts"):
            ...
            for account_id, historical_data in aggregated_results[
                "historical_analysis"
            ]["accounts"].items():
                account_name = self.account_id_to_name.get(account_id, "Unknown")
                ...
                for region, region_data in historical_data.get("regions", {}).items():
                    ...
                    if ec2_instances:
                        total_hours = sum(
                            i.get("running_hours", 0) for i in ec2_instances
                        )
                        total_cost = sum(
                            i.get("historical_cost", 0) for i in ec2_instances
                        )
                    ...
                    if eks_clusters:
                        total_hours = sum(
                            c.get("running_hours", 0) for c in eks_clusters
                        )
        ...
        if aggregated_results.get("forecast_analysis", {}).get("accounts"):
            ...
            for account_id, forecast_data in aggregated_results["forecast_analysis"][
                "accounts"
            ].items():
                account_name = self.account_id_to_name.get(account_id, "Unknown")
                ...
                for region, region_data in forecast_data.get("regions", {}).items():
                    ...
                    if ec2_forecast:
                        total_cost = sum(
                            i.get("forecast_cost", 0) for i in ec2_forecast
                        )
                    ...
                    if eks_forecast:
                        total_cost = sum(
                            c.get("total_forecast", 0) for c in eks_forecast
                        )
    ...
    def generate_html_report(
        ...
        chart_data = self.prepare_chart_data(
            aggregated_results, all_ec2_results, all_eks_results
        )
    ...
    def prepare_chart_data(self, aggregated_results, all_ec2_results, all_eks_results):
        ...
        if total_ec2 > 0:
            chart_data["service_breakdown"].append(
                {"service": "EC2", "cost": total_ec2}
            )
        if total_eks > 0:
            chart_data["service_breakdown"].append(
                {"service": "EKS", "cost": total_eks}
            )
        ...
        for account_id, ec2_data in all_ec2_results.items():
            for region, region_data in ec2_data.get("regions", {}).items():
                ...
                chart_data["regional_distribution"][region]["ec2"] += region_data.get(
                    "total_cost", 0
                )
        ...
        for account_id, eks_data in all_eks_results.items():
            for region, region_data in eks_data.get("regions", {}).items():
                if region not in chart_data["regional_distribution"]:
                    chart_data["regional_distribution"][region] = {"ec2": 0, "eks": 0}
                chart_data["regional_distribution"][region]["eks"] += region_data.get(
                    "total_cost", 0
                )
            ...
            for region, region_data in ec2_data.get("regions", {}).items():
                for instance in region_data.get("instances", []):
                    ...
                    if instance_type not in chart_data["instance_types"]:
                        chart_data["instance_types"][instance_type] = {
                            "count": 0,
                            "cost": 0,
                        }
                    chart_data["instance_types"][instance_type]["count"] += 1
                    chart_data["instance_types"][instance_type]["cost"] += instance[
                        "estimated_cost"
                    ]
            ...
            for region, region_data in eks_data.get("regions", {}).items():
                for cluster in region_data.get("clusters", []):
                    ...
                    if version not in chart_data["cluster_versions"]:
                        chart_data["cluster_versions"][version] = {
                            "count": 0,
                            "cost": 0,
                        }
                    chart_data["cluster_versions"][version]["count"] += 1
                    chart_data["cluster_versions"][version]["cost"] += cluster[
                        "total_cost"
                    ]
    ...
    def generate_html_content(
        ...
        current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        current_user = "varadharajaan"
        formatted_timestamp = datetime.strptime(timestamp, "%Y%m%d_%H%M%S").strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        try:
            current_total = (
                aggregated_results.get("total_cost", 0)
        ...
        try:
            if total_historical > 0 and current_total > 0:
                ((current_total - total_historical) / total_historical) * 100
        except Exception as e:
        ...
        try:
            if current_total > 0 and total_forecast > 0:
                forecast_trend = (
                    (total_forecast - current_total) / current_total
                ) * 100
        except Exception as e:
        ...
        try:
            data_tables_html = self.generate_data_tables_html_inline(
                aggregated_results, current_timestamp, current_user
            )
        except Exception as e:
            self.logger.error(f"Error generating data tables: {e}")
            data_tables_html = (
                '<div class="data-tables"><p>Error generating data tables.</p></div>'
            )

        # Safely get aggregated data
        try:
            accounts_processed = (
                aggregated_results.get("accounts_processed", 0)
                if isinstance(aggregated_results, dict)
                else 0
            )
            ec2_data = (
                aggregated_results.get("ec2", {})
                if isinstance(aggregated_results, dict)
                else {}
            )
            eks_data = (
                aggregated_results.get("eks", {})
                if isinstance(aggregated_results, dict)
                else {}
            )
            ec2_instance_count = (
                ec2_data.get("instance_count", 0) if isinstance(ec2_data, dict) else 0
            )
            eks_cluster_count = (
                eks_data.get("cluster_count", 0) if isinstance(eks_data, dict) else 0
            )
        except Exception as e:
    ...
    def generate_data_tables_html_inline(
        self, aggregated_results, current_timestamp, current_user
    ):
