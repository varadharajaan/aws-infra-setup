...
class EKSClusterManager:
    ...
    def scan_cluster_files(self) -> List[Dict]:
        ...
        for account_dir in os.listdir(self.base_path):
            ...
            for file_name in os.listdir(eks_path):
                ...
                if (
                    matched
                    or file_name.startswith("eks_cluster")
                    or file_name.startswith("eks_cluser")
                ):
                    ...
                    if cluster_data:
                        ...
                        if timestamp_from_filename:
                            cluster_data["timestamp_from_filename"] = (
    ...
    def group_clusters_by_day(self, cluster_files: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for cluster in cluster_files:
            try:
                ...
                timestamp = cluster.get("timestamp") or cluster.get(
                    "timestamp_from_filename", ""
                )
                ...
                logger.warning(
                    f"Error processing timestamp for {cluster.get('file_name', 'unknown')}: {e}"
                )
    ...
    def display_day_options(self, grouped_clusters: Dict[str, List[Dict]]) -> None:
        ...
        sorted_days = sorted(
            [day for day in grouped_clusters.keys() if day != "unknown"]
        )
    ...
    def get_user_day_selection(
        self, grouped_clusters: Dict[str, List[Dict]]
    ) -> List[str]:
            ...
            try:
                ...
                if "-" in selection and not selection.startswith("cluster-"):
                    ...
                    if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                        start_idx = int(parts[0]) - 1
                        end_idx = int(parts[1]) - 1
                        if 0 <= start_idx < len(clusters) and 0 <= end_idx < len(
                            clusters
                        ):
                ...
                # Handle single selection
                    ...
                    if 0 <= idx < len(clusters):
                        ...
                        print(
                            f"Invalid selection. Please enter a number between 1 and {len(clusters)}."
                        )
                        continue
    ...
    def cleanup_prometheus_scrapers(
        self, session: boto3.Session, cluster_name: str, region: str
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                for cluster_arn in clusters_response.get("clusterArns", []):
                    ...
                    if (
                        cluster_name in cluster_name_ecs
                        or "prometheus" in cluster_name_ecs.lower()
                    ):
                        ...
                        for service_arn in services_response.get("serviceArns", []):
                            ...
                            if (
                                cluster_name in service_name
                                or "prometheus" in service_name.lower()
                            ):
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"Error deleting ECS service {service_name}: {e}"
                                    )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Error deleting ECS cluster {cluster_name_ecs}: {e}"
                            )
            ...
            try:
                instances_response = ec2_client.describe_instances(
                    Filters=[
                        {"Name": "tag:Purpose", "Values": ["prometheus", "monitoring"]},
    ...
    def cleanup_lambda_functions(
        ...
        try:
            ...
            print(
                f"   🔍 Searching for Lambda functions related to cluster {cluster_name}..."
            )
            ...
            for page in paginator.paginate():
                for function in page.get("Functions", []):
                    ...
                    if (
                        f"eks-scale-{cluster_suffix}" in function_name
                        or f"eks-down-{cluster_suffix}" in function_name
                        or f"eks-up-{cluster_suffix}" in function_name
                        or cluster_name in function_name
                        or cluster_suffix in function_name
                    ):

                        logger.info(
                            f"Found potential matching Lambda function: {function_name}"
                        )
                        ...
                        try:
                            ...
                            tag_match = (
                                tags.get("Cluster") == cluster_name
                                or tags.get("cluster") == cluster_name
                                or tags.get("ClusterSuffix") == cluster_suffix
                                or cluster_name in tags.get("Purpose", "")
                                or cluster_suffix in tags.get("Purpose", "")
                            )
                            ...
                            if (
                                ...
                                logger.info(
                                    f"Deleting Lambda function: {function_name}"
                                )
                                ...
                                try:
                                    ...
                                    for mapping in mappings_response.get(
                                        "EventSourceMappings", []
                                    ):
                                    ...
                                    logger.warning(
                                        f"Error deleting event source mappings: {e}"
                                    )
                            ...
                            logger.warning(
                                f"Error processing Lambda function {function_name}: {e}"
                            )

            if deleted_functions:
                logger.info(
                    f"Successfully deleted {len(deleted_functions)} Lambda functions: {deleted_functions}"
                )
                print(
                    f"   ✅ Deleted {len(deleted_functions)} Lambda functions related to cluster {cluster_name}"
                )
            else:
                logger.info(
                    f"No Lambda functions found for deletion related to cluster {cluster_name}"
                )
                print(
                    f"   ℹ️ No Lambda functions found related to cluster {cluster_name}"
                )
                    ...
                    try:
                        ...
                        for rule in rules_response.get("Rules", []):
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Error deleting EventBridge rule {rule_name}: {e}"
                                )
                    except ClientError as e:
                        logger.debug(
                            f"No matching rules found for prefix {prefix}: {e}"
                        )

                if deleted_rules:
                    logger.info(
                        f"Successfully deleted {len(deleted_rules)} EventBridge rules"
                    )
                    print(
                        f"   ✅ Deleted {len(deleted_rules)} EventBridge rules related to cluster {cluster_name}"
                    )
                else:
    ...
    def delete_all_event_rules(self, session, cluster_name, region):
        ...
        try:
            ...
            logger.info(
                f"Searching for EventBridge rules related to cluster {cluster_name} in region {region}"
            )
            print(
                f"   🔍 Searching for EventBridge rules with suffix '{cluster_suffix}'..."
            )
            ...
            for page in paginator.paginate():
                for rule in page.get("Rules", []):
                    ...
                    # Method 2: Check if rule has specific cluster suffix (eks-down-diox, eks-up-diox)
                        ...
                        if (
                            f"-{cluster_suffix}" in rule_name
                            or rule_name.lower().endswith(f"-{cluster_suffix}")
                        ):
                            is_cluster_related = True
                        ...
                        try:
                            ...
                            if isinstance(tags, list):
                                ...
                                for key, value in tags.items():
                                    if (
                                        key.lower()
                                        in ["cluster", "clustername", "eks-cluster"]
                                        and value.lower() == cluster_name.lower()
                                    ) or key.lower() == f"kubernetes.io/cluster/{cluster_name.lower()}":
                                        is_cluster_related = True
                                        break
                    ...
                    if is_cluster_related:
                        logger.info(
                            f"Deleting EventBridge rule {rule_name} related to cluster {cluster_name}"
                        )
                        ...
                        try:
                            ...
                            deleted_rules.append(rule_name)
                            logger.info(
                                f"Successfully deleted EventBridge rule {rule_name}"
                            )
            ...
            if deleted_rules:
                ...
                print(
                    f"   ℹ️ No EventBridge rules found related to cluster {cluster_name}"
                )
        ...
        try:
            ...
            for nodegroup in nodegroups:
                ...
                try:
                    ...
                    logger.error(
                        f"Error waiting for node group {nodegroup} deletion: {e}"
                    )
                    return False
    ...
    def cleanup_associated_resources(
        self, session: boto3.Session, cluster_name: str, region: str
    ) -> None:
        ...
        try:
            ...
            try:
                ...
                for sg in security_groups.get("SecurityGroups", []):
                    ...
                    if sg_name == "eks-cluster-sg":
                        ...
                        continue
                    if (
                        sg_name != "default" and sg_name != "eks-cluster-sg"
                    ):  # Don't delete default SG
            ...
            try:
                alarms = cloudwatch_client.describe_alarms(AlarmNamePrefix=cluster_name)
    ...
    def cleanup_node_protection_lambda_functions(
        self, session: boto3.Session, cluster_name: str, region: str
    ) -> bool:
        """Clean up Lambda functions and EventBridge rules associated with node protection monitoring"""
        try:
            logger.info(
                f"Cleaning up node protection Lambda functions for cluster: {cluster_name}"
            )
            print(
                f"   🔍 Searching for node protection Lambda functions related to cluster {cluster_name}..."
            )
            ...
            logger.info(
                f"Using cluster suffix for node protection matching: {cluster_suffix}"
            )
            ...
            for page in paginator.paginate():
                for function in page.get("Functions", []):
                    ...
                    if (
                        f"node-protection-monitor-eks-{cluster_suffix}" in function_name
                        or f"node-protection-monitor-{cluster_name}" in function_name
                        or (
                            function_name.startswith("node-protection-monitor-eks")
                            and cluster_suffix in function_name
                        )
                    ):

                        logger.info(
                            f"Found node protection Lambda function: {function_name}"
                        )
                        ...
                        try:
                            ...
                            tag_match = (
                                ...
                                or tags.get("cluster") == cluster_name
                                or tags.get("ClusterSuffix") == cluster_suffix
                                or "node-protection" in tags.get("Purpose", "").lower()
                                or cluster_name in tags.get("Purpose", "")
                                or cluster_suffix in tags.get("Purpose", "")
                            )
                            ...
                            if (
                                ...
                                in function_name
                                or f"node-protection-monitor-{cluster_name}"
                                in function_name
                                or tag_match
                            ):

                                logger.info(
                                    f"Deleting node protection Lambda function: {function_name}"
                                )
                            ...
                            logger.warning(
                                f"Error processing node protection Lambda function {function_name}: {e}"
                            )
            ...
            if deleted_functions:
                ...
                logger.info(
                    f"No node protection Lambda functions found for deletion related to cluster {cluster_name}"
                )
                print(
                    f"   ℹ️ No node protection Lambda functions found related to cluster {cluster_name}"
                )
            ...
            try:
                ...
                for prefix in rule_prefixes:
                    try:
                        ...
                        for rule in rules_response.get("Rules", []):
                            rule_name = rule["Name"]
                            logger.info(
                                f"Found node protection EventBridge rule: {rule_name}"
                            )
                            ...
                            try:
                                ...
                                deleted_rules.append(rule_name)
                                logger.info(
                                    f"Deleted node protection EventBridge rule: {rule_name}"
                                )
                            except ClientError as e:
                                logger.warning(
                                    f"Error deleting node protection EventBridge rule {rule_name}: {e}"
                                )
                    except ClientError as e:
                        logger.debug(
                            f"No matching node protection rules found for prefix {prefix}: {e}"
                        )
                ...
                try:
                    ...
                    for rule in all_rules_response.get("Rules", []):
                        rule_name = rule["Name"]
                        if (
                            "node-protection" in rule_name.lower()
                            or "nodeprotection" in rule_name.lower()
                        ) and (
                            cluster_name in rule_name or cluster_suffix in rule_name
                        ):

                            if (
                                rule_name not in deleted_rules
                            ):  # Avoid duplicate deletion
                                logger.info(
                                    f"Found additional node protection EventBridge rule: {rule_name}"
                                )
                                try:
                                    # Remove targets first
                                    targets_response = (
                                    ...
                                    deleted_rules.append(rule_name)
                                    logger.info(
                                        f"Deleted additional node protection EventBridge rule: {rule_name}"
                                    )
                                except ClientError as e:
                                    logger.warning(
                    ...
                    logger.debug(
                        f"Error checking for additional node protection rules: {e}"
                    )

                if deleted_rules:
                    logger.info(
                        f"Successfully deleted {len(deleted_rules)} node protection EventBridge rules"
                    )
                    print(
                    ...
                    logger.info(
                        "No relevant node protection EventBridge rules found for deletion"
                    )

            except ClientError as e:
                logger.warning(
                    f"Error cleaning up node protection EventBridge rules: {e}"
                )
    ...
    def delete_cluster(self, cluster_data: Dict) -> bool:
        ...
        try:
            ...
            if not all([account_name, cluster_name, region]):
                logger.error(
                    f"Missing required information in cluster data: {cluster_data.get('file_name')}"
                )
                return False
            ...
            try:
                ...
                if cluster_status in ["DELETING", "FAILED"]:
                    logger.warning(
                        f"Cluster {cluster_name} is already in {cluster_status} state"
                    )
                    ...
                    self.cleanup_node_protection_lambda_functions(
                ...
                if e.response["Error"]["Code"] == "ResourceNotFoundException":
                    logger.info(
                        f"Cluster {cluster_name} not found, cleaning up associated resources"
                    )
                    # Still run cleanup for associated resources
                    self.cleanup_prometheus_scrapers(session, cluster_name, region)
                    self.cleanup_lambda_functions(session, cluster_name, region)
            ...
            if not self.cleanup_prometheus_scrapers(session, cluster_name, region):
                logger.warning(
                    "Some Prometheus resources may not have been cleaned up properly"
                )

            logger.info("Step 2: Cleaning up EventBridge rules...")
            if not self.delete_all_event_rules(session, cluster_name, region):
                logger.warning(
                    "Some EventBridge rules may not have been cleaned up properly"
                )
            ...
            if not self.cleanup_lambda_functions(session, cluster_name, region):
                logger.warning(
                    "Some Lambda functions may not have been cleaned up properly"
                )

            logger.info("Step 3.5: Cleaning up node protection Lambda functions...")
            if not self.cleanup_node_protection_lambda_functions(
                session, cluster_name, region
            ):
                logger.warning(
                    "Some node protection Lambda functions may not have been cleaned up properly"
                )
            ...
            try:
                waiter.wait(
                    name=cluster_name, WaiterConfig={"Delay": 30, "MaxAttempts": 60}
                )
    ...
    def run_cleanup(self) -> None:
        ...
        confirm = (
            input(
                "\nAre you sure you want to delete these clusters and all associated resources? (yes/no): "
            )
            .strip()
            .lower()
        )
        ...
        for i, cluster in enumerate(clusters_to_delete, 1):
            cluster_name = cluster.get("cluster_info", {}).get(
                "cluster_name", "Unknown"
            )
            logger.info(
                f"Processing cluster {i}/{len(clusters_to_delete)}: {cluster_name}"
            )
