...
class EKSClusterManager:
    ...
    def cleanup_lambda_functions(self, session: boto3.Session, cluster_name: str, region: str) -> bool:
        ...
        try:
            ...
            for function in functions_response.get('Functions', []):
                ...
                # Check if function is related to the cluster
                if (cluster_name in function_name or 
                    'node-scheduler' in function_name.lower() or
                    'eks-scheduler' in function_name.lower() or
                    'cluster-scheduler' in function_name.lower() or
                    'autoscaler' in function_name.lower()):
                    ...
                    try:
                        ...
                        # Check if tags indicate association with our cluster
                        if (tags.get('Cluster') == cluster_name or 
                            tags.get('cluster') == cluster_name or
                            cluster_name in tags.get('Purpose', '') or
                            cluster_name in tags.get('purpose', '')):
            ...
            try:
                rules_response = events_client.list_rules(NamePrefix=cluster_name)
                for rule in rules_response.get('Rules', []):
                    rule_name = rule['Name']
                    if cluster_name in rule_name:
    ...
    def delete_cluster(self, cluster_data: Dict) -> bool:
        ...
        try:
            ...
            logger.info("Step 3: Deleting cluster addons...")
            ...
            try:
                ...
                logger.info("Step 7: Cleaning up remaining associated resources...")
