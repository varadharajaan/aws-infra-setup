...
class DynamicNameGenerator:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for coin in data:
                    ...
                    name = coin.get('name', '').lower().replace(' ', '').replace('-', '')
    ...
    def load_api_data(self):
        ...
        self.brand_names = list(set([
            name for name in self.brand_names
            if name.isalpha() and 4 <= len(name) <= 15
        ]))

        self.tech_keywords = list(set([
            name for name in self.tech_keywords
            if name.isalpha() and 4 <= len(name) <= 15
        ]))
    ...
    def generate_custom_word(self):
        ...
        if random.random() < 0.7 and (self.brand_names or self.tech_keywords):
            ...
            fallback_words = ['python', 'django', 'flask', 'react', 'angular', 'nodejs',
                              'docker', 'kubernetes', 'github', 'gitlab', 'jenkins', 'terraform']
