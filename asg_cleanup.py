...
class ASGCleanupManager:
    ...
    def scan_asg_files(self) -> List[Dict]:
        ...
        patterns = [
            re.compile(
                r"^asg_report_(\d{8}_\d{6})\.json$"
            ),  # Exact pattern: asg_report_YYYYMMDD_HHMMSS.json
            re.compile(
                r"^asg_report_(\d{8}T\d{6}\.\d+)\.json$"
            ),  # Alternative with T and milliseconds
        ]
                ...
                if matched:
                    ...
                    if asg_data:
                        ...
                        if timestamp_from_filename:
                            asg_data["timestamp_from_filename"] = (
        ...
        logger.info(
            f"Found {len(asg_files)} valid ASG files (excluding failed reports)"
        )
        return asg_files
    ...
    def group_asgs_by_day(self, asg_files: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for asg in asg_files:
            try:
                ...
                logger.warning(
                    f"Error processing timestamp for {asg.get('file_name', 'unknown')}: {e}"
                )
    ...
    def display_day_options(self, grouped_asgs: Dict[str, List[Dict]]) -> None:
        ...
        sorted_days = sorted(
            [day for day in grouped_asgs.keys() if day != "unknown"], reverse=True
        )
        ...
        for i, asg in enumerate(asgs, 1):
            ...
            if created_asgs:
                for j, asg_info in enumerate(created_asgs):
                    ...
                    print(
                        f"      Instance Types: {', '.join(asg_info.get('instance_types', []))}"
                    )
                    print(
            ...
            try:
                ...
                # Handle comma-separated values
                    ...
                    for item in selection.split(","):
                        ...
                        indices.append(idx)
                    selected_asgs = [
                        asgs[idx] for idx in indices if 0 <= idx < len(asgs)
                    ]
    ...
    def cleanup_monitoring_resources(
        self, session: boto3.Session, asg_name: str, region: str
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                for topic in topics_response.get("Topics", []):
                    ...
                    if (
                        asg_name in topic_name
                        or "asg-notification" in topic_name.lower()
                    ):
            ...
            try:
                ...
                for policy in policies_response.get("ScalingPolicies", []):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error deleting scaling policy {policy_name}: {e}"
                        )
            ...
            try:
                ...
                for action in scheduled_response.get("ScheduledUpdateGroupActions", []):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error deleting scheduled action {action_name}: {e}"
                        )
    ...
    def cleanup_lambda_functions(
        ...
        try:
            ...
            for function in functions_response.get("Functions", []):
                ...
                if (
                    asg_name in function_name
                    or "asg-scaler" in function_name.lower()
                    or "autoscaling" in function_name.lower()
                    ...
                    try:
                        ...
                        if (
                            tags.get("ASG") == asg_name
                            or tags.get("AutoScalingGroup") == asg_name
                            or asg_name in tags.get("Purpose", "")
                            or asg_name in tags.get("Resource", "")
                        ):
            ...
            try:
                ...
                while time.time() - start_time < max_wait_time:
                    try:
                        ...
                        logger.info(
                            f"Still waiting for {len(running_instances)} instances to terminate..."
                        )
                        time.sleep(30)
                ...
                logger.warning(
                    "Timeout waiting for instances to terminate, proceeding anyway"
                )
                return True
        ...
        try:
            ...
            try:
                security_groups = ec2_client.describe_security_groups(
                    Filters=[{"Name": "tag:ASG", "Values": [asg_name]}]
                )
    ...
    def delete_asg(self, asg_data: Dict) -> bool:
        ...
        try:
            ...
            if not created_asgs:
                logger.error(
                    f"No ASGs found in data: {asg_data.get('file_name', 'Unknown')}"
                )
                return False
            ...
            for asg_info in created_asgs:
                ...
                try:
                    ...
                    if not asg_response.get("AutoScalingGroups"):
                        logger.info(
                            f"ASG {asg_name} not found, cleaning up associated resources"
                        )
                ...
                if not self.cleanup_scaling_policies(session, asg_name, region):
                    logger.warning(
                        "Some scaling policies may not have been cleaned up properly"
                    )
                    ...
                    while time.time() - start_time < max_wait_time:
                        try:
                            ...
                            if "does not exist" in str(e):
                                ...
                                logger.warning(
                                    f"Error checking ASG deletion status: {e}"
                                )
                ...
                logger.info(
                    f"ASG {asg_name} and associated resources deleted successfully"
                )
    ...
    def run_cleanup(self) -> None:
        ...
        confirm = (
            input(
                "\nAre you sure you want to delete these ASGs and all associated resources? (yes/no): "
            )
            .strip()
            .lower()
        )
