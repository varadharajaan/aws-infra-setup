...
class LambdaNodeProtectionMonitor:
    ...
    def log_execution_end(self, success=True):
        ...
        self.logger.info(f"{Symbols.STATS} Instances Scanned: {self.execution_stats['instances_scanned']}")
        self.logger.info(f"{Symbols.PROTECTED}  Protection Applied: {self.execution_stats['protection_applied']}")
        self.logger.info(f"[UNLOCKED] Protection Removed: {self.execution_stats['protection_removed']}")
        self.logger.info(f"{Symbols.SECURE} Protection Maintained: {self.execution_stats['protection_maintained']}")
        self.logger.info(f"{Symbols.ERROR} Errors: {self.execution_stats['errors_encountered']}")
        self.logger.info(f"{Symbols.WARN}  Warnings: {self.execution_stats['warnings_issued']}")
        self.logger.info(f"{Symbols.OK} Status: {'SUCCESS' if success else 'FAILED'}")
        self.logger.info("=" * 80)
    ...
    def generate_kubeconfig_with_boto3(self, cluster_name, region, access_key=None, secret_key=None):
        ...
        try:
            ...
            self.logger.info(f"{Symbols.SCAN} Retrieving cluster information for {cluster_name}...")
            ...
            self.logger.info(f"{Symbols.OK} Cluster endpoint: {endpoint}")
            self.logger.info(f"{Symbols.OK} CA data retrieved: {len(ca_data)} characters")

            # Get AWS account information
            identity = sts_client.get_caller_identity()
            account_id = identity['Account']

            self.logger.info(f"{Symbols.OK} Account ID: {account_id}")
            ...
            self.logger.info(f"{Symbols.OK} Kubeconfig generated at: {kubeconfig_path}")
            return kubeconfig_path

        except Exception as e:
            self.logger.error(f"{Symbols.ERROR} Failed to generate kubeconfig: {str(e)}")
            raise
    ...
    def find_kubectl_binary(self):
        ...
        if kubectl_path:
            self.logger.info(f"{Symbols.SCAN} Found kubectl via PATH: {kubectl_path}")
            return kubectl_path

        # Try direct paths
        for path in kubectl_paths:
            if os.path.exists(path) and os.access(path, os.X_OK):
                self.logger.info(f"{Symbols.SCAN} Found kubectl at: {path}")
                return path
    ...
    def annotate_k8s_node_scale_down_disabled(self, kubeconfig_path, node_name):
        ...
        self.logger.info(f"{Symbols.SECURE} Annotating node {node_name} with scale-down protection...")
        ...
        try:
            ...
            if check_result.returncode == 0 and check_result.stdout.strip() == "true":
                self.logger.info(f"{Symbols.OK} Node {node_name} already has scale-down protection")
                return True
            ...
            if annotate_result.returncode == 0:
                self.logger.info(f"{Symbols.OK} Successfully annotated node {node_name}")
                return True
            else:
                self.logger.error(f"{Symbols.ERROR} Failed to annotate node {node_name}: {annotate_result.stderr}")
                return False

        except subprocess.TimeoutExpired:
            self.logger.error(f"{Symbols.ERROR} Timeout annotating node {node_name}")
            return False
        except Exception as e:
            self.logger.error(f"{Symbols.ERROR} Error annotating node {node_name}: {str(e)}")
            return False
    ...
    def get_k8s_nodes_with_instance_ids(self, kubeconfig_path):
        ...
        try:
            ...
            if nodes_result.returncode != 0:
                self.logger.error(f"{Symbols.ERROR} Failed to get nodes: {nodes_result.stderr}")
                return {}
            ...
            for line in nodes_result.stdout.strip().split('\n'):
                ...
                if len(cols) >= 7:
                    ...
                    if instance_id and instance_id.startswith('i-'):
                        ...
                        self.logger.info(f"{Symbols.OK} Mapped instance {instance_id} to node {node_name}")

            self.logger.info(f"{Symbols.STATS} Successfully mapped {len(node_map)} instances to nodes")
            return node_map
            ...
            self.logger.error(f"{Symbols.ERROR} Error getting node information: {str(e)}")
            return {}
    ...
    def create_aws_clients(self, region, access_key=None, secret_key=None):
        ...
        try:
            ...
            self.logger.error(f"{Symbols.ERROR} Failed to create AWS clients: {str(e)}")
            raise
    ...
    def run_protection_monitor_logic(self, cluster_name: str, region: str, access_key: str = None, secret_key: str = None):
        ...
        try:
            ...
            if not nodegroups:
                self.logger.warning(f"{Symbols.WARN}  No nodegroups found for cluster {cluster_name}")
                self.execution_stats['warnings_issued'] += 1
            ...
            self.logger.info(f"{Symbols.OK} Found {len(nodegroups)} nodegroups: {nodegroups}")
            ...
            for i, ng_name in enumerate(nodegroups, 1):
                ...
                try:
                    ...
                    if status != 'ACTIVE':
                        self.logger.warning(f"   {Symbols.WARN}  Nodegroup {ng_name} is not ACTIVE (status: {status})")
                        self.execution_stats['warnings_issued'] += 1
                    ...
                    if not asg_name:
                        self.logger.warning(f"   {Symbols.WARN}  No ASG found for nodegroup {ng_name}")
                        continue
                    ...
                    if not asg_response['AutoScalingGroups']:
                        self.logger.warning(f"   {Symbols.WARN}  ASG {asg_name} not found")
                        continue
                    ...
                    if not instances:
                        self.logger.warning(f"   {Symbols.WARN}  No instances in service for nodegroup {ng_name}")
                        continue
                    ...
                    for instance in instances:
                        ...
                        try:
                            ...
                            if has_protection:
                                ...
                                self.logger.info(f"   │   {Symbols.OK} {instance_id}: Protected")
                            else:
                                ng_unprotected_instances.append(instance_id)
                                all_unprotected_instances.append(instance_info)
                                self.logger.info(f"   │   [UNLOCKED] {instance_id}: Not protected")

                            self.execution_stats['instances_scanned'] += 1

                        except Exception as e:
                            self.logger.error(f"   │   {Symbols.ERROR} Error checking {instance_id}: {str(e)}")
                    ...
                    self.logger.error(f"   {Symbols.ERROR} Error processing nodegroup {ng_name}: {str(e)}")
                    self.execution_stats['errors_encountered'] += 1

            # PHASE 2: Apply protection logic
            self.logger.info(f"{Symbols.PROTECTED}  PHASE 2: APPLYING PROTECTION LOGIC")
            self.logger.info("─" * 60)
            self.logger.info(
                f"{Symbols.STATS} Cluster-wide: {len(all_protected_instances)} protected, {len(all_unprotected_instances)} unprotected")
            ...
            if len(all_protected_instances) == 0 and len(all_unprotected_instances) > 0:
                ...
                self.logger.info(f"{Symbols.TARGET} Applying protection to: {instance_id}")
                ...
                try:
                    ...
                    self.logger.info(f"{Symbols.OK} Protection applied to {instance_id}")
                    ...
                    self.logger.error(f"{Symbols.ERROR} Failed to apply protection: {str(e)}")
                    self.execution_stats['errors_encountered'] += 1
                ...
                self.logger.info(f"{Symbols.TARGET} Perfect state: {target_instance_info['instance_id']} already protected")
                ...
                self.logger.info(f"{Symbols.TARGET} Removing excess protection from {len(remove_instances)} instances")
                ...
                for instance_info in remove_instances:
                    ...
                    try:
                        ...
                        self.logger.info(f"{Symbols.OK} Removed protection from {instance_id}")
                        ...
                        self.logger.error(f"{Symbols.ERROR} Failed to remove protection from {instance_id}: {str(e)}")
                        self.execution_stats['errors_encountered'] += 1
            ...
            if target_instance_info:
                ...
                if node_name and node_name != 'unknown':
                    self.logger.info(f"{Symbols.SECURE} PHASE 3: ANNOTATING KUBERNETES NODE")
                    self.logger.info("─" * 60)
                    self.logger.info(f"{Symbols.TARGET} Target node: {node_name} (instance: {instance_id})")
                    ...
                    if annotation_success:
                        ...
                        self.logger.warning(f"{Symbols.WARN}  Failed to annotate node {node_name}")
                        self.execution_stats['warnings_issued'] += 1
                else:
                    self.logger.warning(f"{Symbols.WARN}  Could not find Kubernetes node for instance {instance_id}")
                    self.execution_stats['warnings_issued'] += 1
            ...
            self.logger.error(f"{Symbols.ERROR} Protection monitor failed: {str(e)}")
            self.execution_stats['errors_encountered'] += 1
            self.log_execution_end(success=False)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"👤 Generated by: varadharajaan")
        logger.info(f"{Symbols.DATE} Template generated: 2025-07-04 13:50:34 UTC")
        logger.info(f"{Symbols.TARGET} Target cluster: {{CLUSTER_NAME}}")
        logger.info(f"{Symbols.REGION} Target region: {{REGION}}")

        # print the event triggered from eventbridge
        logger.info(f"{Symbols.SCAN} Event data: {json.dumps(event, indent=2)}")
        ...
        logger.info(f"{Symbols.TARGET} Final target: {cluster_name} in {region}")
        ...
        logger.info(f"{Symbols.OK} Lambda execution completed: {result['statusCode']}")
        return result
