...
class LambdaNodeProtectionMonitor:
    ...
    def log_execution_start(self, cluster_name, region):
        ...
        self.logger.info("=" * 80)
        self.logger.info("🚀 OPTIMIZED LAMBDA NODE PROTECTION MONITOR - START")
        self.logger.info("=" * 80)
        ...
        self.logger.info(f"📝 Template Generated: {{CURRENT_DATETIME}}")
        self.logger.info(f"🔧 Method: Boto3-based kubeconfig (NO AWS CLI)")
        self.logger.info("=" * 80)
    ...
    def generate_kubeconfig_with_boto3(
        ...
        """
        self.logger.info("🔧 Generating kubeconfig using Boto3...")
        ...
        try:
            ...
            self.logger.error(f"❌ Failed to generate kubeconfig: {str(e)}")
            raise
    ...
    def find_kubectl_binary(self):
        ...
        self.logger.warning("⚠️  kubectl not found in any expected location")
        return None
    ...
    def annotate_k8s_node_scale_down_disabled(self, kubeconfig_path, node_name):
        ...
        try:
            ...
            self.logger.error(f"❌ Timeout annotating node {node_name}")
            return False
        except Exception as e:
            self.logger.error(f"❌ Error annotating node {node_name}: {str(e)}")
            return False
    ...
    def get_k8s_nodes_with_instance_ids(self, kubeconfig_path):
        ...
        kubectl_path = self.find_kubectl_binary()
        if not kubectl_path:
            self.logger.error("❌ kubectl not found. Cannot get node information.")
            return {}

        self.logger.info("🔍 Retrieving Kubernetes nodes with instance IDs...")
        ...
        try:
            ...
            self.logger.error("❌ Timeout getting node information")
            return {}
        except Exception as e:
            self.logger.error(f"❌ Error getting node information: {str(e)}")
            return {}

    def create_aws_clients(self, region, access_key=None, secret_key=None):
        """Create AWS clients with validation"""
        self.logger.info("🔧 Creating AWS clients...")
    ...
    def run_protection_monitor_logic(
        ...
        try:
            ...
            self.logger.info("🔍 PHASE 1: SCANNING NODEGROUPS FOR PROTECTION STATUS")
            self.logger.info("─" * 60)
...
def lambda_handler(event, context):
    ...
    """
    try:
        logger.info("🚀 Optimized Lambda function started")
        logger.info(f"📦 No AWS CLI dependency - using Boto3 for kubeconfig")
