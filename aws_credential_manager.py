    ...
    def display_iam_accounts_with_users(self) -> List[Dict]:
        ...
        for i, (account_name, account_data) in enumerate(accounts.items(), 1):
            ...
            for j, (username, region) in enumerate(users_regions.items(), 1):
                ...
                print(
                    f"         {j}. {username} ({user_info['full_name']}) - Region: {region}"
                )
            print()
        ...
        while True:
            try:
                ...
                if 0 <= account_index < len(root_accounts):
                    ...
                    print(
                        f"❌ Invalid choice. Please enter a number between 1 and {len(root_accounts)}"
                    )
            except ValueError:
    ...
    def select_iam_credentials(self) -> CredentialInfo:
        ...
        while True:
            try:
                ...
                if 0 <= account_index < len(iam_accounts):
                    ...
                    print(
                        f"❌ Invalid choice. Please enter a number between 1 and {len(iam_accounts)}"
                    )
            except ValueError:
                print("❌ Please enter a valid number")
        ...
        while True:
            try:
                ...
                if 0 <= user_index < len(user_list):
                    ...
                    print(
                        f"❌ Invalid choice. Please enter a number between 1 and {len(user_list)}"
                    )
            except ValueError:
                print("❌ Please enter a valid number")
        ...
        try:
            ...
            for filename in os.listdir("./aws/iam/"):
                if filename.startswith("iam_users_credentials_") and filename.endswith(
                    ".json"
                ):
            ...
            sorted_files = sorted(
                iam_cred_files, key=lambda x: x["timestamp"], reverse=True
            )
            ...
            if file_choice == "1" or not file_choice:
                ...
                while True:
                    try:
                        file_index = (
                            int(input(f"Select file (1-{len(sorted_files)}): ").strip())
                            - 1
                        )
                        if 0 <= file_index < len(sorted_files):
                            selected_file = sorted_files[file_index]["path"]
                            print(
                                f"✅ Selected file: {sorted_files[file_index]['filename']}"
                            )
                            break
                        else:
                            print(
                                f"❌ Invalid choice. Please enter a number between 1 and {len(sorted_files)}"
                            )
                    except ValueError:
                        print("❌ Please enter a valid number")
            ...
            if not user_creds:
                print(
                    f"❌ Credentials for user {selected_username} not found in the selected file"
                )
            ...
            return CredentialInfo(
                ...
                email=user_creds.get("real_user", {}).get(
                    "email", selected_account["account_data"].get("email")
                ),
                access_key=user_creds.get(
                    "access_key_id"
                ),  # Using actual IAM user credentials
                secret_key=user_creds.get(
                    "secret_access_key"
                ),  # Using actual IAM user credentials
                credential_type="iam",
                regions=[selected_region],
        ...
        while True:
            try:
                ...
                if 0 <= region_index < len(available_regions):
                    ...
                    print(
                        f"❌ Invalid choice. Please enter a number between 1 and {len(available_regions)}"
                    )
            except ValueError:
                print("❌ Please enter a valid number")
        ...
        try:
            ...
            for username, user_data in matching_users.items():
                ...
                users.append(
                    {
                        "username": username,
                        "access_key": account_data.get(
                            "access_key", ""
                        ),  # Use account credentials
                        "secret_key": account_data.get(
                            "secret_key", ""
                        ),  # Use account credentials
                        "account_id": account_data.get("account_id", "Unknown"),
                        "region": region,
        ...
        try:
            ...
            if os.path.exists(self.mapping_file):
                ...
                if "account_distribution" in mapping_data:
                    ...
                    for account_name, account_info in mapping_data[
