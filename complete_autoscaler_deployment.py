...
class CompleteAutoscalerDeployer:
    ...
    def create_aws_session(
        ...
        try:
            ...
            self.log_step(
                "AWS",
                f"Session created for account: {identity.get('Account')}",
                "SUCCESS",
            )
            return session
        except Exception as e:
    ...
    def setup_kubeconfig(
        ...
        self.log_step(
            "KUBE", f"Cluster access verified. Found {node_count} nodes", "SUCCESS"
        )
    ...
    def discover_and_tag_asgs(
        ...
        try:
            ...
            for asg in response.get("AutoScalingGroups", []):
                ...
                if any(
                    pattern in asg_name.lower() for pattern in ["eks", "nodegroup"]
                ) and any(part in asg_name for part in cluster_name.split("-")):
            ...
            if not cluster_asgs:
                self.log_step(
                    "ASG",
                    "No ASGs found with cluster tags, searching by name patterns...",
                    "WARNING",
                )
            ...
            self.log_step(
                "ASG", f"Found {len(cluster_asgs)} ASGs: {cluster_asgs}", "SUCCESS"
            )
            ...
            for asg_name in cluster_asgs:
                try:
                    ...
                    self.log_step(
                        "ASG", f"Failed to tag ASG {asg_name}: {str(e)}", "ERROR"
                    )

            self.log_step(
                "ASG", f"Successfully tagged {len(tagged_asgs)} ASGs", "SUCCESS"
            )
            return tagged_asgs
...
"""
    ...
    def deploy_autoscaler(self, cluster_name: str, region: str, env: Dict) -> bool:
        ...
        try:
            ...
            if success:
                self.log_step(
                    "DEPLOY", "Autoscaler deployment applied successfully", "SUCCESS"
                )
                return True
            else:
    ...
    def wait_for_autoscaler_ready(self, env: Dict, timeout: int = 300) -> bool:
        ...
        self.log_step(
            "WAIT", f"Waiting for autoscaler to be ready (timeout: {timeout}s)..."
        )
                ...
                for line in lines:
                    ...
                    if len(parts) >= 3:
                        ...
                        if ready_status == "1/1" and pod_status == "Running":
                            self.log_step(
                                "WAIT",
                                f"Autoscaler pod is ready: {pod_name}",
                                "SUCCESS",
                            )
                            return True
                        elif pod_status in [
                            ...
                            self.log_step(
                                "WAIT", f"Pod failed with status: {pod_status}", "ERROR"
                            )
    ...
    def print_success_summary(
        self, cluster_name: str, region: str, tagged_asgs: List[str]
    ):
        ...
        self.print_colored(
            self.colors.GREEN, "✅ AUTOSCALER DEPLOYMENT COMPLETED SUCCESSFULLY!"
        )
        ...
        self.print_colored(
            self.colors.WHITE, f"   • Tagged ASGs: {len(tagged_asgs)}", 1
        )
        self.print_colored(
            self.colors.WHITE, f"   • Deployment Time: {elapsed_time:.1f} seconds", 1
        )
        self.print_colored(
            self.colors.WHITE, f"   • Fast Scaling: 4 minute delays configured", 1
        )
        self.print_colored(
            self.colors.WHITE,
            f"   • Protected Nodes: Nodes with label 'no_delete=true' will be skipped",
            1,
        )

        self.print_colored(self.colors.YELLOW, "\n🧪 Testing Commands:")
        self.print_colored(
            self.colors.WHITE, "   # Scale up test (trigger node addition):", 1
        )
        self.print_colored(
            self.colors.CYAN,
        ...
        self.print_colored(
            self.colors.CYAN, "   kubectl delete deployment test-scale", 1
        )
    ...
    def deploy_complete_autoscaler(
        self,
        cluster_name: str,
        region: str,
        access_key: str,
        ...
        try:
            ...
            self.print_colored(
                self.colors.WHITE, f"   • Fast Scaling: 4 minute delays", 1
            )
            self.print_colored(
                self.colors.WHITE,
                f"   • Protected Nodes: Using 'no_delete=true' label",
                1,
            )
            ...
            if not tagged_asgs:
                self.log_step(
                    "ASG",
                    "No ASGs were tagged, autoscaler may not work properly",
                    "WARNING",
                )
            ...
            if not self.wait_for_autoscaler_ready(env):
                self.log_step(
                    "DEPLOY",
                    "Autoscaler may not be fully ready, but continuing",
                    "WARNING",
                )
