...
class EC2ASGAutomation:
    ...
    def get_global_automation_preferences(self, first_credential: CredentialInfo) -> dict:
        ...
        print("\n[SETTINGS] STEP 2: GLOBAL AUTOMATION PREFERENCES")
    ...
    def select_global_instance_type(self, credential: CredentialInfo, ec2_strategy: str) -> str:
        ...
        print("\n[STATS] GLOBAL EC2 INSTANCE TYPE SELECTION")
        ...
        if ec2_strategy == 'on-demand':
            ...
            print("[STATS] Analyzing spot instances for reference...")
    ...
    def select_global_ondemand_instances(self, credential: CredentialInfo, allowed_types: list) -> dict:
        ...
        print("[STATS] On-Demand Instance Analysis...")
        quota_info = self.spot_analyzer.analyze_service_quotas(credential, allowed_types)
        ...
        while True:
            try:
                ...
                print("[ERROR] Invalid input. Please enter comma-separated numbers, 'all', or 'custom'")
                continue
    ...
    def select_global_spot_instances(self, credential: CredentialInfo, allowed_types: list) -> dict:
        ...
        print("[STATS] Spot Instance Analysis...")
        spot_analyses = self.spot_analyzer.analyze_spot_instances(credential, allowed_types, False)
        ...
        while True:
            try:
                ...
                print("[ERROR] Invalid input. Please enter comma-separated numbers or 'all'")
                continue
            except KeyboardInterrupt:
                print("\n[ERROR] Selection cancelled.")
                return {'spot': []}
    ...
    def select_global_mixed_instances(self, credential: CredentialInfo, allowed_types: list) -> dict:
        ...
        print("[STATS] Mixed Strategy Analysis...")
        ...
        try:
            ...
            if global_config.get('create_ec2', False):
                try:
                    ...
                    if self.dry_run:
                        print(f"[TEST] DRY-RUN: Would create {global_config['instance_type']} instance")
                        result['ec2_instance'] = {
            ...
            if global_config.get('create_asg', False):
                try:
                    ...
                    if self.dry_run:
                        print(f"[TEST] DRY-RUN: Would create ASG with {global_config['asg_strategy']} strategy")
                        result['asg'] = {
    ...
    def run_automation(self):
        ...
        try:
            ...
            print("\n[KEY] STEP 1: ENHANCED CREDENTIAL SELECTION")
            ...
            if multi_credentials.total_users == 0:
                print("[ERROR] No credentials selected. Exiting...")
                return False
            ...
            print("\n[PROTECTED] STEP 2.5: PERFORMING SAFETY CHECKS")
    ...
    def display_automation_summary(self, validated_credentials: MultiUserCredentials, global_config: dict):
        ...
        print(f"   [COMPUTE] Create EC2: {'Yes' if global_config.get('create_ec2') else 'No'}")
        if global_config.get('create_ec2'):
    ...
    def display_final_results(self, results: list, global_config: dict):
        ...
        print("\n" + "="*80)
        print("[PARTY] MULTI-USER AUTOMATION COMPLETED!")
        print("="*80)
            ...
            for result in successful:
                ...
                if result.get('ec2_instance'):
                    ...
                    if ec2_info.get('dry_run'):
                        print(f"   [COMPUTE] EC2: {ec2_info['instance_id']} (DRY-RUN)")
                    else:
                        print(f"   [COMPUTE] EC2: {ec2_info['instance_id']} ({ec2_info.get('instance_type', 'Unknown')})")
...
def main():
    ...
    if args.dry_run:
        print("[TEST] Running in DRY-RUN mode - no actual resources will be created")
    ...
    try:
        ...
        if success:
            ...
            print("\n[ERROR] Automation failed or was interrupted")
            sys.exit(1)
    except Exception as e:
