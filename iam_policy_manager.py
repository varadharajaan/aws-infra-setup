...
class IAMPolicyManager:
    ...
    def delete_custom_policies_from_role(
        self, role_name: str, dry_run: bool = False
    ) -> Dict[str, Any]:
        ...
        if not self.iam_client:
            return {
                ...
                "errors": ["IAM client not initialized"],
            }
        ...
        try:
            self.logger.info(
                f"{'DRY RUN - ' if dry_run else ''}Processing role: {role_name}"
            )
            ...
            for policy in attached_policies:
                ...
                if self._is_custom_policy(policy_arn):
                    results["custom_policies_found"].append(
                        {"name": policy_name, "arn": policy_arn}
                    )
                    self.logger.info(f"Found custom policy: {policy_name}")

                    if not dry_run:
                        delete_result = self._detach_and_delete_custom_policy(
                            role_name, policy_arn, policy_name
                        )
                    ...
                    results["aws_managed_policies_found"].append(
                        {"name": policy_name, "arn": policy_arn}
                    )
    ...
    def delete_all_custom_policies_in_account(
        self, dry_run: bool = False, exclude_policies: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(
                f"{'DRY RUN - ' if dry_run else ''}Starting account-wide policy deletion"
            )
            ...
            for policy in custom_policies:
                ...
                if not dry_run:
                    delete_result = self._detach_and_delete_policy_completely(
    ...
    def delete_custom_roles(
        self, role_names: Optional[List[str]] = None, dry_run: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            if role_names:
                ...
                custom_roles = [
                    role for role in all_roles if self._is_custom_role(role)
                ]
                ...
                results["roles_to_delete"] = (
                    custom_roles  # For now, select all custom roles
                )
            ...
            for role_info in results["roles_to_delete"]:
                ...
                if not dry_run:
                    ...
                    if delete_result["success"]:
                        ...
                        results["failed_deletions"].append(
                            {"role_name": role_name, "reason": delete_result["error"]}
                        )
    ...
    def _detach_and_delete_custom_policy(
        self, role_name: str, policy_arn: str, policy_name: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    detach_result = self._detach_policy_from_all_entities(
                        policy_arn, policy_name
                    )
                    if not detach_result["success"]:
                        return {
                            "success": False,
                            "error": f"Failed to detach from all entities",
                        }
            except ClientError:
                pass
    ...
    def _detach_and_delete_policy_completely(
        self, policy_arn: str, policy_name: str
    ) -> Dict[str, Any]:
