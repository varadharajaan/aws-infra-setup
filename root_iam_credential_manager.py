...
class AWSCredentialManager:
    ...
    def __init__(self, config_dir: str = None):
        ...
        self.print_colored(Colors.CYAN, f"📁 Using config directory: {self.config_dir}")
        ...
        # Set up file paths
        self.aws_accounts_config_file = os.path.join(
            config_dir, "aws_accounts_config.json"
        )
        self.iam_dir = os.path.join(config_dir, "aws", "iam")
        self.iam_users_pattern = os.path.join(
            self.iam_dir, "iam_users_credentials_*.json"
        )
    ...
    def _check_config_files(self):
        ...
        self.print_colored(Colors.CYAN, "🔍 Checking configuration files...")

        # Check aws_accounts_config.json
        if not os.path.exists(self.aws_accounts_config_file):
            self.print_colored(
                Colors.YELLOW,
                f"⚠️  Warning: aws_accounts_config.json not found in {self.config_dir}",
            )
        elif not os.access(self.aws_accounts_config_file, os.R_OK):
            self.print_colored(
                Colors.YELLOW, f"⚠️  Warning: Cannot read aws_accounts_config.json"
            )
        else:
            self.print_colored(Colors.GREEN, f"✅ Found aws_accounts_config.json")

        # Check aws/iam directory
        if not os.path.exists(self.iam_dir):
            self.print_colored(
                Colors.YELLOW,
                f"⚠️  Warning: aws/iam directory not found in {self.config_dir}",
            )
        elif not os.access(self.iam_dir, os.R_OK):
            self.print_colored(
                Colors.YELLOW, f"⚠️  Warning: Cannot read aws/iam directory"
            )
        else:
            self.print_colored(Colors.GREEN, f"✅ Found aws/iam directory")
            iam_files = glob.glob(self.iam_users_pattern)
            if not iam_files:
                self.print_colored(
                    Colors.YELLOW,
                    f"⚠️  Warning: No iam_users_credentials_*.json files found",
                )
            else:
                self.print_colored(
                    Colors.GREEN, f"✅ Found {len(iam_files)} IAM credential files"
                )
    ...
    def load_root_accounts_config(self) -> Optional[Dict]:
        ...
        try:
            if not os.path.exists(self.aws_accounts_config_file):
                self.print_colored(
                    Colors.RED,
                    f"❌ AWS accounts config file not found: {self.aws_accounts_config_file}",
                )
                return None

            if not os.access(self.aws_accounts_config_file, os.R_OK):
                self.print_colored(
                    Colors.RED, f"❌ Cannot read AWS accounts config file"
                )
                return None
            ...
            if "accounts" not in config:
                self.print_colored(
                    Colors.RED, "❌ Invalid config file: missing 'accounts' section"
                )
                return None

            accounts_count = len(config["accounts"])
            self.print_colored(
                Colors.GREEN, f"✅ Loaded {accounts_count} root accounts"
            )
            return config

        except json.JSONDecodeError as e:
            self.print_colored(
                Colors.RED, f"❌ Error parsing root accounts config: {e}"
            )
            return None
        except Exception as e:
            self.print_colored(
                Colors.RED, f"❌ Error loading root accounts config: {e}"
            )
            return None
    ...
    def get_root_account_by_key(self, account_key: str) -> Optional[Dict[str, Any]]:
        ...
        if account_key not in accounts:
            self.print_colored(Colors.RED, f"❌ Root account '{account_key}' not found")
            return None

        account_data = accounts[account_key]
        default_regions = config.get("user_settings", {}).get(
            "user_regions", ["us-east-1"]
        )
    ...
    def get_root_account_by_id(self, account_id: str) -> Optional[Dict[str, Any]]:
        ...
        self.print_colored(Colors.RED, f"❌ No root account found for ID: {account_id}")
        return None
    ...
    def select_root_accounts_interactive(
        ...
        accounts = config.get("accounts", {})
        if not accounts:
            self.print_colored(Colors.RED, "❌ No root accounts found!")
            return None

        self.print_colored(Colors.YELLOW, "\n🔑 Available Root AWS Accounts:")
        ...
        for i, account_key in enumerate(account_keys, 1):
            ...
            email = (
                account["email"][:40] + "..."
                if len(account["email"]) > 43
                else account["email"]
            )

            self.print_colored(
                Colors.CYAN, f"   {i}. {account_key} (ID: {account['account_id']})"
            )
            self.print_colored(
                Colors.WHITE,
                f"      Email: {email}, Users: {account.get('users_per_account', 0)}",
            )
        ...
        while True:
            try:
                if allow_multiple:
                    choice = input(
                        f"Select accounts (1-{len(account_keys)}, comma-separated, range, or 'all') or 'q' to quit: "
                    ).strip()
                else:
                    choice = input(
                        f"Select account (1-{len(account_keys)}) or 'q' to quit: "
                    ).strip()
                ...
                if allow_multiple:
                    if choice.lower() == "all":
                        ...
                        selected_indices = self._parse_selection(
                            choice, len(account_keys)
                        )
                        if not selected_indices:
                            self.print_colored(
                                Colors.RED, "❌ Invalid selection format"
                            )
                            continue
                    ...
                    if 1 <= choice_num <= len(account_keys):
                        ...
                        self.print_colored(
                            Colors.RED,
                            f"❌ Invalid choice. Please enter 1-{len(account_keys)}",
                        )
                        continue
                    ...
                    selected_accounts.append(
                        {
                            ...
                            "region": default_regions[0],
                            "source_type": "root",
                            "users_per_account": account_data.get(
                                "users_per_account", 0
                            ),
                        }
                    )
                ...
                self.print_colored(
                    Colors.GREEN, f"✅ Selected {len(selected_accounts)} account(s)"
                )
            ...
            for file_path in files:
                try:
                    ...
                    if not timestamp_info:
                        ...
                        timestamp_info = {
                            "formatted_timestamp": mod_time.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
    ...
    def load_iam_users_from_file(self, file_path: str) -> Optional[Dict]:
        ...
        try:
            if not os.access(file_path, os.R_OK):
                self.print_colored(
                    Colors.RED, f"❌ Cannot read IAM users file: {file_path}"
                )
                return None

            self.print_colored(
                Colors.CYAN, f"📖 Loading IAM users from: {os.path.basename(file_path)}"
            )
            with open(file_path, "r") as f:
                data = json.load(f)
            ...
            if "accounts" not in data:
                self.print_colored(
                    Colors.RED, "❌ Invalid IAM users file: missing 'accounts' section"
                )
                return None

            total_users = sum(
                len(account_data.get("users", []))
            ...
            self.print_colored(
                Colors.GREEN,
                f"✅ Loaded {total_users} users from {accounts_count} accounts",
            )
            return data

        except Exception as e:
            self.print_colored(Colors.RED, f"❌ Error loading IAM users file: {e}")
            return None
        ...
        while True:
            try:
                ...
                if 1 <= choice_num <= len(iam_files):
                    ...
                    self.print_colored(
                        Colors.GREEN, f"✅ Selected: {selected_file['filename']}"
                    )
    ...
    def select_iam_users_interactive(
        self, file_path: str
    ) -> Optional[List[Dict[str, Any]]]:
        ...
        for i, user in enumerate(all_users, 1):
            if user["account_key"] != current_account:
                ...
                self.print_colored(
                    Colors.PURPLE,
                    f"\n📋 Account: {current_account} (ID: {user['account_id']})",
                )

            real_name_display = (
                user["real_name"][:25] + "..."
                if len(user["real_name"]) > 28
                else user["real_name"]
            )
            email_display = (
                user["email"][:35] + "..." if len(user["email"]) > 38 else user["email"]
            )

            self.print_colored(
                Colors.CYAN, f"   {i}. {user['username']} ({user['region']})"
            )
            self.print_colored(
                Colors.WHITE, f"      Name: {real_name_display}, Email: {email_display}"
            )
        ...
        while True:
            try:
                choice = input(
                    f"Select users (1-{len(all_users)}, comma-separated, range, or 'all') or 'q' to quit: "
                ).strip()
                ...
                if choice.lower() == "q":
                    ...
                    self.print_colored(
                        Colors.GREEN, f"✅ Selected all {len(all_users)} users"
                    )
                    return all_users
                else:
                    ...
                    if selected_indices:
                        ...
                        self.print_colored(
                            Colors.GREEN, f"✅ Selected {len(selected_users)} users"
                        )
                        return selected_users
                    else:
                        self.print_colored(
                            Colors.RED,
                            "❌ Invalid selection format. Examples: 1, 1,3,5, 1-5, all",
                        )

            except Exception as e:
                self.print_colored(Colors.RED, f"❌ Error processing selection: {e}")
    ...
    def _parse_timestamp_from_filename(self, filename: str) -> Optional[Dict[str, str]]:
        ...
        timestamp_match = re.search(
            r"_(\d{4}-\d{2}-\d{2})_(\d{2}-\d{2}-\d{2})\.json$", filename
        )
        if timestamp_match:
            date_str = timestamp_match.group(1)
            time_str = timestamp_match.group(2)
