...
class UltraCleanupASGManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{self.asg_dir}/ultra_asg_cleanup_log_{self.execution_timestamp}.log"
            )
    ...
    def create_asg_client(self, access_key, secret_key, region):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to create ASG client for {region}: {e}"
            )
            raise
    ...
    def create_cloudwatch_client(self, access_key, secret_key, region):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to create CloudWatch client for {region}: {e}"
            )
            raise
    ...
    def delete_asg_related_alarms(self, access_key, secret_key, region, asg_name):
        ...
        try:
            self.log_operation(
                "INFO", f"🔍 Searching for CloudWatch alarms related to ASG: {asg_name}"
            )
            print(
                f"   🔍 Searching for CloudWatch alarms related to ASG: {asg_name}..."
            )
                ...
                for alarm in page.get("MetricAlarms", []):
                    ...
                    for pattern in asg_patterns:
                        if pattern.lower() in alarm_name.lower():
                            is_asg_related = True
                            self.log_operation(
                                "INFO", f"Found alarm by name pattern: {alarm_name}"
                            )
                            break
                    ...
                    if not is_asg_related:
                        for dimension in alarm.get("Dimensions", []):
                            if (
                                dimension.get("Name") == "AutoScalingGroupName"
                                and dimension.get("Value") == asg_name
                            ):
                                is_asg_related = True
                                self.log_operation(
                                    "INFO", f"Found alarm by dimension: {alarm_name}"
                                )
                                break
                    ...
                    if not is_asg_related:
                        ...
                        if asg_name.lower() in description.lower():
                            is_asg_related = True
                            self.log_operation(
                                "INFO", f"Found alarm by description: {alarm_name}"
                            )

                    if is_asg_related:
                        try:
                            self.log_operation(
                                "INFO", f"🗑️  Deleting CloudWatch alarm: {alarm_name}"
                            )
                            ...
                            self.log_operation(
                                "INFO", f"✅ Deleted CloudWatch alarm: {alarm_name}"
                            )
                            ...
                            self.log_operation(
                                "ERROR",
                                f"Failed to delete alarm {alarm_name}: {delete_error}",
                            )
                            print(
                                f"      ❌ Failed to delete alarm {alarm_name}: {delete_error}"
                            )
            ...
            if deleted_alarms:
                self.log_operation(
                    "INFO",
                    f"Deleted {len(deleted_alarms)} CloudWatch alarms for ASG {asg_name}",
                )
                print(
                    f"   ✅ Deleted {len(deleted_alarms)} CloudWatch alarms for ASG {asg_name}"
                )
                ...
                self.log_operation(
                    "INFO", f"No CloudWatch alarms found for ASG {asg_name}"
                )
            ...
            self.log_operation(
                "ERROR", f"Failed to delete CloudWatch alarms for ASG {asg_name}: {e}"
            )
    ...
    def get_all_asgs_in_region(self, asg_client, region, account_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🔍 Scanning for ASGs in {region} ({account_name})"
            )
            ...
            for page in paginator.paginate():
                for asg in page["AutoScalingGroups"]:
                    ...
                    if asg_name.startswith("eks-") or asg_name.startswith("k8s-"):
                        self.log_operation(
                            "INFO",
                            f"Skipping ASG {asg_name} as it is an EKS or K8s managed ASG",
                        )
                        print(
                            f"   ℹ️ Skipping ASG {asg_name} as it is an EKS or K8s managed ASG"
                        )
                        continue
                    ...
                    for tag in asg.get("Tags", []):
                        if tag["Key"] == "Name":
                            ...
                            tag["Key"] == "CreatedAt"
                        ):  # Added for compatibility with ASG creation script
                            created_time = tag["Value"]
                        elif (
                            tag["Key"] == "CreatedBy"
                        ):  # Added for compatibility with ASG creation script
                    ...
                    if "LaunchTemplate" in asg:
                        launch_template_id = asg["LaunchTemplate"].get(
                            "LaunchTemplateId"
                        )
                        launch_template_name = asg["LaunchTemplate"].get(
                            "LaunchTemplateName"
                        )
            ...
            self.log_operation(
                "INFO", f"📦 Found {len(asgs)} ASGs in {region} ({account_name})"
            )
            ...
            account_name = account_info.get("account_key", "Unknown")
            self.log_operation(
                "ERROR", f"Error getting ASGs in {region} ({account_name}): {e}"
            )
    ...
    def delete_asg(self, asg_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🗑️  Deleting ASG {asg_name} in {region} ({account_name})"
            )
            ...
            try:
                ...
                if actions:
                    self.log_operation(
                        "INFO", f"⏰ Found {len(actions)} scheduled action(s)"
                    )
                    ...
                    for action in actions:
                        ...
                        self.log_operation(
                            "INFO", f"🗑️ Deleting scheduled action: {action_name}"
                        )
                ...
                self.log_operation(
                    "WARNING", f"⚠️ Warning: Failed to clean up scheduled actions: {e}"
                )
            ...
            try:
                ...
                if policies:
                    self.log_operation(
                        "INFO", f"📈 Found {len(policies)} scaling policy(s)"
                    )
                    ...
                    for policy in policies:
                        ...
                        self.log_operation(
                            "INFO", f"🗑️ Deleting scaling policy: {policy_name}"
                        )
                        print(f"   🗑️ Deleting scaling policy: {policy_name}")

                        asg_client.delete_policy(
                            AutoScalingGroupName=asg_name, PolicyName=policy_name
                        )
            except Exception as e:
                self.log_operation(
                    "WARNING", f"⚠️ Warning: Failed to clean up scaling policies: {e}"
                )
                print(f"   ⚠️ Warning: Failed to clean up scaling policies: {e}")

            # Step 4: Delete the ASG with ForceDelete to terminate instances
            self.log_operation(
                "INFO", f"💥 Deleting Auto Scaling Group with Force option"
            )
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🧹 Starting ASG cleanup for {account_key} ({account_id}) in {region}",
            )
            print(
            ...
            try:
                ...
                self.log_operation(
                    "ERROR", f"Could not create ASG client for {region}: {client_error}"
                )
            ...
            self.log_operation(
                "INFO", f"📊 {account_key} ({region}) ASG resources summary:"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Error discovering ASG resources in {account_key} ({region}): {e}",
            )
            print(
                f"   ❌ Error discovering ASG resources in {account_key} ({region}): {e}"
            )
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            for alarm in self.cleanup_results["deleted_alarms"]:
                ...
                )  # This might need adjustment based on how we store account info
                region = alarm["region"]
            ...
            self.log_operation(
                "INFO", f"✅ Ultra ASG cleanup report saved to: {report_filename}"
            )
            return report_filename

        except Exception as e:
            self.log_operation(
                "ERROR", f"❌ Failed to save ultra ASG cleanup report: {e}"
            )
            return None
    ...
    def run(self):
        ...
        try:
            ...
            for account_info in selected_accounts:
                ...
                for region in selected_regions:
                    ...
                    try:
                        ...
                        self.log_operation(
                            "ERROR", f"Error processing {account_key} ({region}): {e}"
                        )
            ...
            self.print_colored(Colors.YELLOW, "\n" + "=" * 80)
            self.print_colored(
                Colors.YELLOW, "📋 ALL AUTO SCALING GROUPS ACROSS ACCOUNTS"
            )
            self.print_colored(Colors.YELLOW, "=" * 80)

            # Display ASGs with more informative headers
            print(
                f"{'#':<4} {'ASG Name':<40} {'Size':<15} {'Instances':<10} {'Account':<15} {'Region'}"
            )
            print("-" * 100)
            ...
            for i, asg in enumerate(self.all_asgs, 1):
                ...
                print(
                    f"{i:<4} {asg_name:<40} {size_info:<15} {instances:<10} {account_name:<15} {region}"
                )
            ...
            self.print_colored(
                Colors.WHITE, f"    • {len(selected_asgs)} Auto Scaling Groups"
            )
            self.print_colored(Colors.WHITE, f"    • Associated CloudWatch Alarms")
            self.print_colored(
                Colors.WHITE, f"    • Scaling Policies and Scheduled Actions"
            )
            ...
            for i, asg in enumerate(selected_asgs, 1):
                ...
                try:
                    ...
                    self.print_colored(
                        Colors.RED, f"❌ Error deleting ASG {asg_name}: {e}"
                    )
            ...
            self.print_colored(
                Colors.GREEN, f"✅ Successfully deleted: {successful} ASGs"
            )
            self.print_colored(Colors.RED, f"❌ Failed to delete: {failed} ASGs")
            self.print_colored(
                Colors.WHITE,
                f"📦 Total ASGs deleted: {len(self.cleanup_results['deleted_asgs'])}",
            )
            self.print_colored(
                Colors.WHITE,
                f"🔔 Total alarms deleted: {len(self.cleanup_results['deleted_alarms'])}",
            )

            self.log_operation("INFO", f"ASG CLEANUP COMPLETED")
            self.log_operation("INFO", f"Execution time: {total_time} seconds")
            self.log_operation(
                "INFO", f"ASGs deleted: {len(self.cleanup_results['deleted_asgs'])}"
            )
            self.log_operation(
                "INFO", f"Alarms deleted: {len(self.cleanup_results['deleted_alarms'])}"
            )
            ...
            if self.cleanup_results["deleted_asgs"]:
                ...
                for account, summary in account_summary.items():
                    ...
                    self.print_colored(
                        Colors.WHITE, f"      📦 ASGs deleted: {summary['asgs']}"
                    )
                    self.print_colored(
            ...
            if report_file:
                self.print_colored(
                    Colors.GREEN, f"✅ ASG cleanup report saved to: {report_file}"
                )
            ...
            self.log_operation(
                "ERROR", f"FATAL ERROR in ASG cleanup execution: {str(e)}"
            )
            self.print_colored(Colors.RED, f"\n❌ FATAL ERROR: {e}")
            import traceback
