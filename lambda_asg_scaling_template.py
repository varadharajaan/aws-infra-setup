...
class ASGScaler:
    ...
    def log_execution_start(self, asg_name, direction, ist_time):
        ...
        self.logger.info(f"{Symbols.TARGET} Target ASG: {asg_name}")
        self.logger.info(f"[UP] Scaling Direction: {direction.upper()}")
        self.logger.info(f"[TIME] IST Time: {ist_time}")
        self.logger.info(f"👤 Generated By: {{current_user}}")
        self.logger.info(f"{Symbols.LOG} Template Generated: {{current_date}} {{current_time}} UTC")
        self.logger.info("=" * 80)
    ...
    def log_execution_end(self, success=True):
        ...
        self.logger.info(f"{Symbols.STATS} Instances Before: {self.execution_stats['instances_before']}")
        self.logger.info(f"{Symbols.STATS} Instances After: {self.execution_stats['instances_after']}")
        self.logger.info(f"[UP] Scaling Operations: {self.execution_stats['scaling_operations']}")
        self.logger.info(f"{Symbols.ERROR} Errors Encountered: {self.execution_stats['errors_encountered']}")
    ...
    def create_asg_client(self, region=None):
        ...
        try:
            ...
            self.logger.error(f"{Symbols.ERROR} Failed to create ASG client: {str(e)}")
    ...
    def get_asg_configuration(self, asg_client, asg_name):
        ...
        self.logger.info(f"{Symbols.LIST} Retrieving ASG configuration for: {asg_name}")
        ...
        try:
            ...
            if not response['AutoScalingGroups']:
                self.logger.error(f"{Symbols.ERROR} ASG {asg_name} not found")
            ...
            self.logger.info(f"{Symbols.OK} ASG configuration retrieved:")
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                self.logger.error(f"{Symbols.ERROR} Access denied to ASG {asg_name} - check IAM permissions")
            else:
                self.logger.error(f"{Symbols.ERROR} AWS API error: {error_code} - {e.response['Error']['Message']}")
            self.execution_stats['errors_encountered'] += 1
            raise
        except Exception as e:
            self.logger.error(f"{Symbols.ERROR} Unexpected error retrieving ASG configuration: {str(e)}")
            self.execution_stats['errors_encountered'] += 1
            raise
    ...
    def calculate_new_capacity(self, current_config, direction):
        ...
        if direction.lower() == 'up':
            ...
            if new_desired > max_size:
                ...
                self.logger.warning(f"{Symbols.WARN}  Desired capacity capped at max_size: {max_size}")
                self.execution_stats['warnings_issued'] += 1
            ...
            # Unknown direction
            self.logger.error(f"{Symbols.ERROR} Unknown scaling direction: {direction}")
            raise ValueError(f"Unknown scaling direction: {direction}")

        self.logger.info(f"{Symbols.OK} New capacity calculated:")
    ...
    def update_asg_capacity(self, asg_client, asg_name, new_desired, current_desired):
        ...
        self.logger.info(f"{Symbols.SCAN} Updating ASG desired capacity...")

        try:
            # Check if update is needed
            if new_desired == current_desired:
                self.logger.info(f"{Symbols.SKIP}  No capacity change needed - ASG already at desired capacity: {current_desired}")
                return {
                    'updated': False,
            ...
            self.logger.info(f"{Symbols.OK} ASG capacity update completed successfully")
            ...
            if error_code == 'ScalingActivityInProgressFault':
                self.logger.error(f"{Symbols.ERROR} Scaling activity already in progress for ASG {asg_name}")
            elif error_code == 'ResourceContentionFault':
                self.logger.error(f"{Symbols.ERROR} Resource contention while updating ASG {asg_name}")
            elif error_code == 'ValidationError':
                self.logger.error(f"{Symbols.ERROR} Validation error: {error_message}")
            else:
            ...
            self.logger.error(f"{Symbols.ERROR} Unexpected error updating ASG capacity: {str(e)}")
            self.execution_stats['errors_encountered'] += 1
            raise
...
def lambda_handler(event, context):
    ...
    """
    try:
        logger.info(f"{Symbols.START} ASG Scaling Lambda function invoked")
        logger.info(f"[ROUNDPIN] Function ARN: {context.invoked_function_arn}")
        logger.info(f"{Symbols.DATE} Template generated: {{current_date}} {{current_time}} UTC")
        ...
        logger.info(f"{Symbols.OK} Validation passed - proceeding with scaling operation")
...
2. 🌙 EVENING SCALE-DOWN EVENT (Basic):
{
    "asg_name": "my-web-servers-asg", 
    "direction": "down",
    "ist_time": "6:30 PM ISTf"
}

3. {Symbols.START} SCALE-UP WITH REGION (Production):
{
...
4. [DOWN] SCALE-DOWN WITH REGION (Production):
{
    "asg_name": "prod-web-servers-asg",
    "direction": "down", 
    "region": "us-east-1",
...
8. {Symbols.REGION} CROSS-REGION EVENT (EU West):
{
    ...
    "direction": "up",
    "region": "eu-west-1",
    "ist_time": "2:30 PM ISTf"
}

9. {Symbols.ALERT} EMERGENCY SCALE-DOWN EVENT:
{
...
11. {Symbols.ERROR} ERROR TEST EVENT (Invalid Direction):
{
    "asg_name": "test-asg",
    "direction": "invalid",
    "ist_time": "9:00 AM ISTf"
}

12. {Symbols.SCAN} SPOT INSTANCE ASG SCALING:
{
    "asg_name": "spot-instances-asg",
    "direction": "up",
    "region": "ap-southeast-1", 
    "ist_time": "9:30 AM ISTf"
}
            ...
            "Targets": [
                {
                    "Id": "1", 
                    "Arn": "arn:aws:lambda:us-east-1:123456789012:function:asg-scaling-function",
                    "Input": "{\"asg_name\": \"my-web-servers-asg\", \"direction\": \"down\", \"ist_time\": \"6:30 PM IST\"}f"
                }
            ]
        }
    ]
...
def main():
    ...
    print("=" * 80)
    print(f"{Symbols.DATE} Template generated: {{current_date}} {{current_time}} UTC")
