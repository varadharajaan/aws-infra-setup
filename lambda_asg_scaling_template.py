...
class ASGScaler:
    ...
    def log_execution_start(self, asg_name, direction, ist_time):
        ...
        self.logger.info("=" * 80)
        self.logger.info("🚀 ASG SCALING LAMBDA - EXECUTION START")
        self.logger.info("=" * 80)
        self.logger.info(f"📅 Execution Time: {datetime.utcnow().isoformat()}Z")
        self.logger.info(f"🎯 Target ASG: {asg_name}")
        ...
        self.logger.info(
            f"📝 Template Generated: {{current_date}} {{current_time}} UTC"
        )
        self.logger.info("=" * 80)
    ...
    def log_execution_end(self, success=True):
        ...
        self.logger.info(
            f"📊 Instances Before: {self.execution_stats['instances_before']}"
        )
        self.logger.info(
            f"📊 Instances After: {self.execution_stats['instances_after']}"
        )
        self.logger.info(
            f"📈 Scaling Operations: {self.execution_stats['scaling_operations']}"
        )
        self.logger.info(
            f"❌ Errors Encountered: {self.execution_stats['errors_encountered']}"
        )
        self.logger.info(
    ...
    def create_asg_client(self, region=None):
        ...
        self.logger.info("🔧 Initializing AWS Auto Scaling client...")
        ...
        try:
            ...
            # Test connectivity
            self.logger.info("🧪 Testing AWS client connectivity...")
            try:
                sts_client = (
            ...
            self.logger.error(f"❌ Failed to create ASG client: {str(e)}")
    ...
    def get_asg_configuration(self, asg_client, asg_name):
        ...
        self.logger.info(f"📋 Retrieving ASG configuration for: {asg_name}")
        ...
        try:
            ...
            if not response["AutoScalingGroups"]:
                self.logger.error(f"❌ ASG {asg_name} not found")
            ...
            self.logger.info(f"✅ ASG configuration retrieved:")
            self.logger.info(
                f"   ├── Current desired capacity: {current_config['desired_capacity']}"
            )
            self.logger.info(f"   ├── Min size: {current_config['min_size']}")
            self.logger.info(f"   ├── Max size: {current_config['max_size']}")
            self.logger.info(
                f"   ├── Total instances: {len(current_config['instances'])}"
            )
            self.logger.info(f"   ├── Instance states: {instance_states}")
            self.logger.info(
                f"   ├── Availability zones: {len(current_config['availability_zones'])}"
            )
            self.logger.info(
                f"   ├── Health check type: {current_config['health_check_type']}"
            )
            self.logger.info(
                f"   └── Default cooldown: {current_config['default_cooldown']}s"
            )

            # Store current instance count for statistics
            self.execution_stats["instances_before"] = current_config[
                "desired_capacity"
            ]
            ...
            if error_code == "ResourceNotFoundException":
                ...
                self.logger.error(
                    f"❌ Access denied to ASG {asg_name} - check IAM permissions"
                )
            else:
                self.logger.error(
            ...
            self.logger.error(
                f"❌ Unexpected error retrieving ASG configuration: {str(e)}"
            )
            self.execution_stats["errors_encountered"] += 1
            raise
    ...
    def calculate_new_capacity(self, current_config, direction):
        ...
        self.logger.info(
            f"   ├── Current configuration: desired={current_desired}, min={min_size}, max={max_size}"
        )
        ...
        if direction.lower() == "up":
            ...
            new_desired = (
                ...
                else max(current_desired, min_size)
            )
            ...
            if new_desired > max_size:
                ...
                self.logger.warning(
                    f"⚠️  Desired capacity capped at max_size: {max_size}"
                )
                self.execution_stats["warnings_issued"] += 1
            ...
            # Unknown direction
            self.logger.error(f"❌ Unknown scaling direction: {direction}")
            raise ValueError(f"Unknown scaling direction: {direction}")

        self.logger.info(f"✅ New capacity calculated:")
        self.logger.info(f"   ├── New desired capacity: {new_desired}")
        self.logger.info(f"   ├── Action: {action_description}")
        self.logger.info(
            f"   └── Change: {current_desired} → {new_desired} ({new_desired - current_desired:+d})"
        )
    ...
    def update_asg_capacity(self, asg_client, asg_name, new_desired, current_desired):
        ...
        try:
            ...
            if new_desired == current_desired:
                self.logger.info(
                    f"⏭️  No capacity change needed - ASG already at desired capacity: {current_desired}"
                )
                return {
                    "updated": False,
            ...
            self.logger.info(f"✅ ASG capacity update completed successfully")
            ...
            if error_code == "ScalingActivityInProgressFault":
                self.logger.error(
                    f"❌ Scaling activity already in progress for ASG {asg_name}"
                )
            elif error_code == "ResourceContentionFault":
                self.logger.error(
                    f"❌ Resource contention while updating ASG {asg_name}"
                )
            ...
            self.logger.error(f"❌ Unexpected error updating ASG capacity: {str(e)}")
            self.execution_stats["errors_encountered"] += 1
            raise
    ...
    def scale_asg(self, asg_name, direction, ist_time, region=None):
        ...
        try:
            ...
            new_desired, action_description = self.calculate_new_capacity(
                current_config, direction
            )

            # Update ASG capacity
            update_result = self.update_asg_capacity(
                asg_client, asg_name, new_desired, current_config["desired_capacity"]
            )
            ...
            error_msg = str(e)
            self.logger.error(f"💥 ASG scaling operation failed: {error_msg}")
...
def lambda_handler(event, context):
    ...
    """
    try:
        logger.info(f"🚀 ASG Scaling Lambda function invoked")
        ...
        asg_name = (
            "{{ASG_NAME}}"
            if "{{ASG_NAME}}" != "{{ASG_NAME}}"
            else event.get("asg_name") or os.environ.get("ASG_NAME")
        )
        direction = (
            "{{DIRECTION}}"
            if "{{DIRECTION}}" != "{{DIRECTION}}"
            else event.get("direction") or os.environ.get("SCALING_DIRECTION", "up")
        )
        ...
        if direction not in ["up", "down"]:
            error_msg = (
                f'Invalid scaling direction: {direction}. Must be "up" or "down"'
            )
            logger.error(f"❌ Validation failed: {error_msg}")
            return {
                "statusCode": 400,
        ...
        logger.info(f"✅ Validation passed - proceeding with scaling operation")
...
3. 🚀 SCALE-UP WITH REGION (Production):
{
...
6. 🔧 MAINTENANCE SCALE-DOWN EVENT:
{
...
10. 📭 MINIMAL EVENT (Tests Template Injection):
{
    "direction": "up"
}

11. ❌ ERROR TEST EVENT (Invalid Direction):
{
    "asg_name": "test-asg",
    "direction": "invalid",
    ...
    class MockContext:
        def __init__(self):
            ...
            self.invoked_function_arn = (
                "arn:aws:lambda:{{region}}:123456789012:function:asg-scaling-function"
            )
