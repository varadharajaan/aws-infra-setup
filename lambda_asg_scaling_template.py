...
class ASGScaler:
    ...
    def log_execution_end(self, success=True):
        ...
        self.logger.info(f"📊 Instances Before: {self.execution_stats['instances_before']}")
        self.logger.info(f"📊 Instances After: {self.execution_stats['instances_after']}")
        self.logger.info(f"📈 Scaling Operations: {self.execution_stats['scaling_operations']}")
        self.logger.info(f"❌ Errors Encountered: {self.execution_stats['errors_encountered']}")
    ...
    def get_asg_configuration(self, asg_client, asg_name):
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                self.logger.error(f"❌ Access denied to ASG {asg_name} - check IAM permissions")
            else:
                self.logger.error(f"❌ AWS API error: {error_code} - {e.response['Error']['Message']}")
            self.execution_stats['errors_encountered'] += 1
            raise
        except Exception as e:
            self.logger.error(f"❌ Unexpected error retrieving ASG configuration: {str(e)}")
            self.execution_stats['errors_encountered'] += 1
            raise
    ...
    def calculate_new_capacity(self, current_config, direction):
        ...
        if direction.lower() == 'up':
            ...
            if new_desired > max_size:
                ...
                self.logger.warning(f"⚠️  Desired capacity capped at max_size: {max_size}")
                self.execution_stats['warnings_issued'] += 1
    ...
    def update_asg_capacity(self, asg_client, asg_name, new_desired, current_desired):
        ...
        try:
            ...
            if new_desired == current_desired:
                self.logger.info(f"⏭️  No capacity change needed - ASG already at desired capacity: {current_desired}")
                return {
                    'updated': False,
            ...
            if error_code == 'ScalingActivityInProgressFault':
                self.logger.error(f"❌ Scaling activity already in progress for ASG {asg_name}")
            elif error_code == 'ResourceContentionFault':
                self.logger.error(f"❌ Resource contention while updating ASG {asg_name}")
