...
class UltraAMICleanupManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{log_dir}/ultra_ami_cleanup_log_{self.execution_timestamp}.log"
            )
    ...
    def get_all_amis(self, ec2_client, region, account_name, account_id):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🔍 Scanning for AMIs in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "INFO", f"📸 Found {len(amis)} AMIs in {region} ({account_name})"
            )
            ...
            if amis:
                print(
                    f"      ✓ Available: {available_count}, Other states: {other_count}"
                )
            ...
            self.log_operation(
                "ERROR", f"Error getting AMIs in {region} ({account_name}): {e}"
            )
    ...
    def check_ami_in_use(self, ec2_client, ami_id):
        ...
        try:
            ...
            self.log_operation(
                "WARNING", f"Could not check if AMI {ami_id} is in use: {e}"
            )
            return False, []
    ...
    def deregister_ami(self, ec2_client, ami_info):
        ...
        try:
            ...
            if in_use:
                ...
                self.log_operation(
                    "INFO",
                    f"⏭️  Skipping AMI {ami_id} ({ami_info['name']}) - in use by instances: {', '.join(instance_ids)}",
                )
                print(
                    f"      ⏭️  Skipping {ami_id} ({ami_info['name']}) - in use by {len(instances)} instance(s)"
                )
            ...
            self.log_operation(
                "INFO",
                f"🗑️  Deregistering AMI {ami_id} ({ami_info['name']}) - {ami_info['snapshot_count']} snapshots",
            )
            ...
            if ami_info["snapshot_ids"]:
                print(
                    f"         🗑️  Deleting {len(ami_info['snapshot_ids'])} associated snapshots..."
                )

                for snapshot_id in ami_info["snapshot_ids"]:
                    try:
                        self.log_operation(
                            "INFO", f"Deleting snapshot {snapshot_id} for AMI {ami_id}"
                        )
                        ...
                        error_code = snap_error.response.get("Error", {}).get(
                            "Code", "Unknown"
                        )

                        if error_code == "InvalidSnapshot.InUse":
                            self.log_operation(
                                "WARNING", f"Snapshot {snapshot_id} is in use, skipping"
                            )
                            print(
                                f"         ⚠️  Snapshot {snapshot_id} is in use, skipping"
                            )
                        else:
                            self.log_operation(
                                "ERROR",
                                f"Failed to delete snapshot {snapshot_id}: {snap_error}",
                            )
                            print(
                                f"         ❌ Failed to delete snapshot {snapshot_id}: {snap_error}"
                            )
                        ...
                        self.log_operation(
                            "ERROR",
                            f"Unexpected error deleting snapshot {snapshot_id}: {snap_error}",
                        )
                        print(
                            f"         ❌ Error deleting snapshot {snapshot_id}: {snap_error}"
                        )
            ...
            self.log_operation(
                "ERROR", f"Failed to deregister AMI {ami_info['ami_id']}: {e}"
            )
    ...
    def cleanup_account_region(self, account_name, account_data, region):
        ...
        try:
            ...
            if not amis:
                self.log_operation(
                    "INFO", f"No AMIs found in {account_name} ({region})"
                )
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            report_filename = (
                f"{report_dir}/ultra_ami_cleanup_report_{self.execution_timestamp}.json"
            )
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"💾 Snapshots deleted: {len(self.cleanup_results['deleted_snapshots'])}"
            )
            print(
                f"⏭️  AMIs skipped (in-use): {len(self.cleanup_results['skipped_amis'])}"
            )
            print(
            ...
            self.log_operation(
                "INFO", f"AMIs deleted: {len(self.cleanup_results['deleted_amis'])}"
            )
            self.log_operation(
                "INFO",
                f"Snapshots deleted: {len(self.cleanup_results['deleted_snapshots'])}",
            ...
            if self.cleanup_results["skipped_amis"]:
                ...
                for skipped in self.cleanup_results["skipped_amis"][:5]:
                    print(
                        f"   • {skipped['ami_id']} ({skipped['name']}) - {skipped['reason']}"
                    )
