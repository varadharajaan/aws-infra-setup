...
class EC2CleanupManager:
                ...
                if matched or file_name.startswith("ec2_instance_"):
                    ...
                    if ec2_data:
                        ...
                        if timestamp_from_filename:
                            ec2_data["timestamp_from_filename"] = (
    ...
    def group_ec2s_by_day(self, ec2_files: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for ec2 in ec2_files:
            try:
                ...
                timestamp = ec2.get("timestamp") or ec2.get(
                    "timestamp_from_filename", ""
                )
                ...
                logger.warning(
                    f"Error processing timestamp for {ec2.get('file_name', 'unknown')}: {e}"
                )
    ...
    def display_ec2s_for_selection(self, ec2s: List[Dict]) -> None:
        ...
        for i, ec2 in enumerate(ec2s, 1):
            ...
            print(
                f"  Launch Template: {instance_details.get('launch_template_id', 'N/A')}"
            )
            ...
            try:
                ...
                # Handle comma-separated values
                    ...
                    for item in selection.split(","):
                        ...
                        indices.append(idx)
                    selected_ec2s = [
                        ec2s[idx] for idx in indices if 0 <= idx < len(ec2s)
                    ]
    ...
    def cleanup_monitoring_resources(
        self, session: boto3.Session, instance_id: str, region: str
    ) -> bool:
        ...
        try:
                ...
                for alarm in alarms_response.get("MetricAlarms", []):
                    ...
                    for dimension in alarm.get("Dimensions", []):
                        if (
                            ...
                            and dimension.get("Value") == instance_id
                        ):
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Error deleting alarm {alarm_name}: {e}"
                                )
                            break
            ...
            try:
                ...
                for topic in topics_response.get("Topics", []):
                    ...
                    if (
                        instance_id in topic_name
                        or "ec2-notification" in topic_name.lower()
                    ):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Error deleting SNS topic {topic_name}: {e}"
                            )
    ...
    def cleanup_lambda_functions(
        ...
        try:
            ...
            for function in functions_response.get("Functions", []):
                ...
                if (
                    instance_id in function_name
                    or "ec2-manager" in function_name.lower()
                    or "instance-scheduler" in function_name.lower()
                    or "ec2-scheduler" in function_name.lower()
                    or "instance-monitor" in function_name.lower()
                ):
                    ...
                    try:
                        ...
                        if (
                            tags.get("InstanceId") == instance_id
                            or tags.get("EC2Instance") == instance_id
                            or instance_id in tags.get("Purpose", "")
                            or instance_id in tags.get("Resource", "")
                        ):
            ...
            if deleted_functions:
                logger.info(
                    f"Successfully deleted Lambda functions: {deleted_functions}"
                )
            else:
    ...
    def cleanup_volumes_and_snapshots(
        ...
        try:
            logger.info(
                f"Cleaning up volumes and snapshots for instance: {instance_id}"
            )
            ...
            try:
                ...
                for volume in volumes_response.get("Volumes", []):
                    ...
                    if not delete_on_termination:
                        logger.info(
                            f"Volume {volume_id} will need manual deletion after instance termination"
                        )
                ...
                if volume_ids:
                    ...
                    for snapshot in snapshots_response.get("Snapshots", []):
                        ...
                        logger.info(
                            f"Found snapshot {snapshot_id} for cleanup after instance termination"
                        )
    ...
    def delete_launch_template(
        self, session: boto3.Session, launch_template_id: str, region: str
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Launch template {launch_template_id} deleted successfully"
                )
                return True
                ...
                if e.response["Error"]["Code"] == "InvalidLaunchTemplateId.NotFound":
                    logger.info(
                        f"Launch template {launch_template_id} not found, may already be deleted"
                    )
                    return True
                else:
                    logger.error(
                        f"Error deleting launch template {launch_template_id}: {e}"
                    )
                    return False

        except Exception as e:
            logger.error(
                f"Unexpected error in deleting launch template {launch_template_id}: {e}"
            )
            return False
                ...
                for reservation in instances_response["Reservations"]:
                    for instance in reservation["Instances"]:
                        ...
                        for sg in security_groups:
                            ...
                            if (
                                instance_id in sg_name
                                or "temporary" in sg_name.lower()
                                or "instance-specific" in sg_name.lower()
                            ):

                                logger.info(
                                    f"Attempting to delete security group: {sg_id} ({sg_name})"
                                )
                                try:
                                    ec2_client.delete_security_group(GroupId=sg_id)
                                    logger.info(
                                        f"Security group {sg_id} deleted successfully"
                                    )
                                except ClientError as e:
                                    if "DependencyViolation" in str(e):
                                        logger.info(
                                            f"Security group {sg_id} still in use, skipping"
                                        )
                                    else:
                                        logger.warning(
                                ...
                                logger.info(
                                    f"Keeping security group {sg_id} ({sg_name}) - appears to be shared"
                                )

            except ClientError as e:
                if e.response["Error"]["Code"] == "InvalidInstanceID.NotFound":
                    logger.info(
                        f"Instance {instance_id} not found for security group cleanup"
                    )
                else:
                    ...
                    try:
                        ...
                        for record in records.get("ResourceRecordSets", []):
                            ...
                            if instance_id in record_name:
                                logger.info(
                                    f"Found Route 53 record for cleanup: {record_name}"
                                )
                        ...
                        logger.warning(
                            f"Error checking Route 53 records in zone {zone_id}: {e}"
                        )
    ...
    def delete_ec2_instance(self, ec2_data: Dict) -> bool:
        ...
        try:
            ...
            if not all([account_name, instance_id, region]):
                logger.error(
                    f"Missing required information in EC2 data: {ec2_data.get('file_name')}"
                )
                return False

            logger.info(
                f"Starting comprehensive deletion of EC2 instance: {instance_id}"
            )
            ...
            try:
                ...
                if not instance_found:
                    logger.info(
                        f"Instance {instance_id} not found, cleaning up associated resources"
                    )
                ...
                if current_state in ["terminated", "terminating"]:
                    logger.info(
                        f"Instance {instance_id} is already {current_state}, cleaning up associated resources"
                    )
                    # Still run cleanup for associated resources
                    self.cleanup_monitoring_resources(session, instance_id, region)
                    self.cleanup_lambda_functions(session, instance_id, region)
            ...
            if not self.cleanup_monitoring_resources(session, instance_id, region):
                logger.warning(
                    "Some monitoring resources may not have been cleaned up properly"
                )
            ...
            if not self.cleanup_volumes_and_snapshots(session, instance_id, region):
                logger.warning(
                    "Some volume/snapshot information may not have been processed"
                )
            ...
            if not self.cleanup_security_groups(session, instance_id, region):
                logger.warning(
                    "Some security groups may not have been cleaned up properly"
                )
            ...
            logger.info(
                f"EC2 instance {instance_id} and associated resources deleted successfully"
            )
            return True
    ...
    def run_cleanup(self) -> None:
        ...
        confirm = (
            input(
                "\nAre you sure you want to delete these EC2 instances and all associated resources? (yes/no): "
            )
            .strip()
            .lower()
        )
        ...
        for i, ec2 in enumerate(ec2s_to_delete, 1):
            ...
            logger.info(
                f"Processing EC2 instance {i}/{len(ec2s_to_delete)}: {instance_id}"
            )
