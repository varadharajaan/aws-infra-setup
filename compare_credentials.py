        ...
        if os.path.exists(filename):
            try:
                ...
                if 'accounts' in data and 'account02' in data['accounts']:
                    ...
                    print(f"🔑 account02 access_key: {access_key[:10]}...{access_key[-4:] if len(access_key) > 10 else access_key}")
                    ...
                    for key, value in data.items():
                        if isinstance(value, dict) and 'access_key' in value:
                            ...
                            print(f"🔑 {key} access_key: {access_key[:10]}...{access_key[-4:] if len(access_key) > 10 else access_key}")
