...
Examples:
  ...
  python eks_connector.py i-123,i-456 account01   # Multiple instances, single account
        ...
        parser.add_argument(
            "instances",
            nargs="?",
            help="Comma-separated instance IDs (e.g., i-123,i-456)",
        )
        parser.add_argument(
            "account",
            nargs="?",
            help="Account name to process (single account only in direct mode)",
        )
        ...
        if args.instances and args.account:
            ...
            print(
                f"Direct mode: Processing instances {', '.join(self.direct_instances)} in account {args.account}"
            )
            print(
                "Note: Direct mode supports single account only. Use interactive mode for multi-account processing."
            )
            return True
        elif args.instances or args.account:
            print(
                "Error: Both instance IDs and account name are required for direct mode"
            )
    ...
    def extract_username_from_instruction_filename(self, filename):
        ...
        try:
            ...
            match = re.search(
                r"user_mini_instructions_([^_]+)_([^_]+)_.*?_\d{8}_\d{6}\.txt$",
                filename,
            )
            if match:
    ...
    def get_instance_details_direct(self, instance_ids, account):
        ...
        try:
            ...
            running_instances = [
                inst for inst in instances if inst["state"] == "running"
            ]
            ...
            if len(running_instances) < len(instance_ids):
                ...
                for inst in not_running:
                    logger.warning(
                        f"Instance {inst['instance_id']} is in state '{inst['state']}', skipping"
                    )
    ...
    def discover_user_instruction_files(self):
        ...
        print(
            f"Found {len(all_instruction_files)} user mini-instruction files in current directory:"
        )
        ...
        if not all_instruction_files:
            ...
            print(
                "Expected pattern: user_mini_instructions_{account}_{username}_{cluster}_TIMESTAMP.txt"
            )
            return {}
            ...
            if filename.startswith("user_mini_instructions_"):
                ...
                if len(parts) >= 2:
                    ...
                    cluster_match = re.search(
                        f"user_mini_instructions_{account_name}_{username}_(.+)_{timestamp}\\.txt$",
                        filename,
                    )
                    cluster_name = (
                        cluster_match.group(1) if cluster_match else "unknown"
                    )

                    # NEW: Extract username from cluster name (the proper way)
                    extracted_username = (
                        self.extract_username_from_cluster_name_in_instruction(
                            cluster_name
                        )
                    )
                    ...
                    print(
                        f"    -> Extracted Username from Cluster: {extracted_username}"
                    )
                    ...
                    self.instruction_username_mapping[str(file_path)] = (
                        extracted_username
                    )
                ...
                print(
                    f"  ✗ Could not parse: {file_path.name} (doesn't match expected pattern)"
                )
    ...
    def build_smart_mapping(self):
        ...
        if self.instance_source == "asg":
            ...
            print(
                f"{'ASG Name':<30} {'Account':<15} {'Extracted Username':<20} {'Full Username':<25}"
            )
            print("-" * 100)
            ...
            for asg_info in self.selected_asgs:
                ...
                print(
                    f"{asg_info['name']:<30} {asg_info['account']:<15} {base_username:<20} {full_username:<25}"
                )

        else:  # ec2
            print(f"\n📊 EC2 Instance Username Analysis:")
            print(
                f"{'Instance ID':<20} {'Account':<15} {'Extracted Username':<20} {'Full Username':<25}"
            )
            print("-" * 90)
        ...
        for account, files in all_instruction_files.items():
            ...
            for file_info in files:
                username = file_info[
                    "extracted_username"
                ]  # This is account03_clouduser05 from cluster name
            ...
            for asg_name, full_username in self.asg_username_mapping.items():
                ...
                if asg_account and asg_account in available_instructions:
                    if full_username in available_instructions[asg_account]:
                        auto_mapping[asg_name] = available_instructions[asg_account][
                            full_username
                        ]
                        ...
                        print(
                            f"  ✗ {asg_name} -> {full_username} (no matching instruction file)"
                        )
                else:
                    unmapped_items.append((asg_name, full_username, asg_account))
            ...
            for instance_id, full_username in self.ec2_username_mapping.items():
                ...
                if instance_account and instance_account in available_instructions:
                    if full_username in available_instructions[instance_account]:
                        auto_mapping[instance_id] = available_instructions[
                            instance_account
                        ][full_username]
                        print(f"  ✓ {instance_id} -> {full_username} (auto-matched)")
                    else:
                        unmapped_items.append(
                            (instance_id, full_username, instance_account)
                        )
                        print(
                            f"  ✗ {instance_id} -> {full_username} (no matching instruction file)"
                        )
                else:
                    unmapped_items.append(
                        (instance_id, full_username, instance_account)
        ...
        if auto_mapping:
            ...
            choice = (
                input("Enter 'yes' or 'y' to accept, 'no' to manually map: ")
                .strip()
                .lower()
            )
        ...
        for item_name, username, account in unmapped_items:
            ...
            if account in available_instructions:
                ...
                for i, file_info in enumerate(files, 1):
                    print(
                        f"  {i}. {file_info['extracted_username']} -> {file_info['cluster_name']}"
                    )

                while True:
                    try:
                        selection = input(
                            f"Select instruction file for {item_name} (1-{len(files)}, or 's' to skip): "
                        ).strip()
                        ...
                        if 0 <= idx < len(files):
                            ...
                            print(
                                f"  ✅ Mapped {item_name} -> {files[idx]['extracted_username']}"
                            )
                            break
                        else:
                            print(
                                f"Invalid selection. Please choose 1-{len(files)} or 's' to skip."
                            )
                    except ValueError:
                        print(
                            f"Invalid input. Please enter a number 1-{len(files)} or 's' to skip."
                        )
            else:
    ...
    def select_accounts(self):
        ...
        while True:
            selection = input(
                f"\nSelect accounts (single/comma-separated/range/all): "
            ).strip()
                ...
                for file_path in asg_path.glob("asg_asg-*.json"):
                    ...
                    if match:
                        ...
                        try:
                            ...
                            full_datetime = date_obj.replace(
                            ...
                            logger.warning(
                                f"Could not parse timestamp from {file_path.name}: {e}"
                            )
                            continue
    ...
    def extract_username_from_ec2_filename(self, filename):
        ...
        try:
            ...
            match = re.match(
                r"ec2_instance_[^_]+_[^_]+_([^_]+)_\d{8}_\d{6}\.json$", filename
            )
            if match:
                return match.group(1)
            # Pattern 2: root-accountXX
            match_root = re.match(
                r"ec2_instance_[^_]+_(root-account\d+)_\d{8}_\d{6}\.json$", filename
            )
            if match_root:
                ...
                for file_path in ec2_path.glob("ec2_instance_*.json"):
                    ...
                    if match:
                        ...
                        try:
                            ...
                            files_found += 1
                            print(
                                f"    ✓ Found: {file_path.name} -> Instance: {instance_id}, User: {username}"
                            )
                        except ValueError as e:
                            logger.warning(
                                f"Could not parse timestamp from {file_path.name}: {e}"
                        ...
                        print(
                            f"    ✗ Skipped: {file_path.name} (doesn't match expected pattern)"
                        )
    ...
    def display_asg_files_by_day(self, files_by_day):
        ...
        sorted_days = sorted(
            files_by_day.keys(), reverse=True
        )  # CHANGED: Added reverse=True
        ...
        for i, day in enumerate(sorted_days, 1):
            ...
            account_summary = ", ".join(
                [f"{acc}({count})" for acc, count in accounts_this_day.items()]
            )
            print(
            ...
            for file_info in files_by_day[day]:
                ...
                print(
                    f"   {timestamp_str} - {file_info['path'].name} ({file_info['account']})"
                )
    ...
    def select_asg_files(self):
        ...
        while True:
            selection = input(
                "\nSelect days (single/comma-separated/range/all): "
            ).strip()
    ...
    def extract_ec2_instances_from_files(self):
        ...
        for file_info in self.selected_ec2_files:
            try:
                ...
                logger.error(
                    f"Error reading EC2 instance file {file_info['path']}: {e}"
                )
                continue
    ...
    def display_ec2_instances(self, instances):
        ...
        for account, account_instances in instances_by_account.items():
            ...
            print(
                f"  - {account}: {len(account_instances)} instances - {', '.join(instance_ids)}"
            )
    ...
    def select_asgs(self):
        ...
        while True:
            selection = input(
                "\nSelect ASGs (single/comma-separated/range/all): "
            ).strip()
        ...
        for account, count in account_selected_count.items():
            asg_names = [
                asg["name"] for asg in self.selected_asgs if asg["account"] == account
            ]
    ...
    def select_ec2_instances(self):
        ...
        while True:
            selection = input(
                "\nSelect EC2 instances (single/comma-separated/range/all): "
            ).strip()
        ...
        for account, count in account_selected_count.items():
            instance_ids = [
                inst["instance_id"]
                for inst in self.selected_ec2_instances
    ...
    def get_ec2_instance_details_from_aws(self, instance_info):
        ...
        try:
            ...
            logger.error(
                f"Error getting instance details for {instance_info['instance_id']}: {e}"
            )
            return None
    ...
    def test_ssh_connection(self, instance):
        ...
        try:
            ...
            if "SSH connection successful" in output:
                ...
                logger.info(
                    f"SSH connection successful to {instance['instance_id']} ({ip})"
                )
                print(
    ...
    def log_command_output(
        log_file, username, command, output, error, success, command_number
    ):
    ...
    def process_instance(self, instance, instruction_file_info):
        ...
        logger.info(
            f"Processing instance {instance['instance_id']} in ASG {instance['asg_name']} for account {instance['account']}"
        )
        ...
        output_file = (
            account_dir
            / f"{instance_id}_{account}_{extracted_username}_command_output.txt"
        )
        ...
        # Try SSH as ec2-user
        ssh_success_ec2, ssh_message_ec2, ssh_command_ec2 = self.test_ssh_connection(
            instance
        )
        ec2_results = []
        ec2_successful = 0
    ...
    def save_instance_report(self, instance_result):
        ...
        account_dir = Path(self.reports_path) / account / "instances"
        account_dir.mkdir(parents=True, exist_ok=True)
        account_dir / f"{instance_id}_{account}_{extracted_username}_report.json"
    ...
    def save_asg_report(self, asg_name, username, instance_results):
        ...
        ec2user_total = sum(
            r.get("ec2_user_commands_executed", 0) for r in instance_results
        )
        ec2user_success = sum(
            r.get("ec2_user_commands_successful", 0) for r in instance_results
        )
        ...
        with open(txt_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- demouser: {demouser_success}/{demouser_total} commands successful\n"
            )
            f.write(
                f"- ec2-user: {ec2user_success}/{ec2user_total} commands successful\n"
            )
            f.write(f"\n" + "=" * 80 + "\n")
            ...
            for result in instance_results:
                ...
                f.write(
                    f"SSH Success: {'SUCCESS' if result['ssh_success'] else 'FAILED'}\n"
                )
                ...
                for cmd_result in result["command_results"]:
                    ...
                    f.write(
                        f"[{cmd_result['command_number']}] {cmd_result['command']}\n"
                    )
                    f.write(
                        f"    Status: {status} (exit code: {cmd_result['exit_code']})\n"
                    )
    ...
    def save_account_summary(self, account, instance_results):
        ...
        summary_data = {
            ...
            "command_success_rate": (successful_commands / max(total_commands, 1))
            * 100,
            "instance_summaries": [
                {
    ...
    def save_instance_html_report(self, instance_result):
        ...
        html_file = (
            account_dir / f"{instance_id}_{account}_{extracted_username}_report.html"
        )
    ...
    def run_automation(self):
        ...
        print(
            "Processing Mode: Interactive Multi-Account Processing with Smart Mapping\n"
        )
        ...
        if not self.build_smart_mapping():
            print(
                "❌ Smart mapping failed. Cannot proceed without instruction file mappings."
            )
            return False
        ...
        if self.instance_source == "asg":
            ...
            for asg_info in self.selected_asgs:
                if asg_info["name"] in self.final_mapping:
                    ...
                    print(
                        f"⏭️  Skipping ASG {asg_info['name']} (no instruction file mapping)"
                    )
        else:  # ec2
            # Get instances from selected EC2 files
            for instance_info in self.selected_ec2_instances:
                if instance_info["instance_id"] in self.final_mapping:
                    instance_details = self.get_ec2_instance_details_from_aws(
                        instance_info
                    )
                    if instance_details:
                    ...
                    print(
                        f"⏭️  Skipping instance {instance_info['instance_id']} (no instruction file mapping)"
                    )
        ...
        for mapping_key, instances in instances_by_mapping.items():
            ...
            print(
                f"  - {mapping_key} -> {instruction_info['extracted_username']} ({len(instances)} instances)"
            )
        ...
        for mapping_key, instances in instances_by_mapping.items():
            ...
            logger.info(
                f"Processing {len(instances)} instances for mapping {mapping_key}"
            )
            print(
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                future_to_instance = {
                    executor.submit(
                        self.process_instance, instance, instruction_file
                    ): instance
                    for instance in instances
                }
                ...
                for future in as_completed(future_to_instance):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error processing instance {instance['instance_id']}: {e}"
                        )
            ...
            if self.instance_source == "asg" and mapping_results:
                ...
                try:
                    instruction_file["extracted_username"]
                    json_file, txt_file = self.save_asg_report(
                        mapping_key, instruction_file["username"], mapping_results
                    )
                    print(
                        f"✓ ASG report saved: {mapping_key} -> {json_file.name}, {txt_file.name}"
                    )
                except Exception as e:
    ...
    def print_final_summary(self, results):
        ...
        print(
            f"Processing Mode: Interactive Smart Mapping ({self.instance_source.upper()} Source)"
        )
        print("-" * 80)
        ...
        for account, account_results in by_account.items():
            ...
            for result in account_results:
                ...
                cmd_status = (
                    f"{result['commands_successful']}/{result['commands_executed']}"
                )
                ...
                for asg_name in asg_names:
                    ...
                    for result in account_results:
                        if result.get("asg_name") == asg_name:
                            cluster_name = result.get("cluster_name", "unknown-cluster")
                            extracted_username = self.extract_username_from_cluster(
                                cluster_name
                            )
                            # print(
        ...
        print(
            f"Instance source used: {self.instance_source.upper()} with Smart Username Mapping"
        )
