...
class LaunchTemplateManager:
    ...
    def load_accounts(self) -> Dict:
        ...
        try:
            ...
            print(
                f"✅ Loaded {len(config['accounts'])} accounts from: {self.accounts_file}"
            )
    ...
    def select_accounts(self) -> List[str]:
        ...
        for i, acc_name in enumerate(account_names, 1):
            ...
            print(
                f"{i:2d}. {acc_name} (ID: {acc['account_id']}, Email: {acc['email']})"
            )
        ...
        while True:
            try:
                selection = (
                    input(
                        "\nSelect accounts to process (comma-separated numbers, range with '-', or 'all'): "
                    )
                    .strip()
                    .lower()
                )
    ...
    def select_regions(self) -> List[str]:
        ...
        while True:
            try:
                selection = (
                    input(
                        "\nSelect regions to process (comma-separated numbers, range with '-', or 'all'): "
                    )
                    .strip()
                    .lower()
                )
    ...
    def process_accounts(
        self, selected_accounts: List[str], selected_regions: List[str], age_days: int
    ) -> Dict:
        ...
        print(
            f"\n🔍 Searching for launch templates older than {age_days} days ({cutoff_date.strftime('%Y-%m-%d')})"
        )
        print(
            f"🌐 Scanning {len(selected_regions)} regions across {len(selected_accounts)} accounts"
        )
        ...
        for account_name in selected_accounts:
            ...
            print(f"\n{'='*100}")
            print(
                f"🏢 Processing account: {account_name} (ID: {account_info['account_id']})"
            )
            print(f"{'='*100}")
            ...
            for region in selected_regions:
                ...
                try:
                    ...
                    if not templates:
                        print(f"   📭 No launch templates found in {region}")
                        continue

                    # Sort templates by creation time (newest first)
                    templates.sort(
                        key=lambda x: x.get("CreateTime", datetime.min), reverse=True
                    )

                    print(f"   📋 Found {len(templates)} launch templates in {region}")
                    ...
                    for tpl in templates:
                        ...
                        print(
                            f"   {template_id:<25} {template_name[:38]:<40} {create_time.strftime('%Y-%m-%d %H:%M:%S'):<20} {age_days_actual:<10} {status}"
                        )
                    ...
                    if old_templates:
                        print(
                            f"\n   ⚠️ Found {len(old_templates)} templates older than {age_days} days"
                        )

                        delete_choice = (
                            input(
                                f"   Delete these {len(old_templates)} old templates in {region}? (y/n): "
                            )
                            .strip()
                            .lower()
                        )
                        if delete_choice == "y":
                            ...
                            for tpl in old_templates:
                                ...
                                try:
                                    ...
                                    print(
                                        f"   ✅ Deleted: {template_name} ({template_id})"
                                    )
                                    deleted_count += 1
                                    account_deleted += 1
                                    total_deleted += 1
                            ...
                            print(
                                f"   ✅ Deleted {deleted_count} templates in {region}"
                            )
                        else:
                    ...
                    if protected_templates:
                        print(
                            f"\n   🔒 {len(protected_templates)} templates are protected due to name prefixes"
                        )

                except Exception as e:
                    error_msg = (
                        f"Error processing {region} in account {account_name}: {str(e)}"
                    )
                    print(f"   ❌ {error_msg}")
                    self.results["errors"].append(error_msg)
        ...
        if self.results["errors"]:
            print(
                f"\n⚠️ There were {len(self.results['errors'])} errors during processing"
            )
...
def main():
    print("\n" + "=" * 100)
    print(
        "🚀 Launch Template Manager - List and cleanup old launch templates across AWS accounts"
    )
    print("=" * 100)
    ...
    print(
        f"Found {results['lt_found']} launch templates, deleted {results['lt_deleted']}"
    )
