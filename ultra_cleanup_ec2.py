...
class UltraCleanupEC2Manager:
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation('INFO', f"🧹 Starting EC2 cleanup for {account_key} ({account_id}) in {region}")
            ...
            self.log_operation('INFO', f"📊 {account_key} ({region}) EC2 resources summary:")
            ...
            if instances:
                self.log_operation('INFO', f"🗑️  Terminating {len(instances)} instances in {account_key} ({region}) sequentially")
            ...
            if attached_sgs:
                ...
                for retry in range(max_retries):
                    ...
                    if not remaining_sgs:
                        self.log_operation('INFO', f"✅ All attached security groups deleted in {account_key} ({region})")
            ...
            self.log_operation('INFO', f"✅ EC2 cleanup completed for {account_key} ({region})")
            ...
            print(f"   ❌ Error cleaning up EC2 resources in {account_key} ({region}): {e}")
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            self.log_operation('INFO', f"✅ Ultra EC2 cleanup report saved to: {report_filename}")
            return report_filename

        except Exception as e:
            self.log_operation('ERROR', f"❌ Failed to save ultra EC2 cleanup report: {e}")
            return None
    ...
    def run(self):
        ...
        try:
            ...
            if report_file:
                self.print_colored(Colors.GREEN, f"✅ EC2 cleanup report saved to: {report_file}")
