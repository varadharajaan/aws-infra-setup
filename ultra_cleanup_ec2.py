...
class UltraCleanupEC2Manager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{self.ec2_dir}/ultra_ec2_cleanup_log_{self.execution_timestamp}.log"
            )
            ...
            for page in paginator.paginate():
                for sg in page["SecurityGroups"]:
                    ...
                    if sg_name == "default":
                        self.log_operation(
                            "DEBUG",
                            f"Skipping default security group {sg_id} ({sg_name})",
                        )
                        continue
        ...
        try:
            ...
            for pattern in eks_name_patterns:
                if pattern.lower() in sg_name.lower():
                    self.log_operation(
                        "INFO",
                        f"🎯 EKS pattern match in name: {sg_name} contains '{pattern}'",
                    )
                    return True
            ...
            for tag in sg.get("Tags", []):
                ...
                for pattern in eks_tag_patterns:
                    if pattern in tag_key or pattern in tag_value:
                        self.log_operation(
                            "INFO", f"🏷️ EKS tag found: {tag_key}={tag_value}"
                        )
                        return True
            ...
            self.log_operation(
                "ERROR", f"Error checking if security group {sg_id} is EKS-related: {e}"
            )
    ...
    def is_security_group_used_by_launch_template(self, ec2_client, sg_id):
        ...
        try:
                ...
                for lt in page["LaunchTemplates"]:
                    ...
                    try:
                        ...
                        for version in versions_response["LaunchTemplateVersions"]:
                            ...
                            security_group_ids = launch_template_data.get(
                                "SecurityGroupIds", []
                            )
                            security_groups = launch_template_data.get(
                                "SecurityGroups", []
                            )
                            ...
                            for sg_name in security_groups:
                                if (
                                    sg_id == sg_name
                                ):  # Sometimes names are used instead of IDs
                                    self.log_operation(
                                        "WARNING",
                                        f"🚀 Security group {sg_id} found in Launch Template {lt_id}",
                                    )
            ...
            self.log_operation(
                "ERROR",
                f"Error checking launch templates for security group {sg_id}: {e}",
            )
    ...
    def is_security_group_used_by_asg(self, ec2_client, sg_id, region):
        ...
        try:
            ...
            for page in paginator.paginate():
                for asg in page["AutoScalingGroups"]:
                    ...
                    if "LaunchTemplate" in asg:
                        ...
                        if self.is_security_group_used_by_launch_template(
                            ec2_client, sg_id
                        ):
                            self.log_operation(
                                "WARNING",
                    ...
                    for instance in asg.get("Instances", []):
                        ...
                        try:
                            ...
                            self.log_operation(
                                "DEBUG",
                                f"Could not check instance {instance_id}: {inst_error}",
                            )
                            continue
            ...
            self.log_operation(
                "ERROR",
                f"Error checking Auto Scaling Groups for security group {sg_id}: {e}",
            )
            return True  # Err on the side of caution
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🧹 Starting EC2 cleanup for {account_key} ({account_id}) in {region}",
            )
            print(
                f"\n🧹 Starting EC2 cleanup for {account_key} ({account_id}) in {region}"
            )
            ...
            try:
                ...
                instances = self.get_all_instances_in_region(
                    ec2_client, region, account_info
                )
            ...
            self.log_operation(
                "INFO", f"📊 {account_key} ({region}) EC2 resources summary:"
            )
            ...
            print(
                f"   📊 EC2 resources found: {len(instances)} instances, {len(security_groups)} security groups"
            )
            print(
                f"   📎 Attached SGs: {len(attached_sgs)}, 🔓 Unattached SGs: {len(unattached_sgs)}"
            )

            if not instances and not security_groups:
                self.log_operation(
                    "INFO", f"No EC2 resources found in {account_key} ({region})"
                )
            ...
            if instances:
                self.log_operation(
                    "INFO",
                    f"🗑️  Terminating {len(instances)} instances in {account_key} ({region}) sequentially",
                )
                ...
                for i, instance in enumerate(instances, 1):
                    ...
                    print(
                        f"   [{i}/{len(instances)}] Processing instance {instance_id}..."
                    )
                    ...
                    try:
                        ...
                        failed_count += 1
                        self.log_operation(
                            "ERROR", f"Error terminating instance {instance_id}: {e}"
                        )
                        print(f"   ❌ Error terminating instance {instance_id}: {e}")

                print(
                    f"   ✅ Terminated {terminated_count} instances, ❌ Failed: {failed_count}"
                )
                ...
                if attached_sgs and terminated_count > 0:
                    self.log_operation(
                        "INFO",
                        f"⏳ Waiting 60 seconds for {terminated_count} instances to start terminating...",
                    )
                    print(
            ...
            if unattached_sgs:
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting {len(unattached_sgs)} unattached security groups in {account_key} ({region})",
                )
                print(
                ...
                for i, sg in enumerate(unattached_sgs, 1):
                    sg_id = sg["group_id"]
                    print(
                        f"   [{i}/{len(unattached_sgs)}] Processing security group {sg_id}..."
                    )
                    ...
                    try:
                        ...
                        self.log_operation(
                            "ERROR",
                            f"Error deleting unattached security group {sg_id}: {e}",
                        )
                        print(f"   ❌ Error deleting security group {sg_id}: {e}")

                print(
                    f"   ✅ Deleted {sg_success} unattached security groups, ❌ Failed: {sg_failed}"
                )
            ...
            if attached_sgs:
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting {len(attached_sgs)} attached security groups in {account_key} ({region})",
                )
                print(
                ...
                for retry in range(max_retries):
                    ...
                    for i, sg in enumerate(remaining_sgs, 1):
                        sg_id = sg["group_id"]
                        print(
                            f"   [{i}/{len(remaining_sgs)}] Trying to delete {sg_id}..."
                        )
                        ...
                        try:
                            ...
                            if success:
                                ...
                                self.log_operation(
                                    "INFO", f"✅ Deleted {sg_id} in attempt {retry + 1}"
                                )
                            else:
                                still_remaining.append(sg)
                                self.log_operation(
                                    "WARNING",
                                    f"⏳ {sg_id} still has dependencies, will retry",
                                )
                        except Exception as e:
                            self.log_operation(
                                "ERROR",
                                f"Error deleting attached security group {sg_id}: {e}",
                            )
                            print(f"   ❌ Error deleting security group {sg_id}: {e}")
                            still_remaining.append(sg)
                    ...
                    print(
                        f"   ✅ Deleted {sgs_deleted_this_round} security groups in attempt {retry + 1}, {len(still_remaining)} remaining"
                    )
                    ...
                    if not remaining_sgs:
                        self.log_operation(
                            "INFO",
                            f"✅ All attached security groups deleted in {account_key} ({region})",
                        )
                        print(
                            f"   ✅ All attached security groups deleted successfully!"
                        )
                        break
            ...
            self.log_operation(
                "INFO", f"✅ EC2 cleanup completed for {account_key} ({region})"
            )
            print(f"\n   ✅ EC2 cleanup completed for {account_key} ({region})")
            return True
            ...
            self.log_operation(
                "ERROR",
                f"Error cleaning up EC2 resources in {account_key} ({region}): {e}",
            )
            print(
                f"   ❌ Error cleaning up EC2 resources in {account_key} ({region}): {e}"
            )
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"✅ Ultra EC2 cleanup report saved to: {report_filename}"
            )
            return report_filename

        except Exception as e:
            self.log_operation(
                "ERROR", f"❌ Failed to save ultra EC2 cleanup report: {e}"
            )
            return None
    ...
    def run(self):
        ...
        try:
            ...
            self.print_colored(Colors.RED, "💥 ULTRA EC2 CLEANUP - SEQUENTIAL 💥")
            self.print_colored(Colors.YELLOW, "🚨" * 30)
            self.print_colored(
            ...
            self.print_colored(
                Colors.YELLOW, "\n🔑 Select Root AWS Accounts for EC2 Cleanup:"
            )
            ...
            self.print_colored(
                Colors.RED,
                f"\n⚠️  WARNING: This will delete ALL of the following EC2 resources:",
            )
            ...
            if confirm2 != "yes":
                self.log_operation(
                    "INFO", "Ultra EC2 cleanup cancelled at final confirmation"
                )
                self.print_colored(Colors.RED, "❌ Cleanup cancelled")
                return
            ...
            self.log_operation(
                "INFO",
                f"🚨 EC2 CLEANUP INITIATED - {len(selected_accounts)} accounts, {len(selected_regions)} regions",
            )
            ...
            if (
                ...
                for account, summary in account_summary.items():
                    ...
                    self.print_colored(
                        Colors.WHITE, f"      💻 Instances: {summary['instances']}"
                    )
                    self.print_colored(
                        Colors.WHITE,
            ...
            self.print_colored(Colors.CYAN, f"\n📄 Saving EC2 cleanup report...")
            report_file = self.save_cleanup_report()
            if report_file:
                self.print_colored(
                    Colors.GREEN, f"✅ EC2 cleanup report saved to: {report_file}"
                )
            ...
            self.log_operation(
                "ERROR", f"FATAL ERROR in EC2 cleanup execution: {str(e)}"
            )
            self.print_colored(Colors.RED, f"\n❌ FATAL ERROR: {e}")
            import traceback
...
def main():
    ...
    try:
        ...
        print("\n\n❌ EC2 cleanup interrupted by user")
        exit(1)
    except Exception as e:
