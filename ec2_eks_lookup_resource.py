...
class AWSCostCalculator:
    ...
    def calculate_live_ec2_cost(
        self, instance_data: Dict, live_instance_data: Dict = None
    ) -> Dict:
                ...
                try:
                    if "Z" in time_string:
                        ...
                        if time_string.endswith("IST"):
                            ...
                            return parsed_time.replace(tzinfo=ist_tz).astimezone(
                                timezone.utc
                            )
                        else:
                            # Has +/- timezone offset
        ...
        try:
            ...
            if isinstance(timestamp_str, str):
                ...
                if "+05:30" in timestamp_str or "IST" in timestamp_str:
                    ...
                    clean_time = (
                        timestamp_str.replace("+05:30", "").replace(" IST", "").strip()
                    )
    ...
    def calculate_live_eks_cost(self, cluster_data, live_cluster_data=None):
        ...
        if live_cluster_data and "createdAt" in live_cluster_data:
            ...
            if not created_str:
                ...
                created_str = cluster_data.get(
                    "creation_time", cluster_data.get("created_on", "")
                )
        ...
        hours_running = max(
            0, running_time.total_seconds() / 3600
        )  # Ensure non-negative
        ...
        if live_cluster_data and "nodegroups" in live_cluster_data:
            ...
            actual_nodes = int(
                cluster_data.get("default_nodes", cluster_data.get("min_nodes", 1))
            )
        ...
        # 2. Node compute cost
        hourly_rate = self.get_ec2_hourly_rate(
            instance_type, cluster_data.get("region", "us-east-1")
        )
        node_compute_cost = hours_running * hourly_rate * actual_nodes

        # 3. EBS storage cost (if any)
        storage_hourly_rate = self.get_ebs_hourly_rate(
            "gp3", cluster_data.get("region", "us-east-1")
        )
        node_storage_cost = (
            hours_running * storage_hourly_rate * disk_size_gb * actual_nodes
        )
    ...
    def calculate_eks_cost(self, cluster_data: Dict, hours: int = 24) -> Dict:
        ...
        if created_timestamp:
            try:
                ...
                created_time = datetime.strptime(
                    created_timestamp, "%Y%m%d_%H%M%S"
                ).replace(tzinfo=timezone.utc)
                current_time = self.get_current_time()
                actual_hours = (current_time - created_time).total_seconds() / 3600
                hours = max(1, int(actual_hours))  # At least 1 hour
        ...
        storage_cost_monthly = (
            disk_size_gb * default_nodes * self.EBS_GP3_COST_PER_GB_MONTH
        )
...
class AWSResourceManager:
        ...
        try:
            if isinstance(utc_time_str, str):
                ...
                for fmt in [
                    ...
                    try:
                        utc_dt = datetime.strptime(utc_time_str, fmt).replace(
                            tzinfo=timezone.utc
                        )
                        break
                    except ValueError:
                ...
                utc_dt = (
                    utc_time_str.replace(tzinfo=timezone.utc)
                    if utc_time_str.tzinfo is None
                    else utc_time_str
                )
            else:
    ...
    def group_files_by_date(
        self, file_timestamps: List[Tuple[str, str]]
    ) -> Dict[str, List[str]]:
    ...
    def select_files(self, file_type: str) -> List[str]:
        ...
        if file_type.lower() == "eks":
            patterns = ["eks_cluster_created_*.json", "eks_clusters_created_*.json"]
        else:
            patterns = [
        ...
        for date_str, file_timestamp_pairs in date_groups.items():
            ...
            while True:
                response = (
                    input(f"\n🤔 Process {readable_date} files? (yes/no): ")
                    .strip()
                    .lower()
                )
                if response in ["yes", "y"]:
                    ...
                    print(
                        f"✅ Added {len(file_timestamp_pairs)} file(s) from {readable_date}"
                    )
                    break
    ...
    def get_aws_client(self, service: str, region: str, account_key: str):
        ...
        print(
            f"🔑 Using ROOT credentials for account '{account_key}': {account_info.get('email', 'Unknown')}"
        )
    ...
    def check_eks_cluster_status(
        self, cluster_name: str, region: str, account_key: str
    ) -> Dict:
        ...
        try:
            ...
            return {"error": str(e), "status": "error", "check_time": check_time}
        except Exception as e:
            return {
    ...
    def check_ec2_instance_status(
        self, instance_id: str, region: str, account_key: str
    ) -> Dict:
        ...
        try:
            ...
            try:
                cloudwatch_client = self.get_aws_client(
                    "cloudwatch", region, account_key
                )
    ...
    def display_eks_summary(
        ...
        for idx, cluster in enumerate(clusters):
            ...
            print(
                f"   🏢 Account: {account_key} ({cluster.get('account_id', 'Unknown')})"
            )
            ...
            if show_cost:
                ...
                print(
                    f"   💰 Estimated Cost: ${cost_data['total_cost']:.2f} (for {cost_data['hours_running']:.1f} hours)"
                )
            ...
            cluster_list.append(
                (global_number, cluster_name, account_key, region, cluster)
            )
    ...
    def display_ec2_summary(
        ...
        metadata = data.get("metadata", {})
        print(
            f"📅 Created: {metadata.get('creation_date', 'Unknown')} {metadata.get('creation_time', '')}"
        )
        print(f"👤 Created by: {metadata.get('created_by', 'Unknown')}")
        ...
        for idx, instance in enumerate(instances):
            ...
            print(f"   🌍 Region: {region}")
            print(
                f"   🏢 Account: {account_name} ({instance.get('account_id', 'Unknown')})"
            )
            ...
            if show_cost:
                ...
                print(
                    f"      └─ Compute: ${cost_data['compute_cost']:.2f} ({cost_data['hourly_rate']:.4f}/hr)"
                )
                print(
                    f"      └─ Storage: ${cost_data['storage_cost']:.2f} ({cost_data['disk_size_gb']}GB)"
                )
                total_cost += cost_data["total_cost"]

            # Use global_number in the tuple for proper selection
            instance_list.append(
                (global_number, instance_id, account_name, region, instance)
            )
    ...
    def interactive_live_lookup(self, resource_type: str):
        ...
        for file in files:
            if resource_type == "eks":
                ...
                if data:
                    ...
                    resources = self.display_eks_summary(
                        data, file, show_cost=False, start_number=current_number
                    )
                ...
                if data:
                    # Pass current_number and get back resources with global numbers
                    resources = self.display_ec2_summary(
                        data, file, show_cost=False, start_number=current_number
                    )
                    all_resources.extend(resources)
                    # Update current_number for next file
        ...
        print(
            "💡 Options: single number (5), range (1-3), multiple (1,3,5), or 'all' (default)"
        )
        print(f"💡 Available numbers: 1-{len(all_resources)}")
        print("-" * 70)

        selection_input = input(
            f"Select {resource_type.upper()} for live lookup (default=all): "
        ).strip()
        selected_indices = self.parse_selection_input(
            selection_input, len(all_resources)
        )
        ...
        for global_num in selected_indices:
            ...
            try:
                if resource_type == "eks":
                    global_number, cluster_name, account_key, region, cluster_data = (
                        selected_resource
                    )
                    ...
                    # Perform live lookup
                    status_data = self.check_eks_cluster_status(
                        cluster_name, region, account_key
                    )
                    ...
                    if status_data["status"] == "error":
                        ...
                        print(
                            f"   📝 Reason: Cluster found in JSON but not found in live AWS"
                        )
                        failed_lookups += 1
                        ...
                        print(
                            f"   📅 Created: {self.utc_to_ist(str(cluster['createdAt']))} IST"
                        )
                        ...
                        if nodegroups:
                            ...
                            print(
                                f"   🔗 Node Groups: {len(nodegroups)} active with {total_nodes} total nodes"
                            )
                    ...
                    self.execution_reports.append(
                        {
                            ...
                            "total_nodes": (
                                sum(
                                    [
                                        ng.get("scalingConfig", {}).get(
                                            "desiredSize", 0
                                        )
                                        for ng in status_data.get("nodegroups", [])
                                    ]
                                )
                                if status_data["status"] != "error"
                                else 0
                    ...
                    global_number, instance_id, account_name, region, instance_data = (
                        selected_resource
                    )
                    ...
                    # Perform live lookup
                    status_data = self.check_ec2_instance_status(
                        instance_id, region, account_name
                    )
                    ...
                    if status_data["status"] in ["error", "not_found"]:
                        ...
                        print(f"   Error: {status_data['error']}")
                        print(
                            f"   📝 Reason: Instance found in JSON but not found in live AWS"
                        )
                        failed_lookups += 1
                        not_found_resources.append(
                            {
                        ...
                        print(
                            f"   📅 Launch Time: {self.utc_to_ist(str(instance['LaunchTime']))} IST"
                        )
                    ...
                    self.execution_reports.append(
                        {
                            ...
                            "status": (
                                "success"
                                if status_data["status"] not in ["error", "not_found"]
                                else "error"
                            ),
                            "data": status_data,
                            "timestamp": current_time,
        ...
        if not_found_resources:
            ...
            for resource in not_found_resources:
                print(
                    f"📝 #{resource['number']} {resource['type']}: {resource['id']} (Account: {resource['account']}, Region: {resource['region']})"
                )
        else:
            print(f"\n✅ All resources successfully found in live AWS")

        # Save consolidated report
        saved_file = self.save_consolidated_execution_report(
            "live_lookup", resource_type
        )
        if saved_file:
    ...
    def display_live_eks_status(self, cluster_name: str, region: str, account_key: str):
        ...
        if nodegroups:
            ...
            for ng in nodegroups:
                ...
                print(
                    f"    - Scaling: Min={scaling.get('minSize', 0)}, "
    ...
    def direct_resource_lookup(self, resource_id: str):
        ...
        if result:
            ...
            print(
                f"❌ {resource_type.upper()} resource '{resource_id}' not found in state files"
            )
    ...
    def save_consolidated_execution_report(
        self, operation_type: str, resource_type: str
    ):
        ...
        try:
            ...
            timestamp = datetime.now(timezone(timedelta(hours=5, minutes=30))).strftime(
                "%Y%m%d_%H%M%S"
            )
            filename = (
                f"{resource_type}_{operation_type}_execution_report_{timestamp}.txt"
            )
            ...
            with open(full_path, "w", encoding="utf-8") as f:
                f.write(
                    f"AWS {resource_type.upper()} {operation_type.upper()} EXECUTION REPORT\n"
                )
                f.write("=" * 70 + "\n")
                ...
                successful_count = len(
                    [r for r in self.execution_reports if r["status"] == "success"]
                )
                error_count = len(
                    [r for r in self.execution_reports if r["status"] == "error"]
                )
                ...
                for i, report in enumerate(self.execution_reports, 1):
                    ...
                    f.write(
                        f"   Status: {'✅ SUCCESS' if report['status'] == 'success' else '❌ ERROR'}\n"
                    )
                    ...
                    if report["status"] == "success":
                        if report["type"] == "eks_status":
                            f.write(
                                f"   Total Nodes: {report.get('total_nodes', 'Unknown')}\n"
                            )
                            ...
                            f.write(
                                f"   Running Hours: {cost_data.get('hours_running', 0):.2f}\n"
                            )
                ...
                if operation_type in ["live_cost", "cost_calculation"]:
                    total_cost = sum(
                        [
                            r.get("cost_data", {}).get("total_cost", 0)
    ...
    def calculate_resource_costs(self, resource_type: str):
        ...
        selection_input = input(
            f"Select {resource_type.upper()} for live cost calculation (default=all): "
        ).strip()
        selected_indices = self.parse_selection_input(
            selection_input, len(all_resources)
        )
        ...
        for account_name, group_data in account_groups.items():
            ...
            for selected_resource in group_data["resources"]:
                if resource_type == "eks":
                    ...
                    try:
                        ...
                        live_status = self.check_eks_cluster_status(
                            cluster_name, region, account_key
                        )

                        if live_status["status"] == "error":
                            print(
                                f"⚠️ SKIPPED - EKS Cluster {global_number}: {cluster_name}"
                            )
                            print(f"   ❌ Error: {live_status['error']}")
                            print(
                                f"   📝 Reason: Cluster not found in live AWS or access denied"
                            )
                            skipped_resources += 1
                            account_skipped += 1
                        ...
                        if live_status["status"] == "success":
                            ...
                            live_cluster_data["nodegroups"] = live_status.get(
                                "nodegroups", []
                            )

                        cost_data = self.cost_calculator.calculate_live_eks_cost(
                            cluster_data, live_cluster_data
                        )
                        ...
                        print(f"   🌍 Region: {region}")
                        print(
                            f"   📊 Status: {cost_data.get('current_status', 'unknown')}"
                        )
                        print(
                            f"   📅 Created: {self.utc_to_ist(str(live_cluster_data.get('createdAt', 'Unknown') if live_cluster_data else 'Unknown'))} IST"
                        )
                        print(f"   ⏰ Running Hours: {cost_data['hours_running']:.2f}")
                        print(f"   🔢 Total Nodes: {total_nodes}")
                        print(
                            f"   🎛️ Control Plane: ${cost_data['control_plane_cost']:.2f} (${self.cost_calculator.EKS_CLUSTER_HOURLY_COST:.2f}/hr)"
                        )
                        print(
                            f"   🖥️ Node Compute: ${cost_data['node_compute_cost']:.2f} ({total_nodes} × {cost_data['instance_type']})"
                        )
                        print(
                            f"   💾 Node Storage: ${cost_data['node_storage_cost']:.2f} ({cost_data['disk_size_gb']}GB per node)"
                        )
                        print(f"   💰 Total Cost: ${cost_data['total_cost']:.2f}")
                        print(
                            f"   ⏰ Calculated at: {cost_data['calculation_time'].astimezone(timezone(timedelta(hours=5, minutes=30))).strftime('%Y-%m-%d %H:%M:%S')} IST"
                        )
                    ...
                    try:
                        ...
                        live_status = self.check_ec2_instance_status(
                            instance_id, region, account_name_inner
                        )

                        if live_status["status"] in ["error", "not_found"]:
                            print(
                                f"⚠️ SKIPPED - EC2 Instance {global_number}: {instance_id}"
                            )
                            print(f"   ❌ Error: {live_status['error']}")
                            print(
                                f"   📝 Reason: Instance not found in live AWS or access denied"
                            )
                            skipped_resources += 1
                            account_skipped += 1
                        ...
                        cost_data = self.cost_calculator.calculate_live_ec2_cost(
                            instance_data, live_instance_data
                        )

                        print(f"\n💻 EC2 Instance {global_number}: {instance_id}")
                        print(f"   🌍 Region: {region}")
                        print(
                            f"   🏷️ Type: {cost_data['instance_type']} | 📊 State: {cost_data.get('current_state', 'unknown')}"
                        )
                        print(
                            f"   📅 Launch Time: {self.utc_to_ist(str(live_instance_data.get('LaunchTime', 'Unknown') if live_instance_data else 'Unknown'))} IST"
                        )
                        print(f"   ⏰ Running Hours: {cost_data['hours_running']:.2f}")
                        print(
                            f"   🖥️ Compute: ${cost_data['compute_cost']:.2f} (${cost_data['hourly_rate']:.4f}/hr)"
                        )
                        print(
                            f"   💾 Storage: ${cost_data['storage_cost']:.2f} ({cost_data['disk_size_gb']}GB)"
                        )
            ...
            print(
                f"📊 Resources in this account: {group_data['count']} {resource_type.upper()} (✅ {account_successful} successful, ⚠️ {account_skipped} skipped)"
            )
            print("-" * 60)
        ...
        for account_name in account_groups.keys():
            ...
            print(
                f"   🏢 {account_name}: ${account_total:.2f} ({percentage:.1f}%) - {account_groups[account_name]['count']} resources"
            )

        # Save consolidated execution report instead of individual files
        if self.execution_reports:
            saved_file = self.save_consolidated_execution_report(
                "live_cost", resource_type
            )
            if saved_file:
                print(
    ...
    def ask_resource_type(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                resource_id = input(
                    "\nEnter resource ID (cluster name or instance ID): "
                ).strip()
                if resource_id:
