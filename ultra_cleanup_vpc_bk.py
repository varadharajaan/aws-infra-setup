...
class UltraCleanupCustomVPCManager:
    ...
    def delete_vpc(self, ec2_client, vpc_info, force_delete=True):
        ...
        try:
            ...
            self.log_operation('INFO', f"✅ Successfully deleted VPC {vpc_id} ({vpc_name})")
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation('INFO', f"🧹 Starting VPC cleanup for {account_name} in {region}")
            ...
            self.log_operation('INFO', f"🗑️  Deleting {len(vpcs)} custom VPCs in {account_name} ({region})")
            ...
            for retry in range(max_retries):
                ...
                for i, vpc in enumerate(remaining_vpcs, 1):
                    ...
                    try:
                        ...
                        if success:
                            ...
                            self.log_operation('INFO', f"✅ Deleted {vpc_id} in attempt {retry + 1}")
                        else:
                            still_remaining.append(vpc)
                            self.log_operation('WARNING', f"⏳ {vpc_id} still has dependencies, will retry")
                    except Exception as e:
                ...
                print(f"   ✅ Deleted {vpcs_deleted_this_round} VPCs in attempt {retry + 1}, {len(still_remaining)} remaining")
                
                remaining_vpcs = still_remaining
                
                if not remaining_vpcs:
                    self.log_operation('INFO', f"✅ All custom VPCs deleted in {account_name} ({region})")
            ...
            if remaining_vpcs:
                self.log_operation('WARNING', f"⚠️  {len(remaining_vpcs)} VPCs could not be deleted after {max_retries} retries")
                print(f"   ⚠️  {len(remaining_vpcs)} VPCs could not be deleted after {max_retries} retries")
                self.log_operation('WARNING', f"Remaining VPCs: {[vpc['vpc_id'] for vpc in remaining_vpcs]}")
            
            self.log_operation('INFO', f"✅ VPC cleanup completed for {account_name} ({region})")
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            self.log_operation('INFO', f"✅ VPC cleanup report saved to: {report_filename}")
    ...
    def run(self):
        ...
        try:
            self.log_operation('INFO', "🚨 STARTING ULTRA CUSTOM VPC CLEANUP SESSION 🚨")
            ...
            self.log_operation('INFO', f"🚨 VPC CLEANUP INITIATED - {len(accounts)} accounts, {len(regions)} regions")
