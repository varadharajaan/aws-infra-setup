...
class UltraCleanupCustomVPCManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{log_dir}/ultra_vpc_cleanup_log_{self.execution_timestamp}.log"
            )
            ...
            self.operation_logger.info("🚨 ULTRA CUSTOM VPC CLEANUP SESSION STARTED 🚨")
            self.operation_logger.info("=" * 100)
    ...
    def create_vpc_client(self, credentials, region):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to create VPC client for {region}: {e}"
            )
            raise
    ...
    def get_custom_vpcs(self, ec2_client, region, account_name):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🔍 Scanning for custom VPCs in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "INFO", f"🏗️  Found {len(vpcs)} custom VPCs in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "ERROR", f"Error getting custom VPCs in {region} ({account_name}): {e}"
            )
    ...
    def get_vpc_dependencies(self, ec2_client, vpc_id):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Error getting VPC dependencies for {vpc_id}: {e}"
            )
    ...
    def cleanup_vpc_dependencies(self, ec2_client, vpc_id, vpc_name):
        ...
        self.log_operation(
            "INFO", f"🧹 Cleaning VPC dependencies for {vpc_id} ({vpc_name})"
        )
        print(f"   🧹 Cleaning VPC dependencies for {vpc_id} ({vpc_name})...")
        ...
        for endpoint in dependencies["vpc_endpoints"]:
            try:
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete VPC endpoint {endpoint_id}: {e}"
                )
        ...
        for eip in dependencies["elastic_ips"]:
            try:
                ...
                if allocation_id:
                    self.log_operation(
                        "INFO", f"   Releasing elastic IP {public_ip} ({allocation_id})"
                    )
                ...
                self.log_operation(
                    "WARNING", f"   Failed to release elastic IP {public_ip}: {e}"
                )
                cleanup_success = False
        ...
        for eni in dependencies["network_interfaces"]:
            try:
                ...
                if eni.get("Attachment"):
                    ...
                    if attachment_id:
                        self.log_operation(
                            "INFO", f"   Detaching network interface {eni_id}"
                        )
                ...
                self.cleanup_results["deleted_dependencies"][
                    "network_interfaces"
                ].append(
                    {
                        "interface_id": eni_id,
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete network interface {eni_id}: {e}"
                )
                cleanup_success = False
        ...
        for nat in dependencies["nat_gateways"]:
            try:
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete NAT gateway {nat_id}: {e}"
                )
                cleanup_success = False
        ...
        if nat_gateway_ids:
            self.log_operation(
                "INFO",
                f"   Waiting for {len(nat_gateway_ids)} NAT gateways to be deleted...",
            )
        ...
        for subnet in dependencies["subnets"]:
            try:
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete subnet {subnet_id}: {e}"
                )
                cleanup_success = False
        ...
        for sg in dependencies["security_groups"]:
            try:
                ...
                self.log_operation(
                    "INFO", f"   Deleting security group {sg_id} ({sg_name})"
                )
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete security group {sg_id}: {e}"
                )
                cleanup_success = False
        ...
        for rt in dependencies["route_tables"]:
            try:
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete route table {rt_id}: {e}"
                )
                cleanup_success = False
        ...
        for nacl in dependencies["network_acls"]:
            try:
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete network ACL {nacl_id}: {e}"
                )
                cleanup_success = False
        ...
        for igw in dependencies["internet_gateways"]:
            try:
                ...
                self.log_operation(
                    "INFO", f"   Detaching internet gateway {igw_id} from VPC {vpc_id}"
                )
                ...
                self.log_operation(
                    "WARNING", f"   Failed to delete internet gateway {igw_id}: {e}"
                )
                cleanup_success = False
        ...
        self.log_operation(
            "INFO",
            f"   Deleted {total_dependencies} dependencies, waiting 30 seconds for cleanup...",
        )
    ...
    def wait_for_nat_gateways_deletion(
        self, ec2_client, nat_gateway_ids, max_wait_time=300
    ):
        """Wait for NAT gateways to be deleted"""
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                self.log_operation(
                    "INFO",
                    f"   Waiting for {len(active_gateways)} NAT gateways to be deleted...",
                )
                time.sleep(15)
                ...
                if e.response["Error"]["Code"] == "InvalidNatGatewayID.NotFound":
                    ...
                    self.log_operation(
                        "WARNING", f"Error checking NAT gateway status: {e}"
                    )
                    break
            except Exception as e:
        ...
        try:
            ...
            if non_default_egress:
                ec2_client.revoke_security_group_egress(
                    GroupId=sg_id, IpPermissions=non_default_egress
                )

            return True
        except Exception as e:
            self.log_operation(
                "WARNING", f"Failed to clear rules for security group {sg_id}: {e}"
            )
            return False
    ...
    def delete_vpc(self, ec2_client, vpc_info, force_delete=True):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🗑️  Deleting custom VPC {vpc_id} ({vpc_name}) in {region} ({account_name})",
            )
            print(f"   🗑️  Deleting custom VPC {vpc_id} ({vpc_name})...")

            # Clean dependencies first
            if force_delete:
                dependencies_cleaned = self.cleanup_vpc_dependencies(
                    ec2_client, vpc_id, vpc_name
                )
                if not dependencies_cleaned:
                    self.log_operation(
                        "WARNING",
                        f"Some dependencies could not be cleaned for VPC {vpc_id}",
                    )
            ...
            self.log_operation(
                "INFO", f"✅ Successfully deleted VPC {vpc_id} ({vpc_name})"
            )
            ...
            if error_code == "InvalidVpcID.NotFound":
                ...
                self.log_operation(
                    "WARNING", f"Cannot delete VPC {vpc_id}: dependency violation"
                )
                print(f"   ⚠️ Cannot delete VPC {vpc_id}: still has dependencies")
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🧹 Starting VPC cleanup for {account_name} in {region}"
            )
            print(f"\n🧹 Starting VPC cleanup for {account_name} in {region}")
            ...
            print(f"   📊 Resources found: {len(vpcs)} custom VPCs")

            if not vpcs:
                self.log_operation(
                    "INFO", f"No custom VPCs found in {account_name} ({region})"
                )
                print(f"   ✅ No custom VPCs to clean up in {region}")
                return True

            # Delete VPCs with retries for dependency issues
            self.log_operation(
                "INFO",
                f"🗑️  Deleting {len(vpcs)} custom VPCs in {account_name} ({region})",
            )
            print(f"\n   🗑️  Deleting {len(vpcs)} custom VPCs...")
            ...
            for retry in range(max_retries):
                self.log_operation(
                    "INFO", f"🔄 VPC deletion attempt {retry + 1}/{max_retries}"
                )
                ...
                for i, vpc in enumerate(remaining_vpcs, 1):
                    vpc_id = vpc["vpc_id"]
                    print(
                        f"   [{i}/{len(remaining_vpcs)}] Trying to delete VPC {vpc_id}..."
                    )
                    ...
                    try:
                        ...
                        if success:
                            ...
                            self.log_operation(
                                "INFO", f"✅ Deleted {vpc_id} in attempt {retry + 1}"
                            )
                        else:
                            still_remaining.append(vpc)
                            self.log_operation(
                                "WARNING",
                                f"⏳ {vpc_id} still has dependencies, will retry",
                            )
                    except Exception as e:
                ...
                print(
                    f"   ✅ Deleted {vpcs_deleted_this_round} VPCs in attempt {retry + 1}, {len(still_remaining)} remaining"
                )
                ...
                if not remaining_vpcs:
                    self.log_operation(
                        "INFO",
                        f"✅ All custom VPCs deleted in {account_name} ({region})",
                    )
                    print(f"   ✅ All custom VPCs deleted successfully!")
                    break
            ...
            if remaining_vpcs:
                self.log_operation(
                    "WARNING",
                    f"⚠️  {len(remaining_vpcs)} VPCs could not be deleted after {max_retries} retries",
                )
                print(
                    f"   ⚠️  {len(remaining_vpcs)} VPCs could not be deleted after {max_retries} retries"
                )
                self.log_operation(
                    "WARNING",
                    f"Remaining VPCs: {[vpc['vpc_id'] for vpc in remaining_vpcs]}",
                )

            self.log_operation(
                "INFO", f"✅ VPC cleanup completed for {account_name} ({region})"
            )
            print(f"\n   ✅ VPC cleanup completed for {account_name} ({region})")
            return True

        except Exception as e:
            self.log_operation(
                "ERROR", f"Error cleaning up VPCs in {account_name} ({region}): {e}"
            )
            print(f"   ❌ Error cleaning up VPCs in {account_name} ({region}): {e}")
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            report_filename = (
                f"{report_dir}/ultra_vpc_cleanup_report_{self.execution_timestamp}.json"
            )
            ...
            total_dependencies_deleted = sum(
                len(deps)
                for deps in self.cleanup_results["deleted_dependencies"].values()
            )
            ...
            report_data = {
                ...
                "summary": {
                    ...
                    "dependency_breakdown": {
                        dep_type: len(deps)
                        for dep_type, deps in self.cleanup_results[
                            "deleted_dependencies"
                        ].items()
                    },
                },
                "detailed_results": {
                    ...
                    "deleted_dependencies": self.cleanup_results[
                        "deleted_dependencies"
                    ],
            ...
            self.log_operation(
                "INFO", f"✅ VPC cleanup report saved to: {report_filename}"
            )
            return report_filename

        except Exception as e:
            self.log_operation("ERROR", f"❌ Failed to save VPC cleanup report: {e}")
            return None
    ...
    def run(self):
        ...
        try:
            self.log_operation(
                "INFO", "🚨 STARTING ULTRA CUSTOM VPC CLEANUP SESSION 🚨"
            )

            print("🚨" * 30)
            print("💥 ULTRA CUSTOM VPC CLEANUP 💥")
            print("🚨" * 30)
            ...
            print(f"\n🔐 Loading AWS account credentials...")
            ...
            # Confirmation
            print(
                f"\n⚠️  WARNING: This will delete ALL custom VPCs and their dependencies"
            )
            print(f"    across {len(accounts)} accounts in {len(regions)} regions")
            print(
                f"    This includes subnets, route tables, internet gateways, NAT gateways, etc."
            )
            ...
            if confirm2 != "yes":
                self.log_operation(
                    "INFO", "VPC cleanup cancelled at final confirmation"
                )
                print("❌ Cleanup cancelled")
                return

            # Start cleanup
            print(f"\n💥 STARTING VPC CLEANUP...")
            self.log_operation(
                "INFO",
                f"🚨 VPC CLEANUP INITIATED - {len(accounts)} accounts, {len(regions)} regions",
            )
            ...
            for account in accounts:
                for region in regions:
                    ...
                    try:
                        ...
                        self.log_operation(
                            "ERROR",
                            f"Task failed for {account['name']} ({region}): {e}",
                        )
                        print(f"❌ Task failed for {account['name']} ({region}): {e}")
            ...
            print(
                f"🏗️  Custom VPCs deleted: {len(self.cleanup_results['deleted_vpcs'])}"
            )
            ...
            print(f"🧹 Total dependencies deleted: {total_dependencies}")
            ...
            if self.cleanup_results["deleted_vpcs"]:
                print(f"\n📊 VPC Deletion Summary by Account:")
                account_summary = {}
                for vpc in self.cleanup_results["deleted_vpcs"]:
                    account = vpc["account_name"]
                ...
                for account, summary in account_summary.items():
                    ...
                    print(f"   🏦 {account}: {summary['vpcs']} VPCs in {regions_list}")
            ...
            # Save report
            print(f"\n📄 Saving VPC cleanup report...")
            report_file = self.save_cleanup_report()
            if report_file:
                print(f"✅ VPC cleanup report saved to: {report_file}")
            ...
            self.log_operation(
                "ERROR", f"FATAL ERROR in VPC cleanup execution: {str(e)}"
            )
            print(f"\n❌ FATAL ERROR: {e}")
            import traceback
