...
class AWSVPCRecoveryTool:
    ...
    def load_configuration(self):
        ...
        try:
            ...
            print(
                f"🌍 Available {len(self.check_regions)} regions: {self.check_regions}"
            )
    ...
    def select_accounts_and_regions(self):
        ...
        print(f"\n🌍 Region Selection:")
        print(
            f"  1. Recover ALL configured regions ({len(self.check_regions)} regions)"
        )
    ...
    def get_next_available_cidr(
        self, existing_cidrs, base_cidr="172.31.0.0/16", subnet_size=20
    ):
        ...
        try:
            ...
            while current_ip < vpc_network.broadcast_address:
                try:
                    ...
                    candidate_subnet = ipaddress.IPv4Network(
                        f"{current_ip}/{subnet_prefix_len}", strict=False
                    )
    ...
    def analyze_vpc_state(self, ec2_client, account_name, region):
        ...
        try:
            ...
            min_required_azs = min(
                2, len(available_azs)
            )  # At least 2 AZs or all available AZs if less than 2
            ...
            igw_id = (
                igw_response["InternetGateways"][0]["InternetGatewayId"]
                if has_igw
                else None
            )
            ...
            needs_subnets = (len(covered_azs) < min_required_azs) or (
                len(subnets) < min_required_azs
            )
        ...
        try:
            ...
            if analysis["needs_subnets"]:
                ...
                if not target_azs and current_subnet_count < min_required_azs:
                    ...
                    target_azs = available_azs[
                        : min_required_azs - current_subnet_count
                    ]

                print(f"      🔧 Creating subnets for better AZ coverage...")
                print(
                    f"         Current: {current_subnet_count} subnets in {len(covered_azs)} AZs: {list(covered_azs)}"
                )
                print(
                    f"         Target: {len(target_azs)} additional subnets in AZs: {target_azs}"
                )
                ...
                for az in target_azs:
                    ...
                    try:
                        ...
                        subnets_created.append(
                            {"subnet_id": subnet_id, "cidr": subnet_cidr, "az": az}
                        )

                        print(
                            f"         • Created subnet {subnet_id} in {az} ({subnet_cidr})"
                        )
                        self.log_action(
                            account_name,
                            region,
                            "CREATE_SUBNET",
                            f"{subnet_id}:{subnet_cidr}:{az}",
                        )
                ...
                print(
                    f"         ✅ Created {len(subnets_created)} subnets successfully"
                )
            else:
                print(
                    f"      ✅ Subnet configuration already adequate ({analysis['subnet_count']} subnets in {len(analysis['covered_azs'])} AZs)"
                )
            ...
            for subnet in all_subnets_response["Subnets"]:
                ...
                if subnet_id not in explicitly_associated:
                    try:
                        ...
                        print(
                            f"         • Associated {subnet_id} -> {rt_id} ({assoc_id})"
                        )
                        self.log_action(
                            account_name,
                            region,
                            "CREATE_ASSOCIATION",
                            f"{subnet_id}:{rt_id}:{assoc_id}",
                        )
            ...
            if associations_created > 0:
                print(
                    f"         ✅ Created {associations_created} explicit associations"
                )
            else:
            ...
            if not analysis["has_internet_route"]:
                ...
                try:
                    ...
                    if existing_internet_route:
                        current_target = existing_internet_route.get(
                            "GatewayId", "unknown"
                        )
                        if current_target != igw_id:
                            ...
                            print(
                                f"         • Deleted conflicting route (was: {current_target})"
                            )
            ...
            if not final_analysis.get("recovery_needed", True):
                ...
                self.log_action(
                    account_name, region, "RECOVERY_PARTIAL", "Some issues remain"
                )
                return False
        ...
        try:
            ...
            report_data = {
                ...
                "recovery_summary": {
                    ...
                    "success_rate": (
                        f"{(successful_recoveries/total_recoveries*100):.1f}%"
                ...
                "action_statistics": {
                    "vpc_creations": len(
                        [
                            ...
                            if a["action"] == "CREATE_DEFAULT_VPC"
                        ]
                    ),
                    "igw_creations": len(
                        [
                            a
                            for a in self.recovery_actions
                            if a["action"] == "CREATE_IGW"
                        ]
                    ),
                    "subnet_creations": len(
                        [
                            a
                            for a in self.recovery_actions
                            if a["action"] == "CREATE_SUBNET"
                        ]
                    ),
                    "association_creations": len(
                        [
                            a
                            for a in self.recovery_actions
                            if a["action"] == "CREATE_ASSOCIATION"
                        ]
                    ),
                    "route_creations": len(
                        [
                            a
                            for a in self.recovery_actions
                            if a["action"] == "CREATE_INTERNET_ROUTE"
                        ]
                    ),
                },
            }
    ...
    def run_recovery(self):
        ...
        print(
            f"   🎯 Total VPCs to recover: {len(selected_accounts) * len(selected_regions)}"
        )

        confirm = (
            input(f"\n⚠️  This will recover/create VPC resources. Continue? (y/N): ")
            .lower()
            .strip()
        )
        if confirm != "y":
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_task = {
                executor.submit(self.recover_vpc_resources, account, region): (
                    account,
                    region,
                )
        ...
        print(f"{'='*80}")
        print(
            f"✅ Successfully recovered: {successful}/{len(results)} VPCs ({successful/len(results)*100:.1f}%)"
        )
