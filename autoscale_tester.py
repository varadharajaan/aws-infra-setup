...
class AutoscalerTester:
    ...
    def get_user_input(self, prompt: str, default: str = "y") -> str:
        ...
        try:
            response = input(
                f"{self.colors.YELLOW}{prompt} [{default}]: {self.colors.ENDC}"
            ).strip()
    ...
    def wait_with_countdown(self, seconds: int, message: str):
        ...
        for i in range(seconds, 0, -1):
            print(
                f"\r{self.colors.CYAN}    Waiting {i} seconds...{self.colors.ENDC}",
                end="",
                flush=True,
            )
            time.sleep(1)
    ...
    def check_nodes(self) -> int:
        ...
        if success:
            ...
            self.print_colored(
                self.colors.WHITE, f"Current node count: {node_count}", 1
            )
            return node_count
        else:
    ...
    def show_autoscaler_logs(self, lines: int = 10):
        ...
        self.print_colored(
            self.colors.BLUE, f"📋 Recent autoscaler logs ({lines} lines):"
        )
        success, stdout, stderr = self.run_command(
            [
                "kubectl",
...
"""
        ...
        success, stdout, stderr = self.run_command(
            ["kubectl", "apply", "-f", "/tmp/stress-test.yaml"]
        )

        if success:
            self.print_colored(
                self.colors.GREEN,
                "✅ Stress test applications deployed successfully",
                1,
            )
            ...
            success, stdout, stderr = self.run_command(
                [
                    ...
                    "deployments",
                    "-l",
                    "app in (stress-test,cpu-intensive)",
                ]
            )
            if success:
    ...
    def run_scale_up_scenario(self):
        ...
        self.print_colored(
            self.colors.YELLOW,
            "🎯 Scale-Up Test: Increasing workload to trigger node addition",
        )
        ...
        self.print_colored(
            self.colors.WHITE,
            f"Initial state: {initial_nodes} nodes, {initial_pods['Running']} running pods",
            1,
        )

        # Scale up stress-test deployment
        self.print_colored(
            self.colors.BLUE, "📊 Step 1: Scaling stress-test to 5 replicas..."
        )
        success, stdout, stderr = self.run_command(
        ...
        if success:
            self.print_colored(
                self.colors.GREEN, "✅ Stress-test scaled to 5 replicas", 1
            )
        else:
            self.print_colored(
                self.colors.RED, f"❌ Failed to scale stress-test: {stderr}", 1
            )
        ...
        self.print_colored(
            self.colors.BLUE, "📊 Step 2: Scaling cpu-intensive to 3 replicas..."
        )
        success, stdout, stderr = self.run_command(
            ["kubectl", "scale", "deployment", "cpu-intensive", "--replicas=3"]
        )
        if success:
            self.print_colored(
                self.colors.GREEN, "✅ CPU-intensive scaled to 3 replicas", 1
            )
        else:
            self.print_colored(
                self.colors.RED, f"❌ Failed to scale cpu-intensive: {stderr}", 1
            )
        ...
        for i in range(5):  # Monitor for 5 minutes
            ...
            self.print_colored(
                self.colors.WHITE,
                f"Test app pods - Running: {current_pods['Running']}, Pending: {current_pods['Pending']}",
                1,
            )
            ...
            if current_nodes > initial_nodes:
                self.print_colored(
                    self.colors.GREEN,
                    f"🎉 SUCCESS: Nodes scaled from {initial_nodes} to {current_nodes}!",
                    1,
                )
                break
                ...
                self.print_colored(
                    self.colors.YELLOW,
                    f"⏳ Pods pending, autoscaler should add nodes soon...",
                    1,
                )
            else:
                self.print_colored(
                    self.colors.BLUE,
                    f"ℹ️  No pending pods, checking if more capacity needed...",
                    1,
                )
        ...
        self.print_colored(
            self.colors.WHITE,
            f"Nodes: {initial_nodes} → {final_nodes} (Change: +{final_nodes - initial_nodes})",
            1,
        )
        self.print_colored(
            self.colors.WHITE,
            f"Running pods: {initial_pods['Running']} → {final_pods['Running']}",
            1,
        )
        self.print_colored(
            self.colors.WHITE, f"Pending pods: {final_pods['Pending']}", 1
        )
    ...
    def run_scale_down_scenario(self):
        ...
        self.print_colored(
            self.colors.YELLOW,
            "🎯 Scale-Down Test: Reducing workload to trigger node removal",
        )
        ...
        self.print_colored(
            self.colors.BLUE, "📊 Step 1: Scaling stress-test to 1 replica..."
        )
        success, stdout, stderr = self.run_command(
        ...
        if success:
            self.print_colored(
                self.colors.GREEN, "✅ Stress-test scaled down to 1 replica", 1
            )
        else:
            self.print_colored(
                self.colors.RED, f"❌ Failed to scale down stress-test: {stderr}", 1
            )

        self.wait_with_countdown(30, "Waiting for pods to terminate...")

        self.print_colored(
            self.colors.BLUE, "📊 Step 2: Scaling cpu-intensive to 0 replicas..."
        )
        success, stdout, stderr = self.run_command(
            ["kubectl", "scale", "deployment", "cpu-intensive", "--replicas=0"]
        )
        if success:
            self.print_colored(
                self.colors.GREEN, "✅ CPU-intensive scaled down to 0 replicas", 1
            )
        else:
            self.print_colored(
                self.colors.RED, f"❌ Failed to scale down cpu-intensive: {stderr}", 1
            )

        # Monitor scale-down (takes longer due to 2m delays)
        self.print_colored(
            self.colors.YELLOW,
            "⏳ Scale-down monitoring (autoscaler waits 2 minutes before removing nodes)",
            1,
        )
        ...
        for i in range(4):  # Monitor for 8 minutes (scale-down takes longer)
            ...
            self.print_colored(
                self.colors.WHITE,
                f"Test app pods - Running: {current_pods['Running']}",
                1,
            )
            ...
            if current_nodes < initial_nodes:
                self.print_colored(
                    self.colors.GREEN,
                    f"🎉 SUCCESS: Nodes scaled down from {initial_nodes} to {current_nodes}!",
                    1,
                )
                break
            else:
                self.print_colored(
                    self.colors.BLUE,
                    f"ℹ️  Nodes unchanged, autoscaler waiting for scale-down delay...",
                    1,
                )

            if i < 3:  # Don't wait after last iteration
                self.wait_with_countdown(
                    120, "Waiting 2 minutes for scale-down delay..."
                )
        ...
        self.print_colored(
            self.colors.WHITE, f"Running test pods: {final_pods['Running']}", 1
        )
        ...
        if final_nodes < initial_nodes:
            ...
            self.print_colored(
                self.colors.YELLOW,
                "⚠️  Scale-down may take more time (can take up to 10+ minutes)",
                1,
            )
    ...
    def cleanup_test_deployments(self):
        ...
        for deployment in deployments:
            ...
            if success:
                ...
                self.print_colored(
                    self.colors.YELLOW,
                    f"⚠️  {deployment} may not exist or already deleted",
                    1,
                )
        ...
        if remaining_pods["Running"] == 0:
            self.print_colored(
                self.colors.GREEN, "✅ All test pods cleaned up successfully", 1
            )
        else:
            self.print_colored(
                self.colors.YELLOW,
                f"⚠️  {remaining_pods['Running']} test pods still running",
                1,
            )
    ...
    def run_interactive_testing(self):
        ...
        self.print_colored(
            self.colors.CYAN, "This will test your autoscaler with real workloads!"
        )
        ...
        try:
            ...
            cleanup = self.get_user_input(
                "🗑️ Clean up test deployments before exit? (y/n)", "y"
            )
            if cleanup.lower() in ["y", "yes"]:
                self.cleanup_test_deployments()
...
def deploy_my_autoscaler():
    ...
    try:
        response = input(
            f"\n🤔 Would you like to test the autoscaler with sample deployments? (y/n) [y]: "
        ).strip()
        ...
        if test_autoscaler:
            ...
            print(
                f"  kubectl set resources deployment test-scale --requests=cpu=1000m,memory=1Gi"
            )
