...
class UltraCleanupSageMakerManager:
        ...
        try:
            ...
            try:
                ...
                if ep_list:
                    ...
                    for ep in ep_list:
                        try:
                            self.print_colored(Colors.CYAN,f"[DELETE] Deleting endpoint: {ep['EndpointName']}")
                        ...
                        try:
                            if nb['NotebookInstanceStatus']!='Deleting':
                                if nb['NotebookInstanceStatus']=='InService':
                                    ...
                                    time.sleep(10)
                                self.print_colored(Colors.CYAN,f"[DELETE] Deleting notebook: {nb['NotebookInstanceName']}")
    ...
    def interactive_cleanup(self):
        self.print_colored(Colors.BLUE,"\n"+"="*100+"\n[START] ULTRA AWS SAGEMAKER CLEANUP\n"+"="*100)
