...
class UltraCleanupIAMManager:
    ...
    def get_all_iam_users(self, iam_client, account_info):
        ...
        try:
            ...
            self.log_operation('INFO', f"[USER] Found {len(users)} IAM users in {account_name}")
            print(f"   [USER] Found {len(users)} IAM users in {account_name}")
    ...
    def run(self):
        ...
        try:
            ...
            self.print_colored(Colors.YELLOW, "\n[KEY] Select Root AWS Accounts for IAM Cleanup:")
            ...
            if cleanup_users and all_users:
                self.print_colored(Colors.YELLOW, "\n" + "=" * 80)
                self.print_colored(Colors.YELLOW, "[USER] IAM USERS SELECTION")
                self.print_colored(Colors.YELLOW, "=" * 80)
