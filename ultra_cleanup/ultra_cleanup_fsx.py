...
class UltraCleanupFSxManager:
    ...
    def log_action(self,message,level="INFO"):
        with open(self.log_file,'a') as f:f.write(f"{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} | {level:8} | {message}\n")
            ...
            try:
                ...
                if filesystems:
                    ...
                    for fs in filesystems:
                        try:
                            self.print_colored(Colors.CYAN,f"[DELETE] Deleting file system: {fs['FileSystemId']} ({fs['FileSystemType']})")
    ...
    def cleanup_account_fsx(self,account_name,credentials):
        self.print_colored(Colors.BLUE,f"\n{'='*100}\n[START] Processing Account: {account_name}\n{'='*100}")
    ...
    def interactive_cleanup(self):
        self.print_colored(Colors.BLUE,"\n"+"="*100+"\n[START] ULTRA AWS FSX CLEANUP MANAGER\n"+"="*100)
        config=self.cred_manager.load_root_accounts_config()
        if not config or 'accounts' not in config:self.print_colored(Colors.RED,"[ERROR] No accounts found!");return
        accounts=config['accounts'];account_list=list(accounts.keys())
        self.print_colored(Colors.YELLOW,"\n[KEY] Available Accounts:");print("="*100)
        ...
        self.print_colored(Colors.GREEN,f"\n[OK] FSx cleanup completed! Report: {report_path}")
