...
class EnhancedAWSCredentialManager:
    ...
    def get_root_credential_regions(self) -> list:
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"✅ Selected {len(selected_regions)} region(s): {', '.join(selected_regions)}"
                )
    ...
    def select_multiple_accounts(self) -> List[Dict]:
        ...
        while True:
            try:
                selection = input(
                    f"\n🔢 Select account(s) (1-{len(account_list)}): "
                ).strip()
                ...
                for account in selected_accounts:
                    print(
                        f"   • {account['account_name']} ({account['account_data'].get('account_id', 'Unknown')})"
                    )
    ...
    def select_multiple_root_credentials(self) -> MultiUserCredentials:
        ...
        if method_choice == "1":
            ...
            for account in selected_accounts:
                ...
                print(
                    f"✅ Added root credentials for {account_name} in {len(selected_regions)} region(s)"
                )
            ...
            for account in selected_accounts:
                ...
                root_credentials.append(cred_info)
                print(
                    f"✅ Added root credentials for {account_name} in {len(selected_regions)} region(s): {', '.join(selected_regions)}"
                )
    ...
    def select_iam_credential_file(self) -> Optional[str]:
        ...
        try:
            ...
            print("\nSelection Options:")
            print(
                "  • Enter number (1-{}) to select specific file".format(
                    len(sorted_files)
                )
            )
            ...
            while True:
                try:
                    choice = input(
                        f"\n🔢 Select IAM credential file (1-{len(sorted_files)}, Enter for latest): "
                    ).strip()
                    ...
                    if choice == "q":
                        ...
                        print(
                            f"   📅 Modified: {datetime.fromtimestamp(selected_file['timestamp']).strftime('%Y-%m-%d %H:%M:%S')}"
                        )
    ...
    def select_multiple_iam_credentials(self) -> MultiUserCredentials:
        ...
        while True:
            try:
                selection = input(
                    f"\n🔢 Select account(s) (1-{len(iam_accounts)}): "
                ).strip()
        ...
        for account in selected_accounts:
            ...
            while True:
                try:
                    user_selection = input(
                        f"\n🔢 Select user(s) for {account_name} (1-{len(users)}): "
                    ).strip()
            ...
            for user in selected_users:
                ...
                print(
                    f"✅ Added IAM credentials for {user.get('username')} in {account_name}"
                )
    ...
    def validate_credentials(self, cred_info: CredentialInfo) -> bool:
        ...
        try:
            print(
                f"🔍 Validating {cred_info.credential_type} credentials for {cred_info.account_name}..."
            )
            ...
            if actual_account_id == cred_info.account_id:
                print(
                    f"✅ Credentials validated successfully for {cred_info.account_name}"
                )
