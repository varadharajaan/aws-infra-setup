...
class AWSDefaultVPCChecker:
    ...
    def load_configuration(self):
        ...
        try:
            ...
            print(f"{Symbols.STATS} Found {len(self.aws_accounts)} AWS accounts: {list(self.aws_accounts.keys())}")
            print(f"{Symbols.REGION} Will check {len(self.check_regions)} regions: {self.check_regions}")
            
        except FileNotFoundError as e:
            print(f"{Symbols.ERROR} Configuration file error: {e}")
            sys.exit(1)
    ...
    def fix_missing_default_vpcs(self, results):
        ...
        if choice_num == 1:
            ...
            print(f"\n{Symbols.TARGET} Will create {len(missing_vpcs)} missing default VPCs")
        else:
            issues_to_fix = missing_vpcs + compliance_issues
            print(f"\n{Symbols.TARGET} Will fix {len(missing_vpcs)} missing VPCs and {len(compliance_issues)} compliance issues")
        ...
        print(f"\n{Symbols.SCAN} Starting remediation...")
        fixed_count = 0
        failed_count = 0
            ...
            try:
                ...
                if success:
                    ...
                    if self.verify_fixes(account, region):
                        fixed_count += 1
                        print(f"   {Symbols.OK} {account} | {region} - Fixed and verified")
                    else:
                        failed_count += 1
                        print(f"   {Symbols.WARN}  {account} | {region} - Partially fixed")
                else:
                    failed_count += 1
                    print(f"   {Symbols.ERROR} {account} | {region} - Failed")
                    
            except Exception as e:
                failed_count += 1
                print(f"   {Symbols.ERROR} {account} | {region} - Error: {e}")
        
        # Summary of remediation    
        print(f"\n{Symbols.STATS} Remediation Summary:")
        print(f"   {Symbols.OK} Fixed: {fixed_count}")
        print(f"   {Symbols.ERROR} Failed: {failed_count}")
    ...
    def create_default_vpc(self, account_name, region):
        ...
        try:
            ...
            print(f"      [BUILD]  Created default VPC: {vpc_id}")
            return True
            
        except ClientError as e:
            if 'DefaultVpcAlreadyExists' in str(e):
                print(f"      {Symbols.WARN}  Default VPC already exists")
                return True
            else:
                print(f"      {Symbols.ERROR} AWS Error: {e}")
                return False
        except Exception as e:
    ...
    def fix_vpc_compliance(self, account_name, region, issue_result):
        ...
        try:
            ...
            if current_count > rules['max_subnets']:
                print(f"      {Symbols.CLEANUP} Cleaning up excess subnets: {current_count} > {rules['max_subnets']}")
                ...
                if cleanup_success:
                    ...
                    print(f"      {Symbols.ERROR} Cleanup failed: {cleanup_result}")
                    return False
            ...
            if not main_route_table_id:
                print(f"      {Symbols.ERROR} Could not find main route table for VPC {vpc_id}")
                return False
                ...
                if existing_internet_route:
                    ...
                    if igw_created or current_target != igw_id:
                        try:
                            ...
                            print(f"      {Symbols.WARN}  Could not fix internet route: {e}")
                            return False
                    else:
                    ...
                    try:
                        ...
                        print(f"      {Symbols.WARN}  Could not add internet route: {e}")
                        return False
                    ...
                    try:
                        ...
                        if 'InvalidSubnet.Conflict' in str(e):
                            ...
                            for retry_octet in range(32, 240, 16):
                                ...
                                print(f"      {Symbols.WARN}  Could not create subnet in {az}")
                        else:
                            print(f"      {Symbols.WARN}  Could not create subnet in {az}: {e}")
            ...
            for subnet in current_subnets_response['Subnets']:
                ...
                if not subnet.get('MapPublicIpOnLaunch', False):
                    try:
                        ...
                        print(f"      {Symbols.WARN}  Could not enable auto-assign public IP for {subnet_id}: {e}")
            ...
            print(f"      {Symbols.ERROR} Error fixing compliance: {e}")
            return False

    def verify_fixes(self, account_name, region):
        """Verify that fixes were applied correctly"""
        try:
            print(f"      {Symbols.SCAN} Verifying fixes for {account_name} | {region}...")
            
            # Re-run the check to verify
            verification_result = self.check_account_region(account_name, region)
            
            if verification_result['status'] != 'SUCCESS':
                print(f"      {Symbols.ERROR} Verification failed: {verification_result.get('error', 'Unknown error')}")
                return False
            ...
            if is_compliant:
                print(f"      {Symbols.OK} Verification passed: All requirements met")
                return True
            else:
                ...
                print(f"      {Symbols.WARN}  Verification incomplete: {', '.join(issues)}")
                return False
                
        except Exception as e:
            print(f"      {Symbols.ERROR} Verification error: {e}")
            return False

    def display_detailed_results(self, results):
        """Display detailed results for all accounts and regions"""
        print(f"\n{'='*100}")
        print(f"{Symbols.SCAN} DETAILED AWS DEFAULT VPC ANALYSIS REPORT")
        print(f"{'='*100}")
        print(f"{Symbols.DATE} Generated: {self.current_time} UTC")
        print(f"👤 Generated by: {self.current_user}")
        print(f"{Symbols.REGION} Regions checked: {', '.join(self.check_regions)}")
        print(f"{Symbols.ACCOUNT} Accounts checked: {len(self.aws_accounts)}")
        ...
        for account_name, summary in accounts_summary.items():
            ...
            print(f"\n{Symbols.ACCOUNT} ACCOUNT: {account_name.upper()} ({account_id})")
            print(f"{'─'*80}")
            print(f"   {Symbols.STATS} Summary: {summary['compliant_regions']}/{summary['total_regions']} regions fully compliant")
            
            if summary['no_vpc_regions'] > 0:
                print(f"   {Symbols.WARN}  {summary['no_vpc_regions']} regions missing default VPC")
            if summary['failed_regions'] > 0:
                print(f"   {Symbols.ERROR} {summary['failed_regions']} regions with issues")
            ...
            for region, result in summary['regions'].items():
                ...
                if result['status'] == 'SUCCESS':
                    ...
                    if assessment.get('fully_compliant', False):
                        status_icon = f"{Symbols.OK}"
                        status_text = "FULLY COMPLIANT"
                    else:
                        status_icon = f"{Symbols.WARN} "
                        status_text = "HAS ISSUES"
                    
                    print(f"\n   {Symbols.REGION} {region}: {status_icon} {status_text}")
                    ...
                    if vpc_check.get('exists'):
                        ...
                        print(f"      [BUILD]  Default VPC: {vpc_id} ({vpc_cidr})")
                    ...
                    subnet_icon = f"{Symbols.OK}" if meets_min and within_max and valid_dist else f"{Symbols.ERROR}"
                    ...
                    access_icon = f"{Symbols.OK}" if all_have_access else f"{Symbols.ERROR}"
                    ...
                    if not all_have_access:
                        for subnet_id, access_info in subnet_access.items():
                            access_status = "[OK]" if access_info['has_internet_access'] else f"{Symbols.ERROR}"
                            rt_id = access_info['route_table_id']
                            print(f"         • {subnet_id}: {access_status} (Route Table: {rt_id})")
                
                elif result['status'] == 'NO_DEFAULT_VPC':
                    print(f"\n   {Symbols.REGION} {region}: {Symbols.ERROR} NO DEFAULT VPC")
                    print(f"      {Symbols.TIP} Run: aws ec2 create-default-vpc --region {region}")
                
                else:
                    print(f"\n   {Symbols.REGION} {region}: {Symbols.ERROR} ERROR")
        ...
        print(f"\n{'='*100}")
        print(f"[UP] OVERALL SUMMARY")
        print(f"{'='*100}")
        print(f"{Symbols.OK} Fully Compliant: {compliant_regions}/{total_regions} regions ({compliant_regions/total_regions*100:.1f}%)")
        print(f"{Symbols.ERROR} Missing Default VPC: {no_vpc_regions} regions")
        print(f"{Symbols.WARN}  Has Issues: {total_regions - compliant_regions - no_vpc_regions - error_regions} regions")
    ...
    def save_results_to_file(self, results):
        ...
        try:
            ...
            print(f"\n{Symbols.INSTANCE} Detailed report saved to: {filename}")
            return filename
            
        except Exception as e:
            print(f"{Symbols.ERROR} Failed to save report: {e}")
            return None
    ...
    def select_accounts_and_regions(self):
        ...
        print(f"\n{Symbols.LIST} Account Selection:")
        ...
        while True:
            try:
                ...
                if choice_num == 1:
                    ...
                    print(f"\n{Symbols.LIST} Available Accounts:")
                    ...
                    while True:
                        ...
                        if error:
                            print(f"{Symbols.ERROR} {error}. Please try again.")
                            continue
                        
                        if not account_numbers:
                            print(f"{Symbols.ERROR} No valid account numbers entered. Please try again.")
                            continue
                        
                        selected_accounts = [account_names[i-1] for i in account_numbers]
                        print(f"{Symbols.OK} Selected accounts: {', '.join(selected_accounts)}")
                        break
                    break
                else:
        ...
        print(f"\n{Symbols.REGION} Region Selection:")
        ...
        while True:
            try:
                ...
                if choice_num == 1:
                    ...
                    print(f"\n{Symbols.REGION} Available Regions:")
                    ...
                    while True:
                        ...
                        if not region_numbers:
                            print(f"{Symbols.ERROR} No valid region numbers entered. Please try again.")
                            continue
                        
                        selected_regions = [self.check_regions[i-1] for i in region_numbers]
                        print(f"{Symbols.OK} Selected regions: {', '.join(selected_regions)}")
                        break
                    break
                else:
                    print("[ERROR] Invalid choice. Please enter 1 or 2")
    ...
    def run(self):
        ...
        print(f"{Symbols.START} AWS Default VPC Configuration Checker")
        ...
        print(f"\n{Symbols.STATS} Check Summary:")
        print(f"   {Symbols.ACCOUNT} Accounts: {len(selected_accounts)} ({', '.join(selected_accounts)})")
        print(f"   {Symbols.REGION} Regions: {len(selected_regions)} ({', '.join(selected_regions)})")
        print(f"   {Symbols.SCAN} Total checks: {len(selected_accounts) * len(selected_regions)}")
        ...
        confirm = input(f"\n{Symbols.OK} Proceed with the check? (y/N): ").lower().strip()
        if confirm != 'y':
            print(f"{Symbols.ERROR} Check cancelled")
            return
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    # Show progress
                    status = "[OK]" if result.get('checks', {}).get('overall_assessment', {}).get('fully_compliant', False) else f"{Symbols.WARN}"
        ...
        save_report = input(f"\n{Symbols.INSTANCE} Save detailed report to JSON file? (y/N): ").lower().strip()
        if save_report == 'y':
...
def main():
    ...
    try:
        ...
        print(f"\n\n{Symbols.ERROR} Check interrupted by user")
        sys.exit(1)
    except Exception as e:
