...
class AWSDefaultVPCChecker:
    ...
    def load_configuration(self):
        ...
        try:
            ...
            self.check_regions = self.user_settings.get(
                "user_regions",
            ...
            print(
                f"📊 Found {len(self.aws_accounts)} AWS accounts: {list(self.aws_accounts.keys())}"
            )
            print(
                f"🌍 Will check {len(self.check_regions)} regions: {self.check_regions}"
            )
                ...
                for route in rt.get("Routes", []):
                    route_info = {
                        ...
                        "target": route.get(
                            "GatewayId",
                            route.get(
                                "InstanceId", route.get("NetworkInterfaceId", "local")
                            ),
                        ),
                    ...
                    if route.get("DestinationCidrBlock") == "0.0.0.0/0":
                        ...
                        rt_info["internet_route_target"] = route.get(
                            "GatewayId", "unknown"
                        )
    ...
    def validate_subnet_distribution(self, subnet_details, region):
        ...
        if subnet_count < rules["min_subnets"]:
            return (
                False,
                f"Insufficient subnets: {subnet_count} < {rules['min_subnets']} (minimum)",
            )

        if subnet_count > rules["max_subnets"]:
            return (
                True,
                f"Excess subnets: {subnet_count} > {rules['max_subnets']} (maximum)",
            )
        ...
        if len(unique_azs) != len(azs):
            return (
                False,
                f"Subnets not in different AZs: {len(unique_azs)} unique AZs for {len(azs)} subnets",
            )

        return (
            True,
            f"Valid distribution: {subnet_count} subnets in {len(unique_azs)} different AZs",
        )
    ...
    def cleanup_excess_subnets(self, ec2_client, vpc_id, region):
        ...
        try:
            ...
            if len(subnets) <= rules["max_subnets"]:
                return (
                    True,
                    f"No cleanup needed: {len(subnets)} subnets within limit of {rules['max_subnets']}",
                )
            ...
            for subnet in subnets_to_delete:
                ...
                try:
                        ...
                        for association in rt.get("Associations", []):
                            if association.get("SubnetId") == subnet_id:
                                ...
                                cleanup_actions.append(
                                    f"Disassociated {subnet_id} from route table {rt['RouteTableId']}"
                                )
                    ...
                    if "DependencyViolation" in str(e):
                        cleanup_actions.append(
                            f"Cannot delete {subnet_id}: has dependencies (instances/ENIs)"
                        )
                    else:
    ...
    def check_account_region(self, account_name, region):
        ...
        try:
            ...
            subnet_count, subnet_details = self.check_default_subnets(
                ec2_client, vpc_id
            )
            is_valid_distribution, distribution_msg = self.validate_subnet_distribution(
                subnet_details, region
            )
            ...
            subnet_ids = (
                [subnet["subnet_id"] for subnet in subnet_details]
                if isinstance(subnet_details, list)
                else []
            )
            ...
            route_analysis, subnet_internet_access = self.check_route_tables_and_routes(
                ec2_client, vpc_id, subnet_ids
            )
            ...
            result["checks"]["overall_assessment"] = {
                ...
                "fully_compliant": (
                    vpc_exists
                    and subnet_count >= rules["min_subnets"]
                    and subnet_count <= rules["max_subnets"]
                    and is_valid_distribution
                    and igw_exists
                    and all_subnets_have_internet
                    and all_subnets_auto_assign_ip
                ),
            }
    ...
    def fix_vpc_compliance(self, account_name, region, issue_result):
        ...
        try:
            ...
            if current_count > rules["max_subnets"]:
                print(
                    f"      🧹 Cleaning up excess subnets: {current_count} > {rules['max_subnets']}"
                )
                cleanup_success, cleanup_result = self.cleanup_excess_subnets(
                    ec2_client, vpc_id, region
                )
            ...
            updated_count, updated_details = self.check_default_subnets(
                ec2_client, vpc_id
            )
            ...
            if updated_count >= rules["min_subnets"]:
                ...
                is_valid, msg = self.validate_subnet_distribution(
                    updated_details, region
                )
                if is_valid:
                    fixed_items.append(
                        f"Adequate subnet coverage: {updated_count} subnets in different AZs"
                    )
                else:
                ...
                fixed_items.append(
                    f"Need to create {needed_subnets} additional subnets"
                )
            ...
            if igw_check.get("exists", False):
                ...
                igw_created = True
                fixed_items.append(
                    f"Created and attached NEW Internet Gateway: {igw_id}"
                )
                ...
                if existing_internet_route:
                    ...
                    if igw_created or current_target != igw_id:
                        try:
                            ...
                            fixed_items.append(
                                f"Updated internet route: {rt_id} -> {igw_id}"
                            )
                        ...
                        fixed_items.append(
                            f"Internet route already correct: {rt_id} -> {igw_id}"
                        )
                else:
                    try:
                        ec2_client.create_route(
            ...
            if updated_count < rules["min_subnets"]:
                ...
                if isinstance(updated_details, list):
                    used_azs = {
                        subnet["availability_zone"] for subnet in updated_details
                    }
                    ...
                    try:
                        ...
                        try:
                            association_response = ec2_client.associate_route_table(
                                RouteTableId=main_route_table_id, SubnetId=subnet_id
                            )
                            association_response["AssociationId"]
                            fixed_items.append(
                                f"Created subnet: {subnet_id} ({subnet_cidr}) in {az}"
                            )
                        except ClientError as assoc_error:
                            fixed_items.append(
                                f"Created subnet: {subnet_id} ({subnet_cidr}) in {az} (implicit association)"
                            )
                        ...
                        if "InvalidSubnet.Conflict" in str(e):
                            ...
                            for retry_octet in range(32, 240, 16):
                                try:
                                    ...
                                    fixed_items.append(
                                        f"Created subnet: {subnet_id} ({retry_cidr}) in {az}"
                                    )
                                    break
                                except ClientError:
                                    continue
            ...
            for subnet in current_subnets_response["Subnets"]:
                ...
                if not subnet.get("MapPublicIpOnLaunch", False):
                    try:
                        ...
                        fixed_items.append(
                            f"Enabled auto-assign public IP: {subnet_id}"
                        )
                    except ClientError as e:
                        print(
                            f"      ⚠️  Could not enable auto-assign public IP for {subnet_id}: {e}"
                        )
    ...
    def display_detailed_results(self, results):
        ...
        for account_name, summary in accounts_summary.items():
            ...
            print(f"{'─'*80}")
            print(
                f"   📊 Summary: {summary['compliant_regions']}/{summary['total_regions']} regions fully compliant"
            )
            ...
            for region, result in summary["regions"].items():
                ...
                if result["status"] == "SUCCESS":
                    ...
                    print(
                        f"      📏 Regional rules: {rules['min_subnets']}-{rules['max_subnets']} subnets"
                    )
                    ...
                    subnet_icon = (
                        "✅" if meets_min and within_max and valid_dist else "❌"
                    )
                    print(
                        f"      🌐 Subnets: {subnet_icon} {subnet_count} subnets ({rules['min_subnets']}-{rules['max_subnets']} required)"
                    )

                    if not valid_dist:
                        dist_msg = subnet_check.get(
                            "distribution_message", "Invalid distribution"
                        )
                        print(f"         ⚠️  {dist_msg}")
                    ...
                    if isinstance(subnet_check.get("details"), list):
                        for subnet in subnet_check["details"]:
                            ...
                            print(
                                f"         • {subnet['subnet_id']} ({subnet['availability_zone']}) - Auto IP: {auto_ip_icon}"
                            )
                    ...
                    all_have_access = assessment.get(
                        "all_subnets_have_internet_access", False
                    )
                    access_icon = "✅" if all_have_access else "❌"
                    print(
                        f"      🔗 Internet Access: {access_icon} All subnets routed to internet"
                    )

                    if not all_have_access:
                        for subnet_id, access_info in subnet_access.items():
                            access_status = (
                                "✅" if access_info["has_internet_access"] else "❌"
                            )
                            rt_id = access_info["route_table_id"]
                            print(
                                f"         • {subnet_id}: {access_status} (Route Table: {rt_id})"
                            )
        ...
        print(f"{'='*100}")
        print(
            f"✅ Fully Compliant: {compliant_regions}/{total_regions} regions ({compliant_regions/total_regions*100:.1f}%)"
        )
        print(f"❌ Missing Default VPC: {no_vpc_regions} regions")
        print(
            f"⚠️  Has Issues: {total_regions - compliant_regions - no_vpc_regions - error_regions} regions"
        )
        ...
        while True:
            try:
                ...
                if choice_num == 1:
                    ...
                    while True:
                        account_input = input(
                            f"\n🔢 Enter account numbers (e.g., 1,3,5 or 1-3 or 2): "
                        ).strip()
                        account_numbers, error = self.parse_number_input(
                            account_input, len(account_names)
                        )
                        ...
                        if not account_numbers:
                            print(
                                f"❌ No valid account numbers entered. Please try again."
                            )
                            continue

                        selected_accounts = [
                            account_names[i - 1] for i in account_numbers
                        ]
        ...
        while True:
            try:
                ...
                if choice_num == 1:
                    ...
                    while True:
                        region_input = input(
                            f"\n🔢 Enter region numbers (e.g., 1,3,5 or 1-3 or 2): "
                        ).strip()
                        region_numbers, error = self.parse_number_input(
                            region_input, len(self.check_regions)
                        )
                        ...
                        if not region_numbers:
                            print(
                                f"❌ No valid region numbers entered. Please try again."
                            )
                            continue

                        selected_regions = [
                            self.check_regions[i - 1] for i in region_numbers
                        ]
    ...
    def run(self):
        ...
        print(f"\n📊 Check Summary:")
        print(
            f"   🏦 Accounts: {len(selected_accounts)} ({', '.join(selected_accounts)})"
        )
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            # Submit all tasks
            future_to_task = {
                executor.submit(self.check_account_region, account, region): (
                    account,
                    region,
                )
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    print(
                        f"   {status} {account} | {region} ({completed}/{len(tasks)})"
                    )
