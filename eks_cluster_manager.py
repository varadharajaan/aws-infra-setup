...
class EKSClusterManager:
    ...
    def setup_logging(self):
        ...
        self.logger.info(
            f"EKS Cluster Manager initialized - Session ID: {self.execution_timestamp}"
        )
    ...
    def render_fluentbit_configmap(
        self,
        cluster_name,
        region_name,
        http_server_toggle,
    ...
    def get_cloudwatch_daemonset_manifest_fixed(
        self, cluster_name: str, region: str, account_id: str
    ) -> str:
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to process DaemonSet manifest: {str(e)}"
            )
    ...
    def get_application_signals_operator_manifest(
        self, cluster_name: str, region: str, account_id: str
    ) -> str:
        ...
        try:
            ...
            self.log_operation(
                "ERROR",
                f"Failed to load Application Signals operator manifest: {str(e)}",
            )
            return ""

    def get_adot_collector_manifest(
        self, cluster_name: str, region: str, account_id: str
    ) -> str:
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to load ADOT Collector manifest: {str(e)}"
            )
            return ""
    ...
    def get_auto_instrumentation_manifest(self, cluster_name: str, region: str) -> str:
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to load auto-instrumentation manifest: {str(e)}"
            )
            return ""
    ...
    def enable_application_signals(
        ...
        try:
            self.log_operation(
                "INFO",
                f"Enabling CloudWatch Application Signals for cluster {cluster_name}",
            )
            self.print_colored(
                Colors.YELLOW,
                f"📊 Enabling CloudWatch Application Signals for {cluster_name}...",
            )
            ...
            if not kubectl_available:
                self.log_operation(
                    "WARNING",
                    f"kubectl not found. Cannot deploy Application Signals for {cluster_name}",
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"⚠️  kubectl not found. Application Signals deployment skipped.",
                )
                return False
            ...
            self.print_colored(
                Colors.CYAN, "   🔄 Updating kubeconfig for Application Signals..."
            )
            update_result = subprocess.run(
                update_cmd, env=env, capture_output=True, text=True, timeout=120
            )

            if update_result.returncode != 0:
                self.log_operation(
                    "ERROR", f"Failed to update kubeconfig: {update_result.stderr}"
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to update kubeconfig: {update_result.stderr}",
                )
                return False

            # Step 1: Create IAM role first
            self.print_colored(
                Colors.CYAN, "   🔐 Setting up IAM role for Application Signals..."
            )
            ...
            if not role_arn:
                self.print_colored(
                    Colors.YELLOW,
                    "   ⚠️  IAM role creation failed, continuing with deployment...",
                )

            # Step 2: Enable Application Signals service
            self.print_colored(
                Colors.CYAN, "   📡 Enabling Application Signals service..."
            )
            ...
            try:
                ...
                enable_result = subprocess.run(
                    enable_cmd, env=env, capture_output=True, text=True, timeout=120
                )

                if enable_result.returncode == 0:
                    self.print_colored(
                        Colors.GREEN, "   ✅ Application Signals service enabled"
                    )
                    self.log_operation(
                        "INFO",
                        f"Application Signals service enabled for {cluster_name}",
                    )
                else:
                    self.log_operation(
                        "WARNING",
                        f"Application Signals service enablement failed: {enable_result.stderr}",
                    )
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  Application Signals service enablement failed, continuing...",
                    )
            except Exception as e:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️  Could not enable Application Signals service: {str(e)}",
                )

            # Step 3: Deploy Application Signals operator
            self.print_colored(
                Colors.CYAN, "   🚀 Deploying Application Signals operator..."
            )

            operator_manifest = self.get_application_signals_operator_manifest(
            ...
            if self.apply_kubernetes_manifest_fixed(
                cluster_name,
                region,
                admin_access_key,
                admin_secret_key,
                ...
                self.print_colored(
                    Colors.GREEN, "   ✅ Application Signals operator deployed"
                )
                self.log_operation(
                    "INFO", f"Application Signals operator deployed for {cluster_name}"
                )
            else:
                self.print_colored(
                    Colors.YELLOW,
                    "   ⚠️  Application Signals operator deployment failed",
                )
                return False
            ...
            self.print_colored(
                Colors.CYAN, "   📊 Deploying ADOT Collector for Application Signals..."
            )

            adot_manifest = self.get_adot_collector_manifest(
                cluster_name, region, account_id
            )
            if not adot_manifest:
                self.print_colored(
                    Colors.RED, "   ❌ Failed to load ADOT Collector manifest"
                )
                return False

            if self.apply_kubernetes_manifest_fixed(
                cluster_name, region, admin_access_key, admin_secret_key, adot_manifest
            ):
                self.print_colored(Colors.GREEN, "   ✅ ADOT Collector deployed")
                self.log_operation(
                    "INFO", f"ADOT Collector deployed for {cluster_name}"
                )
            else:
                self.print_colored(
                    Colors.YELLOW, "   ⚠️  ADOT Collector deployment failed"
                )
                return False
            ...
            self.print_colored(
                Colors.CYAN,
                "   🔍 Deploying auto-instrumentation for all supported languages...",
            )

            auto_instrumentation_manifest = self.get_auto_instrumentation_manifest(
                cluster_name, region
            )
            if not auto_instrumentation_manifest:
                self.print_colored(
                    Colors.RED, "   ❌ Failed to load auto-instrumentation manifest"
                )
                return False
            ...
            if self.apply_kubernetes_manifest_fixed(
                ...
                admin_access_key,
                admin_secret_key,
                auto_instrumentation_manifest,
            ):
                self.print_colored(Colors.GREEN, "   ✅ Auto-instrumentation deployed")
                self.log_operation(
                    "INFO", f"Auto-instrumentation deployed for {cluster_name}"
                )
            else:
                self.print_colored(
                    Colors.YELLOW, "   ⚠️  Auto-instrumentation deployment failed"
                )
                return False

            # Step 6: Verify deployment
            self.print_colored(
                Colors.CYAN, "   ⏳ Verifying Application Signals deployment..."
            )
            time.sleep(30)
            ...
            verify_cmd = [
                ...
                "pods",
                "-n",
                "aws-application-signals-system",
                "--no-headers",
            ]
            verify_result = subprocess.run(
                verify_cmd, env=env, capture_output=True, text=True, timeout=60
            )
            ...
            if verify_result.returncode == 0:
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Application Signals pods: {len(running_pods)} ready out of {len(pod_lines)} total",
                )
                self.log_operation(
                    "INFO",
                    f"Application Signals deployment verified: {len(running_pods)} pods ready",
                )

                # Access information
                self.print_colored(
                    Colors.CYAN, f"📊 Access Application Signals in AWS Console:"
                )
                self.print_colored(
                    Colors.CYAN, f"   CloudWatch → Application Signals → Services"
                )
                self.print_colored(Colors.CYAN, f"   Filter by cluster: {cluster_name}")
                self.print_colored(
                    Colors.CYAN,
                    f"   Supported languages: Java, Python, .NET, Node.js, Go",
                )
                self.print_colored(Colors.CYAN, f"")
                self.print_colored(
                    Colors.CYAN,
                    f"📋 To auto-instrument your applications, add this annotation:",
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   instrumentation.opentelemetry.io/inject-java: 'aws-application-signals-system/application-signals-instrumentation'",
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   (Replace 'java' with: python, nodejs, dotnet, or go as needed)",
                )

                return True
            else:
                self.log_operation(
                    "WARNING", f"Could not verify Application Signals deployment"
                )
                return (
                    True  # Still consider successful since deployment commands worked
                )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to enable Application Signals for {cluster_name}: {error_msg}",
            )
            self.print_colored(
                Colors.RED, f"❌ Application Signals deployment failed: {error_msg}"
            )
            return False
        ...
        try:
            ...
            trust_policy = {
                ...
                "Statement": [
                    {
                        "Effect": "Allow",
                        "Principal": {"Service": "application-signals.amazonaws.com"},
            ...
            try:
                ...
                self.log_operation(
                    "INFO",
                    f"Created Application Signals IAM role: {role_response['Role']['Arn']}",
                )
                ...
                self.log_operation(
                    "INFO",
                    f"Using existing Application Signals IAM role: {role_response['Role']['Arn']}",
                )
                return role_response["Role"]["Arn"]

        except Exception as e:
            self.log_operation(
                "ERROR", f"Failed to create Application Signals IAM role: {str(e)}"
            )
            return None

    #####
    def ensure_addon_service_roles(
        self, eks_client, cluster_name: str, account_id: str
    ) -> None:
        """
        ...
        for addon_name in addon_names:
            try:
                ...
                if current_role and not current_role == None:
                    self.log_operation(
                        "INFO",
                        f"{addon_name}: serviceAccountRoleArn already set ({current_role}), skipping.",
                    )
                    continue

                self.log_operation(
                    "INFO",
                    f"{addon_name}: serviceAccountRoleArn not set, applying NodeInstanceRole.",
                )
                ...
                self.print_colored(
                    Colors.GREEN, f"✅ Applied NodeInstanceRole to {addon_name}"
                )
            except eks_client.exceptions.ResourceNotFoundException:
                self.log_operation(
                    "WARNING",
                    f"{addon_name} not found on cluster {cluster_name}, skipping.",
                )
            except Exception as e:
                self.log_operation("ERROR", f"Failed to update {addon_name}: {str(e)}")

                self.print_colored(
                    Colors.RED, f"❌ Failed to update {addon_name}: {str(e)}"
                )
    ...
    def format_instance_types_summary(self, instance_selections: Dict) -> str:
        ...
        if "on-demand" in instance_selections and instance_selections["on-demand"]:
            summary_parts.append(
                f"OnDemand: {', '.join(instance_selections['on-demand'])}"
            )
        ...
        if "on_demand_percentage" in instance_selections:
            summary_parts.append(
                f"({instance_selections['on_demand_percentage']}% OnDemand)"
            )

        return " | ".join(summary_parts) if summary_parts else "None"

    def select_subnets_for_nodegroup(
        self, all_subnet_ids: List[str], preference: str, ec2_client
    ) -> List[str]:
    ...
    ###
    def _setup_autoscaler_iam_permissions(
        self, admin_session, cluster_name: str, account_id: str
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.log_operation(
                    "INFO", f"Created Enhanced Cluster Autoscaler policy: {policy_arn}"
                )
                ...
                self.log_operation(
                    "INFO",
                    f"Using existing Enhanced Cluster Autoscaler policy: {policy_arn}",
                )
            ...
            for role_name in node_role_names:
                try:
                    ...
                    self.print_colored(
                        Colors.GREEN, f"   ✅ IAM policy attached to role: {role_name}"
                    )
                    ...
                    self.log_operation(
                        "DEBUG",
                        f"Could not attach policy to role {role_name}: {str(e)}",
                    )

            if not attached_to_role:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ Could not attach IAM policy to any node role"
                )
    ...
    def debug_cluster_autoscaler(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> dict:
    ...
    def _print_usage_instructions_fixed(self, cluster_name: str, region: str):
        ...
        self.print_colored(
            Colors.BLUE, "    🎉 CLUSTER AUTOSCALER DEPLOYMENT SUCCESSFUL!"
        )
        ...
        self.print_colored(
            Colors.GREEN,
            f"   • Deployment Time: {time.strftime('%Y-%m-%d %H:%M:%S UTC', time.gmtime())}",
        )
        ...
        self.print_colored(
            Colors.WHITE,
            f"     kubectl logs -n kube-system -l app=cluster-autoscaler -f",
        )

        self.print_colored(Colors.CYAN, f"   • Check autoscaler pod status:")
        self.print_colored(
            Colors.WHITE,
            f"     kubectl get pods -n kube-system -l app=cluster-autoscaler",
        )
        ...
        self.print_colored(
            Colors.YELLOW, f"   • Scale up test (trigger node addition):"
        )
        self.print_colored(
            Colors.WHITE, f"     kubectl scale deployment stress-test --replicas=8"
        )

        self.print_colored(
            Colors.YELLOW, f"   • Scale down test (trigger node removal):"
        )
        self.print_colored(
            Colors.WHITE, f"     kubectl scale deployment stress-test --replicas=1"
        )
        ...
        self.print_colored(
            Colors.GREEN,
            f"   • Scale Up: New nodes added when pods are pending (2-5 minutes)",
        )
        self.print_colored(
            Colors.GREEN, f"   • Scale Down: Unused nodes removed after 10+ minutes"
        )
        self.print_colored(
            Colors.GREEN, f"   • Auto-discovery: ASGs tagged for autoscaler discovery"
        )
    ...
    def should_deploy_cloudwatch_agent(self):
        ...
        if self.check_container_insights_enabled():
            self.log_operation(
                "INFO",
                "Container Insights already enabled - skipping custom CloudWatch agent deployment",
            )
            return False
        return True

    def ensure_ec2_key_pair(
        self, ec2_client, key_name: str = "k8s_demo_key", key_dir: str = "."
    ) -> str:
        """
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"EC2 key pair '{key_name}' already exists in AWS."
            )
            return key_name
            ...
            if "InvalidKeyPair.NotFound" in str(e):
                self.log_operation(
                    "INFO",
                    f"Key pair '{key_name}' not found in AWS, importing from local file...",
                )
                ...
                try:
                    ...
                    if not os.path.exists(public_key_path):
                        self.log_operation(
                            "ERROR", f"Public key file not found: {public_key_path}"
                        )
                    ...
                    if not public_key_material.startswith(
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"✅ Imported public key as EC2 key pair '{key_name}'",
                    )
                    return key_name
                    ...
                    self.print_colored(
                        Colors.RED, f"❌ Public key file not found: {public_key_path}"
                    )
                    raise
                except ValueError as ve:
                    self.log_operation("ERROR", f"Invalid public key format: {str(ve)}")
                    self.print_colored(
                        Colors.RED, f"❌ Invalid public key format in {public_key_path}"
                    )
                    raise
                except Exception as upload_error:
                    self.log_operation(
                        "ERROR", f"Error importing public key: {str(upload_error)}"
                    )
                    self.print_colored(
                        Colors.RED,
                        f"❌ Error importing public key: {str(upload_error)}",
                    )
                    raise
            else:
    ...
    def setup_cloudwatch_alarms_multi_nodegroup(
        self,
        cluster_name: str,
        region: str,
        cloudwatch_client,
        ...
        for nodegroup_name in nodegroup_names:
            success = self.setup_cloudwatch_alarms(
                cluster_name, region, cloudwatch_client, nodegroup_name, account_id
            )
            if not success:
                all_success = False
    ...
    def save_cluster_details_enhanced(
        self,
        credential_info,
        cluster_name,
        region,
    ...
    def generate_mini_instructions(
        self, credential_info, cluster_name, region, username
    ):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"Mini instruction file generated: {instruction_mini_file}"
            )
            self.print_colored(
                Colors.GREEN,
            ...
            self.log_operation(
                "ERROR", f"Failed to generate mini instruction file: {str(e)}"
            )
            self.print_colored(
                Colors.RED, f"❌ Failed to generate mini instruction file: {str(e)}"
            )
            return None

    def generate_user_instructions_enhanced(
        self, credential_info, cluster_name, region, username, nodegroup_configs
    ):
        ...
        try:
            ...
            with open(instruction_file, "w") as f:
                ...
                for config in nodegroup_configs:
                    f.write(
                        f"# Scale {config['name']} (current: min={config['min_nodes']}, desired={config['desired_nodes']}, max={config['max_nodes']})\n"
                    )
                    f.write(f"# Scale up to 2 nodes:\n")
                    f.write(
                        f"aws eks update-nodegroup-config --cluster-name {cluster_name} --nodegroup-name {config['name']} --scaling-config minSize=1,maxSize={config['max_nodes']},desiredSize=2 --region {region} --profile {username}\n\n"
                    )
                    f.write(f"# Scale down to 1 node:\n")
                    f.write(
                        f"aws eks update-nodegroup-config --cluster-name {cluster_name} --nodegroup-name {config['name']} --scaling-config minSize=1,maxSize={config['max_nodes']},desiredSize=1 --region {region} --profile {username}\n\n"
                    )
                    f.write(f"# Scale to zero (for cost savings):\n")
                    f.write(
                        f"aws eks update-nodegroup-config --cluster-name {cluster_name} --nodegroup-name {config['name']} --scaling-config minSize=0,maxSize={config['max_nodes']},desiredSize=0 --region {region} --profile {username}\n\n"
                    )

                f.write("### Create New Nodegroup (Default: min=1, desired=1, max=3)\n")
                f.write(
                    "# Replace <NEW_NODEGROUP_NAME> with your desired nodegroup name\n"
                )
                f.write(
                    "# Replace <NODE_ROLE_ARN> with your EKS node instance role ARN\n"
                )
                ...
                f.write(
                    "*************** EXAMPLE COMMANDS NOT TO RUN DIRECTLY ***************\n"
                )
                f.write(
                    " -----------------------------------------------------------------------------\n"
                )
                ...
                if nodegroup_configs:
                    f.write(
                        "# Example using similar configuration as existing nodegroups:\n"
                    )

                f.write(
                    f"""aws eks create-nodegroup \\
    ...
    """
                ...
                f.write(
                    "# Generic template - replace <NODEGROUP_NAME> with actual nodegroup name\n\n"
                )

                # Scaling templates
                f.write("# Scale any nodegroup by name:\n")
                f.write(
                    f"aws eks update-nodegroup-config --cluster-name {cluster_name} --nodegroup-name <NODEGROUP_NAME> --scaling-config minSize=1,maxSize=5,desiredSize=2 --region {region} --profile {username}\n\n"
                )

                f.write("# Update instance types (requires new nodegroup):\n")
                f.write(
                    f"# Note: Cannot change instance types of existing nodegroup. Create new one and migrate workloads.\n\n"
                )

                f.write("# Update AMI version:\n")
                f.write(
                    f"aws eks update-nodegroup-version --cluster-name {cluster_name} --nodegroup-name <NODEGROUP_NAME> --region {region} --profile {username}\n\n"
                )
                ...
                f.write(
                    f"aws eks describe-nodegroup --cluster-name {cluster_name} --nodegroup-name <NODEGROUP_NAME> --region {region} --profile {username}\n\n"
                )

                f.write(f"# Get nodegroup scaling configuration:\n")
                f.write(
                    f"aws eks describe-nodegroup --cluster-name {cluster_name} --nodegroup-name <NODEGROUP_NAME> --region {region} --profile {username} --query 'nodegroup.scalingConfig'\n\n"
                )

                f.write(f"# Get nodegroup status:\n")
                f.write(
                    f"aws eks describe-nodegroup --cluster-name {cluster_name} --nodegroup-name <NODEGROUP_NAME> --region {region} --profile {username} --query 'nodegroup.status' --output text\n\n"
                )

                f.write("### Delete Nodegroup\n")
                f.write("# WARNING: This will terminate all nodes in the nodegroup\n")
                f.write(
                    f"aws eks delete-nodegroup --cluster-name {cluster_name} --nodegroup-name <NODEGROUP_NAME> --region {region} --profile {username}\n\n"
                )
                ...
                f.write(
                    f"""for ng in $(aws eks list-nodegroups --cluster-name {cluster_name} --region {region} --profile {username} --query 'nodegroups[]' --output text); do
    ...
    """
                ...
                f.write(
                    f"""watch -n 30 "aws eks list-nodegroups --cluster-name {cluster_name} --region {region} --profile {username} --output table && echo && kubectl get nodes"
    ...
    """
                ...
                f.write(
                    f"""aws eks list-nodegroups --cluster-name {cluster_name} --region {region} --profile {username} --query 'nodegroups[]' --output text | while read ng; do
        echo "=== $ng ==="
    ...
    """
                ...
                f.write(
                    "# 2. Ensure your user has been granted access to the cluster\n"
                )
                ...
                f.write(
                    "# 2. Verify subnet IDs are correct and have available capacity\n"
                )
                f.write(
                    "# 3. Ensure instance type is available in the selected subnets\n"
                )
                ...
                f.write(
                    "# 3. Monitor EC2 Auto Scaling Groups for underlying changes\n\n"
                )
                ...
                f.write(
                    "- EKS Nodegroup Management: https://docs.aws.amazon.com/eks/latest/userguide/managed-node-groups.html\n"
                )
                f.write(
            ...
            self.log_operation(
                "INFO", f"Enhanced user instructions saved to: {instruction_file}"
            )
            self.print_colored(
                Colors.GREEN,
                f"📄 Enhanced user instructions saved to: {instruction_file}",
            )
    ...
    def print_enhanced_cluster_summary_multi_nodegroup(
        self, cluster_name: str, cluster_info: dict
    ):
        ...
        self.print_colored(
            Colors.GREEN, f"🎉 Enhanced Cluster Summary for {cluster_name}:"
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ EKS Version: {cluster_info.get('eks_version', 'Unknown')}",
        )
        self.print_colored(
            Colors.GREEN, f"   ✅ AMI Type: {cluster_info.get('ami_type', 'Unknown')}"
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ Total Nodegroups: {len(nodegroups_created)}/{len(nodegroup_configs)}",
        )
        ...
        for config in nodegroup_configs:
            if config["strategy"] == "mixed":
                ...
                both_created = (
                    ondemand_name in nodegroups_created
                    and spot_name in nodegroups_created
                )
                status = (
                    "✅"
                    if both_created
                        ...
                        if (
                            ondemand_name in nodegroups_created
                            or spot_name in nodegroups_created
                        )
                        else "❌"
                    )
                )
            ...
            instance_summary = self.format_instance_types_summary(
                config["instance_selections"]
            )
            self.print_colored(
                (
                    Colors.GREEN
                    if status == "✅"
                    else Colors.YELLOW if status == "⚠️" else Colors.RED
                ),
        ...
        self.print_colored(
            Colors.GREEN,
            f"   ✅ Essential Add-ons: {'Installed' if cluster_info.get('addons_installed') else 'Failed'}",
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ Container Insights: {'Enabled' if cluster_info.get('container_insights_enabled') else 'Failed'}",
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ Cluster Autoscaler: {'Enabled' if cluster_info.get('autoscaler_enabled') else 'Failed'}",
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ Scheduled Scaling: {'Enabled' if cluster_info.get('scheduled_scaling_enabled') else 'Failed'}",
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ CloudWatch Agent: {'Deployed' if cluster_info.get('cloudwatch_agent_enabled') else 'Failed'}",
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ CloudWatch Alarms: {'Configured' if cluster_info.get('cloudwatch_alarms_enabled') else 'Failed'}",
        )
        self.print_colored(
            Colors.GREEN,
            f"   ✅ Cost Monitoring: {'Enabled' if cluster_info.get('cost_alarms_enabled') else 'Failed'}",
        )
        ...
        if health_status:
            ...
            self.print_colored(
                Colors.GREEN, f"   ✅ Health Check: HEALTHY (Score: {health_score}/100)"
            )
        else:
            ...
            self.print_colored(
                Colors.YELLOW,
                f"   ⚠️  Health Check: NEEDS ATTENTION ({issues} issues, {warnings} warnings)",
            )
        ...
        if auth_status and access_verified:
            self.print_colored(
                Colors.GREEN, f"   ✅ User Access: Configured & Verified"
            )
        elif auth_status:
            self.print_colored(
                Colors.YELLOW, f"   ⚠️  User Access: Configured (verification pending)"
            )
        else:
    ...
    def create_eks_control_plane(
        ...
        try:
            ...
            if not any(eks_version.startswith(v) for v in supported_versions):
                print(
                    f"⚠️ Warning: EKS version {eks_version} may not be fully supported"
                )
                proceed = (
                    input(f"Proceed with version {eks_version}? (y/N): ")
                    .strip()
                    .lower()
                )
            ...
            waiter.wait(
                name=cluster_name, WaiterConfig={"Delay": 30, "MaxAttempts": 40}
            )
    ...
    def short_cluster_name(self, cluster_name: str) -> str:
        ...
        region = (
            parts[4].replace("us", "u").replace("west", "w").replace("east", "e")
            if len(parts) > 4
            else ""
        )
    ...
    def create_mixed_nodegroup(
        ...
        try:
            ...
            if on_demand_percentage is None:
                self.log_operation(
                    "ERROR",
                    f"No on_demand_percentage specified for mixed nodegroup {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create mixed nodegroup: on_demand_percentage not specified",
                )
                return False
            ...
            if not on_demand_types:
                self.log_operation(
                    "ERROR",
                    f"No on-demand instance types provided for mixed nodegroup {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create mixed nodegroup: on-demand instance types not provided",
                )
                return False
            ...
            if not spot_types:
                self.log_operation(
                    "ERROR",
                    f"No spot instance types provided for mixed nodegroup {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create mixed nodegroup: spot instance types not provided",
                )
                return False
            ...
            if not on_demand_types:
                self.log_operation(
                    "ERROR",
                    f"Empty on-demand instance types list after processing for {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create mixed nodegroup: Empty on-demand instance types",
                )
                return False
            ...
            if not spot_types:
                self.log_operation(
                    "ERROR",
                    f"Empty spot instance types list after processing for {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create mixed nodegroup: Empty spot instance types",
                )
                return False

            self.print_colored(
                Colors.CYAN,
                f"Creating mixed strategy with {on_demand_percentage}% On-Demand, {100-on_demand_percentage}% Spot",
            )
            ...
            if on_demand_percentage > 0:
                ondemand_desired = max(
                    1, int(total_desired * on_demand_percentage / 100)
                )
            ...
            self.print_colored(
                Colors.CYAN,
                f"   On-Demand: Min={ondemand_min}, Desired={ondemand_desired}, Max={ondemand_max}",
            )
            self.print_colored(
                Colors.CYAN,
                f"   Spot: Min={spot_min}, Desired={spot_desired}, Max={spot_max}",
            )
            ...
            if on_demand_types and ondemand_max > 0:
                ...
                self.print_colored(
                    Colors.CYAN, f"   Instance Types: {', '.join(on_demand_types)}"
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   Scaling: Min={ondemand_min}, Desired={ondemand_desired}, Max={ondemand_max}",
                )

                # Generate comprehensive tags for On-Demand instances
                ondemand_instance_tags = self.generate_instance_tags(
                    cluster_name, ondemand_ng_name, "Mixed-OnDemand"
                )
                ondemand_instance_tags["ParentNodegroup"] = nodegroup_name
                ondemand_instance_tags["OnDemandPercentage"] = str(on_demand_percentage)
                if (
                    ondemand_instance_tags["Name"] == None
                    or ondemand_instance_tags["Name"] == ""
                ):
                ...
                try:
                    eks_client.create_nodegroup(
                        ...
                        remoteAccess={"ec2SshKey": ec2_key_name},
                        tags=ondemand_instance_tags,
                        labels={
                            "nodegroup-name": ondemand_ng_name,
                            "instance-type": self.sanitize_label_value(
                                "-".join(on_demand_types)
                            )[:63],
                    ...
                    self.print_colored(
                        Colors.CYAN,
                        f"⏳ Waiting for On-Demand nodegroup {ondemand_ng_name} to be active...",
                    )
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"✅ On-Demand nodegroup {ondemand_ng_name} is now active",
                    )
                    success_count += 1
                    ...
                    self.log_operation(
                        "ERROR", f"Failed to create On-Demand nodegroup: {str(e)}"
                    )
                    self.print_colored(
                        Colors.RED, f"❌ Failed to create On-Demand nodegroup: {str(e)}"
                    )
            ...
            if spot_types and spot_max > 0:
                ...
                self.print_colored(
                    Colors.CYAN, f"   Instance Types: {', '.join(spot_types)}"
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   Scaling: Min={spot_min}, Desired={spot_desired}, Max={spot_max}",
                )

                # Generate comprehensive tags for Spot instances
                spot_instance_tags = self.generate_instance_tags(
                    cluster_name, spot_ng_name, "Mixed-Spot"
                )
                ...
                try:
                    ...
                    self.print_colored(
                        Colors.CYAN,
                        f"⏳ Waiting for Spot nodegroup {spot_ng_name} to be active...",
                    )
                    waiter = eks_client.get_waiter("nodegroup_active")
                    waiter.wait(
                        clusterName=cluster_name,
                    ...
                    self.print_colored(
                        Colors.GREEN, f"✅ Spot nodegroup {spot_ng_name} is now active"
                    )
                    success_count += 1
                    ...
                    self.log_operation(
                        "ERROR", f"Failed to create Spot nodegroup: {str(e)}"
                    )
                    self.print_colored(
                        Colors.RED, f"❌ Failed to create Spot nodegroup: {str(e)}"
                    )
            ...
            if success_count > 0:
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Created {success_count} nodegroups: {', '.join(created_nodegroups)}",
                )
                self.print_colored(
                    Colors.GREEN,
                    f"   📊 Distribution: {on_demand_percentage}% On-Demand, {100-on_demand_percentage}% Spot",
                )
                return True
            else:
                self.print_colored(
    ...
    def create_spot_nodegroup(
        ...
        try:
            ...
            if not instance_types:
                self.log_operation(
                    "ERROR",
                    f"Empty instance types list after processing for spot nodegroup {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create spot nodegroup: Empty instance types list",
                )
                return False

            self.print_colored(Colors.CYAN, f"Creating spot nodegroup {nodegroup_name}")
            self.print_colored(
                Colors.CYAN, f"Instance types: {' '.join(instance_types)}"
            )
            self.print_colored(Colors.CYAN, f"AMI type: {ami_type}")
            self.print_colored(
                Colors.CYAN,
                f"Scaling: Min={min_size}, Desired={desired_size}, Max={max_size}",
            )

            # Generate comprehensive tags for instances
            instance_tags = self.generate_instance_tags(
                cluster_name, nodegroup_name, "Spot"
            )
            ...
            self.print_colored(
                Colors.CYAN,
                f"⏳ Waiting for nodegroup {nodegroup_name} to be active...",
            )
            waiter = eks_client.get_waiter("nodegroup_active")
            waiter.wait(
                clusterName=cluster_name,
            ...
            self.print_colored(
                Colors.GREEN, f"✅ Nodegroup {nodegroup_name} is now active"
            )
            return True
    ...
    def create_ondemand_nodegroup(
        ...
        try:
            ...
            if not instance_types:
                self.log_operation(
                    "ERROR",
                    f"No instance types provided for on-demand nodegroup {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create nodegroup: No instance types provided",
                )
                return False
            ...
            if not instance_types:
                self.log_operation(
                    "ERROR",
                    f"Empty instance types list after processing for nodegroup {nodegroup_name}",
                )
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create nodegroup: Empty instance types list",
                )
                return False

            self.print_colored(
                Colors.CYAN, f"Creating on-demand nodegroup {nodegroup_name}"
            )
            self.print_colored(
                Colors.CYAN, f"Instance types: {', '.join(instance_types)}"
            )
            ...
            # Generate comprehensive tags for instances
            instance_tags = self.generate_instance_tags(
                cluster_name, nodegroup_name, "On-Demand"
            )
            ...
            self.print_colored(
                Colors.RED, f"❌ Error creating on-demand nodegroup: {e}"
            )
            return False
    ...
    def ensure_iam_roles(self, iam_client, account_id: str) -> Tuple[str, str]:
        ...
        try:
            ...
            self.log_operation(
                "DEBUG", f"EKS service role {eks_role_name} already exists"
            )
        ...
        try:
            ...
            self.log_operation(
                "DEBUG", f"Node instance role {node_role_name} already exists"
            )
        except iam_client.exceptions.NoSuchEntityException:
        ...
        self.log_operation(
            "INFO", f"Checking and attaching AWS CSI policy for EBS driver"
        )
        ...
        try:
            ...
            self.log_operation(
                "DEBUG", f"AWS CSI policy {csi_policy_name} already exists"
            )
        except iam_client.exceptions.NoSuchEntityException:
            ...
            if os.path.exists("aws_csi_policy.json"):
                ...
                self.log_operation(
                    "DEBUG", f"Loaded CSI policy from aws_csi_policy.json"
                )
            else:
                # Create default CSI policy document if file doesn't exist
                self.log_operation(
                    "WARNING", f"aws_csi_policy.json not found, using default policy"
                )
        ...
        try:
            iam_client.attach_role_policy(
                RoleName=node_role_name, PolicyArn=csi_policy_arn
            )
    ...
    def get_or_create_vpc_resources(
        ...
        try:
            ...
            if unsupported_azs:
                self.log_operation(
                    "DEBUG", f"Unsupported AZs in {region}: {unsupported_azs}"
                )
            ...
            for subnet in subnets["Subnets"]:
                ...
                if az not in unsupported_azs:
                    ...
                    self.log_operation(
                        "DEBUG",
                        f"Skipping subnet {subnet['SubnetId']} in unsupported AZ: {az}",
                    )
            ...
            if len(subnet_ids) < 2:
                ...
                self.log_operation(
                    "WARNING",
                    f"Only found {len(subnet_ids)} supported subnets in {region}, minimum required: {min_subnets}",
                )
                ...
                if len(supported_subnets) > 0:
                    ...
                    self.log_operation(
                        "INFO", f"Using {len(subnet_ids)} available supported subnets"
                    )

                    if len(subnet_ids) < min_subnets:
                        self.log_operation(
                            "ERROR",
                            f"Insufficient supported subnets: found {len(subnet_ids)}, need {min_subnets}",
                        )
                        raise Exception(
                            f"Insufficient supported subnets in {region}: found {len(subnet_ids)}, need at least {min_subnets}"
                        )
                else:
                    self.log_operation(
                        "ERROR", f"No supported subnets found in {region}"
                    )
                    raise Exception(f"No supported subnets found in {region}")

            self.log_operation(
                "DEBUG", f"Found {len(subnet_ids)} supported subnets: {subnet_ids}"
            )
            ...
            try:
                ...
                if security_groups["SecurityGroups"]:
                    ...
                    self.log_operation(
                        "DEBUG", f"VPC {vpc_id} has CIDR block: {vpc_cidr}"
                    )
                    ...
                    has_vpc_rule = any(
                        ...
                        and rule.get("CidrIpv4") == vpc_cidr
                        and rule.get("IpProtocol") == "-1"
                    ...
                    vpc_cidr = vpc_response["Vpcs"][0]["CidrBlock"]
                    self.log_operation(
                        "INFO", f"VPC {vpc_id} has CIDR block: {vpc_cidr}"
                    )

                    # Add the VPC access rule
                    self.log_operation(
                        "INFO", f"Adding VPC access rule to new security group {sg_id}"
                    )
                    ec2_client.authorize_security_group_ingress(
                        GroupId=sg_id,
                        IpPermissions=[
                ...
                self.log_operation(
                    "WARNING", f"Using default security group due to: {str(e)}"
                )
            ...
            self.log_operation(
                "ERROR", f"Failed to get VPC resources in {region}: {str(e)}"
            )
            raise

    def enable_cluster_access_modes(
        self,
        cluster_name: str,
        region: str,
        account_id: str,
        ...
        try:
            self.log_operation(
                "INFO",
                f"Enabling both ConfigMap and API access for cluster {cluster_name}",
            )
            self.print_colored(
                Colors.YELLOW,
                f"🔐 Configuring dual access (ConfigMap + API) for cluster {cluster_name}",
            )
            ...
            self.log_operation(
                "INFO", f"Configuring access for {creator_type}: {user_arn}"
            )
            ...
            auth_mode = (
                eks_client.describe_cluster(name=cluster_name)["cluster"]
                .get("accessConfig", {})
                .get("authenticationMode", "CONFIG_MAP")
            )

            if auth_mode in ["API", "API_AND_CONFIG_MAP"]:
                api_success = self._enable_api_access(
                    eks_client, cluster_name, user_arn, username
                )
            else:
                api_success = False
                self.log_operation(
            ...
            if configmap_success and api_success:
                self.print_colored(
                    Colors.GREEN,
                    f"✅ Both ConfigMap and API access enabled successfully",
                )
                return True
            elif configmap_success:
                self.print_colored(
                    Colors.YELLOW, f"⚠️ ConfigMap access enabled, API access failed"
                )
                return True
            elif api_success:
                self.print_colored(
                    Colors.YELLOW, f"⚠️ API access enabled, ConfigMap access failed"
                )
                return True
            else:
            ...
            self.log_operation(
                "ERROR", f"Error enabling cluster access modes: {str(e)}"
            )
            self.print_colored(
                Colors.RED, f"❌ Error enabling cluster access modes: {str(e)}"
            )
            return False

    def _enable_api_access(
        self, eks_client, cluster_name: str, user_arn: str, username: str
    ) -> bool:
        """Enable API access for the user"""
        try:
            self.log_operation("INFO", f"Enabling API access for user {username}")
            self.print_colored(
                Colors.CYAN, f"🔑 Enabling API access for user {username}"
            )

            # Create access entry for the user
            try:
                eks_client.create_access_entry(
                    clusterName=cluster_name, principalArn=user_arn, type="STANDARD"
                )
                self.log_operation("INFO", f"Created access entry for {user_arn}")

            except eks_client.exceptions.ResourceInUseException:
                self.log_operation(
                    "INFO", f"Access entry already exists for {user_arn}"
                )
                self.print_colored(
                    Colors.YELLOW, f"⚠️ Access entry already exists for {username}"
                )
            ...
            try:
                ...
                self.log_operation(
                    "INFO", f"Associated admin policy with access entry for {user_arn}"
                )
                self.print_colored(
                    Colors.GREEN, f"✅ API access enabled for user {username}"
                )
                return True

            except eks_client.exceptions.ResourceInUseException:
                self.log_operation(
                    "INFO", f"Access policy already associated for {user_arn}"
                )
                self.print_colored(
                    Colors.YELLOW, f"⚠️ Access policy already associated for {username}"
                )
                return True

        except Exception as e:
            self.log_operation(
                "WARNING", f"Failed to enable API access for {username}: {str(e)}"
            )
            self.print_colored(
                Colors.YELLOW, f"⚠️ API access setup failed for {username}: {str(e)}"
            )
            return False

    def configure_aws_auth_configmap(
        self,
        cluster_name: str,
        region: str,
        account_id: str,
        ...
        try:
            self.log_operation(
                "INFO", f"Configuring aws-auth ConfigMap for cluster {cluster_name}"
            )
            self.print_colored(
                Colors.YELLOW,
                f"🔐 Configuring aws-auth ConfigMap for cluster {cluster_name}",
            )
            ...
            try:
                ...
                is_root_user = (
                    ...
                    or caller_user_id == account_id
                    or "root" in caller_arn.lower()
                )
                ...
                self.log_operation(
                    "WARNING", f"Could not determine caller identity: {str(e)}"
                )
            ...
            if is_root_user:
                ...
                self.log_operation(
                    "INFO", f"Configuring access for root user: {root_arn}"
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   👑 Creator is root user - configuring root access only",
                )
                ...
                self.log_operation(
                    "INFO", f"Configuring access for IAM user: {username} ({user_arn})"
                )
                self.log_operation(
                    "INFO", f"Configuring access for root user: {root_arn}"
                )
            ...
            if auth_mode in ["CONFIG_MAP", "API_AND_CONFIG_MAP"]:
                ...
                configmap_file = os.path.join(
                    temp_dir, f"aws-auth-{cluster_name}-{self.execution_timestamp}.yaml"
                )
                try:
                    with open(configmap_file, "w") as f:
                        yaml.dump(aws_auth_config, f)
                    self.log_operation(
                        "INFO", f"Created ConfigMap file: {configmap_file}"
                    )
                except Exception as e:
                    self.log_operation(
                        "ERROR", f"Failed to create ConfigMap file: {str(e)}"
                    )
                    return False
                ...
                if not kubectl_available:
                    ...
                    self.print_colored(
                        Colors.YELLOW, f"⚠️  kubectl not found. Manual setup required."
                    )
                    ...
                    try:
                        with open(instruction_file, "w") as f:
                            f.write(
                                f"# Manual aws-auth ConfigMap Setup for {cluster_name}\n"
                            )
                            f.write(
                                f"# Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}\n"
                            )
                            ...
                            f.write(f"# Region: {region}\n")
                            f.write(
                                f"# Creator type: {'Root User' if is_root_user else 'IAM User'}\n\n"
                            )
                            ...
                            f.write(
                                f"export AWS_SECRET_ACCESS_KEY={admin_secret_key}\n"
                            )
                            ...
                            if is_root_user:
                                ...
                                f.write(f"\n## Test Root Access\n")
                                f.write(
                                    f"# Set root credentials (replace with actual root credentials)\n"
                                )
                                f.write(f"export AWS_ACCESS_KEY_ID=ROOT_ACCESS_KEY\n")
                                f.write(
                                    f"export AWS_SECRET_ACCESS_KEY=ROOT_SECRET_KEY\n"
                                )
                        ...
                        self.log_operation(
                            "INFO",
                            f"Manual setup instructions saved to: {instruction_file}",
                        )
                        self.print_colored(
                            Colors.CYAN,
                            f"📋 Manual setup instructions: {instruction_file}",
                        )

                    except Exception as e:
                        self.log_operation(
                            "WARNING", f"Failed to create instruction file: {str(e)}"
                        )
                ...
                self.log_operation(
                    "INFO", f"Applying ConfigMap using admin credentials"
                )
                self.print_colored(
                    Colors.YELLOW, f"🚀 Applying ConfigMap with admin credentials..."
                )
                ...
                try:
                    ...
                    self.log_operation(
                        "INFO", f"Updating kubeconfig with admin credentials"
                    )
                    update_result = subprocess.run(
                        update_cmd,
                        env=myenv,
                    ...
                    if update_result.returncode == 0:
                        self.log_operation(
                            "INFO",
                            f"Successfully updated kubeconfig with admin credentials",
                        )
                    else:
                        self.log_operation(
                            "ERROR",
                    ...
                    self.log_operation(
                        "INFO", f"Applying ConfigMap: {' '.join(apply_cmd)}"
                    )
                    ...
                    if apply_result.returncode == 0:
                        self.log_operation(
                            "INFO",
                            f"Successfully applied aws-auth ConfigMap for {cluster_name}",
                        )
                        self.print_colored(
                            Colors.GREEN, f"✅ ConfigMap applied successfully"
                        )
                        ...
                        if verify_result.returncode == 0:
                            self.log_operation(
                                "INFO", f"ConfigMap verification successful"
                            )
                            self.log_operation(
                                "DEBUG", f"ConfigMap content: {verify_result.stdout}"
                            )
                        else:
                            self.log_operation(
                                "WARNING",
                                f"ConfigMap verification failed: {verify_result.stderr}",
                            )
                        ...
                        self.log_operation(
                            "ERROR",
                            f"Failed to apply aws-auth ConfigMap: {apply_result.stderr}",
                        )
                        self.print_colored(
                            Colors.RED,
                            f"❌ Failed to apply ConfigMap: {apply_result.stderr}",
                        )
                        success = False

                except subprocess.TimeoutExpired:
                    self.log_operation(
                        "ERROR", f"kubectl/aws command timed out for {cluster_name}"
                    )
                    self.print_colored(Colors.RED, f"❌ Command timed out")
                    success = False
                except Exception as e:
                    self.log_operation(
                        "ERROR", f"Failed to execute kubectl/aws commands: {str(e)}"
                    )
                    self.print_colored(
                        Colors.RED, f"❌ Command execution failed: {str(e)}"
                    )
                    success = False
                ...
                try:
                    if os.path.exists(configmap_file):
                        ...
                        self.log_operation(
                            "INFO", f"Cleaned up temporary ConfigMap file"
                        )
                except Exception as e:
                    self.log_operation(
                        "WARNING", f"Failed to clean up ConfigMap file: {str(e)}"
                    )
            else:
            ...
            if success:
                if is_root_user:
                    self.print_colored(
                        Colors.GREEN, f"✅ Root user configured for cluster access"
                    )
                else:
                    username = user_data.get("username", "unknown")
                    self.print_colored(
                ...
                if not is_root_user:
                    try:
                        ...
                        username = user_data.get("username", "unknown")
                        self.log_operation(
                            "INFO", f"Testing user access for {username}"
                        )
                        ...
                        user_env["AWS_SECRET_ACCESS_KEY"] = user_data.get(
                            "secret_key", ""
                        )
                        ...
                        user_update_result = subprocess.run(
                            user_update_cmd,
                            env=user_env,
                            capture_output=True,
                            text=True,
                        ...
                        if user_update_result.returncode == 0:
                            ...
                            if test_result.returncode == 0:
                                self.log_operation(
                                    "INFO",
                                    f"User access test successful for {username}",
                                )
                                self.print_colored(
                                    Colors.GREEN,
                                    f"✅ User access verified - can access cluster",
                                )
                            else:
                                self.log_operation(
                                    "WARNING",
                                    f"User access test failed: {test_result.stderr}",
                                )
                                self.print_colored(
                                    Colors.YELLOW,
                        ...
                        self.log_operation(
                            "WARNING", f"User access test failed: {str(e)}"
                        )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to configure aws-auth ConfigMap for {cluster_name}: {error_msg}",
            )
            self.print_colored(
                Colors.RED, f"❌ ConfigMap configuration failed: {error_msg}"
            )
            return False

    def health_check_cluster(
        self,
        cluster_name: str,
        region: str,
        admin_access_key: str,
        ...
        try:
            self.log_operation(
                "INFO",
                f"Performing comprehensive health check for cluster {cluster_name}",
            )
            ...
            try:
                ...
                if cluster_status != "ACTIVE":
                    ...
                    health_status["issues"].append(
                        f"Cluster status is {cluster_status}, expected ACTIVE"
                    )
                    self.print_colored(
                        Colors.RED, f"   ❌ Cluster status: {cluster_status}"
                    )
                else:
                    health_status["success_items"].append(
                        f"Cluster is ACTIVE (version {cluster_version})"
                    )
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Cluster status: {cluster_status} (v{cluster_version})",
                    )

                # Check cluster logging
                logging_config = cluster.get("logging", {}).get("clusterLogging", [])
                if logging_config and any(
                    log.get("enabled", False) for log in logging_config
                ):
                    health_status["success_items"].append(
                        "CloudWatch logging is enabled"
                    )
                    self.print_colored(
                        Colors.GREEN, f"   ✅ CloudWatch logging: Enabled"
                    )
                else:
                    health_status["warnings"].append(
                        "CloudWatch logging may not be fully configured"
                    )
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️  CloudWatch logging: Limited or disabled"
                    )
            ...
            try:
                ...
                for ng_name in nodegroups_response["nodegroups"]:
                    ...
                    if ng_status != "ACTIVE":
                        health_status["overall_healthy"] = False
                        health_status["issues"].append(
                            f"NodeGroup {ng_name} status is {ng_status}"
                        )
                        self.print_colored(
                            Colors.RED, f"   ❌ NodeGroup {ng_name}: {ng_status}"
                        )
                    else:
                        active_nodegroups += 1
                        health_status["success_items"].append(
                            f"NodeGroup {ng_name} is ACTIVE ({capacity_type}, {', '.join(instance_types)})"
                        )
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ NodeGroup {ng_name}: {ng_status} ({capacity_type}, {ami_type})",
                        )
                        self.print_colored(
                            Colors.CYAN,
                            f"      Scaling: {scaling_config.get('desiredSize', 0)}/{scaling_config.get('maxSize', 0)} nodes",
                        )
                ...
                self.print_colored(
                    Colors.RED, f"   ❌ NodeGroup check failed: {str(e)}"
                )
            ...
            try:
                ...
                for addon_name in addons_response["addons"]:
                    ...
                    if addon_status not in ["ACTIVE", "RUNNING"]:
                        if addon_name in essential_addons:
                            health_status["overall_healthy"] = False
                            health_status["issues"].append(
                                f"Essential add-on {addon_name} status is {addon_status}"
                            )
                            self.print_colored(
                                Colors.RED,
                                f"   ❌ Add-on {addon_name}: {addon_status} (ESSENTIAL)",
                            )
                        else:
                            health_status["warnings"].append(
                                f"Non-essential add-on {addon_name} status is {addon_status}"
                            )
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️  Add-on {addon_name}: {addon_status}",
                            )
                    else:
                        active_addons += 1
                        health_status["success_items"].append(
                            f"Add-on {addon_name} is {addon_status} (v{addon_version})"
                        )
                        addon_type = (
                            "ESSENTIAL"
                            if addon_name in essential_addons
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Add-on {addon_name}: {addon_status} (v{addon_version}, {addon_type})",
                        )
                ...
                installed_essential = [
                    name
                    for name in addons_response["addons"]
                    if name in essential_addons
                ]
                missing_essential = [
                    name for name in essential_addons if name not in installed_essential
                ]

                if missing_essential:
                    health_status["warnings"].append(
                        f"Missing essential add-ons: {', '.join(missing_essential)}"
                    )
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  Missing essential add-ons: {', '.join(missing_essential)}",
                    )
                ...
                if kubectl_available:
                    ...
                    if update_result.returncode == 0:
                        ...
                        if nodes_result.returncode == 0:
                            ...
                            not_ready_nodes = [
                                line for line in node_lines if "NotReady" in line
                            ]
                            ...
                            if len(ready_nodes) == 0:
                                ...
                                health_status["warnings"].append(
                                    f"{len(not_ready_nodes)} nodes are not ready"
                                )
                                self.print_colored(
                                    Colors.YELLOW,
                                    f"   ⚠️  Nodes ready: {len(ready_nodes)}/{len(node_lines)} ({len(not_ready_nodes)} not ready)",
                                )
                            else:
                                health_status["success_items"].append(
                                    f"All {len(ready_nodes)} nodes are ready"
                                )
                                self.print_colored(
                                    Colors.GREEN,
                                    f"   ✅ All nodes ready: {len(ready_nodes)}/{len(node_lines)}",
                                )
                            ...
                            if pods_result.returncode == 0:
                                ...
                                if len(failed_pods) > 0:
                                    health_status["warnings"].append(
                                        f"{len(failed_pods)} system pods have failed"
                                    )
                                    self.print_colored(
                                        Colors.YELLOW,
                                        f"   ⚠️  System pods: {len(running_pods)}/{len(pod_lines)} running ({len(failed_pods)} failed)",
                                    )
                                else:
                                    health_status["success_items"].append(
                                        f"All {len(running_pods)} system pods are running"
                                    )
                                    self.print_colored(
                                        Colors.GREEN,
                                        f"   ✅ System pods: {len(running_pods)}/{len(pod_lines)} running",
                                    )
                        else:
                            health_status["warnings"].append(
                                "Could not retrieve node status via kubectl"
                            )
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️  Could not check nodes via kubectl",
                            )
                    else:
                        health_status["warnings"].append(
                            "Could not update kubeconfig for kubectl access"
                        )
                else:
                    health_status["warnings"].append(
                        "kubectl not available for node status check"
                    )
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  kubectl not available for detailed node check",
                    )

            except Exception as e:
                health_status["warnings"].append(
                    f"Failed to check nodes via kubectl: {str(e)}"
                )
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️  kubectl check failed: {str(e)}"
                )
            ...
            health_status["summary"] = {
                ...
                "health_score": max(
                    0, 100 - (total_issues * 20) - (total_warnings * 5)
                ),
            }
            ...
            if health_status["overall_healthy"]:
                self.log_operation(
                    "INFO",
                    f"Health check PASSED for {cluster_name} - Score: {health_status['summary']['health_score']}/100",
                )
                self.print_colored(
                    Colors.GREEN,
                    f"   🎉 Overall Health: HEALTHY (Score: {health_status['summary']['health_score']}/100)",
                )
            else:
                self.log_operation(
                    "WARNING",
                    f"Health check FAILED for {cluster_name} - {total_issues} issues, {total_warnings} warnings",
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️  Overall Health: NEEDS ATTENTION ({total_issues} issues, {total_warnings} warnings)",
                )
            ...
            error_msg = str(e)
            self.log_operation(
                "ERROR", f"Health check exception for {cluster_name}: {error_msg}"
            )
    ...
    def create_kubectl_awscli_layer(
        self, lambda_client, layer_name: str = "kubectl-layer"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.print_colored(
                    Colors.GREEN, "   [OK] kubectl downloaded and made executable"
                )
    ...
    """
                ...
                try:
                    ...
                    layer_exists = any(
                        ...
                        for layer in existing_layers.get("Layers", [])
                    )
                    ...
                    # Create new layer
                    self.print_colored(
                        Colors.BLUE, f"   📦 Publishing kubectl layer..."
                    )
                    ...
                    self.print_colored(
                        Colors.GREEN, f"   ✅ kubectl layer created: {layer_arn}"
                    )
                    ...
                    self.print_colored(
                        Colors.RED, f"   ❌ Failed to create kubectl layer: {str(e)}"
                    )
                    return None

        except Exception as e:
            self.print_colored(
                Colors.RED, f"[ERROR] Failed to create kubectl layer: {str(e)}"
            )
            return None

    def create_minimal_kubectl_layer(
        self, region: str, admin_access_key: str, admin_secret_key: str
    ) -> str:
        """
        ...
        try:
            ...
            try:
                ...
                if response.get("LayerVersions"):
                    ...
                    self.print_colored(
                        Colors.CYAN, f"   📦 Using existing minimal layer: {layer_arn}"
                    )
                    return layer_arn
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ kubectl downloaded ({kubectl_size_mb:.1f} MB)"
                )
                ...
                with zipfile.ZipFile(
                    temp_zip_path, "w", zipfile.ZIP_DEFLATED, compresslevel=9
                ) as zip_file:
                ...
                self.print_colored(
                    Colors.CYAN, f"   📏 Layer size: {layer_size_mb:.1f} MB"
                )
                ...
                with open(temp_zip_path, "rb") as zip_data:
                    ...
                    layer_arn = response["LayerVersionArn"]
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Minimal layer created: {layer_arn}"
                    )
                    return layer_arn
            ...
            self.print_colored(
                Colors.RED, f"❌ Minimal layer creation failed: {str(e)}"
            )
            return None

    def setup_node_protection_monitoring(
        self,
        cluster_name: str,
        region: str,
        admin_access_key: str,
        admin_secret_key: str,
        nodegroup_names: list,
    ) -> bool:
        """
        ...
        try:
            ...
            self.print_colored(
                Colors.CYAN, "   📦 Creating kubectl layer (optimized)..."
            )
            ...
            self.print_colored(
                Colors.GREEN, f"   ✅ kubectl layer created: {kubectl_layer_arn}"
            )
            ...
            try:
                ...
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️  IAM role already exists: {role_name}"
                )
            ...
            for policy_arn in policies:
                try:
                    ...
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Attached policy: {policy_arn}"
                    )
                except Exception as e:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  Policy may already be attached: {policy_arn}",
                    )
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ Created optimized inline policy"
                )
            except Exception as e:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️  Inline policy may already exist: {str(e)}"
                )
            ...
            self.print_colored(
                Colors.CYAN, f"   ⏱️  Waiting for IAM role to be ready..."
            )
            time.sleep(10)
            ...
            self.print_colored(
                Colors.CYAN,
                f"   🚀 Creating optimized Lambda function: {function_name}",
            )
            ...
            try:
                ...
                with open(
                    "lambda_node_protection_template.py", "r", encoding="utf-8"
                ) as f:
                    lambda_template = f.read()

                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Lambda template loaded successfully with UTF-8 encoding",
                )
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ Template variables replaced successfully"
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   📅 Generated: {current_datetime} UTC by {self.current_user}",
                )
                ...
                self.print_colored(
                    Colors.RED,
                    f"   ❌ Unicode decode error reading template file: {str(e)}",
                )
                self.print_colored(
                    Colors.YELLOW,
                ...
                self.print_colored(
                    Colors.RED, f"   ❌ Failed to read Lambda template: {str(e)}"
                )
                return False
            ...
            try:
                ...
                self.print_colored(
                    Colors.RED,
                    f"   ❌ Failed to create Lambda deployment package: {str(e)}",
                )
                return False
            ...
            try:
                ...
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️  Lambda function already exists: {function_name}",
                )
            except lambda_client.exceptions.ResourceNotFoundException:
                self.print_colored(
                    Colors.CYAN, f"   📝 Creating new optimized Lambda function..."
                )
            ...
            if not function_exists:
                try:
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Created optimized Lambda function: {function_name}",
                    )
                except Exception as e:
                    self.print_colored(
                        Colors.RED, f"   ❌ Failed to create Lambda function: {str(e)}"
                    )
                    return False
            else:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        lambda_client.update_function_code(
                            FunctionName=function_name, ZipFile=lambda_zip_content
                        )
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Updated optimized Lambda function: {function_name}",
                        )
                        break
                        ...
                        self.print_colored(
                            Colors.RED,
                            f"   ❌ Failed to update Lambda function: {str(e)}",
                        )
                        return False
            ...
            self.print_colored(
                Colors.CYAN, f"   📅 Creating EventBridge rule: {rule_name}"
            )
            ...
            event_pattern = {
                ...
                "detail": {"state": ["terminated", "running"]},
            }
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ Created EventBridge rule: {rule_name}"
                )
            except Exception as e:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️  EventBridge rule may already exist: {str(e)}"
                )

            # Step 5: Add Lambda permission for EventBridge
            self.print_colored(
                Colors.CYAN, f"   🔒 Adding Lambda permission for EventBridge..."
            )
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ Added Lambda permission for EventBridge"
                )
            except lambda_client.exceptions.ResourceConflictException:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️  Lambda permission already exists"
                )
            except Exception as e:
                self.print_colored(
                    Colors.RED, f"   ❌ Failed to add Lambda permission: {str(e)}"
                )
            ...
            try:
                ...
                self.print_colored(
                    Colors.RED, f"   ❌ Failed to create EventBridge target: {str(e)}"
                )
                return False
            ...
            #     self.print_colored(Colors.GREEN, f"   ✅ Created scheduled monitoring rule (5-minute interval)")
                ...
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️  Scheduled monitoring rule already exists"
                )
            except Exception as e:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️  Failed to create scheduled monitoring: {str(e)}",
                )

            self.print_colored(
                Colors.GREEN,
                f"✅ Optimized node protection monitoring setup completed!",
            )
            self.print_colored(Colors.CYAN, f"   📋 Function: {function_name}")
            self.print_colored(Colors.CYAN, f"   🔑 Role: {role_name}")
            self.print_colored(
                Colors.CYAN, f"   📦 Layer: kubectl only (~15MB vs 70MB)"
            )
            self.print_colored(
                Colors.CYAN, f"   🔧 Method: Boto3 kubeconfig generation"
            )
            self.print_colored(Colors.CYAN, f"   ⏱️  Timeout: 180s (optimized)")
            self.print_colored(Colors.CYAN, f"   💾 Memory: 256MB (optimized)")
            self.print_colored(
                Colors.CYAN, f"   🎯 Monitoring: {len(nodegroup_names)} nodegroups"
            )
            ...
            self.print_colored(
                Colors.CYAN, f"   📝 Encoding: UTF-8 (prevents charmap errors)"
            )
            self.print_colored(
                Colors.CYAN, f"   📦 Package: Proper ZIP format (prevents unzip errors)"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to setup optimized node protection monitoring: {str(e)}",
            )
            self.print_colored(
                Colors.RED,
                f"❌ Optimized node protection monitoring setup failed: {str(e)}",
            )
            return False
    ...
    def create_kubectl_only_layer(self, lambda_client):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.print_colored(
                    Colors.CYAN, f"   📥 Downloading kubectl from {kubectl_url}"
                )
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ kubectl-only layer created: {layer_arn}"
                )
            ...
            self.print_colored(
                Colors.RED, f"   ❌ Failed to create kubectl-only layer: {str(e)}"
            )
            return None
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    self.print_colored(
                        Colors.GREEN, f"   ✅ PyYAML installed successfully"
                    )

                except subprocess.CalledProcessError as e:
                    self.print_colored(
                        Colors.RED,
                        f"   ❌ Failed to install PyYAML: {e.stderr.decode()}",
                    )
                    return None
                ...
                self.print_colored(
                    Colors.CYAN, f"   📊 Layer size: {layer_size_mb:.2f} MB"
                )
                ...
                layer_arn = layer_response["LayerVersionArn"]
                self.print_colored(
                    Colors.GREEN, f"   ✅ kubectl + PyYAML layer created: {layer_arn}"
                )
            ...
            self.print_colored(
                Colors.RED, f"   ❌ Failed to create kubectl + PyYAML layer: {str(e)}"
            )
            return None

    def create_node_termination_eventbridge_rule(
        self,
        region: str,
        account_id: str,
        lambda_arn: str,
        ...
        try:
            ...
            if current_account != account_id:
                self.print_colored(
                    Colors.RED,
                    f"❌ Account mismatch: Current={current_account}, Expected={account_id}",
                )
                return False
            ...
            self.logger.info(
                "Event pattern updated to trigger on EC2 instance creation and deletion events:"
            )
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ EventBridge rule {rule_name} created"
                )
            except Exception as e:
                if "already exists" in str(e):
                    self.print_colored(
                        Colors.CYAN,
                        f"   📝 EventBridge rule {rule_name} already exists",
                    )
                else:
                    raise e
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ Lambda permission added for EventBridge"
                )
            except lambda_client.exceptions.ResourceConflictException:
                self.print_colored(
                    Colors.CYAN, f"   📝 Lambda permission already exists"
                )
            except Exception as e:
                if "Cross-account access" in str(e):
                    ...
                    try:
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Lambda permission recreated successfully",
                        )
                    except Exception as e2:
                        self.print_colored(
                            Colors.YELLOW, f"   ⚠️ Permission issue: {str(e2)}"
                        )
            ...
            self.print_colored(
                Colors.GREEN, f"   ✅ Lambda target added to EventBridge rule"
            )
            return True
            ...
            self.print_colored(
                Colors.RED, f"❌ Failed to create EventBridge rule: {str(e)}"
            )
            return False

    def apply_no_delete_to_matching_nodegroups(
        self, cluster_name, region, access_key, secret_key
    ):
        ...
        print("=" * 80)
        print(
            f"📅 Current Date and Time (UTC - YYYY-MM-DD HH:MM:SS formatted): {current_datetime}"
        )
        print(f"👤 Current User's Login: {current_user}")
        print(f"🏷️  Cluster Name: {cluster_name}")
        print(f"🌍 Region: {region}")
        print(
            f"🎯 Target Pattern: nodegroup-*-ondemand (where * is 0-9) if not found then Target Pattern: nodegroup-*-"
        )
        print("=" * 80)
        ...
        try:
            ...
            print(
                f"🔧 Updating kubeconfig for cluster: {cluster_name} in region: {region}"
            )
            subprocess.run(
                [
                    "aws",
            ...
            matching_nodegroups = [
                ng for ng in all_nodegroups if primary_pattern.match(ng)
            ]
            ...
            if not matching_nodegroups:
                ...
                matching_nodegroups = [
                    ng for ng in all_nodegroups if secondary_pattern.match(ng)
                ]
            ...
            if not matching_nodegroups:
                ...
                matching_nodegroups = [
                    ng for ng in all_nodegroups if fallback_pattern.match(ng)
                ]
                ...
                if nodegroup_label and nodegroup_label in matching_nodegroups:
                    ...
                    if current_no_delete == "true":
                        ...
                        try:
                            ...
                            print(
                                f"✅ Successfully applied NO_DELETE protection to node: {node_name}"
                            )
                            print(
                                f"   📋 Labels applied: {list(protection_labels.keys())}"
                            )
                            ...
                            if e.stderr:
                                ...
                                print(
                                    f"   Error details when applying NO_DELETE Protection Label: {error_msg}"
                                )

                            results["errors"] += 1
                        except Exception as e:
                            print(
                                f"❌ Unexpected error applying labels to {node_name}: {e}"
                            )
                            results["errors"] += 1
                else:
                    results["skipped"] += 1
            ...
            if results["nodes_to_verify"]:
                print(
                    f"🔍 Verifying {len(results['nodes_to_verify'])} newly labeled nodes..."
                )
                    ...
                    try:
                        ...
                        for label_key, expected_value in expected_labels.items():
                            ...
                            if actual_value == expected_value:
                                ...
                                print(
                                    f"   ❌ {label_key}: expected='{expected_value}', actual='{actual_value}'"
                                )
                        ...
                        if all_labels_present:
                            print(
                                f"   🎯 All labels verified successfully on {node_name}"
                            )
                            verification_results["verified_successfully"] += 1
                        else:
                            print(
                                f"   ⚠️ Some labels missing on {node_name}: {missing_labels}"
                            )
                            verification_results["verification_failed"] += 1
                            verification_results["missing_labels"] += len(
                                missing_labels
                            )
                ...
                try:
                    ...
                    verify_result = subprocess.run(
                        [
                            ...
                            "nodes",
                            "-l",
                            f"eks.amazonaws.com/nodegroup={nodegroup}",
                            "-o",
                    ...
                    count_result = subprocess.run(
                        [
                            ...
                            "nodes",
                            "-l",
                            f"eks.amazonaws.com/nodegroup={nodegroup},NO_DELETE=true",
                            "--no-headers",
                        ],
                        capture_output=True,
                        text=True,
            ...
            print(
                f"✅ Nodes verified successfully: {verification_results['verified_successfully']}"
            )
            print(
                f"❌ Nodes with verification issues: {verification_results['verification_failed']}"
            )
            ...
            return {
                ...
                "verification": {
                    "verified_successfully": verification_results[
    ...
    def protect_nodes_with_no_delete_label(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> bool:
        """
        ...
        try:
            self.log_operation(
                "INFO",
                f"Prot" f"ecting nodes with NO_DELETE label in cluster {cluster_name}",
            )
            self.print_colored(
                Colors.YELLOW, f"🔒 Protecting nodes with NO_DELETE label..."
            )
            ...
            if not kubectl_available:
                self.log_operation(
                    "WARNING", f"kubectl not found. Cannot protect nodes"
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"kubectl not found. Manual node protection required.",
                )
                return False
            ...
            if update_result.returncode != 0:
                self.print_colored(
                    Colors.RED, f"Failed to update kubeconfig: {update_result.stderr}"
                )
                return False
            ...
            nodes_result = subprocess.run(
                get_nodes_cmd, env=env, capture_output=True, text=True, timeout=60
            )

            if nodes_result.returncode != 0:
                self.print_colored(
                    Colors.YELLOW,
                    f"No nodes found with NO_DELETE label or kubectl error",
                )
            ...
            for line in node_lines:
                ...
                if (
                    ...
                    self.print_colored(
                        Colors.CYAN, f" Node {node_name} already protected"
                    )
                    protected_nodes += 1
                    continue
                ...
                annotate_result = subprocess.run(
                    annotate_cmd, env=env, capture_output=True, text=True, timeout=30
                )
                count = count + 1
                ...
                if annotate_result.returncode == 0:
                    self.print_colored(
                        Colors.GREEN,
                        f"Protected node {node_name} from autoscaler scale-down",
                    )
                    protected_nodes += 1
                else:
                    self.print_colored(
                        Colors.RED,
                        f"Failed to protect node {node_name}: {annotate_result.stderr}",
                    )
            ...
            if protected_nodes > 0:
                ...
                self.log_operation(
                    "INFO",
                    f"Protected {protected_nodes} nodes from autoscaler scale-down",
                )
            else:
    ...
    def generate_cost_alarm_summary_report(self, cluster_name: str) -> str:
        ...
        if (
            not hasattr(self, "cost_alarm_details")
            or cluster_name not in self.cost_alarm_details
        ):
        ...
        for alarm_name, threshold in details.get("thresholds", {}).items():
            service = (
                "EKS"
                if "daily-cost" in alarm_name
                else (
                    "EC2"
                    if "ec2-cost" in alarm_name
                    else "EBS" if "ebs-cost" in alarm_name else "Unknown"
                )
            )
    ...
    def generate_health_check_report(self, cluster_name: str) -> str:
        ...
        if not hasattr(self, "health_check_results") or cluster_name not in getattr(
    ...
    def display_cost_estimation(
        self, instance_type: str, capacity_type: str, node_count: int = 1
    ):
    ...
    def show_cluster_summary(self, cluster_configs) -> bool:
        ...
        print(
            f"💻 Instance types: {', '.join(set(cluster.get('instance_type', 'c6a.large') for cluster in cluster_configs))}"
        )
    ...
    def setup_cost_alarms(
        self, cluster_name: str, region: str, cloudwatch_client, account_id: str
    ) -> bool:
        """Setup cost monitoring alarms with proper service names and realistic expectations"""
        try:
            self.log_operation(
                "INFO", f"Setting up cost monitoring alarms for cluster {cluster_name}"
            )
            ...
            for alarm_config in cost_alarm_configs:
                ...
                try:
                    ...
                    self.log_operation(
                        "ERROR",
                        f"Failed to create cost alarm {alarm_config['name']}: {str(e)}",
                    )
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️ Failed cost alarm: {alarm_config['name']}"
                    )
            ...
            if cost_alarms_created > 0:
                self.print_colored(
                    Colors.CYAN,
                    "   ℹ️  Note: Cost alarms may show 'Insufficient Data' for 24-48 hours",
                )
                self.print_colored(
                    Colors.CYAN,
                    "      This is normal as AWS billing metrics update daily",
                )

            success_rate = (
                (cost_alarms_created / total_cost_alarms) * 100
                if total_cost_alarms > 0
                else 0
            )
            self.log_operation(
                "INFO",
        ...
        try:
            ...
            for alarm_config in cost_alarm_configs:
                ...
                try:
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Cost alarm created: {alarm_config['name']} (${alarm_config['threshold']})",
                    )
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️  Failed to create cost alarm: {alarm_config['name']}",
                    )
            ...
            self.print_colored(
                Colors.GREEN,
                f"   📊 Cost alarms: {cost_alarms_created}/{total_cost_alarms} created ({success_rate:.1f}%)",
            )
            ...
            self.cost_alarm_details[cluster_name] = {
                ...
                "thresholds": {
                    config["name"]: config["threshold"] for config in cost_alarm_configs
                },
            }
    ...
    def _get_unsupported_azs(self, region: str) -> Set[str]:
        ...
        try:
            ...
            mapping_file_path = os.path.join(
                os.path.dirname(__file__), "ec2-region-ami-mapping.json"
            )

            if not os.path.exists(mapping_file_path):
                self.log_operation(
                    "WARNING", f"Mapping file not found: {mapping_file_path}"
                )
                return set()
            ...
            if (
                "eks_unsupported_azs" in mapping_data
                and region in mapping_data["eks_unsupported_azs"]
            ):
                unsupported_azs = set(mapping_data["eks_unsupported_azs"][region])
                self.log_operation(
                    "DEBUG",
                    f"Loaded {len(unsupported_azs)} unsupported AZs for {region} from mapping file",
                )
            else:
                self.log_operation(
                    "DEBUG",
                    f"No unsupported AZs found for region {region} in mapping file",
                )
            ...
            self.log_operation(
                "WARNING", f"Failed to load unsupported AZs from mapping file: {str(e)}"
            )
            return set()
    ...
    def _get_min_subnets_required(self) -> int:
        ...
        try:
            ...
            if (
                "eks_config" in mapping_data
                and "min_subnets_required" in mapping_data["eks_config"]
            ):
    ...
    def install_essential_addons(
        ...
        try:
            self.log_operation(
                "INFO", f"Installing essential add-ons for cluster {cluster_name}"
            )
            ...
            try:
                ...
                self.log_operation(
                    "WARNING",
                    f"Could not detect EKS version, using default {eks_version}: {str(e)}",
                )
            ...
            for addon in addons:
                try:
                    self.print_colored(
                        Colors.CYAN,
                        f"   📦 Installing {addon['addonName']} ({addon['description']})...",
                    )
                    ...
                    try:
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ {addon['addonName']} installed successfully",
                        )
                        self.log_operation(
                            "INFO",
                            f"Add-on {addon['addonName']} installed successfully for {cluster_name}",
                        )
                    except Exception as waiter_error:
                        # Check addon status
                        try:
                            addon_info = eks_client.describe_addon(
                                clusterName=cluster_name, addonName=addon["addonName"]
                            )

                            status = addon_info["addon"]["status"]
                            if (
                                status in ["DEGRADED", "UPDATE_FAILED"]
                                and "csi-driver" in addon["addonName"]
                            ):
                                self.print_colored(
                                    Colors.YELLOW,
                                    f"   ⚠️ {addon['addonName']} installed but status is {status} (may work normally)",
                                )
                                self.log_operation(
                                    "WARNING",
                                    f"Add-on {addon['addonName']} installed with status {status}",
                                )
                                successful_addons.append(addon["addonName"])
                            else:
                                ...
                                self.print_colored(
                                    Colors.YELLOW,
                                    f"   ⚠️ {addon['addonName']} installation completed but status is {status}",
                                )
                                self.log_operation(
                                    "WARNING",
                                    f"Add-on {addon['addonName']} status: {status}",
                                )
                        except Exception as describe_error:
                            failed_addons.append(addon["addonName"])
                            self.log_operation(
                                "WARNING",
                                f"Failed to verify {addon['addonName']} status: {str(describe_error)}",
                            )
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️ Failed to verify {addon['addonName']} status: {str(describe_error)}",
                            )

                except Exception as e:
                    failed_addons.append(addon["addonName"])
                    self.log_operation(
                        "WARNING", f"Failed to install {addon['addonName']}: {str(e)}"
                    )
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Failed to install {addon['addonName']}: {str(e)}",
                    )

            self.print_colored(
                Colors.GREEN,
                f"✅ Add-ons installation completed: {len(successful_addons)} successful, {len(failed_addons)} failed",
            )
            self.log_operation(
                "INFO",
                f"Add-ons installation completed for {cluster_name}: {successful_addons}",
            )
            ...
            self.log_operation(
                "ERROR", f"Failed to install add-ons for {cluster_name}: {str(e)}"
            )
    ...
    def attach_csi_policies_to_node_role(self, iam_client, account_id: str) -> bool:
        ...
        try:
            ...
            for policy_arn in aws_managed_policies:
                try:
                    iam_client.attach_role_policy(
                        RoleName=node_role_name, PolicyArn=policy_arn
                    )
                    policy_name = policy_arn.split("/")[-1]
                    self.log_operation(
                        "INFO",
                        f"Attached AWS managed policy {policy_name} to {node_role_name}",
                    )
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Attached {policy_name} to NodeInstanceRole",
                    )
                    ...
                    if "attached" in str(e).lower() or "already" in str(e).lower():
                        ...
                        self.log_operation(
                            "INFO",
                            f"AWS managed policy {policy_name} already attached to {node_role_name}",
                        )
                        self.print_colored(
                            Colors.CYAN,
                            f"   ℹ️  {policy_name} already attached to NodeInstanceRole",
                        )
                        managed_policies_attached += 1
                    else:
                        policy_name = policy_arn.split("/")[-1]
                        self.log_operation(
                            "WARNING", f"Failed to attach {policy_name}: {str(e)}"
                        )
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️ Failed to attach {policy_name}: {str(e)}",
                        )

            # Summary
            total_policies = len(aws_managed_policies) + (
                1 if custom_policy_attached else 0
            )
            attached_policies = managed_policies_attached + (
                1 if custom_policy_attached else 0
            )

            self.print_colored(
                Colors.GREEN,
                f"   📊 Policy attachment summary: {attached_policies}/{total_policies} policies attached",
            )
            self.log_operation(
                "INFO",
                f"CSI policies attachment completed: {attached_policies}/{total_policies} successful",
            )
    ...
    def create_and_attach_custom_csi_policy(
        self, iam_client, account_id: str, node_role_name: str
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.log_operation(
                    "INFO", f"Custom CSI policy {custom_policy_name} already exists"
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   ℹ️  Custom CSI policy {custom_policy_name} already exists",
                )

            except iam_client.exceptions.NoSuchEntityException:
                # Step 3: Create the custom policy if it doesn't exist
                self.log_operation(
                    "INFO", f"Creating custom CSI policy {custom_policy_name}"
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   🔧 Creating custom CSI policy {custom_policy_name}",
                )
                ...
                try:
                    ...
                    self.log_operation(
                        "INFO", f"Created custom CSI policy: {custom_policy_arn}"
                    )
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Created custom CSI policy: {custom_policy_name}",
                    )
                    ...
                    self.log_operation(
                        "ERROR", f"Failed to create custom CSI policy: {str(e)}"
                    )
                    self.print_colored(
                        Colors.RED,
                        f"   ❌ Failed to create custom CSI policy: {str(e)}",
                    )
                    return False
            ...
            try:
                iam_client.attach_role_policy(
                    RoleName=node_role_name, PolicyArn=custom_policy_arn
                )
                self.log_operation(
                    "INFO",
                    f"Attached custom CSI policy {custom_policy_name} to {node_role_name}",
                )
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Attached custom CSI policy {custom_policy_name} to NodeInstanceRole",
                )
                return True
                ...
                if "attached" in str(e).lower() or "already" in str(e).lower():
                    self.log_operation(
                        "INFO",
                        f"Custom CSI policy {custom_policy_name} already attached to {node_role_name}",
                    )
                    self.print_colored(
                        Colors.CYAN,
                        f"   ℹ️  Custom CSI policy {custom_policy_name} already attached to NodeInstanceRole",
                    )
                    return True
                else:
                    self.log_operation(
                        "ERROR",
                        f"Failed to attach custom CSI policy to {node_role_name}: {str(e)}",
                    )
                    self.print_colored(
                        Colors.RED,
                        f"   ❌ Failed to attach custom CSI policy: {str(e)}",
                    )
                    return False

        except Exception as e:
            self.log_operation(
                "ERROR", f"Failed to create and attach custom CSI policy: {str(e)}"
            )
            self.print_colored(
                Colors.RED, f"   ❌ Custom CSI policy setup failed: {str(e)}"
            )
            return False
    ...
    def load_custom_csi_policy(self) -> str:
        ...
        try:
            ...
            if not os.path.exists(policy_file):
                self.log_operation(
                    "ERROR", f"Custom CSI policy file not found: {policy_file}"
                )
                self.print_colored(
                    Colors.RED, f"   ❌ Custom CSI policy file not found: {policy_file}"
                )
                return None
            ...
            try:
                ...
                self.print_colored(
                    Colors.RED, f"   ❌ Invalid JSON in {policy_file}: {str(e)}"
                )
                return None

            self.log_operation(
                "INFO", f"Successfully loaded custom CSI policy from {policy_file}"
            )
            self.print_colored(
                Colors.GREEN, f"   ✅ Loaded custom CSI policy from {policy_file}"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to load custom CSI policy from {policy_file}: {str(e)}",
            )
            self.print_colored(
                Colors.RED, f"   ❌ Failed to load custom CSI policy: {str(e)}"
            )
            return None
    ...
    def verify_user_access(
        ...
        try:
            ...
            print(
                f"{BLUE}[INFO] Verifying user access for {username} to cluster {cluster_name}{RESET}"
            )
            print(
                f"{YELLOW}[SECURITY] Verifying user access to cluster {cluster_name}...{RESET}"
            )
            ...
            try:
                cluster_info = eks_client.describe_cluster(name=cluster_name)
                print(
                    f"{GREEN}   [OK] User {username} can access cluster information{RESET}"
                )
                print(
                    f"{BLUE}[INFO] User {username} can access cluster {cluster_name}{RESET}"
                )

                # Check cluster endpoint access configuration
                endpoint_access = cluster_info["cluster"].get("resourcesVpcConfig", {})
                endpoint_public_access = endpoint_access.get(
                    "endpointPublicAccess", False
                )
                endpoint_private_access = endpoint_access.get(
                    "endpointPrivateAccess", False
                )
                public_access_cidrs = endpoint_access.get(
                    "publicAccessCidrs", ["0.0.0.0/0"]
                )

                print(f"{CYAN}   [STATS] Cluster Endpoint Configuration:{RESET}")
                print(
                    f"{CYAN}      - Public Access: {'Enabled' if endpoint_public_access else 'Disabled'}{RESET}"
                )
                print(
                    f"{CYAN}      - Private Access: {'Enabled' if endpoint_private_access else 'Disabled'}{RESET}"
                )
                print(
                    f"{CYAN}      - Public Access CIDRs: {', '.join(public_access_cidrs)}{RESET}"
                )
                ...
                print(
                    f"{RED}   [ERROR] User {username} cannot access cluster information: {str(e)}{RESET}"
                )
                return False
            ...
            if kubectl_available:
                ...
                update_result = subprocess.run(
                    update_cmd, env=my_env, capture_output=True, text=True, timeout=120
                )

                if update_result.returncode != 0:
                    print(
                        f"{RED}   [ERROR] Failed to update kubeconfig: {update_result.stderr}{RESET}"
                    )
                    return False
                ...
                nodes_result = subprocess.run(
                    nodes_cmd, env=my_env, capture_output=True, text=True, timeout=60
                )
                ...
                if nodes_result.returncode == 0:
                    ...
                    print(
                        f"{GREEN}   [OK] kubectl access successful - {node_count} nodes found{RESET}"
                    )
                    print(
                        f"{BLUE}[INFO] User {username} has kubectl access to cluster {cluster_name}{RESET}"
                    )
                else:
                    print(
                        f"{RED}   [ERROR] kubectl access failed: {nodes_result.stderr}{RESET}"
                    )
                    return False
                ...
                pods_result = subprocess.run(
                    pods_cmd, env=my_env, capture_output=True, text=True, timeout=60
                )
                ...
                if pods_result.returncode == 0:
                    ...
                    print(
                        f"{GREEN}   [OK] kubectl pod access successful - {pod_count} pods found{RESET}"
                    )
                    print(
                        f"{BLUE}[INFO] User {username} can access pods in cluster {cluster_name}{RESET}"
                    )

                    # Display kubectl access command for user
                    print(f"{CYAN}📋 Kubectl access command:{RESET}")
                    print(
                        f"{CYAN}   aws eks update-kubeconfig --region {region} --name {cluster_name}{RESET}"
                    )

                    return True
                else:
                    print(
                        f"{RED}   [ERROR] kubectl pod access failed: {pods_result.stderr}{RESET}"
                    )
                    return False
            else:
                print(
                    f"{YELLOW}   ⚠️ kubectl not available. Skipping kubectl access verification.{RESET}"
                )
    ...
    def setup_cloudwatch_alarms(
        ...
        try:
            self.log_operation(
                "INFO", f"Setting up CloudWatch alarms for cluster {cluster_name}"
            )
            ...
            available_metrics = self._discover_available_metrics(
                cloudwatch_client, cluster_name, nodegroup_name
            )
            ...
            composite_success = self.create_composite_alarms(
                cloudwatch_client, cluster_name, alarms_created
            )

            # Calculate success rates
            basic_alarm_success_rate = (
                (alarms_created / total_alarms) * 100 if total_alarms > 0 else 0
            )
            ...
            self.print_colored(
                Colors.RED, f"❌ CloudWatch alarms setup failed: {str(e)}"
            )
            return False

    def setup_cloudwatch_alarms_v1(
        self,
        cluster_name: str,
        region: str,
        cloudwatch_client,
        ...
        try:
            ...
            for alarm_config in alarm_configs:
                ...
                try:
                    cloudwatch_client.put_metric_alarm(
                        ...
                        Unit=(
                            "Percent"
                            if "utilization" in alarm_config["metric_name"].lower()
                            else "Count"
                        ),
                    )
                    ...
                    self.log_operation(
                        "ERROR",
                        f"Failed to create alarm {alarm_config['name']}: {str(e)}",
                    )

            # Create composite alarms for critical conditions
            composite_alarms_success = self.create_composite_alarms(
                cloudwatch_client, cluster_name, alarm_configs
            )
            ...
            overall_success = (
                basic_alarm_success_rate >= 70 and composite_alarms_success
            )
            ...
            self.log_operation(
                "INFO",
                f"  - Basic alarms: {alarms_created}/{total_alarms} created ({basic_alarm_success_rate:.1f}%)",
            )
            self.log_operation(
                "INFO",
                f"  - Composite alarms: {'Success' if composite_alarms_success else 'Failed'}",
            )
            self.log_operation(
                "INFO",
                f"  - Overall status: {'Success' if overall_success else 'Partial/Failed'}",
            )
            ...
            self.alarm_details[cluster_name] = {
                ...
                "alarm_names": [
                    config["name"] for config in alarm_configs if alarms_created > 0
                ],
            }
    ...
    def _create_container_insights_alarms(
        self, cloudwatch_client, cluster_name: str, available_metrics: dict
    ) -> tuple:
        ...
        for alarm_config in ci_alarm_configs:
            ...
            try:
                ...
                if self._verify_metric_exists(
                    ...
                    self.log_operation(
                        "INFO",
                        f"Created Container Insights alarm: {alarm_config['name']}",
                    )
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Created CI alarm: {alarm_config['name']}"
                    )
                else:
                    self.log_operation(
                        "WARNING",
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Metric not found, skipped: {alarm_config['name']}",
                    )
                ...
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ Failed to create CI alarm: {alarm_config['name']}",
                )

        return (created, total)

    def _create_ec2_based_alarms(
        self,
        cloudwatch_client,
        cluster_name: str,
        nodegroup_name: str,
        ...
        if not available_metrics.get("ec2") or not available_metrics.get("asg_names"):
            self.log_operation(
                "WARNING", f"No EC2 metrics available for cluster {cluster_name}"
            )
            return (0, 0)
        ...
        for alarm_config in all_ec2_alarms:
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN, f"   ✅ Created alarm: {alarm_config['name']}"
                )

            except Exception as e:
                self.log_operation(
                    "ERROR",
                    f"Failed to create EC2 alarm {alarm_config['name']}: {str(e)}",
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ Failed to create alarm: {alarm_config['name']}",
                )

        return (created, total)

    def _discover_available_metrics(
        self, cloudwatch_client, cluster_name: str, nodegroup_name: str
    ) -> dict:
        ...
        try:
            ...
            try:
                ...
                for metric in ec2_metrics.get("Metrics", []):
                    for dimension in metric.get("Dimensions", []):
                        if dimension["Name"] == "AutoScalingGroupName":
                            ...
                            if (
                                cluster_name.split("-")[-1] in asg_name
                                or nodegroup_name in asg_name
                            ):
                ...
                self.log_operation(
                    "WARNING", f"Failed to discover EC2 metrics: {str(e)}"
                )
            ...
            try:
                ...
                if ci_metrics.get("Metrics"):
                    ...
                    self.log_operation(
                        "INFO",
                        f"Container Insights metrics available for cluster {cluster_name}",
                    )
                else:
                    self.log_operation(
                        "INFO",
                        f"Container Insights metrics not available for cluster {cluster_name}",
                    )

            except Exception as e:
                self.log_operation(
                    "WARNING", f"Failed to check Container Insights metrics: {str(e)}"
                )

            # Check for CloudWatch Agent metrics
            try:
                cwa_metrics = cloudwatch_client.list_metrics(
                    Namespace="CWAgent", MetricName="cpu_usage_active"
                )
                ...
                self.log_operation(
                    "WARNING", f"Failed to check CloudWatch Agent metrics: {str(e)}"
                )
            ...
            self.log_operation(
                "ERROR", f"Failed to discover available metrics: {str(e)}"
            )
            return {
                "ec2": False,
                "container_insights": False,
    ...
    def _verify_metric_exists(
        self, cloudwatch_client, namespace: str, metric_name: str, dimensions: list
    ) -> bool:
        ...
        try:
            ...
            if metrics_found > 0:
                self.log_operation(
                    "DEBUG",
                    f"Metric {namespace}/{metric_name} exists ({metrics_found} instances)",
                )
                return True
            else:
                self.log_operation(
                    "DEBUG", f"Metric {namespace}/{metric_name} not found"
                )
                return False
            ...
            self.log_operation(
                "WARNING",
                f"Failed to verify metric {namespace}/{metric_name}: {str(e)}",
            )
            return False

    def create_composite_alarms(
        self, cloudwatch_client, cluster_name: str, basic_alarms_created: int
    ) -> bool:
        """Create composite alarms only if we have basic alarms to reference"""
        try:
            if basic_alarms_created == 0:
                self.log_operation(
                    "WARNING", f"No basic alarms created, skipping composite alarms"
                )
            ...
            for composite_config in composite_configs:
                ...
                try:
                    ...
                    self.log_operation(
                        "INFO", f"Created composite alarm: {composite_config['name']}"
                    )
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Composite alarm: {composite_config['name']}",
                    )
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Composite alarm skipped: {composite_config['name']}",
                    )

            success_rate = (
                (composite_alarms_created / total_composite_alarms) * 100
                if total_composite_alarms > 0
                else 100
            )
            self.log_operation(
                "INFO",
    ...
    def create_composite_alarms_v1(
        self, cloudwatch_client, cluster_name: str, alarm_configs: list
    ) -> bool:
        """Create composite alarms for critical conditions"""
        try:
            composite_alarms_created = 0
            ...
            for composite_config in composite_configs:
                ...
                try:
                    ...
                    self.log_operation(
                        "INFO",
                        f"Created composite alarm: {composite_config['name']} (Severity: {composite_config['severity']})",
                    )
            ...
            self.log_operation(
                "INFO",
                f"Composite alarms: {composite_alarms_created}/{total_composite_alarms} created ({composite_success_rate:.1f}%)",
            )
                ...
                self.alarm_details[cluster_name].update(
                    {
                        ...
                        "composite_alarm_names": [
                            config["name"] for config in composite_configs
                        ],
                    }
                )
    ...
    def select_instance_type(self, user_name: str = None) -> str:
        ...
        while True:
            try:
                choice = input(
                    f"Select instance type (1-{len(allowed_types)}) [default: {default_type}]: "
                ).strip()
                ...
                if 1 <= choice_num <= len(allowed_types):
                    ...
                    print(
                        f"❌ Please enter a number between 1 and {len(allowed_types)}"
                    )
            except ValueError:
    ...
    def select_capacity_type(self, user_name: str = None) -> str:
        ...
        print(
            "  2. Spot (cheaper, but can be terminated, best for non-critical workloads)"
        )
        ...
        while True:
            try:
                choice = (
                    input(f"Select capacity type (1-3) [default: Spot]: ")
                    .strip()
                    .lower()
                )
    ...
    def load_ec2_config(self) -> Dict:
        ...
        try:
            if not os.path.exists(config_file):
                self.log_operation(
                    "WARNING", f"Config file {config_file} not found, using defaults"
                )
            ...
            with open(config_file, "r") as f:
                config = json.load(f)
                self.log_operation(
                    "INFO", f"Loaded EC2 configuration from {config_file}"
                )
                return config
        except Exception as e:
    ...
    def get_diversified_instance_types(self, primary_instance_type: str) -> List[str]:
        ...
        diversified_types = instance_families.get(
            primary_instance_type, [primary_instance_type]
        )

        self.log_operation(
            "INFO", f"Using diversified instance types: {diversified_types}"
        )
    ...
    def debug_nodegroup_configs(self, stage: str, nodegroup_configs) -> None:
        ...
        if nodegroup_configs is None:
            ...
            self.print_colored(
                "GREEN",
                f"[DEBUG {stage}] nodegroup_configs is set: {len(nodegroup_configs)} configs",
            )
            for i, config in enumerate(nodegroup_configs):
                self.print_colored(
                    "CYAN",
                    f"  Config {i+1}: {config.get('name')}, strategy: {config.get('strategy')}",
                )

    def get_cloudwatch_configmap_manifest_fixed(
        self, config: dict, cluster_name: str, region: str
    ) -> str:
        ...
        manifest = {
            "apiVersion": "v1",
            "kind": "ConfigMap",
            "metadata": {"name": "cwagentconfig", "namespace": "amazon-cloudwatch"},
            "data": {"cwagentconfig.json": config_json},
        }
    ...
    def apply_kubernetes_manifest_fixed(
        self,
        cluster_name: str,
        region: str,
        access_key: str,
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(
                    kubectl_cmd, capture_output=True, text=True, env=env, timeout=120
                )
            ...
            self.log_operation(
                "ERROR", f"Failed to apply Kubernetes manifest: {str(e)}"
            )

        return False

    def wait_for_daemonset_ready_fixed(
        self,
        cluster_name: str,
        region: str,
        access_key: str,
        ...
        try:
            ...
            result = subprocess.run(
                wait_cmd, capture_output=True, text=True, env=env, timeout=timeout + 30
            )
            ...
            if result.returncode == 0:
                ...
                self.log_operation(
                    "WARNING",
                    f"DaemonSet {daemonset_name} not ready within timeout: {result.stderr}",
                )
    ...
    def deploy_cloudwatch_agent_fixed_bk(
        self,
        cluster_name: str,
        region: str,
        access_key: str,
        ...
        try:
            self.log_operation(
                "INFO", f"Deploying CloudWatch agent for cluster {cluster_name}"
            )
            ...
            if not kubectl_available:
                self.log_operation(
                    "WARNING",
                    f"kubectl not found. Cannot deploy CloudWatch agent for {cluster_name}",
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"⚠️  kubectl not found. CloudWatch agent deployment skipped.",
                )
                return False
            ...
            service_account_manifest = (
                self.get_cloudwatch_service_account_manifest_fixed()
            )
            configmap_manifest = self.get_cloudwatch_configmap_manifest_fixed(
                cloudwatch_config, cluster_name, region
            )
            daemonset_manifest = self.get_cloudwatch_daemonset_manifest_fixed(
                cluster_name, region, account_id
            )
            ...
            try:
                ...
                self.log_operation(
                    "INFO", f"Applied CloudWatch service account manifest"
                )
                ...
                subprocess.run(
                    delete_cmd, env=env, capture_output=True, text=True, timeout=60
                )
                self.log_operation(
                    "INFO", f"Removed existing CloudWatch DaemonSet if present"
                )
                ...
                self.log_operation(
                    "ERROR", f"Failed to apply CloudWatch manifests: {str(e)}"
                )
                return False
    ...
    def deploy_cloudwatch_agent_fixed(
        self,
        cluster_name: str,
        region: str,
        access_key: str,
        ...
        try:
            ...
            self.print_colored(
                Colors.YELLOW, f"🔧 Deploying CloudWatch agent for {cluster_name}..."
            )
            ...
            try:
                # 1. Create namespace if it doesn't exist
                self.print_colored(
                    Colors.CYAN, "   📦 Creating CloudWatch namespace..."
                )
                ...
                if not namespace_success:
                    self.print_colored(
                        Colors.YELLOW,
                        "   ⚠️ Could not create namespace, but continuing...",
                    )

                # 2. Apply service account
                self.print_colored(
                    Colors.CYAN, "   📦 Creating CloudWatch service account..."
                )
                self.apply_kubernetes_manifest_fixed(
                    cluster_name,
                    region,
                ...
                # 3. Apply configmap
                self.print_colored(
                    Colors.CYAN, "   📦 Creating CloudWatch config map..."
                )
                ...
                if not configmap_success:
                    self.print_colored(
                        Colors.RED, "   ❌ Failed to create ConfigMap, cannot proceed"
                    )
                    return False
                ...
                self.print_colored(
                    Colors.CYAN,
                    "   🗑️ Completely removing existing CloudWatch agent DaemonSet...",
                )
                ...
                if check_result.returncode == 0 and check_result.stdout.strip():
                    ...
                    self.print_colored(
                        Colors.CYAN, f"   🗑️ Forcefully deleted existing DaemonSet"
                    )
                    ...
                    verify_result = subprocess.run(
                        verify_cmd, env=env, capture_output=True, text=True, timeout=30
                    )

                    if verify_result.returncode == 0 and verify_result.stdout.strip():
                        self.print_colored(
                            Colors.YELLOW,
                            "   ⚠️ DaemonSet still exists, waiting longer...",
                        )
                        time.sleep(10)  # Wait more if it still exists
                else:
                    self.print_colored(
                        Colors.CYAN,
                        "   ✅ No existing DaemonSet found, proceeding with creation",
                    )
                ...
                for attempt in range(max_retries):
                    self.print_colored(
                        Colors.CYAN,
                        f"   📦 Creating CloudWatch DaemonSet (attempt {attempt+1}/{max_retries})...",
                    )
                    ...
                    if daemonset_success:
                        self.print_colored(
                            Colors.GREEN,
                            "   ✅ CloudWatch agent DaemonSet deployed successfully",
                        )
                        break
                    elif attempt < max_retries - 1:
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️ Deployment failed, retrying in 5 seconds...",
                        )
                        time.sleep(5)
                    else:
                        self.print_colored(
                            Colors.RED,
                            "   ❌ Failed to deploy CloudWatch agent DaemonSet after retries",
                        )
                        return False
                ...
                self.print_colored(
                    Colors.CYAN,
                    "   ⏳ Waiting for CloudWatch agent DaemonSet to be ready...",
                )
                ...
                if ready_success:
                    self.print_colored(
                        Colors.GREEN, "   ✅ CloudWatch agent DaemonSet is ready"
                    )
                else:
                    self.print_colored(
                        Colors.YELLOW,
                        "   ⚠️ CloudWatch agent DaemonSet readiness check timed out",
                    )

                # Apply Fluent Bit for log collection
                self.print_colored(
                    Colors.CYAN, "   📦 Setting up Fluent Bit for log collection..."
                )
                fluentbit_yaml = self.render_fluentbit_configmap(
                    cluster_name=cluster_name,
                    region_name=region,
                ...
                if fluentbit_result:
                    self.print_colored(
                        Colors.GREEN, "   ✅ Fluent Bit configuration applied"
                    )
                else:
                    self.print_colored(
                        Colors.YELLOW, "   ⚠️ Could not apply Fluent Bit configuration"
                    )

                self.print_colored(
                    Colors.GREEN, "✅ CloudWatch monitoring deployment completed"
                )
                return True
                ...
                self.print_colored(
                    Colors.RED, f"❌ Failed to apply CloudWatch manifests: {str(e)}"
                )
                return False
            ...
            self.log_operation("ERROR", f"Failed to deploy CloudWatch agent: {str(e)}")
            self.print_colored(
                Colors.RED, f"❌ Failed to deploy CloudWatch agent: {str(e)}"
            )
            return False

    def get_cloudwatch_daemonset_manifest_fixed_chatgpt(
        self, cluster_name: str, region: str, account_id: str
    ) -> str:
        """Load and prepare the cloudwatch-daemonset.yaml manifest with proper annotations"""
        # Load the template
        ...
        try:
            ...
            updated_manifest = yaml.dump(
                daemonset_yaml, default_flow_style=False, indent=2
            )
            return updated_manifest
    ...
    ########
    def setup_scheduled_scaling_multi_nodegroup_bk(
        self,
        cluster_name: str,
        region: str,
        admin_access_key: str,
        ...
        try:
            if not nodegroup_names:
                self.log_operation(
                    "WARNING", f"No nodegroups to configure scheduled scaling for"
                )
                return False
            ...
            for ng_name in nodegroup_names:
                try:
                    ...
                    self.log_operation(
                        "WARNING",
                        f"Could not get current scaling config for {ng_name}: {str(e)}",
                    )
                    nodegroup_configs[ng_name] = {
                        "current_min": 0,
                        "current_desired": 0,
            ...
            self.print_colored(
                Colors.CYAN, "   Default scale-up: 11:00 AM IST (5:30 AM UTC)"
            )
            self.print_colored(
                Colors.CYAN, "   Default scale-down: 9:00 PM IST (3:30 PM UTC)"
            )
            ...
            if change_times == "y":
                ...
                scale_up_time_input = input(
                    "   Enter scale-up time (HH:MM AM/PM IST): "
                ).strip()
                scale_down_time_input = input(
                    "   Enter scale-down time (HH:MM AM/PM IST): "
                ).strip()
            ...
            self.print_colored(
                Colors.CYAN, "   Default scale-up: min=1, desired=1, max=8"
            )
            self.print_colored(
                Colors.CYAN, "   Default scale-down: min=0, desired=0, max=8"
            )
            ...
            if change_sizes == "y":
                ...
                try:
                    ...
                    scale_down_desired = int(
                        input("     Desired nodes for scale-down: ")
                    )
                    scale_down_max = int(input("     Max nodes for scale-down: "))

                except ValueError:
                    self.print_colored(
                        Colors.RED, "   ❌ Invalid input. Using default values."
                    )
                    # Use default values
            ...
            max_current_desired = max(
                [config["current_desired"] for config in nodegroup_configs.values()]
            )
            if max_current_desired > scale_down_desired:
                ...
                continue_with_settings = (
                    input("   Continue with these settings? (y/N): ").strip().lower()
                )
            ...
            self.print_colored(
                Colors.CYAN,
                f"      - Scale up: {scale_up_time} (cron: {scale_up_cron})",
            )
            self.print_colored(
                Colors.CYAN,
                f"      - Scale down: {scale_down_time} (cron: {scale_down_cron})",
            )
            ...
            self.print_colored(
                Colors.CYAN, "   [SECURITY] Creating IAM role for scheduled scaling..."
            )

            # Use a shorter name - EKS-{cluster_suffix}-ScaleRole
            short_cluster_suffix = cluster_name.split("-")[
                -1
            ]  # Just take the random suffix
            ...
            try:
                ...
                self.log_operation(
                    "INFO",
                    f"Created Lambda role for scheduled scaling: {lambda_role_arn}",
                )
                ...
                lambda_role_arn = role_response["Role"]["Arn"]
                self.log_operation(
                    "INFO", f"Using existing Lambda role: {lambda_role_arn}"
                )

            # Create a multi-nodegroup Lambda function
            self.print_colored(
                Colors.CYAN,
                "   🔧 Creating Lambda function for multi-nodegroup scaling...",
            )

            # Load lambda code from template file if it exists, otherwise use embedded template
            try:
                template_file = os.path.join(
                    os.path.dirname(__file__), "lambda_eks_scaling_template.py"
                )
                ...
                if not os.path.exists(template_file):
                    self.log_operation(
                        "INFO", f"Creating lambda template file: {template_file}"
                    )
                ...
                self.log_operation(
                    "INFO", f"Loaded Lambda template from file: {template_file}"
                )
            ...
            try:
                ...
                self.log_operation(
                    "INFO", f"Updated existing Lambda function: {function_arn}"
                )

            # Create EventBridge rules for scaling
            self.print_colored(
                Colors.CYAN, f"   📅 Creating scheduled scaling rules (IST timezone):"
            )
            self.print_colored(
                Colors.CYAN,
                f"      - Scale up: {scale_up_time} → {scale_up_desired} node(s)",
            )
            self.print_colored(
                Colors.CYAN,
                f"      - Scale down: {scale_down_time} → {scale_down_desired} node(s)",
            )
            ...
            self.print_colored(
                Colors.CYAN, f"   🌏 Timezone: Indian Standard Time (UTC+5:30)"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to setup multi-nodegroup scheduled scaling: {error_msg}",
            )
            self.print_colored(
                Colors.RED, f"❌ Scheduled scaling setup failed: {error_msg}"
            )
            return False

    def setup_scheduled_scaling_multi_nodegroup(
        self,
        cluster_name: str,
        region: str,
        admin_access_key: str,
        ...
        try:
            ...
            if change_times:
                while True:
                    scale_up_time = input(
                        "   Enter scale-up time (format: HH:MM AM/PM IST): "
                    ).strip()
                    if not scale_up_time:
                        scale_up_time = "11:00 AM IST"
                        scale_up_cron = (
                            "30 5 * * ? *"  # 5:30 AM UTC = 11:00 AM IST (weekdays only)
                        )
                        break
                    ...
                    try:
                        ...
                        self.print_colored(
                            Colors.YELLOW,
                            "   ⚠️  Invalid time format. Please use format like '8:30 AM IST'",
                        )

                while True:
                    scale_down_time = input(
                        "   Enter scale-down time (format: HH:MM AM/PM IST): "
                    ).strip()
                    ...
                    try:
                        ...
                        self.print_colored(
                            Colors.YELLOW,
                            "   ⚠️  Invalid time format. Please use format like '6:30 PM IST'",
                        )
            else:
                # scale_up_time = "11:00 AM IST"
                # scale_up_cron = "30 5 * * ? *"  # 5:30 AM UTC = 11:00 AM IST
            ...
            if change_sizes:
                try:
                    scale_up_min = int(
                        input("   Scale-up minimum nodes (default: 1): ").strip() or "1"
                    )
                    scale_up_desired = int(
                        input("   Scale-up desired nodes (default: 1): ").strip() or "1"
                    )
                    scale_up_max = int(
                        input("   Scale-up maximum nodes (default: 3): ").strip() or "3"
                    )

                    scale_down_min = int(
                        input("   Scale-down minimum nodes (default: 0): ").strip()
                        or "0"
                    )
                    scale_down_desired = int(
                        input("   Scale-down desired nodes (default: 0): ").strip()
                        or "0"
                    )
                    scale_down_max = int(
                        input("   Scale-down maximum nodes (default: 3): ").strip()
                        or "3"
                    )
                    ...
                    if (
                        scale_up_min < 0
                        or scale_up_desired < 0
                        or scale_up_max < 0
                        or scale_down_min < 0
                        or scale_down_desired < 0
                        or scale_down_max < 0
                    ):
                        self.print_colored(
                            Colors.YELLOW,
                            "   ⚠️  Negative values not allowed, using defaults.",
                        )
                        scale_up_min, scale_up_desired, scale_up_max = 1, 1, 8
                        scale_down_min, scale_down_desired, scale_down_max = 0, 0, 8

                    if (
                        scale_up_min > scale_up_desired
                        or scale_up_desired > scale_up_max
                    ):
                        self.print_colored(
                            Colors.YELLOW,
                            "   ⚠️  Invalid scale-up values (should be min ≤ desired ≤ max), adjusting...",
                        )
                        scale_up_max = max(scale_up_max, scale_up_desired, scale_up_min)
                        scale_up_min = min(scale_up_min, scale_up_desired)
                        scale_up_desired = max(
                            scale_up_min, min(scale_up_desired, scale_up_max)
                        )

                    if (
                        scale_down_min > scale_down_desired
                        or scale_down_desired > scale_down_max
                    ):
                        self.print_colored(
                            Colors.YELLOW,
                            "   ⚠️  Invalid scale-down values (should be min ≤ desired ≤ max), adjusting...",
                        )
                        scale_down_max = max(
                            scale_down_max, scale_down_desired, scale_down_min
                        )
                        scale_down_min = min(scale_down_min, scale_down_desired)
                        scale_down_desired = max(
                            scale_down_min, min(scale_down_desired, scale_down_max)
                        )

                except ValueError:
                    self.print_colored(
                        Colors.YELLOW, "   ⚠️  Invalid number format, using defaults."
                    )
                    scale_up_min, scale_up_desired, scale_up_max = 1, 1, 8
                    scale_down_min, scale_down_desired, scale_down_max = 0, 0, 8
            else:
            ...
            if size_reduction_detected:
                ...
                self.print_colored(
                    Colors.YELLOW,
                    "      This will cause nodes to be terminated during scale-down.",
                )
                confirm = "yes"
                # confirm = input("   Continue with these settings? (y/N): ").strip().lower()
                if confirm not in ["y", "yes"]:
                    self.print_colored(
                        Colors.YELLOW, "   ⚠️  Scheduled scaling setup canceled."
                    )
                    return False

            # Step 3: Create IAM role for Lambda function - with shorter name
            self.print_colored(
                Colors.CYAN, "   🔐 Creating IAM role for scheduled scaling..."
            )
            ...
            try:
                ...
                # Get current timestamp for replacement
                timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")

                # Replace placeholders in template with actual values
                lambda_code = (
                    lambda_template.replace("{{cluster_name}}", cluster_name)
                    .replace("{{region}}", region)
                    .replace("{{CURRENT_USER}}", self.current_user)
                    .replace("{{CURRENT_DATETIME}}", timestamp)
                )
                ...
                lambda_code = (
                    self._get_multi_nodegroup_lambda_template()
    ...
    def cleanup_temp_file(self, file_path: str) -> None:
        ...
        try:
            if os.path.exists(file_path):
                ...
                self.log_operation(
                    "WARNING", f"Temporary file not found during cleanup: {file_path}"
                )
        except Exception as e:
            self.log_operation(
                "ERROR", f"Error during cleanup of temp file {file_path}: {str(e)}"
            )

    def install_enhanced_addons(
        self,
        eks_client,
        cluster_name: str,
        region: str,
        ...
        try:
            self.log_operation(
                "INFO", f"Installing enhanced add-ons for cluster {cluster_name}"
            )
            self.print_colored(
                Colors.YELLOW, f"🔧 Installing enhanced add-ons for {cluster_name}..."
            )
            ...
            try:
                self.print_colored(
                    Colors.CYAN, f"   📦 Installing Amazon EFS CSI Driver..."
                )
                ...
                try:
                    ...
                    self.log_operation(
                        "INFO",
                        f"EFS CSI Driver policy already exists: {efs_policy_arn}",
                    )
                except iam_client.exceptions.NoSuchEntityException:
                    policy = iam_client.create_policy(
                    ...
                    self.log_operation(
                        "INFO", f"Created EFS CSI Driver policy: {efs_policy_arn}"
                    )

                # Attach policy to node role
                try:
                    iam_client.attach_role_policy(
                        RoleName="NodeInstanceRole", PolicyArn=efs_policy_arn
                    )
                    self.log_operation(
                        "INFO", f"Attached EFS CSI Driver policy to NodeInstanceRole"
                    )
                except Exception as e:
                    self.log_operation(
                        "WARNING", f"Failed to attach EFS CSI Driver policy: {str(e)}"
                    )
                ...
                try:
                    eks_client.create_addon(
                        ...
                        addonName="aws-efs-csi-driver",
                        addonVersion=(
                            addon_version if addon_version != "latest" else None
                        ),
                        resolveConflicts="OVERWRITE",
                    )
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Amazon EFS CSI Driver installed successfully",
                    )
                    self.log_operation(
                        "INFO",
                        f"Amazon EFS CSI Driver installed successfully for {cluster_name}",
                    )
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Failed to install Amazon EFS CSI Driver: {str(e)}",
                    )
                    self.log_operation(
                        "WARNING", f"Failed to install Amazon EFS CSI Driver: {str(e)}"
                    )

            except Exception as e:
                self.print_colored(
                    Colors.RED, f"   ❌ EFS CSI Driver installation error: {str(e)}"
                )
                self.log_operation(
                    "ERROR", f"EFS CSI Driver installation error: {str(e)}"
                )

            # 2. Install Node monitoring agent (CloudWatch agent with Node metrics)
            try:
                self.print_colored(
                    Colors.CYAN, f"   📦 Installing Node monitoring agent..."
                )
                ...
                if not kubectl_available:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ kubectl not found. Node monitoring agent installation skipped.",
                    )
                    self.log_operation(
                        "WARNING",
                        f"kubectl not found. Node monitoring agent installation skipped.",
                    )
                else:
                    # Setup environment
                    env = os.environ.copy()
                    ...
                    try:
                        ...
                        if apply_result.returncode == 0:
                            self.print_colored(
                                Colors.GREEN,
                                f"   ✅ Node monitoring agent configured successfully",
                            )
                            self.log_operation(
                                "INFO",
                                f"Node monitoring agent configured for {cluster_name}",
                            )
                            results["node_monitoring_agent"] = True
                        else:
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️ Failed to configure Node monitoring agent: {apply_result.stderr}",
                            )
                            self.log_operation(
                                "WARNING",
                                f"Failed to configure Node monitoring agent: {apply_result.stderr}",
                            )
                ...
                self.print_colored(
                    Colors.RED,
                    f"   ❌ Node monitoring agent installation error: {str(e)}",
                )
                self.log_operation(
                    "ERROR", f"Node monitoring agent installation error: {str(e)}"
                )

            # 3. Install Amazon EKS Pod Identity Agent
            try:
                self.print_colored(
                    Colors.CYAN, f"   📦 Installing Amazon EKS Pod Identity Agent..."
                )
                ...
                try:
                    eks_client.create_addon(
                        ...
                        addonVersion=(
                            identity_version if identity_version != "latest" else None
                        ),
                        resolveConflicts="OVERWRITE",
                    )
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Amazon EKS Pod Identity Agent installed successfully",
                    )
                    self.log_operation(
                        "INFO",
                        f"Amazon EKS Pod Identity Agent installed successfully for {cluster_name}",
                    )
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Failed to install Amazon EKS Pod Identity Agent: {str(e)}",
                    )
                    self.log_operation(
                        "WARNING",
                        f"Failed to install Amazon EKS Pod Identity Agent: {str(e)}",
                    )

            except Exception as e:
                self.print_colored(
                    Colors.RED,
                    f"   ❌ EKS Pod Identity Agent installation error: {str(e)}",
                )
                self.log_operation(
                    "ERROR", f"EKS Pod Identity Agent installation error: {str(e)}"
                )
            ...
            self.print_colored(
                Colors.GREEN,
                f"✅ Enhanced add-ons installation completed: {successful}/{total} successful",
            )
            self.log_operation(
                "INFO",
                f"Enhanced add-ons installation completed for {cluster_name}: {successful}/{total} successful",
            )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to install enhanced add-ons for {cluster_name}: {str(e)}",
            )
            self.print_colored(
                Colors.RED, f"❌ Enhanced add-ons installation failed: {str(e)}"
            )
            return results

    def verify_scheduled_scaling(
        self,
        cluster_name: str,
        region: str,
        admin_access_key: str,
        ...
        try:
            self.print_colored(
                Colors.YELLOW, f"🔍 Verifying scheduled scaling for {cluster_name}..."
            )
            ...
            try:
                ...
                if lambda_response and "Configuration" in lambda_response:
                    ...
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Lambda function verified: {function_name}"
                    )

                    # Show additional Lambda info
                    last_modified = lambda_response["Configuration"].get(
                        "LastModified", "Unknown"
                    )
                    runtime = lambda_response["Configuration"].get("Runtime", "Unknown")
                    memory = lambda_response["Configuration"].get(
                        "MemorySize", "Unknown"
                    )
                    timeout = lambda_response["Configuration"].get("Timeout", "Unknown")

                    self.print_colored(
                        Colors.CYAN, f"      - Last Modified: {last_modified}"
                    )
                    self.print_colored(Colors.CYAN, f"      - Runtime: {runtime}")
                    self.print_colored(
                        Colors.CYAN, f"      - Memory: {memory}MB, Timeout: {timeout}s"
                    )
            except Exception as e:
                self.print_colored(
                    Colors.RED, f"   ❌ Lambda function verification failed: {str(e)}"
                )
            ...
            try:
                ...
                if up_rule_response:
                    ...
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Scale-up rule verified: {scale_up_rule}"
                    )
                    ...
                    self.print_colored(
                        Colors.CYAN, f"      - Schedule: {schedule} ({state})"
                    )
                    ...
                    if "Targets" in targets:
                        first_target = (
                            targets["Targets"][0] if targets["Targets"] else {}
                        )
                        if "Input" in first_target:
                            ...
                            self.print_colored(
                                Colors.CYAN,
                                f"      - Action: Scale to {desired} nodes at {ist_time}",
                            )
                            self.print_colored(
                                Colors.CYAN, f"      - Min/Max: {min_size}/{max_size}"
                            )
            except Exception as e:
                self.print_colored(
                    Colors.RED, f"   ❌ Scale-up rule verification failed: {str(e)}"
                )
            ...
            try:
                ...
                if down_rule_response:
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ Scale-down rule verified: {scale_down_rule}",
                    )
                ...
                self.print_colored(
                    Colors.RED, f"   ❌ Scale-down rule verification failed: {str(e)}"
                )
            ...
            if all_verified:
                self.print_colored(
                    Colors.GREEN, f"✅ Scheduled scaling verification successful"
                )
                self.print_colored(
                    Colors.GREEN,
                    f"   All components verified: Lambda function, scale-up rule, scale-down rule",
                )
            else:
                self.print_colored(
                    Colors.YELLOW, f"⚠️ Scheduled scaling verification incomplete"
                )
                self.print_colored(
                    Colors.YELLOW, f"   Some components could not be verified"
                )
            ...
            self.print_colored(
                Colors.RED, f"❌ Scheduled scaling verification failed: {str(e)}"
            )
            return False

    def download_and_patch_fluentbit_config(
        self, manifest_url: str, cluster_name: str
    ) -> str:
        """
        ...
        try:
            self.log_operation(
                "INFO", f"Downloading and patching manifest from: {manifest_url}"
            )
            response = requests.get(manifest_url)
            if response.status_code != 200:
                raise Exception(
                    f"Failed to download manifest: HTTP {response.status_code}"
                )
            ...
            with tempfile.NamedTemporaryFile(
                ...
                self.log_operation(
                    "INFO", f"Patched manifest written to: {temp_file.name}"
                )
                return temp_file.name
        except Exception as e:
            self.log_operation(
                "ERROR", f"Error patching fluent-bit configmap: {str(e)}"
            )
            raise

    #######

    def verify_enhanced_cluster_components(
        self,
        cluster_name: str,
        region: str,
        access_key: str,
        ...
        """
        self.log_operation(
            "INFO", f"Verifying enhanced cluster components for {cluster_name}"
        )
        self.print_colored(
            Colors.YELLOW,
            f"🔍 Verifying enhanced cluster components for {cluster_name}...",
        )
        ...
        try:
            ...
            self.print_colored(
                Colors.CYAN, "📊 Verifying CloudWatch Container Insights..."
            )
            container_insights_verified = self._verify_container_insights(
            ...
            autoscaler_verified = self._verify_cluster_autoscaler(
                cluster_name, region, access_key, secret_key
            )
            ...
            try:
                ...
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ Lambda function not found: {function_name}"
                )
            ...
            try:
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Scheduling rules verified: {scale_up_rule}, {scale_down_rule}",
                )
            except Exception:
            ...
            if kubectl_available:
                ...
                try:
                    ...
                    agent_result = subprocess.run(
                        agent_cmd, env=env, capture_output=True, text=True, timeout=60
                    )
                    ...
                    if agent_result.returncode == 0:
                        ...
                        if running_pods:
                            ...
                            self.print_colored(
                                Colors.GREEN,
                                f"   ✅ CloudWatch agent pods: {len(running_pods)} running",
                            )
                        else:
                            self.print_colored(
                                Colors.YELLOW,
                                f"   ⚠️ No running CloudWatch agent pods found",
                            )
                    else:
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️ Could not check CloudWatch agent pods: {agent_result.stderr}",
                        )
                except Exception as e:
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️ Error checking CloudWatch agent: {str(e)}"
                    )
            else:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ kubectl not available, skipping CloudWatch agent check",
                )
            ...
            try:
                alarms_response = cloudwatch_client.describe_alarms(
                    AlarmNamePrefix=alarm_prefix, AlarmTypes=["MetricAlarm"]
                )
                ...
                composite_response = cloudwatch_client.describe_alarms(
                    AlarmNamePrefix=alarm_prefix, AlarmTypes=["CompositeAlarm"]
                )
                ...
                if len(metric_alarms) > 0:
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ CloudWatch metric alarms: {len(metric_alarms)} found",
                    )
                    verification_results["cloudwatch_alarms"] = True
                else:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ No CloudWatch metric alarms found with prefix {alarm_prefix}",
                    )

                if len(composite_alarms) > 0:
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ CloudWatch composite alarms: {len(composite_alarms)} found",
                    )
                else:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ No CloudWatch composite alarms found with prefix {alarm_prefix}",
                    )

            except Exception as e:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ Error checking CloudWatch alarms: {str(e)}"
                )
            ...
            try:
                ...
                for pattern in cost_alarm_patterns:
                    try:
                        cost_response = cloudwatch_client.describe_alarms(
                            AlarmNamePrefix=pattern, AlarmTypes=["MetricAlarm"]
                        )
                        ...
                        if pattern_alarms:
                            self.print_colored(
                                Colors.GREEN,
                                f"   ✅ Cost alarms for {pattern}: {len(pattern_alarms)} found",
                            )
                    except Exception:
                        continue
                ...
                if cost_alarms_found == 0:
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️ No cost monitoring alarms found"
                    )
            except Exception as e:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ Error checking cost alarms: {str(e)}"
                )
            ...
            try:
                ...
                essential_addons = ["vpc-cni", "coredns", "kube-proxy"]
                found_essential = [
                    addon for addon in addons if addon in essential_addons
                ]
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Essential add-ons: {len(found_essential)}/{len(essential_addons)} found",
                )

                if efs_addon:
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Amazon EFS CSI Driver: Installed"
                    )
                else:
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️ Amazon EFS CSI Driver: Not installed"
                    )

                if pod_identity:
                    self.print_colored(
                        Colors.GREEN, f"   ✅ Amazon EKS Pod Identity Agent: Installed"
                    )
                else:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Amazon EKS Pod Identity Agent: Not installed",
                    )

            except Exception as e:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ Error checking add-ons: {str(e)}"
                )
            ...
            addon_success = sum(
                1 for _, value in verification_results["addons"].items() if value
            )
            total_items = len(
                [
                    key
                    for key in verification_results
                    if key != "addons" and key != "overall"
                ]
            )
            ...
            self.print_colored(Colors.GREEN, f"\n📋 Verification Summary:")
            self.print_colored(
                Colors.GREEN if verification_results["overall"] else Colors.YELLOW,
                f"   ✓ Core functionality: {verification_results['overall']}",
            )
            self.print_colored(
                Colors.CYAN, f"   ✓ Components verified: {success_count}/{total_items}"
            )
            self.print_colored(
                Colors.CYAN, f"   ✓ Add-ons verified: {addon_success}/{total_addons}"
            )
            ...
            self.log_operation(
                "ERROR", f"Error during component verification: {str(e)}"
            )
            self.print_colored(
                Colors.RED, f"❌ Component verification failed: {str(e)}"
            )
            return verification_results

    def setup_and_verify_all_components(
        self,
        cluster_name: str,
        region: str,
        access_key: str,
        ...
        """
        self.log_operation(
            "INFO", f"Setting up and verifying all components for {cluster_name}"
        )
        self.print_colored(
            Colors.YELLOW,
        ...
        if False:
            ...
            try:
                protection_status = self.verify_node_protection_status(
                    cluster_name, region, access_key, secret_key
                )

                if protection_status:
                    components_status["node_protection"]["enabled"] = (
                        protection_status.get("protected_nodes", 0) > 0
                    )
                    components_status["node_protection"]["protected_nodegroups"] = (
                        protection_status.get("protected_nodegroups", [])
                    )
                    components_status["node_protection"]["protected_nodes_count"] = (
                        protection_status.get("protected_nodes", 0)
                    )
                    ...
                    if components_status["node_protection"]["enabled"]:
                        ...
                        for ng in protection_status.get("protected_nodegroups", []):
                            self.print_colored(
                                Colors.CYAN, f"      - Protected nodegroup: {ng}"
                            )
                    else:
                        self.print_colored(
                            Colors.YELLOW, f"   ⚠️ No node protection found"
                        )
                else:
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️ Could not verify node protection status"
                    )

            except Exception as e:
                self.log_operation(
                    "WARNING", f"Could not verify node protection: {str(e)}"
                )
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ Node protection verification failed: {str(e)}"
                )
        else:
            print(
            ...
            try:
                ...
                for function_name in possible_function_names:
                    try:
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Node protection monitoring Lambda found: {function_name}",
                        )
                        ...
                        runtime = lambda_response["Configuration"].get(
                            "Runtime", "Unknown"
                        )
                        self.print_colored(
                            Colors.CYAN, f"      - Last Modified: {last_modified}"
                ...
                if not monitoring_found:
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ No node protection monitoring Lambda found",
                    )
                    self.print_colored(
                        Colors.CYAN,
                        f"      - Searched for: {', '.join(possible_function_names)}",
                    )

            except Exception as e:
                self.log_operation(
                    "WARNING", f"Could not check node protection monitoring: {str(e)}"
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ Node protection monitoring check failed: {str(e)}",
                )
        else:
            print(
        ...
        if False:
            ...
            for key in components_status:
                if (
                    key != "node_protection"
                    and key in verification_results
                    and verification_results[key]
                ):
        ...
        if False:
            ...
            if node_protection["enabled"]:
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Protected Nodes: {node_protection['protected_nodes_count']}",
                )
                self.print_colored(
                    Colors.GREEN,
                ...
                if node_protection["monitoring_setup"]:
                    ...
                    self.print_colored(
                        Colors.YELLOW, f"   ⚠️ Monitoring Lambda: NOT FOUND"
                    )
            else:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ Protection Status: NOT ENABLED"
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ Consider enabling node protection for critical workloads",
                )
        else:
    ...
    def verify_node_protection_status(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> dict:
        """
        ...
        try:
            ...
            if not kubectl_available:
                self.log_operation(
                    "WARNING", f"kubectl not found. Cannot verify node protection"
                )
                return {}
            ...
            nodes_result = subprocess.run(
                nodes_cmd, env=env, capture_output=True, text=True, timeout=60
            )

            if nodes_result.returncode != 0:
                self.log_operation(
                    "WARNING", f"Could not get protected nodes: {nodes_result.stderr}"
                )
                return {
                    "protected_nodes": 0,
                    "protected_nodegroups": [],
            ...
            try:
                ...
                self.log_operation(
                    "ERROR", f"Failed to parse kubectl JSON output: {str(e)}"
                )
                return {}

        except Exception as e:
            self.log_operation(
                "ERROR", f"Failed to verify node protection status: {str(e)}"
            )
            return {}

    #######

    def verify_cloudwatch_insights(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> bool:
        ...
        try:
            self.log_operation(
                "INFO",
                f"Verifying CloudWatch Container Insights for cluster {cluster_name}",
            )
            self.print_colored(
                Colors.YELLOW, f"🔍 Verifying CloudWatch Container Insights..."
            )
            ...
            if not kubectl_available:
                self.log_operation(
                    "WARNING", f"kubectl not found. Cannot verify Container Insights"
                )
                self.print_colored(
                    Colors.YELLOW, f"⚠️ kubectl not found. Manual verification required."
                )
                return False
                ...
                try:
                    ...
                    if verify_result.returncode == 0 and verify_result.stdout.strip():
                        ...
                        if insights_pods:
                            ...
                            self.print_colored(
                                Colors.GREEN,
                                f"   ✅ Found Container Insights pods in namespace '{namespace}': {len(running_pods)}/{len(insights_pods)} running",
                            )
                            ...
                            for pod in insights_pods[:3]:  # Show first 3 pods
                                ...
                                if len(pod_parts) >= 3:
                                    ...
                                    self.print_colored(
                                        status_color,
                                        f"      - {pod_name}: {pod_status}",
                                    )
                        else:
                            self.print_colored(
                                Colors.CYAN,
                                f"   📍 No Container Insights pods found in namespace '{namespace}'",
                            )

                except Exception as e:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Error checking namespace '{namespace}': {str(e)}",
                    )

            if found_insights_pods and total_running_pods > 0:
                self.print_colored(
                    Colors.GREEN,
                    f"✅ CloudWatch Container Insights: {total_running_pods} total pods running",
                )
                ...
                self.print_colored(
                    Colors.CYAN,
                    f"   - CloudWatch → Insights → Container Insights → {cluster_name}",
                )
                self.print_colored(
                    Colors.CYAN,
                    f"   - CloudWatch → Logs → Log groups → /aws/containerinsights/{cluster_name}",
                )
                ...
                self.print_colored(
                    Colors.YELLOW,
                    f"⚠️ CloudWatch Container Insights: No running pods found",
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"   - Container Insights may still be starting, check again in a few minutes",
                )
                return False

        except Exception as e:
            self.log_operation(
                "ERROR", f"Failed to verify Container Insights: {str(e)}"
            )
            self.print_colored(
                Colors.RED, f"❌ Container Insights verification failed: {str(e)}"
            )
            return False

    def _verify_container_insights(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> bool:
        ...
        try:
            ...
            if not kubectl_available:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ kubectl not found, skipping Container Insights check",
                )
                return False
            ...
            for namespace in namespaces_to_check:
                try:
                    ...
                    pods_result = subprocess.run(
                        pods_cmd, env=env, capture_output=True, text=True, timeout=60
                    )
                    ...
                    if pods_result.returncode == 0 and pods_result.stdout.strip():
                        ...
                        if insights_pods:
                            running_pods = [
                                line for line in insights_pods if "Running" in line
                            ]
                            total_running_pods += len(running_pods)
                            ...
                            if running_pods:
                                self.print_colored(
                                    Colors.GREEN,
                                    f"   ✅ Container Insights pods in '{namespace}': {len(running_pods)}/{len(insights_pods)} running",
                                )
            ...
            if total_running_pods > 0:
                ...
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ No running Container Insights pods found"
                )
                return False

        except Exception as e:
            self.print_colored(
                Colors.YELLOW, f"   ⚠️ Error checking Container Insights: {str(e)}"
            )
            return False

    def _verify_cluster_autoscaler(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> bool:
        ...
        try:
            ...
            self.print_colored(
                Colors.CYAN, "   🔍 Running cluster autoscaler diagnostics..."
            )
            debug_info = self.debug_cluster_autoscaler(
                cluster_name, region, access_key, secret_key
            )

            # Print the debug info with proper formatting
            self.print_colored(
                Colors.CYAN, "   📋 Cluster Autoscaler Debug Information:"
            )
            ...
            if "error" in debug_info:
                ...
                if debug_info.get("deployment_exists", False):
                    self.print_colored(
                        Colors.GREEN, f"      ✅ Autoscaler deployment exists"
                    )
                else:
                    self.print_colored(
                        Colors.YELLOW, f"      ⚠️ Autoscaler deployment not found"
                    )
                    if "deployment_error" in debug_info:
                        self.print_colored(
                            Colors.YELLOW,
                            f"         Error: {debug_info['deployment_error']}",
                        )
                ...
                if debug_info.get("pods_output"):
                    ...
                    if (
                        len(pod_lines) > 1
                    ):  # If there are pods (more than just the header)
                        self.print_colored(
                            Colors.GREEN,
                            f"      ✅ Found {len(pod_lines) - 1} autoscaler pods:",
                        )
                        ...
                        if len(pod_lines) > 5:
                            self.print_colored(
                                Colors.CYAN,
                                f"         ...(and {len(pod_lines) - 5} more)",
                            )
                    else:
                        self.print_colored(
                            Colors.YELLOW, f"      ⚠️ No autoscaler pods found"
                        )
                ...
                if debug_info.get("logs"):
                    ...
                    if log_lines and log_lines[0].strip():
                        self.print_colored(
                            Colors.GREEN, f"      ✅ Autoscaler logs available:"
                        )
                        for i, line in enumerate(
                            log_lines[:3]
                            ...
                            if line.strip():
                                self.print_colored(
                                    Colors.CYAN, f"         {line[:100]}..."
                                )
                        if len(log_lines) > 3:
                            self.print_colored(
                                Colors.CYAN,
                                f"         ...(and {len(log_lines) - 3} more lines)",
                            )
                    else:
                        self.print_colored(
                            Colors.YELLOW, f"      ⚠️ No autoscaler logs found"
                        )
                ...
                if debug_info.get("events"):
                    ...
                    if len(event_lines) > 1:  # More than just header
                        self.print_colored(
                            Colors.GREEN, f"      ✅ Recent cluster events:"
                        )
                        for i, line in enumerate(
                            event_lines[:3]
                        ):  # Show first 3 events
                        ...
                        if len(event_lines) > 4:
                            self.print_colored(
                                Colors.CYAN,
                                f"         ...(and {len(event_lines) - 4} more events)",
                            )
            ...
            if not kubectl_available:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ kubectl not found, skipping Cluster Autoscaler check",
                )
                return False
            ...
            for selector in autoscaler_selectors:
                try:
                    ...
                    if (
                        autoscaler_result.returncode == 0
                        and autoscaler_result.stdout.strip()
                    ):
                        pod_lines = [
                            line.strip()
                            for line in autoscaler_result.stdout.strip().split("\n")
                            if line.strip()
                        ]
                        running_pods = [line for line in pod_lines if "Running" in line]
                        ...
                        if running_pods:
                            ...
                            self.print_colored(
                                Colors.GREEN,
                                f"   ✅ Cluster Autoscaler pods (selector: {selector}): {len(running_pods)} running",
                            )
            ...
            if not found_autoscaler:
                ...
                try:
                    ...
                    if all_pods_result.returncode == 0:
                        pod_lines = [
                            line.strip()
                            for line in all_pods_result.stdout.strip().split("\n")
                            if line.strip()
                        ]
                        autoscaler_pods = [
                            line for line in pod_lines if "autoscaler" in line.lower()
                        ]

                        if autoscaler_pods:
                            running_pods = [
                                line for line in autoscaler_pods if "Running" in line
                            ]
                            if running_pods:
                                found_autoscaler = True
                                self.print_colored(
                                    Colors.GREEN,
                                    f"   ✅ Cluster Autoscaler pods (by name): {len(running_pods)} running",
                                )

                except Exception as e:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Error searching for autoscaler pods: {str(e)}",
                    )
            ...
            if found_autoscaler:
                ...
                try:
                    ...
                    sa_result = subprocess.run(
                        sa_cmd, env=env, capture_output=True, text=True, timeout=30
                    )

                    if sa_result.returncode == 0 and sa_result.stdout.strip():
                        self.print_colored(
                            Colors.GREEN,
                            f"   ✅ Cluster Autoscaler service account verified",
                        )
                    else:
                        self.print_colored(
                            Colors.YELLOW,
                            f"   ⚠️ Cluster Autoscaler service account not found (but pods are running)",
                        )

                except Exception as e:
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Could not verify service account: {str(e)}",
                    )

                return True
            else:
                self.print_colored(
                    Colors.YELLOW, f"   ⚠️ No running Cluster Autoscaler pods found"
                )
                return False

        except Exception as e:
            self.print_colored(
                Colors.YELLOW, f"   ⚠️ Error checking Cluster Autoscaler: {str(e)}"
            )
            return False
    ...
    def enable_container_insights(
        ...
        try:
            self.log_operation(
                "INFO",
                f"Enabling CloudWatch Container Insights for cluster {cluster_name}",
            )
            self.print_colored(
                Colors.YELLOW,
                f"📊 Enabling CloudWatch Container Insights for {cluster_name}...",
            )
            ...
            if not kubectl_available:
                self.log_operation(
                    "WARNING",
                    f"kubectl not found. Cannot deploy Container Insights for {cluster_name}",
                )
                self.print_colored(
                    Colors.YELLOW,
                    f"⚠️  kubectl not found. Container Insights deployment skipped.",
                )
                return False
            ...
            try:
                ...
                self.print_colored(
                    Colors.CYAN, "   📦 Creating amazon-cloudwatch namespace..."
                )
                namespace_cmd = ["kubectl", "create", "namespace", "amazon-cloudwatch"]
                subprocess.run(
                    namespace_cmd, env=env, capture_output=True, text=True, timeout=60
                )
                ...
                self.log_operation(
                    "INFO",
                    f"amazon-cloudwatch namespace already exists or failed to create",
                )
            ...
            try:
                ...
                insights_result = subprocess.run(
                    insights_cmd, env=env, capture_output=True, text=True, timeout=300
                )

                if insights_result.returncode == 0:
                    self.print_colored(
                        Colors.GREEN,
                        "   ✅ Container Insights add-on deployed via AWS CLI",
                    )
                    self.log_operation(
                        "INFO", f"Container Insights add-on deployed for {cluster_name}"
                    )

                    # Wait for addon to be active
                    self.print_colored(
                        Colors.CYAN,
                        "   ⏳ Waiting for Container Insights add-on to be active...",
                    )
                    time.sleep(30)
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Add-on deployment failed, trying manual deployment...",
                    )
                    return self._deploy_container_insights_manual(
                        cluster_name, region, env
                    )

            except Exception as e:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ Add-on deployment error: {str(e)}, trying manual deployment...",
                )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to enable Container Insights for {cluster_name}: {error_msg}",
            )
            self.print_colored(
                Colors.RED, f"❌ Container Insights deployment failed: {error_msg}"
            )
            return False

    def _deploy_container_insights_manual(
        self, cluster_name: str, region: str, env: dict
    ) -> bool:
        ...
        try:
            ...
            self.print_colored(
                Colors.CYAN, "   📋 Applying Container Insights manifests manually..."
            )
            ...
            for i, manifest_url in enumerate(manifests, 1):
                try:
                    self.print_colored(
                        Colors.CYAN, f"   📦 Applying manifest {i}/{len(manifests)}..."
                    )
                    ...
                    if "fluent-bit-cluster-info-configmap.yaml" in manifest_url:
                        ...
                        if response.status_code == 200:
                            # Replace placeholders
                            patched_manifest = response.text.replace(
                                "{{cluster_name}}", cluster_name
                            )
                            patched_manifest = patched_manifest.replace(
                                "{{region_name}}", region
                            )
                            patched_manifest = patched_manifest.replace(
                                "{{http_server_toggle}}", "On"
                        ...
                        patched_path = self.download_and_patch_fluentbit_config(
                            manifest_url, cluster_name
                        )
                    ...
                    if apply_result.returncode == 0:
                        ...
                        self.log_operation(
                            "WARNING",
                            f"Failed to apply manifest {manifest_url}: {apply_result.stderr}",
                        )

                except Exception as e:
                    self.log_operation(
                        "WARNING", f"Failed to apply manifest {manifest_url}: {str(e)}"
                    )

            # FIXED: Wait longer for pods to be created and check status
            self.print_colored(
                Colors.CYAN, "   ⏳ Waiting for Container Insights pods to start..."
            )
            ...
            if success_count >= 2:  # At least 2 manifests applied successfully
                self.print_colored(
                    Colors.GREEN,
                    f"   ✅ Container Insights deployed manually ({success_count}/{len(manifests)} manifests applied)",
                )

                # Verify deployment
                verify_result = self.verify_cloudwatch_insights(
                    cluster_name,
                    region,
                ...
                if verify_result:
                    self.print_colored(
                        Colors.CYAN, f"📊 Access Container Insights in AWS Console:"
                    )
                    self.print_colored(
                        Colors.CYAN, f"   CloudWatch → Insights → Container Insights"
                    )
                    self.print_colored(
                        Colors.CYAN, f"   Filter by cluster: {cluster_name}"
                    )
                ...
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ Container Insights deployment incomplete ({success_count}/{len(manifests)} manifests applied)",
                )
                return False

        except Exception as e:
            self.log_operation(
                "ERROR", f"Manual Container Insights deployment failed: {str(e)}"
            )
            self.print_colored(Colors.RED, f"❌ Manual deployment failed: {str(e)}")
            return False

    def generate_instance_tags(
        self, cluster_name: str, nodegroup_name: str, strategy: str
    ) -> Dict:
        ...
        self.log_operation(
            "DEBUG",
            f"Generating instance tags for {short_name} in cluster {cluster_name} with strategy {strategy}",
        )
    ...
    def add_name_tag_to_nodegroup_asg(
        self,
        cluster_name: str,
        nodegroup_name: str,
        strategy: str,
        ...
        try:
            ...
            self.print_colored(
                Colors.CYAN, f"   🔍 Finding ASG for nodegroup {nodegroup_name}..."
            )
            ...
            for asg_resource in resources.get("autoScalingGroups", []):
                ...
                try:
                    ...
                    if asg_details["AutoScalingGroups"]:
                        asg_tags = {
                            tag["Key"]: tag["Value"]
                            for tag in asg_details["AutoScalingGroups"][0].get(
                                "Tags", []
                            )
                        }
                        ...
                        if (
                            asg_tags.get("eks:nodegroup-name") == nodegroup_name
                            and asg_tags.get("eks:cluster-name") == cluster_name
                        ):
                            asg_name = potential_asg_name
                            self.print_colored(
                                Colors.GREEN,
                                f"   ✅ Verified ASG belongs to nodegroup: {asg_name}",
                            )
                            break
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Could not verify ASG {potential_asg_name}: {str(e)}",
                    )
                    continue
            ...
            if not asg_name:
                self.print_colored(
                    Colors.YELLOW,
                    f"   ⚠️ No verified ASG found for nodegroup {nodegroup_name}",
                )
                return False
            ...
            self.print_colored(
                Colors.GREEN, f"   ✅ Added naming tags to verified ASG {asg_name}"
            )
            self.print_colored(
                Colors.GREEN, f"   📝 Instance base name: {base_instance_name}"
            )
            ...
            self.print_colored(
                Colors.RED, f"   ❌ Failed to add Name tag to ASG: {str(e)}"
            )
            self.log_operation(
                "ERROR",
                f"Failed to add Name tag to ASG for nodegroup {nodegroup_name}: {str(e)}",
            )
            return False
    ...
    def create_cluster(self, config: Dict) -> bool:
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"Starting creation of cluster {cluster_name} in {region}"
            )
            self.print_colored(
            ...
            self.print_colored(
                Colors.YELLOW, f"   Account: {account_name} ({account_id})"
            )
            ...
            self.log_operation(
                "DEBUG",
                f"Received nodegroup_configs in create_cluster: {nodegroup_configs}",
            )
            ...
            if not nodegroup_configs:
                ...
                default_choice = (
                    "1"
                    if default_strategy == "on-demand"
                    else "2" if default_strategy == "spot" else "3"
                )

                while True:
                    strategy_choice = input(
                        f"Select nodegroup strategy (1-3) [default: {default_choice}]: "
                    ).strip()
                ...
                self.print_colored(
                    Colors.GREEN, f"✅ Selected nodegroup strategy: {strategy.upper()}"
                )
                ...
                try:
                    min_size = int(
                        input(f"Minimum nodes [default: {default_min}]: ").strip()
                        or default_min
                    )
                    desired_size = int(
                        input(f"Desired nodes [default: {default_desired}]: ").strip()
                        or default_desired
                    )
                    max_size = int(
                        input(f"Maximum nodes [default: {default_max}]: ").strip()
                        or default_max
                    )
                    ...
                    if min_size > desired_size or desired_size > max_size:
                        print(
                            "❌ Invalid values (should be min ≤ desired ≤ max). Adjusting..."
                        )
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"✅ Nodegroup sizing: Min={min_size}, Desired={desired_size}, Max={max_size}",
                )
                ...
                if strategy == "mixed":
                    ...
                    try:
                        on_demand_percentage = int(
                            input(
                                f"Percentage of On-Demand capacity (0-100) [default: {default_percentage}%]: "
                            ).strip()
                            or default_percentage
                        )
                        if on_demand_percentage < 0 or on_demand_percentage > 100:
                            print(
                                "❌ Percentage must be between 0 and 100. Using default."
                            )
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"✅ Mixed strategy: {on_demand_percentage}% On-Demand, {100-on_demand_percentage}% Spot",
                    )
                    ...
                    self.print_colored(
                        Colors.GREEN, f"✅ Spot instance types: {spot_types}"
                    )

                elif strategy == "on-demand":
                    print("\n💻 On-Demand Instance Configuration:", instance_type)
                    instance_selections = {"on-demand": [instance_type]}
                ...
                default_subnet = "1"
                subnet_choice = input(
                    f"Select subnet preference (1-3) [default: {default_subnet}]: "
                ).strip()
                ...
                self.print_colored(
                    Colors.GREEN, f"✅ Subnet preference: {subnet_preference.upper()}"
                )
                ...
                self.log_operation(
                    "DEBUG", f"Generated nodegroup_configs: {nodegroup_configs}"
                )

                # 1.6 Ask for add-ons and Container Insights
                if not hasattr(self, "setup_addons") or not hasattr(
                    self, "setup_container_insights"
                ):
                    ...
                    self.setup_container_insights = config.get(
                        "enable_container_insights", True
                    )

                    if self.setup_addons:
                        self.print_colored(
                            Colors.GREEN, "✅ Essential add-ons will be installed"
                        )
                    else:
                        self.print_colored(
                            Colors.YELLOW, "⚠️ Essential add-ons will NOT be installed"
                        )

                    if self.setup_container_insights:
                        self.print_colored(
                            Colors.GREEN,
                            "✅ CloudWatch Container Insights will be enabled",
                        )
                    else:
                        self.print_colored(
                            Colors.YELLOW,
                            "⚠️ CloudWatch Container Insights will NOT be enabled",
                        )
                ...
                for ng_config in nodegroup_configs:
                    ...
                    if ng_config["strategy"] == "on-demand":
                        primary_instance = ng_config["instance_selections"].get(
                            "on-demand", [instance_type]
                        )[0]
                    ...
                    self.display_cost_estimation(
                        primary_instance,
                        ng_config["strategy"],
                        ng_config["desired_nodes"],
                    )
            else:
                ...
                self.log_operation(
                    "INFO", f"Using existing nodegroup_configs: {nodegroup_configs}"
                )
                strategy = nodegroup_configs[0][
                    "strategy"
                ]  # Use the strategy from the first nodegroup
            ...
            for i, ng_config in enumerate(nodegroup_configs, 1):
                ...
                print(
                    f"Scaling: Min={ng_config['min_nodes']}, Desired={ng_config['desired_nodes']}, Max={ng_config['max_nodes']}"
                )

                # Display instance type information
                if ng_config["strategy"] == "mixed":
                    on_demand_types = ng_config["instance_selections"].get(
                        "on-demand", ["Not specified"]
                    )
                    spot_types = ng_config["instance_selections"].get(
                        "spot", ["Not specified"]
                    )
                    on_demand_percentage = ng_config["instance_selections"].get(
                    ...
                    print(
                        f"Instance Types: On-Demand: {', '.join(on_demand_types)}, Spot: {', '.join(spot_types)}"
                    )
                elif ng_config["strategy"] == "spot":
                    spot_types = ng_config["instance_selections"].get(
                        "spot", ["Not specified"]
            ...
            if confirmation == "n":
                self.print_colored(
                    Colors.YELLOW, "❌ Cluster creation cancelled by user"
                )
                return False
            ...
            subnet_ids, security_group_id = self.get_or_create_vpc_resources(
                ec2_client, region
            )

            # Step 5: Create EKS control plane
            self.print_colored(
                Colors.CYAN, f"   🚀 Creating EKS control plane {cluster_name}..."
            )
            ...
            try:
                ...
                try:
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"   ⚠️ Cluster {cluster_name} already exists, skipping creation",
                    )
                except eks_client.exceptions.ResourceNotFoundException:
                ...
                if not cluster_exists:
                    ...
                    self.log_operation(
                        "INFO",
                        f"Creating EKS control plane {cluster_name} with version {eks_version}",
                    )
                    ...
                    eks_client.create_cluster(
                        ...
                        tags=self.generate_instance_tags(
                            cluster_name, "control-plane", "managed"
                        ),
                    )
                    ...
                    self.print_colored(
                        Colors.CYAN,
                        f"   ⏳ Waiting for cluster {cluster_name} to be active...",
                    )
                    waiter = eks_client.get_waiter("cluster_active")
                    waiter.wait(
                        name=cluster_name, WaiterConfig={"Delay": 30, "MaxAttempts": 40}
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"   ✅ EKS control plane {cluster_name} is now active",
                    )
                    self.log_operation(
                        "INFO", f"EKS control plane {cluster_name} created successfully"
                    )

            except Exception as e:
                self.log_operation(
                    "ERROR",
                    f"Failed to create EKS control plane {cluster_name}: {str(e)}",
                )
                self.print_colored(
                    Colors.RED, f"❌ Failed to create EKS control plane: {str(e)}"
                )
                return False
            ...
            auth_configured = self.enable_cluster_access_modes(
                cluster_name, region, account_id, config, access_key, secret_key
            )

            # Step 7: Create nodegroups based on strategy
            self.print_colored(
                Colors.CYAN, f"   🚀 Creating nodegroups with {strategy} strategy..."
            )
            ...
            try:
                for ng_config in nodegroup_configs:
                    self.log_operation(
                        "INFO", f"Creating nodegroup with config: {ng_config}"
                    )
                    ...
                    selected_subnets = self.select_subnets_for_nodegroup(
                        subnet_ids, subnet_preference, ec2_client
                    )

                    self.log_operation(
                        "INFO",
                        f"Creating nodegroup {nodegroup_name} with strategy {strategy}",
                    )
                    ...
                    if strategy == "on-demand":
                        success = self.create_ondemand_nodegroup(
                            ...
                            selected_subnets,
                            ami_type,
                            instance_selections.get("on-demand", ["c6a.large"]),
                            min_size,
                            desired_size,
                            max_size,
                            ec2_key_name,
                        ...
                        if success:
                            ...
                            self.add_name_tag_to_nodegroup_asg(
                                cluster_name,
                                nodegroup_name,
                                strategy,
                                region,
                        ...
                        success = self.create_spot_nodegroup(
                            ...
                            selected_subnets,
                            ami_type,
                            instance_selections.get("spot", ["c6a.large"]),
                            min_size,
                            desired_size,
                            max_size,
                            ec2_key_name,
                ...
                if not nodegroups_created:
                    self.log_operation(
                        "ERROR",
                        f"Failed to create nodegroups for cluster {cluster_name}",
                    )
                ...
                self.log_operation(
                    "INFO",
                    f"Successfully created nodegroups: {', '.join(nodegroups_created)}",
                )

            except Exception as e:
                self.log_operation("ERROR", f"Failed to create nodegroups: {str(e)}")
                self.print_colored(
                    Colors.RED, f"❌ Failed to create nodegroups: {str(e)}"
                )
                return False
            ...
            if False:
                ...
                self.print_colored(
                    Colors.CYAN,
                    f"   📋 Ensuring only one node per nodegroup has NO_DELETE label...",
                )
                self.protect_nodes_with_no_delete_label(
                    cluster_name, region, access_key, secret_key
                )
                ...
                if protection_result.get("success"):
                    self.print_colored(
                        Colors.GREEN, f"✅ Initial node protection applied"
                    )
                    ...
                    if monitoring_setup:
                        self.print_colored(
                            Colors.GREEN,
                            f"✅ Automated node protection monitoring enabled",
                        )
                        self.print_colored(
                            Colors.CYAN,
                ...
                self.print_colored(
                    Colors.YELLOW,
                    f"⚠️ Skipping node protection setup - manual monitoring required",
                )
            ...
            if self.setup_addons:
                ...
                self.log_operation(
                    "INFO",
                    f"Essential add-ons installation {'successful' if addons_installed else 'failed'}",
                )
            else:
                addons_installed = False
                self.log_operation(
                    "INFO", "Essential add-ons installation skipped by user"
                )
            ...
            features_status = {
                ...
                "container_insights_enabled": components_status.get(
                    "container_insights", False
                ),
                "autoscaler_enabled": components_status.get(
                    "cluster_autoscaler", False
                ),
                "scheduled_scaling_enabled": components_status.get(
                    "scheduled_scaling", False
                ),
                "cloudwatch_agent_enabled": components_status.get(
                    "cloudwatch_agent", False
                ),
                "cloudwatch_alarms_enabled": components_status.get(
                    "cloudwatch_alarms", False
                ),
                ...
                "no_delete_label_applied": components_status["node_protection"][
                    "enabled"
                ],
                "no_delete_lamba_monitoring": components_status["node_protection"][
                    "monitoring_setup"
                ],
            }
            ...
            self.generate_mini_instructions(
                credential_info, cluster_name, region, username
            )

            # Print enhanced cluster summary
            self.print_enhanced_cluster_summary_multi_nodegroup(
                cluster_name, features_status
            )

            # Log successful cluster creation
            self.log_operation(
                "INFO", f"Successfully created and configured cluster {cluster_name}"
            )
            return True
            ...
            self.log_operation(
                "ERROR",
                f"Failed to create cluster {config.get('cluster_name', 'unknown')}: {error_msg}",
            )
    ...
    def ensure_single_no_delete_per_nodegroup(
        cluster_name, region, access_key, secret_key
    ):
        """
        ...
        for node in nodes:
            ...
            nodegroups.setdefault(nodegroup, []).append(
                {"name": node_name, "has_no_delete": labels.get("NO_DELETE") == "true"}
            )
    ...
    def create_multiple_clusters(self, cluster_configs: List[Dict]) -> bool:
        ...
        for i, config in enumerate(cluster_configs, 1):
            ...
            self.logger.info(
                f"  {i}. {cluster_name} - Region: {region}, User: {username}"
            )
        ...
        for i, config in enumerate(cluster_configs, 1):
            ...
            self.logger.info(
                f"[{i}/{total_clusters}] Creating cluster {cluster_name} for {username} in {region}"
            )
            print(
            ...
            try:
                ...
                self.print_colored(
                    Colors.RED, f"❌ Error creating cluster {cluster_name}: {error_msg}"
                )
                ...
                if i < total_clusters:
                    continue_choice = (
                        input(
                            f"\n⚠️ Failed to create cluster {cluster_name}. Continue with remaining {total_clusters - i} clusters? (y/N): "
                        )
                        .strip()
                        .lower()
                    )
                    if continue_choice not in ["y", "yes"]:
                        self.logger.warning(
                            f"Batch creation aborted by user after failure of {cluster_name}"
                        )
                        break

        # Generate and log final summary
        success_rate = (
            len(created_clusters) / total_clusters * 100 if total_clusters > 0 else 0
        )
        ...
        summary = {
            ...
            "failed_clusters": {
                cluster: error_details.get(cluster, "Unknown error")
        ...
        if failed_clusters:
            try:
                ...
                report_file = os.path.join(
                    log_dir, f"cluster_creation_errors_{self.execution_timestamp}.json"
                )
                error_report = {
                    "timestamp": datetime.now().isoformat(),
                    "total_clusters": total_clusters,
    ...
    def process_single_user(self, user_data: Dict) -> bool:
        ...
        try:
            self.logger.info(
                f"Processing single user: {user_data.get('username', 'unknown')}"
            )
            ...
            if "nodegroup_configs" in user_data and user_data["nodegroup_configs"]:
                self.logger.info(
                    f"Found nodegroup_configs in user_data: {user_data['nodegroup_configs']}"
                )
            else:
                self.logger.warning(
                    f"No nodegroup_configs found in user_data for {username}"
                )
            ...
            if result:
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"✅ Successfully created cluster {cluster_name} for {username}",
                )
            else:
                self.logger.error(
                    json.dumps(
                ...
                self.print_colored(
                    Colors.RED,
                    f"❌ Failed to create cluster {cluster_name} for {username}",
                )
            ...
            self.print_colored(
                Colors.RED,
                f"❌ Error creating cluster for {user_data.get('username', 'unknown')}: {error_msg}",
            )
            return False

    def process_multiple_user_selection_bk(
        self, selected_users: List[Dict], automation_options
    ) -> bool:
        """
        ...
        for user in selected_users:
            user_config = {
                ...
                "cluster_name": self.generate_cluster_name(
                    user.get("username", "unknown"), user.get("region", "us-east-1")
                ),
                ...
                "enable_container_insights": automation_options.get(
                    "enable_container_insights", False
                ),
                "ami_type": automation_options.get(
                    "ami_type", "AL2_x86_64"
                ),  # Default to Amazon Linux 2 x86_64',
                "eks_version": automation_options.get(
                    "eks_version", "1.33"
                ),  # Default EKS version'
            }
            ...
            if "nodegroup_configs" in user and user["nodegroup_configs"] is not None:
                ...
                self.logger.info(
                    f"Added nodegroup_configs for user {user['username']}: {user['nodegroup_configs']}"
                )
            else:
        ...
        for i, config in enumerate(cluster_configs):
            has_nodegroup_configs = (
                "nodegroup_configs" in config
                and config["nodegroup_configs"] is not None
            )
            self.logger.info(
                f"Cluster config {i+1} has nodegroup_configs: {has_nodegroup_configs}"
            )

        # Create clusters with shared configuration
        return self.create_multiple_clusters(cluster_configs)

    def process_multiple_user_selection(
        self, selected_users: List[Dict], automation_options: Dict
    ) -> bool:
        """
        ...
        for i, user in enumerate(selected_users, 1):
            ...
            self.logger.info(
                f"  {i}. {username} - Region: {region}, Account: {account_name}"
            )

        # Check for selected add-ons and features
        self.setup_addons = automation_options.get("install_addons", True)
        self.setup_container_insights = automation_options.get(
            "enable_container_insights", True
        )
        ...
        for i, user in enumerate(selected_users, 1):
            ...
            self.logger.info(
                f"[{i}/{total_users}] Processing user {username} for cluster {cluster_name}"
            )
            print(
            ...
            try:
                    ...
                    json.dumps(
                        {
                            ...
                            "has_nodegroup_configs": "nodegroup_configs" in user
                            and bool(user["nodegroup_configs"]),
                        }
                    )
                )
                ...
                if "nodegroup_configs" in user and user["nodegroup_configs"]:
                    ...
                    self.logger.info(
                        f"Using nodegroup configurations from user: {nodegroup_count} nodegroups"
                    )
                    ...
                    self.logger.warning(
                        f"No nodegroup configurations found for user {username}"
                    )
                ...
                if result:
                    ...
                    self.print_colored(
                        Colors.GREEN, f"✅ Successfully created cluster {cluster_name}"
                    )
                else:
                    failed_clusters.append(cluster_name)
                    error_message = "Create cluster method returned False"
                    ...
                    self.print_colored(
                        Colors.RED, f"❌ Failed to create cluster {cluster_name}"
                    )
                    ...
                    if i < total_users:
                        continue_choice = (
                            input(
                                f"\n⚠️ Failed to create cluster for {username}. Continue with remaining {total_users - i} users? (y/N): "
                            )
                            .strip()
                            .lower()
                        )
                        if continue_choice not in ["y", "yes"]:
                            self.logger.warning(
                                f"Multi-user cluster creation aborted by user after failure of {cluster_name}"
                            )
                            break
                ...
                if i < total_users:
                    continue_choice = (
                        input(
                            f"\n⚠️ Error creating cluster for {username}. Continue with remaining {total_users - i} users? (y/N): "
                        )
                        .strip()
                        .lower()
                    )
                    if continue_choice not in ["y", "yes"]:
                        self.logger.warning(
                            f"Multi-user cluster creation aborted by user after exception for {cluster_name}"
                        )
                        break

        # Generate and log final summary
        success_rate = (
            len(created_clusters) / total_users * 100 if total_users > 0 else 0
        )
        ...
        print(
            f"Successfully created: {len(created_clusters)} clusters ({success_rate:.1f}%)"
        )
    ...
    def disable_public_access(self, eks_client, cluster_name: str) -> bool:
        ...
        try:
            self.log_operation(
                "INFO", f"Disabling public access for EKS cluster {cluster_name}..."
            )
            ...
            self.log_operation(
                "SUCCESS",
                f"Successfully disabled public access for EKS cluster {cluster_name}",
            )
            return True
            ...
            self.log_operation(
                "ERROR",
                f"Error disabling public access for cluster {cluster_name}: {str(e)}",
            )
            return False
