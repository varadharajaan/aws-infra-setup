...
class UltraCleanupELBManager:
    ...
    def get_all_load_balancers_in_region(self, elb_client, elbv2_client, region, account_info):
        ...
        try:
            ...
            print(f"   ⚖️  Found {len(load_balancers)} load balancers ({classic_count} Classic, "
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            if load_balancers:
                ...
                if deleted_count > 0:
                    self.log_operation('INFO', f"⏳ Waiting 30 seconds for load balancers to be deleted...")
                    print(f"   ⏳ Waiting 30 seconds for load balancers to be deleted...")
                    time.sleep(30)
    ...
    def run(self):
        ...
        try:
            ...
            self.print_colored(Colors.RED, "💥 SIMPLIFIED ELB CLEANUP - KUBERNETES ENHANCED 💥")
            self.print_colored(Colors.YELLOW, "🚨" * 30)
            ...
            self.print_colored(Colors.GREEN, f"🎯 Enhanced Kubernetes ELB Detection: ENABLED")
            self.print_colored(Colors.CYAN, f"🎯 Simplified Scope: Load Balancers + Target Groups + Security Groups ONLY")
            ...
            self.print_colored(Colors.CYAN, f"🎯 Scope: SIMPLIFIED - LBs + TGs + SGs ONLY")
            self.print_colored(Colors.YELLOW, "=" * 80)
            ...
            self.print_colored(Colors.GREEN, f"    ✅ VPCs, subnets, and other resources will be LEFT ALONE")
            self.print_colored(Colors.GREEN, f"    🎯 Enhanced detection for Kubernetes-managed load balancers")
            ...
            self.print_colored(Colors.GREEN, f"🎯 Kubernetes LBs deleted: {total_k8s_lbs}")
            self.print_colored(Colors.WHITE,
            ...
            if (self.cleanup_results['deleted_load_balancers'] or
                ...
                for account, summary in account_summary.items():
                    ...
                    if summary['kubernetes_lbs'] > 0:
                        self.print_colored(Colors.GREEN, f"      🎯 Kubernetes LBs: {summary['kubernetes_lbs']}")
