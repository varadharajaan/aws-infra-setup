...
class UltraCleanupELBManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{self.elb_dir}/ultra_elb_cleanup_log_{self.execution_timestamp}.log"
            )
    ...
    def is_kubernetes_managed_elb(
        self, lb_name: str, lb_tags: List[Dict], lb_description: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        for tag in lb_tags:
            ...
            if tag_key.startswith("kubernetes.io/cluster/"):
                ...
                detection_info["cluster_name"] = tag_key.replace(
    ...
    def get_load_balancer_tags(
        self, elb_client, lb_name: str, lb_type: str
    ) -> List[Dict]:
        ...
        try:
            ...
            self.log_operation(
                "WARNING", f"Could not get tags for {lb_type} LB {lb_name}: {e}"
            )
            return []
    ...
    def create_aws_clients(self, access_key, secret_key, region):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to create AWS clients for {region}: {e}"
            )
            raise

    def get_all_load_balancers_in_region(
        self, elb_client, elbv2_client, region, account_info
    ):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🔍 Scanning for load balancers in {region} ({account_name})"
            )
            ...
            try:
                ...
                for page in paginator.paginate():
                    for lb in page["LoadBalancerDescriptions"]:
                        ...
                        lb_tags = self.get_load_balancer_tags(
                            elb_client, lb_name, "classic"
                        )
                        ...
                        k8s_info = self.is_kubernetes_managed_elb(
                            lb_name, lb_tags, lb_description
                        )
                        ...
                        if k8s_info["is_kubernetes"]:
                            ...
                            self.log_operation(
                                "INFO",
                                f"🎯 Kubernetes Classic ELB detected: {lb_name} "
                                f"(method: {k8s_info['detection_method']}, "
                                f"confidence: {k8s_info['confidence']})",
                            )
                            if k8s_info["cluster_name"]:
                                self.log_operation(
                                    "INFO", f"   Cluster: {k8s_info['cluster_name']}"
                                )
                            if k8s_info["service_name"]:
                                self.log_operation(
                                    "INFO", f"   Service: {k8s_info['service_name']}"
                                )

            except Exception as e:
                self.log_operation(
                    "WARNING", f"Error getting Classic Load Balancers: {e}"
                )
            ...
            try:
                ...
                for page in paginator.paginate():
                    for lb in page["LoadBalancers"]:
                        ...
                        try:
                            ...
                            self.log_operation(
                                "WARNING",
                                f"Could not get tags for {lb_type} LB {lb_name}: {e}",
                            )
                        ...
                        if k8s_info["is_kubernetes"]:
                            ...
                            self.log_operation(
                                "INFO",
                                f"🎯 Kubernetes {lb_type.upper()} detected: {lb_name} "
                                f"(method: {k8s_info['detection_method']}, "
                                f"confidence: {k8s_info['confidence']})",
                            )
                ...
                self.log_operation(
                    "WARNING", f"Error getting ALB/NLB Load Balancers: {e}"
                )

            # Enhanced logging with breakdown
            self.log_operation(
                "INFO",
                f"⚖️  Found {len(load_balancers)} total load balancers in {region} ({account_name})",
            )
            self.log_operation("INFO", f"   📊 Classic ELBs: {classic_count}")
            self.log_operation("INFO", f"   📊 Application LBs: {alb_count}")
            self.log_operation("INFO", f"   📊 Network LBs: {nlb_count}")
            self.log_operation("INFO", f"   🎯 Kubernetes-managed: {kubernetes_count}")

            print(
                f"   ⚖️  Found {len(load_balancers)} load balancers ({classic_count} Classic, "
            ...
            self.log_operation(
                "ERROR",
                f"Error getting load balancers in {region} ({account_name}): {e}",
            )
    ...
    def get_all_target_groups_in_region(self, elbv2_client, region, account_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🔍 Scanning for target groups in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "INFO",
                f"🎯 Found {len(target_groups)} target groups in {region} ({account_name})",
            )
            print(
                f"   🎯 Found {len(target_groups)} target groups in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Error getting target groups in {region} ({account_name}): {e}",
            )
            print(f"   ❌ Error getting target groups in {region}: {e}")
            return []

    def get_elb_security_groups_in_region(
        self, ec2_client, load_balancers, region, account_info
    ):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🔍 Scanning for ELB security groups in {region} ({account_name})",
            )
            print(
                f"   🔍 Scanning for ELB security groups in {region} ({account_name})..."
            )
                    ...
                    try:
                        ...
                        sg_info = {
                            ...
                            "attached_load_balancers": [
                                lb["name"]
                                for lb in load_balancers
                                if sg_id in lb.get("security_groups", [])
                            ],
                        }
                        ...
                        self.log_operation(
                            "WARNING",
                            f"Could not get details for security group {sg_id}: {e}",
                        )
            ...
            self.log_operation(
                "ERROR",
                f"Error getting ELB security groups in {region} ({account_name}): {e}",
            )
    ...
    def delete_load_balancer(self, elb_client, elbv2_client, lb_info):
        ...
        try:
            ...
            if k8s_info.get("is_kubernetes"):
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting Kubernetes-managed {lb_type} load balancer {lb_name} in {region} ({account_name})",
                )
                self.log_operation(
                    "INFO",
                ...
                print(f"   🗑️  Deleting Kubernetes {lb_type} load balancer {lb_name}...")
            else:
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting {lb_type} load balancer {lb_name} in {region} ({account_name})",
                )
                ...
                try:
                    if lb_type == "classic":
                        ...
                        elbv2_client.delete_load_balancer(
                            LoadBalancerArn=lb_info["arn"]
                        )

                    self.log_operation(
                        "INFO",
                        f"✅ Successfully deleted {lb_type} load balancer {lb_name}",
                    )
                    print(
                        f"   ✅ Successfully deleted {lb_type} load balancer {lb_name}"
                    )
                    ...
                    if error_code == "LoadBalancerNotFound":
                        self.log_operation(
                            "INFO", f"Load balancer {lb_name} already deleted"
                        )
                        return True
                    elif (
                        error_code in ["InvalidLoadBalancerName", "ValidationError"]
            ...
            account_name = lb_info["account_info"].get("account_key", "Unknown")
            self.log_operation(
                "ERROR", f"Failed to delete load balancer {lb_name}: {e}"
            )
    ...
    def delete_target_group(self, elbv2_client, tg_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🗑️  Deleting target group {tg_name} in {region} ({account_name})",
            )
            ...
            self.log_operation(
                "INFO", f"✅ Successfully deleted target group {tg_name}"
            )
    ...
    def clear_security_group_rules(self, ec2_client, sg_id):
        ...
        try:
            ...
            if rules_cleared > 0:
                self.log_operation(
                    "INFO", f"Waiting 5 seconds for rule changes to propagate..."
                )
                time.sleep(5)
            ...
            self.log_operation(
                "ERROR", f"Error clearing rules for security group {sg_id}: {e}"
            )
            return False
            ...
            self.cleanup_results["deleted_security_groups"].append(
                {
                    ...
                    "attached_load_balancers": sg_info.get(
                        "attached_load_balancers", []
                    ),
                    "region": region,
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🧹 Starting ELB cleanup for {account_key} ({account_id}) in {region}",
            )
            print(
            ...
            # Create AWS clients
            ec2_client, elb_client, elbv2_client = self.create_aws_clients(
            ...
            try:
                ...
                load_balancers = self.get_all_load_balancers_in_region(
                    elb_client, elbv2_client, region, account_info
                )

                # Get all target groups
                target_groups = self.get_all_target_groups_in_region(
                    elbv2_client, region, account_info
                )

                # Get ONLY security groups attached to load balancers
                elb_security_groups = self.get_elb_security_groups_in_region(
                    ec2_client, load_balancers, region, account_info
                )
            ...
            kubernetes_lbs = [
                lb
                for lb in load_balancers
                if lb.get("kubernetes_info", {}).get("is_kubernetes", False)
            ]
            ...
            self.log_operation(
                "INFO", f"📊 {account_key} ({region}) ELB resources summary:"
            )
            self.log_operation("INFO", f"   ⚖️  Load Balancers: {len(load_balancers)}")
            self.log_operation("INFO", f"   🎯 Kubernetes LBs: {len(kubernetes_lbs)}")
            self.log_operation("INFO", f"   🎯 Target Groups: {len(target_groups)}")
            self.log_operation(
                "INFO", f"   🛡️  ELB Security Groups: {len(elb_security_groups)}"
            )

            print(
                f"   📊 ELB resources found: {len(load_balancers)} LBs ({len(kubernetes_lbs)} K8s), "
            ...
            if not load_balancers and not target_groups and not elb_security_groups:
                self.log_operation(
                    "INFO", f"No ELB resources found in {account_key} ({region})"
                )
            ...
            if load_balancers:
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting {len(load_balancers)} load balancers in {account_key} ({region})",
                )
                print(f"\n   🗑️  Deleting {len(load_balancers)} load balancers...")
                ...
                for i, lb in enumerate(load_balancers, 1):
                    ...
                    if k8s_info.get("is_kubernetes"):
                        print(
                            f"   [{i}/{len(load_balancers)}] Processing Kubernetes load balancer {lb_name}..."
                        )
                    else:
                        print(
                            f"   [{i}/{len(load_balancers)}] Processing load balancer {lb_name}..."
                        )

                    try:
                        success = self.delete_load_balancer(
                            elb_client, elbv2_client, lb
                        )
                        if success:
                            deleted_count += 1
                        else:
                        ...
                        failed_count += 1
                        self.log_operation(
                            "ERROR", f"Error deleting load balancer {lb_name}: {e}"
                        )
                        print(f"   ❌ Error deleting load balancer {lb_name}: {e}")

                print(
                    f"   ✅ Deleted {deleted_count} load balancers, ❌ Failed: {failed_count}"
                )
                ...
                if deleted_count > 0:
                    self.log_operation(
                        "INFO",
                        f"⏳ Waiting 30 seconds for load balancers to be deleted...",
                    )
                    print(
                        f"   ⏳ Waiting 30 seconds for load balancers to be deleted..."
                    )
            ...
            if target_groups:
                self.log_operation(
                    "INFO",
                    f"🗑️  Deleting {len(target_groups)} target groups in {account_key} ({region})",
                )
                print(f"\n   🗑️  Deleting {len(target_groups)} target groups...")
                ...
                for i, tg in enumerate(target_groups, 1):
                    tg_name = tg["name"]
                    print(
                        f"   [{i}/{len(target_groups)}] Processing target group {tg_name}..."
                    )
                    ...
                    try:
                        ...
                        failed_count += 1
                        self.log_operation(
                            "ERROR", f"Error deleting target group {tg_name}: {e}"
                        )
                        print(f"   ❌ Error deleting target group {tg_name}: {e}")

                print(
                    f"   ✅ Deleted {deleted_count} target groups, ❌ Failed: {failed_count}"
                )
            ...
            if elb_security_groups:
                ...
                for retry in range(max_retries):
                    ...
                    for i, sg in enumerate(remaining_sgs, 1):
                        sg_id = sg["group_id"]
                        print(
                            f"   [{i}/{len(remaining_sgs)}] Trying to delete ELB security group {sg_id}..."
                        )
                        ...
                        try:
                            ...
                            self.log_operation(
                                "ERROR",
                                f"Error deleting ELB security group {sg_id}: {e}",
                            )
                            print(
                                f"   ❌ Error deleting ELB security group {sg_id}: {e}"
                            )
                    ...
                    if not remaining_sgs:
                        self.log_operation(
                            "INFO",
                            f"✅ All ELB security groups deleted in {account_key} ({region})",
                        )
            ...
            self.log_operation(
                "INFO", f"✅ ELB cleanup completed for {account_key} ({region})"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Error cleaning up ELB resources in {account_key} ({region}): {e}",
            )
            print(
                f"   ❌ Error cleaning up ELB resources in {account_key} ({region}): {e}"
            )
        ...
        try:
            ...
            report_data = {
                ...
                "detailed_results": {
                    ...
                    "deleted_load_balancers": self.cleanup_results[
                        "deleted_load_balancers"
                    ],
                    "deleted_target_groups": self.cleanup_results[
                        "deleted_target_groups"
                    ],
            ...
            self.log_operation(
                "INFO", f"✅ Ultra ELB cleanup report saved to: {report_filename}"
            )
            return report_filename

        except Exception as e:
            self.log_operation(
                "ERROR", f"❌ Failed to save ultra ELB cleanup report: {e}"
            )
            return None
    ...
    def run(self):
        ...
        try:
            self.log_operation("INFO", "🚨 STARTING SIMPLIFIED ELB CLEANUP SESSION 🚨")

            self.print_colored(Colors.YELLOW, "🚨" * 30)
            self.print_colored(
                Colors.RED, "💥 SIMPLIFIED ELB CLEANUP - KUBERNETES ENHANCED 💥"
            )
            self.print_colored(Colors.YELLOW, "🚨" * 30)
            self.print_colored(
            ...
            self.print_colored(
                Colors.GREEN, f"🎯 Enhanced Kubernetes ELB Detection: ENABLED"
            )
            self.print_colored(
                Colors.CYAN,
                f"🎯 Simplified Scope: Load Balancers + Target Groups + Security Groups ONLY",
            )
            ...
            self.print_colored(
                Colors.YELLOW, f"\n🎯 SIMPLIFIED ELB CLEANUP CONFIGURATION"
            )
            self.print_colored(Colors.YELLOW, "=" * 80)
            ...
            self.print_colored(Colors.GREEN, f"🎯 Kubernetes ELB Detection: ENHANCED")
            self.print_colored(
                Colors.CYAN, f"🎯 Scope: SIMPLIFIED - LBs + TGs + SGs ONLY"
            )
            self.print_colored(Colors.YELLOW, "=" * 80)
            ...
            self.print_colored(
                Colors.RED,
                f"\n⚠️  WARNING: This will delete the following ELB resources ONLY:",
            )
            self.print_colored(
                Colors.WHITE,
                f"    • Classic Load Balancers (ELB) - INCLUDING Kubernetes-managed",
            )
            ...
            self.print_colored(
                Colors.WHITE, f"    • Security Groups attached to Load Balancers ONLY"
            )
            self.print_colored(
                Colors.GREEN,
                f"    ✅ VPCs, subnets, and other resources will be LEFT ALONE",
            )
            self.print_colored(
                Colors.GREEN,
                f"    🎯 Enhanced detection for Kubernetes-managed load balancers",
            )
            self.print_colored(
                Colors.WHITE,
            ...
            confirm1 = (
                input(f"\nContinue with simplified ELB cleanup? (y/n): ")
                .strip()
                .lower()
            )
            ...
            if confirm2 != "yes":
                self.log_operation(
                    "INFO", "Simplified ELB cleanup cancelled at final confirmation"
                )
                self.print_colored(Colors.RED, "❌ Cleanup cancelled")
                return
            ...
            self.print_colored(
                Colors.RED,
                f"\n💥 STARTING SIMPLIFIED KUBERNETES-ENHANCED ELB CLEANUP...",
            )
            self.log_operation(
                "INFO",
                f"🚨 SIMPLIFIED ELB CLEANUP INITIATED - {len(selected_accounts)} accounts, {len(selected_regions)} regions",
            )
            ...
            self.print_colored(
                Colors.YELLOW,
                f"\n💥" + "=" * 25 + " SIMPLIFIED ELB CLEANUP COMPLETE " + "=" * 25,
            )
            self.print_colored(
            ...
            self.print_colored(
                Colors.WHITE,
                f"⚖️  Load balancers deleted: {len(self.cleanup_results['deleted_load_balancers'])}",
            )
            self.print_colored(
                Colors.GREEN, f"🎯 Kubernetes LBs deleted: {total_k8s_lbs}"
            )
            self.print_colored(
                Colors.WHITE,
                f"🎯 Target groups deleted: {len(self.cleanup_results['deleted_target_groups'])}",
            )
            self.print_colored(
                Colors.WHITE,
            ...
            self.log_operation(
                "INFO",
                f"Load balancers deleted: {len(self.cleanup_results['deleted_load_balancers'])}",
            )
            self.log_operation(
                "INFO", f"Kubernetes load balancers deleted: {total_k8s_lbs}"
            )
            self.log_operation(
                "INFO",
                f"Target groups deleted: {len(self.cleanup_results['deleted_target_groups'])}",
            ...
            if (
                self.cleanup_results["deleted_load_balancers"]
                or self.cleanup_results["deleted_target_groups"]
                ...
                for account, summary in account_summary.items():
                    ...
                    self.print_colored(
                        Colors.WHITE,
                        f"      ⚖️  Load Balancers: {summary['load_balancers']}",
                    )
                    if summary["kubernetes_lbs"] > 0:
                        self.print_colored(
                            Colors.GREEN,
                            f"      🎯 Kubernetes LBs: {summary['kubernetes_lbs']}",
                        )
                    self.print_colored(
                        Colors.WHITE,
                        f"      🎯 Target Groups: {summary['target_groups']}",
                    )
                    self.print_colored(
                        Colors.WHITE,
                        f"      🛡️  Security Groups: {summary['security_groups']}",
                    )
                    self.print_colored(
            ...
            self.print_colored(
                Colors.CYAN, f"\n📄 Saving simplified ELB cleanup report..."
            )
            report_file = self.save_cleanup_report()
            if report_file:
                self.print_colored(
                    Colors.GREEN, f"✅ ELB cleanup report saved to: {report_file}"
                )
            ...
            self.print_colored(
                Colors.RED,
                f"\n💥 SIMPLIFIED KUBERNETES-ENHANCED ELB CLEANUP COMPLETE! 💥",
            )
            self.print_colored(Colors.YELLOW, "🚨" * 30)

        except Exception as e:
            self.log_operation(
                "ERROR", f"FATAL ERROR in simplified ELB cleanup execution: {str(e)}"
            )
            self.print_colored(Colors.RED, f"\n❌ FATAL ERROR: {e}")
            import traceback
