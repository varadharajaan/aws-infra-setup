...
class IAMLogFileCleanupManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{log_dir}/iam_log_cleanup_{self.execution_timestamp}.log"
            )
            ...
            self.logger.info(
                f"🧹 IAM LOG FILE CLEANUP SESSION STARTED (Pattern: {self.file_pattern})"
            )
            self.logger.info("=" * 100)
    ...
    def load_aws_accounts_config(self):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"✅ AWS accounts configuration loaded from: {self.config_file}"
            )
            ...
            self.log_operation(
                "ERROR", f"Failed to load AWS accounts configuration: {e}"
            )
            raise
    ...
    def find_iam_credential_files(self):
        ...
        self.log_operation(
            "INFO",
            f"🔍 Scanning for IAM credential files matching '{self.file_pattern}' in {self.creds_base_dir}",
        )
        ...
        if not os.path.exists(self.creds_base_dir):
            ...
            try:
                ...
                self.log_operation(
                    "ERROR", f"Could not create IAM credentials directory: {e}"
                )

            self.log_operation(
                "WARNING",
                f"No IAM credential files found as directory was just created: {self.creds_base_dir}",
            )
            return []
                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            try:
                                ...
                                self.log_operation(
                                    "WARNING", f"Invalid JSON in file: {file_path}"
                                )
                    except Exception as e:
                        self.log_operation(
        ...
        self.log_operation(
            "INFO",
            f"📊 Found {valid_files} valid IAM credential files out of {total_files} total files",
        )
        ...
        for cred_info in cred_files:
            ...
            for account_name, account_data in accounts_data.items():
                ...
                for user in account_data.get("users", []):
                    ...
                    if (
                        user_key not in users_by_name
                        or file_date > users_by_name[user_key]["file_date"]
                    ):
        ...
        for cred_info in cred_files:
            ...
            for account_name, account_data in accounts_data.items():
                ...
                for group in account_data.get("groups", []):
                    ...
                    if (
                        group_key not in groups_by_name
                        or file_date > groups_by_name[group_key]["file_date"]
                    ):
                        group_info = {
    ...
    def create_iam_client(self, account_name):
        ...
        try:
            ...
            if not account_data:
                self.log_operation(
                    "ERROR", f"Account {account_name} not found in configuration"
                )
                return None
            ...
            self.log_operation(
                "ERROR", f"Failed to create IAM client for {account_name}: {e}"
            )
            return None
    ...
    def delete_user_access_keys(self, iam_client, username):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Error deleting access keys for user {username}: {e}"
            )
            return False
    ...
    def delete_login_profile(self, iam_client, username):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Error deleting login profile for user {username}: {e}"
            )
            return False
    ...
    def remove_user_from_groups(self, iam_client, username):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Error removing user {username} from groups: {e}"
            )
            return False
    ...
    def detach_user_policies(self, iam_client, username):
        ...
        try:
            ...
            for policy in response.get("AttachedPolicies", []):
                ...
                self.log_operation(
                    "INFO",
                    f"Detaching policy {policy_name} (ARN: {policy_arn}) from user {username}",
                )
                try:
                    ...
                    self.log_operation(
                        "WARNING",
                        f"Failed to detach policy {policy_name} ({policy_arn}) from user {username}: {e}",
                    )
            ...
            self.log_operation(
                "ERROR", f"Error detaching policies from user {username}: {e}"
            )
            return False
    ...
    def delete_user_inline_policies(self, iam_client, username):
        ...
        try:
            ...
            for policy_name in response.get("PolicyNames", []):
                self.log_operation(
                    "INFO", f"Deleting inline policy {policy_name} for user {username}"
                )
            ...
            self.log_operation(
                "ERROR", f"Error deleting inline policies for user {username}: {e}"
            )
            return False
    ...
    def delete_iam_user(self, user_info):
        ...
        try:
            ...
            if not iam_client:
                self.log_operation(
                    "ERROR",
                    f"Cannot delete user {username}: Failed to create IAM client",
                )
                return False
            ...
            try:
                ...
                for mfa in response.get("MFADevices", []):
                    self.log_operation(
                        "INFO",
                        f"Deactivating MFA device {mfa['SerialNumber']} for {username}",
                    )
                    iam_client.deactivate_mfa_device(
                        UserName=username, SerialNumber=mfa["SerialNumber"]
                    )
            except Exception as e:
                self.log_operation(
                    "WARNING", f"Error handling MFA devices for {username}: {e}"
                )
            ...
            self.log_operation(
                "INFO",
                f"✅ Successfully deleted IAM user: {username} in {account_name}",
            )
            ...
            if "NoSuchEntity" in str(e):
                self.log_operation(
                    "INFO", f"User {username} does not exist in account {account_name}"
                )
                return True
            else:
                self.log_operation(
                    "ERROR",
                    f"Error deleting IAM user {username} in {account_name}: {e}",
                )
            ...
            self.log_operation(
                "ERROR",
                f"Unexpected error deleting IAM user {username} in {account_name}: {e}",
            )
    ...
    def detach_group_policies(self, iam_client, group_name):
        ...
        try:
            ...
            for policy in response.get("AttachedPolicies", []):
                ...
                self.log_operation(
                    "INFO",
                    f"Detaching policy {policy['PolicyName']} from group {group_name}",
                )
            ...
            self.log_operation(
                "ERROR", f"Error detaching policies from group {group_name}: {e}"
            )
            return False
    ...
    def delete_group_inline_policies(self, iam_client, group_name):
        ...
        try:
            ...
            for policy_name in response.get("PolicyNames", []):
                self.log_operation(
                    "INFO",
                    f"Deleting inline policy {policy_name} for group {group_name}",
                )
            ...
            self.log_operation(
                "ERROR", f"Error deleting inline policies for group {group_name}: {e}"
            )
            return False
    ...
    def remove_all_users_from_group(self, iam_client, group_name):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Error removing users from group {group_name}: {e}"
            )
            return False
    ...
    def delete_iam_group(self, group_info):
        ...
        try:
            ...
            if not iam_client:
                self.log_operation(
                    "ERROR",
                    f"Cannot delete group {group_name}: Failed to create IAM client",
                )
                return False
            ...
            self.log_operation(
                "INFO",
                f"✅ Successfully deleted IAM group: {group_name} in {account_name}",
            )
            ...
            if "NoSuchEntity" in str(e):
                self.log_operation(
                    "INFO",
                    f"Group {group_name} does not exist in account {account_name}",
                )
                return True
            else:
                self.log_operation(
                    "ERROR",
                    f"Error deleting IAM group {group_name} in {account_name}: {e}",
                )
            ...
            self.log_operation(
                "ERROR",
                f"Unexpected error deleting IAM group {group_name} in {account_name}: {e}",
            )
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            report_filename = (
                f"{report_dir}/iam_cleanup_report_{self.execution_timestamp}.json"
            )
    ...
    def run(self):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"Starting IAM credential file discovery for pattern: {self.file_pattern}",
            )
            ...
            if all_groups:
                print(
                    f"📊 Found {len(all_groups)} IAM groups across {len(cred_files)} credential files"
                )
            ...
            for age in age_groups:
                ...
                age_label = (
                    "today" if age == 0 else f"{age} day{'s' if age != 1 else ''} old"
                )
            ...
            for age in sorted(age_groups):
                ...
                if max_age_to_process >= 0 and age > max_age_to_process:
                    self.log_operation(
                        "INFO",
                        f"Stopping at age {age} days (limit was {max_age_to_process} days)",
                    )
                    break
                ...
                if age >= 1:
                    process_age = (
                        input(
                            f"\nProcess credentials that are {age_label}? (y/n/all): "
                        )
                        .strip()
                        .lower()
                    )
                ...
                age_groups = [
                    group for group in all_groups if group["file_age_days"] == age
                ]
                ...
                if age_users:
                    ...
                    print("=" * 80)
                    print(
                        f"{'#':<4} {'Username':<30} {'Account':<15} {'Real Name':<25} {'Email'}"
                    )
                    print("-" * 90)
                    ...
                    for i, user in enumerate(age_users, 1):
                        ...
                        real_name = user.get("real_user", {}).get(
                            "full_name", "Unknown"
                        )
                        email = user.get("real_user", {}).get("email", "Unknown")
                        print(
                            f"{i:<4} {username:<30} {account_name:<15} {real_name:<25} {email}"
                        )
                    ...
                    if user_selection != "skip":
                        ...
                        if not user_selection or user_selection == "all":
                            ...
                            print(
                                f"✅ Selected all {len(age_users)} users from this age group"
                            )
                        else:
                            try:
                                indices = self.parse_selection(
                                    user_selection, len(age_users)
                                )
                                selected_users = [age_users[i - 1] for i in indices]
                                print(
                                    f"✅ Selected {len(selected_users)} users from this age group"
                                )
                            except ValueError as e:
                ...
                if age_groups:
                    ...
                    print("=" * 80)
                    print(
                        f"{'#':<4} {'Group Name':<30} {'Account':<15} {'Members Count'}"
                    )
                    print("-" * 80)
                    ...
                    for i, group in enumerate(age_groups, 1):
                        ...
                        print(
                            f"{i:<4} {group_name:<30} {account_name:<15} {members_count}"
                        )
                    ...
                    if group_selection != "skip":
                        ...
                        if not group_selection or group_selection == "all":
                            ...
                            print(
                                f"✅ Selected all {len(age_groups)} groups from this age group"
                            )
                        else:
                            try:
                                indices = self.parse_selection(
                                    group_selection, len(age_groups)
                                )
                                selected_groups = [age_groups[i - 1] for i in indices]
                                print(
                                    f"✅ Selected {len(selected_groups)} groups from this age group"
                                )
                            except ValueError as e:
                                print(f"❌ Invalid selection: {e}")
                                continue
            ...
            print(
                f"    User access keys, policies, and group memberships will also be deleted."
            )
                ...
                for group in groups_to_delete:
                    try:
                        if self.delete_iam_group(group):
                            successful_groups += 1
                            print(
                                f"✅ Deleted group: {group['group_name']} in account {group['account_name']}"
                            )
                        else:
                            failed_groups += 1
                            print(
                                f"❌ Failed to delete group: {group['group_name']} in account {group['account_name']}"
                            )
                    except Exception as e:
                        self.log_operation(
                            "ERROR", f"Error deleting group {group['group_name']}: {e}"
                        )
                        failed_groups += 1
                ...
                for user in users_to_delete:
                    try:
                        if self.delete_iam_user(user):
                            successful_users += 1
                            print(
                                f"✅ Deleted user: {user['username']} in account {user['account_name']}"
                            )
                        else:
                            failed_users += 1
                            print(
                                f"❌ Failed to delete user: {user['username']} in account {user['account_name']}"
                            )
                    except Exception as e:
                        self.log_operation(
                            "ERROR", f"Error deleting user {user['username']}: {e}"
                        )
                        failed_users += 1
            ...
            print(
                f"✅ Successfully deleted: {len(self.cleanup_results['users_deleted'])} users"
            )
            print(
                f"✅ Successfully deleted: {len(self.cleanup_results['groups_deleted'])} groups"
            )
            print(
...
def main():
    ...
    print(
        "This utility finds IAM users and groups from credential files and deletes them from AWS"
    )
    ...
    creds_dir = input(
        f"IAM credentials directory path (default: {default_creds_dir}): "
    ).strip()
    if not creds_dir:
    ...
    try:
        cleanup_manager = IAMLogFileCleanupManager(
            config_file=config_file, creds_base_dir=creds_dir, file_pattern=pattern
        )
        cleanup_manager.run()
    except Exception as e:
        print(f"\n❌ Fatal error: {e}")
