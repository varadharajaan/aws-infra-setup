...
class ASGLogFileCleanupManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{log_dir}/asg_log_cleanup_{self.execution_timestamp}.log"
            )
            ...
            self.logger.info(
                f"🧹 ASG LOG FILE CLEANUP SESSION STARTED (Pattern: {self.target_pattern})"
            )
            self.logger.info("=" * 100)
    ...
    def find_asg_log_files(self):
        ...
        if not os.path.exists(self.aws_base_dir):
            self.log_operation(
                "ERROR", f"AWS base directory does not exist: {self.aws_base_dir}"
            )
            return []
                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            try:
                                ...
                                asg_name = None
                                if (
                                    "asg_configuration" in asg_data
                                    and "name" in asg_data["asg_configuration"]
                                ):
                                    ...
                                    name_match = re.search(
                                        r"asg_(.*?)_\d{8}_\d{6}\.json", filename
                                    )
                                    if name_match:
                                ...
                                if (
                                    "asg_configuration" in asg_data
                                    and "region" in asg_data["asg_configuration"]
                                ):
                                    asg_info["region"] = asg_data["asg_configuration"][
                                        "region"
                                    ]
                                    asg_info["launch_template_id"] = asg_data[
                                        "asg_configuration"
                                    ].get("launch_template_id")
                                elif "region" in asg_data:
                                    asg_info["region"] = asg_data["region"]
                                    asg_info["launch_template_id"] = asg_data.get(
                                        "launch_template_id"
                                    )
                                ...
                                if "account_info" in asg_data:
                                    ...
                                    asg_info["account_name"] = account_data.get(
                                        "account_name"
                                    )
                                    asg_info["account_id"] = account_data.get(
                                        "account_id"
                                    )
                                ...
                                asg_info["created_by"] = asg_data.get(
                                    "created_by", "Unknown"
                                )

                                # Get creation_date and creation_time from metadata, fallback to Unknown
                                if "metadata" in asg_data:
                                    asg_info["creation_date"] = asg_data[
                                        "metadata"
                                    ].get("creation_date", "Unknown")
                                    asg_info["creation_time"] = asg_data[
                                        "metadata"
                                    ].get("creation_time", "Unknown")
                                else:
    ...
    def display_asg_files_by_day(self, files_by_day):
        ...
        for i, day in enumerate(sorted_days, 1):
            ...
            for file_info in files_by_day[day]:
                ...
                account = file_info.get("account_name", "Unknown")
                print(
                    f"   {timestamp_str} - {file_info['asg_name']} - {file_info['file_name']} ({account})"
                )
    ...
    def extract_unique_asgs(self, asg_files):
        ...
        for asg_info in asg_files:
            ...
            if (
                asg_name not in asgs_by_name
                or asg_info["file_date"] > asgs_by_name[asg_name]["file_date"]
            ):
    ...
    def create_asg_client(self, account_name, region):
        ...
        try:
            ...
            self.log_operation(
                "ERROR",
                f"Failed to create ASG client for {account_name} in {region}: {e}",
            )
            return None
    ...
    def delete_asg(self, asg_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO", f"🗑️ Deleting ASG: {asg_name} in {account_name} ({region})"
            )
            ...
            if not all([asg_name, account_name, region]):
                ...
                self.log_operation(
                    "ERROR", f"Cannot delete ASG: Missing {', '.join(missing)}"
                )
                return False
            ...
            try:
                ...
                if not response["AutoScalingGroups"]:
                    self.log_operation(
                        "INFO",
                        f"ASG {asg_name} does not exist in {account_name} ({region})",
                    )
                    return False
            ...
            try:
                self.log_operation(
                    "INFO", f"Checking for scheduled actions on {asg_name}"
                )
                ...
                for action in actions_response.get("ScheduledUpdateGroupActions", []):
                    ...
                    self.log_operation(
                        "INFO", f"Deleting scheduled action: {action_name}"
                    )
                    asg_client.delete_scheduled_action(
                        AutoScalingGroupName=asg_name, ScheduledActionName=action_name
                    )
            except Exception as e:
            ...
            try:
                self.log_operation(
                    "INFO", f"Checking for scaling policies on {asg_name}"
                )
                ...
                for policy in policies_response.get("ScalingPolicies", []):
                    ...
                    self.log_operation(
                        "INFO", f"Deleting scaling policy: {policy_name}"
                    )
            ...
            self.log_operation(
                "ERROR", f"Failed to delete ASG {asg_info['asg_name']}: {e}"
            )
            ...
            new_file_path = self.move_processed_file(
                asg_info["file_path"], asg_info["asg_name"], success=False
            )
    ...
    def run(self):
        ...
        try:
            print("\n" + "=" * 80)
            print(
                f"🧹 AUTO SCALING GROUP CLEANUP UTILITY - PATTERN: {self.target_pattern}"
            )
            print("=" * 80)
            ...
            self.log_operation(
                "INFO",
                f"Starting ASG log file discovery for pattern: {self.target_pattern}",
            )
            ...
            print("\n" + "=" * 80)
            print(
                f"🔍 ASG SELECTION FROM SELECTED FILES - PATTERN: {self.target_pattern}"
            )
            print("=" * 80)

            print(f"\n📋 Available ASGs from selected files:")
            print(
                f"{'#':<4} {'ASG Name':<40} {'Account':<15} {'Age':<8} {'Region':<12} {'Date':<12}"
            )
            print("-" * 100)
            ...
            for i, asg in enumerate(unique_asgs, 1):
                ...
                print(
                    f"{i:<4} {asg_name:<40} {account_name:<15} {age_str:<8} {region:<12} {date_str:<12}"
                )
            ...
            for asg in asgs_to_delete:
                try:
                    ...
                    self.log_operation(
                        "ERROR", f"Error deleting ASG {asg['asg_name']}: {e}"
                    )
                    failed += 1
...
def main():
    ...
    print(
        "This utility finds specific ASG patterns from log files and deletes them from AWS"
    )
    ...
    aws_dir = input(
        f"AWS logs base directory path (default: {default_aws_dir}): "
    ).strip()
    if not aws_dir:
    ...
    # Get target pattern
    pattern = input(
        f"ASG name pattern to target (default: {default_pattern}): "
    ).strip()
    if not pattern:
    ...
    try:
        cleanup_manager = ASGLogFileCleanupManager(
            config_file=config_file, aws_base_dir=aws_dir, target_pattern=pattern
        )
        cleanup_manager.run()
    except Exception as e:
        print(f"\n❌ Fatal error: {e}")
