...
class EKSNodeGroupScaler:
    ...
    def log_execution_start(self, cluster_name, region, action, ist_time):
        ...
        self.logger.info("=" * 80)
        self.logger.info("[START] EKS NODEGROUP SCALING LAMBDA - EXECUTION START")
        self.logger.info("=" * 80)
        self.logger.info(f"{Symbols.DATE} Execution Time: {datetime.now(datetime.UTC).isoformat()}Z")
        self.logger.info(f"{Symbols.TARGET} Target Cluster: {cluster_name}")
        self.logger.info(f"{Symbols.REGION} Target Region: {region}")
        self.logger.info(f"[FAST] Scaling Action: {action.upper()}")
        self.logger.info(f"[TIME] IST Time: {ist_time}")
        self.logger.info(f"👤 Generated By: {{CURRENT_USER}}")
        self.logger.info(f"{Symbols.LOG} Template Generated: {{CURRENT_DATETIME}}")
        self.logger.info("=" * 80)
    ...
    def log_execution_end(self, success=True):
        ...
        self.logger.info("=" * 80)
        self.logger.info(f"{Symbols.TIMER}  Total Execution Time: {execution_time:.2f} seconds")
        self.logger.info(f"{Symbols.STATS} Nodegroups Processed: {self.execution_stats['nodegroups_processed']}")
        self.logger.info(f"{Symbols.OK} Successful Operations: {self.execution_stats['successful_operations']}")
        self.logger.info(f"{Symbols.SKIP}  Skipped Operations: {self.execution_stats['skipped_operations']}")
        self.logger.info(f"{Symbols.ERROR} Failed Operations: {self.execution_stats['failed_operations']}")
        self.logger.info(f"[UP] Total Scaling Changes: {self.execution_stats['total_scaling_changes']}")
        self.logger.info(f"{Symbols.WARN}  Warnings Issued: {self.execution_stats['warnings_issued']}")
        self.logger.info(
            f"{Symbols.TARGET} Success Rate: {(self.execution_stats['successful_operations'] + self.execution_stats['skipped_operations']) / max(1, self.execution_stats['nodegroups_processed']) * 100:.1f}%")
        self.logger.info(f"{Symbols.OK} Execution Status: {'SUCCESS' if success else 'FAILED'}")
        self.logger.info("=" * 80)
    ...
    def calculate_ist_time(self):
        ...
        try:
            ...
            self.logger.info(f"{Symbols.OK} Current IST time calculated: {ist_time}")
            return ist_time

        except Exception as e:
            self.logger.warning(f"{Symbols.WARN}  Error calculating IST time: {str(e)} - using UTC time")
            self.execution_stats['warnings_issued'] += 1
            utc_time = datetime.now(datetime.UTC).strftime('%I:%M %p UTC')
            return utc_time
    ...
    def validate_nodegroup_config(self, ng_config, index):
        ...
        if not nodegroup_name:
            self.logger.warning(f"{Symbols.WARN}  [{index}] Skipping nodegroup with missing name: {ng_config}")
            self.execution_stats['warnings_issued'] += 1
            return None
        ...
        if min_size > max_size:
            self.logger.warning(
                f"{Symbols.WARN}  [{index}] Invalid config for {nodegroup_name}: min_size ({min_size}) > max_size ({max_size})")
            self.execution_stats['warnings_issued'] += 1

        if desired_size < min_size or desired_size > max_size:
            self.logger.warning(
                f"{Symbols.WARN}  [{index}] Invalid config for {nodegroup_name}: desired_size ({desired_size}) not between min_size ({min_size}) and max_size ({max_size})")
            self.execution_stats['warnings_issued'] += 1

        self.logger.info(
            f"{Symbols.OK} [{index}] Validated config for {nodegroup_name}: min={min_size}, desired={desired_size}, max={max_size}")
    ...
    def get_current_nodegroup_config(self, eks_client, cluster_name, nodegroup_name):
        ...
        self.logger.info(f"   ├── {Symbols.LIST} Retrieving current configuration...")
        ...
        try:
            ...
            self.logger.info(
                f"   ├── {Symbols.STATS} Current scaling: min={current_config['min']}, desired={current_config['desired']}, max={current_config['max']}")
            self.logger.info(f"   ├── [UP] Status: {status}")
            self.logger.info(f"   ├── {Symbols.INSTANCE} Capacity Type: {capacity_type}")
            self.logger.info(f"   ├── [DESKTOP]  Instance Types: {instance_types}")

            if status != 'ACTIVE':
                self.logger.warning(f"   {Symbols.WARN}  Nodegroup status is {status}, not ACTIVE")
                self.execution_stats['warnings_issued'] += 1
            ...
            if error_code == 'ResourceNotFoundException':
                self.logger.error(f"   {Symbols.ERROR} Nodegroup {nodegroup_name} not found in cluster {cluster_name}")
            elif error_code == 'AccessDeniedException':
                self.logger.error(f"   {Symbols.ERROR} Access denied to nodegroup {nodegroup_name} - check IAM permissions")
            else:
                self.logger.error(f"   {Symbols.ERROR} AWS API error: {error_code} - {e.response['Error']['Message']}")
            raise
        except Exception as e:
            self.logger.error(f"   {Symbols.ERROR} Unexpected error retrieving nodegroup config: {str(e)}")
            raise
    ...
    def update_nodegroup_scaling(self, eks_client, cluster_name, nodegroup_name, target_config):
        ...
        self.logger.info(f"   ├── {Symbols.SCAN} Initiating scaling configuration update...")
        ...
        try:
            ...
            self.logger.info(f"   ├── {Symbols.OK} Scaling update initiated successfully")
            self.logger.info(f"   ├── 🆔 Update ID: {update_id}")
            self.logger.info(f"   ├── {Symbols.STATS} Update Status: {update_status}")
            self.logger.info(
                f"   └── {Symbols.TARGET} New configuration will be: min={target_config['min_size']}, desired={target_config['desired_size']}, max={target_config['max_size']}")
            ...
            if error_code == 'InvalidParameterException':
                self.logger.error(f"   {Symbols.ERROR} Invalid scaling parameters: {error_message}")
            elif error_code == 'ResourceInUseException':
                self.logger.error(f"   {Symbols.ERROR} Nodegroup is currently being updated: {error_message}")
            elif error_code == 'InvalidRequestException':
                self.logger.error(f"   {Symbols.ERROR} Invalid request: {error_message}")
            else:
                self.logger.error(f"   {Symbols.ERROR} AWS API error: {error_code} - {error_message}")
            raise
        except Exception as e:
            self.logger.error(f"   {Symbols.ERROR} Unexpected error updating nodegroup: {str(e)}")
            raise
    ...
    def process_nodegroup(self, eks_client, cluster_name, ng_config, index, total):
        ...
        self.logger.info(f"[PACKAGE] [{index}/{total}] Processing nodegroup: {nodegroup_name}")
        self.logger.info(
            f"   ├── {Symbols.TARGET} Target: min={target_config['min_size']}, desired={target_config['desired_size']}, max={target_config['max_size']}")
        ...
        try:
            ...
            if (current_config['min'] == target_config['min_size'] and
                ...
                self.logger.info(f"   ├── {Symbols.SKIP}  No changes needed - scaling configuration already matches target")
                self.logger.info(f"   └── {Symbols.OK} Skipping update for {nodegroup_name}")
            ...
            error_msg = str(e)
            self.logger.error(f"   {Symbols.ERROR} Failed to process nodegroup {nodegroup_name}: {error_msg}")
    ...
    def scale_nodegroups(self, cluster_name, region, action, ist_time, nodegroups_config):
        ...
        try:
            ...
            self.logger.info("[CONFIG] Initializing AWS EKS client...")
            ...
            try:
                ...
                self.logger.info(f"{Symbols.OK} AWS connectivity verified - Account: {identity.get('Account')}")
                self.logger.info(f"   User/Role ARN: {identity.get('Arn')}")
            except Exception as e:
                self.logger.warning(f"{Symbols.WARN}  Could not verify AWS identity: {str(e)}")
                self.execution_stats['warnings_issued'] += 1

            # Validate input
            if not nodegroups_config:
                error_msg = "No nodegroups specified in the event"
                self.logger.error(f"{Symbols.ERROR} Validation failed: {error_msg}")
                return {
                    'statusCode': 400,
                    'body': json.dumps({
                        'success': False,
            ...
            self.logger.info(f"{Symbols.LIST} PHASE 1: VALIDATING {len(nodegroups_config)} NODEGROUP CONFIGURATIONS")
            self.logger.info("─" * 60)
            ...
            if not valid_configs:
                ...
                self.logger.error(f"{Symbols.ERROR} {error_msg}")
            ...
            self.logger.info(
                f"{Symbols.OK} Validation complete: {len(valid_configs)}/{len(nodegroups_config)} configurations valid")

            # Process scaling operations
            self.logger.info(f"{Symbols.START} PHASE 2: EXECUTING SCALING OPERATIONS")
            self.logger.info("─" * 60)
            ...
            self.logger.info(f"{Symbols.STATS} SCALING OPERATION SUMMARY:")
            ...
            error_msg = str(e)
            self.logger.error(f"[BOOM] Scaling operation failed with unexpected error: {error_msg}")
            self.log_execution_end(success=False)
...
def lambda_handler(event, context):
    ...
    """
    try:
        logger.info(f"{Symbols.START} EKS Scaling Lambda function invoked")
        logger.info(f"[ROUNDPIN] Function ARN: {context.invoked_function_arn}")
        logger.info(f"{Symbols.DATE} Template generated: {{CURRENT_DATETIME}}")
        logger.info(f"👤 Generated by user: {{CURRENT_USER}}")
        logger.info(f"{Symbols.TIMER}  Remaining execution time: {context.get_remaining_time_in_millis()}ms")
        logger.info(f"{Symbols.INSTANCE} Memory limit: {context.memory_limit_in_mb}MB")
        ...
        if not cluster_name:
            ...
            logger.error(f"{Symbols.ERROR} Validation failed: {error_msg}")
            return {
                'statusCode': 400,
                'body': json.dumps({
                    'success': False,
...
3. {Symbols.START} MULTI-NODEGROUP SCALE-UP EVENT (Production):
{
    "action": "scale_up",
...
4. [DOWN] MULTI-NODEGROUP SCALE-DOWN EVENT (Production):
{
    "action": "scale_down",
...
6. [CONFIG] MAINTENANCE MODE EVENT (Scale to Minimum):
{
...
8. {Symbols.REGION} CROSS-REGION EVENT (EU West):
{
    "action": "scale_up",
...
9. {Symbols.ERROR} ERROR TEST EVENT (Invalid Configuration):
{
    "action": "scale_up",
...
10. [MAILBOX] EMPTY EVENT (Tests Environment Variables):
{
    "action": "test_connectionf"
}

11. {Symbols.SCAN} SPOT INSTANCE SCALING EVENT:
{
...
12. {Symbols.ALERT} EMERGENCY SCALE DOWN EVENT:
{
...
═══════════════════════════════════════════════════════════════════════════════
{Symbols.LOG} EVENTBRIDGE SCHEDULED EVENT EXAMPLES
═══════════════════════════════════════════════════════════════════════════════
            ...
            "Targets": [
                {
                    "Id": "1", 
                    "Arn": "arn:aws:lambda:us-east-1:123456789012:function:eks-scaling-function",
                    "Input": "{\"action\": \"scale_down\", \"ist_time\": \"6:30 PM IST\", \"nodegroups\": [{\"name\": \"eks-nodegroup-general\", \"desired_size\": 0, \"min_size\": 0, \"max_size\": 5}]}f"
                }
            ]
        }
    ]
...
═══════════════════════════════════════════════════════════════════════════════
{Symbols.STATS} TEMPLATE INJECTION VARIABLES
═══════════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    print("=" * 80)
    print(f"{Symbols.DATE} Template generated: {{CURRENT_DATETIME}}")
    ...
    try:
        print(f"\n{Symbols.START} Testing SCALE UP operation...")
        ...
        print(f"\n{Symbols.START} Testing SCALE DOWN operation...")
        ...
        print(f"\n{Symbols.OK} Local testing completed!")
