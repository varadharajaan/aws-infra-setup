...
class EKSNodeGroupScaler:
    ...
    def log_execution_end(self, success=True):
        ...
        self.logger.info(f"[STATS] Nodegroups Processed: {self.execution_stats['nodegroups_processed']}")
        self.logger.info(f"[OK] Successful Operations: {self.execution_stats['successful_operations']}")
        self.logger.info(f"[SKIP]  Skipped Operations: {self.execution_stats['skipped_operations']}")
        self.logger.info(f"[ERROR] Failed Operations: {self.execution_stats['failed_operations']}")
        ...
        self.logger.info(
            f"[TARGET] Success Rate: {(self.execution_stats['successful_operations'] + self.execution_stats['skipped_operations']) / max(1, self.execution_stats['nodegroups_processed']) * 100:.1f}%")
    ...
    def validate_nodegroup_config(self, ng_config, index):
        ...
        if not nodegroup_name:
            self.logger.warning(f"[WARN]  [{index}] Skipping nodegroup with missing name: {ng_config}")
            self.execution_stats['warnings_issued'] += 1
            return None
        ...
        if min_size > max_size:
            self.logger.warning(
                f"[WARN]  [{index}] Invalid config for {nodegroup_name}: min_size ({min_size}) > max_size ({max_size})")
            self.execution_stats['warnings_issued'] += 1

        if desired_size < min_size or desired_size > max_size:
            self.logger.warning(
                f"[WARN]  [{index}] Invalid config for {nodegroup_name}: desired_size ({desired_size}) not between min_size ({min_size}) and max_size ({max_size})")
            self.execution_stats['warnings_issued'] += 1

        self.logger.info(
            f"[OK] [{index}] Validated config for {nodegroup_name}: min={min_size}, desired={desired_size}, max={max_size}")
    ...
    def get_current_nodegroup_config(self, eks_client, cluster_name, nodegroup_name):
        ...
        self.logger.info(f"   ├── [LIST] Retrieving current configuration...")
        ...
        try:
            ...
            self.logger.info(
                f"   ├── [STATS] Current scaling: min={current_config['min']}, desired={current_config['desired']}, max={current_config['max']}")
            self.logger.info(f"   ├── 📈 Status: {status}")
            self.logger.info(f"   ├── [INSTANCE] Capacity Type: {capacity_type}")
            self.logger.info(f"   ├── [DESKTOP]  Instance Types: {instance_types}")

            if status != 'ACTIVE':
                self.logger.warning(f"   [WARN]  Nodegroup status is {status}, not ACTIVE")
                self.execution_stats['warnings_issued'] += 1
            ...
            if error_code == 'ResourceNotFoundException':
                self.logger.error(f"   [ERROR] Nodegroup {nodegroup_name} not found in cluster {cluster_name}")
            elif error_code == 'AccessDeniedException':
                self.logger.error(f"   [ERROR] Access denied to nodegroup {nodegroup_name} - check IAM permissions")
            else:
                self.logger.error(f"   [ERROR] AWS API error: {error_code} - {e.response['Error']['Message']}")
            raise
        except Exception as e:
            self.logger.error(f"   [ERROR] Unexpected error retrieving nodegroup config: {str(e)}")
            raise
    ...
    def update_nodegroup_scaling(self, eks_client, cluster_name, nodegroup_name, target_config):
        ...
        try:
            ...
            self.logger.info(f"   ├── [OK] Scaling update initiated successfully")
            self.logger.info(f"   ├── 🆔 Update ID: {update_id}")
            self.logger.info(f"   ├── [STATS] Update Status: {update_status}")
            self.logger.info(
                f"   └── [TARGET] New configuration will be: min={target_config['min_size']}, desired={target_config['desired_size']}, max={target_config['max_size']}")
            ...
            if error_code == 'InvalidParameterException':
                self.logger.error(f"   [ERROR] Invalid scaling parameters: {error_message}")
            elif error_code == 'ResourceInUseException':
                self.logger.error(f"   [ERROR] Nodegroup is currently being updated: {error_message}")
            elif error_code == 'InvalidRequestException':
                self.logger.error(f"   [ERROR] Invalid request: {error_message}")
            else:
                self.logger.error(f"   [ERROR] AWS API error: {error_code} - {error_message}")
            raise
        except Exception as e:
            self.logger.error(f"   [ERROR] Unexpected error updating nodegroup: {str(e)}")
            raise
    ...
    def process_nodegroup(self, eks_client, cluster_name, ng_config, index, total):
        ...
        self.logger.info(
            f"   ├── [TARGET] Target: min={target_config['min_size']}, desired={target_config['desired_size']}, max={target_config['max_size']}")
        ...
        try:
            ...
            if (current_config['min'] == target_config['min_size'] and
                ...
                self.logger.info(f"   ├── [SKIP]  No changes needed - scaling configuration already matches target")
                self.logger.info(f"   └── [OK] Skipping update for {nodegroup_name}")
            ...
            error_msg = str(e)
            self.logger.error(f"   [ERROR] Failed to process nodegroup {nodegroup_name}: {error_msg}")
    ...
    def scale_nodegroups(self, cluster_name, region, action, ist_time, nodegroups_config):
        ...
        try:
            ...
            if not nodegroups_config:
                ...
                self.logger.error(f"[ERROR] Validation failed: {error_msg}")
                return {
                    'statusCode': 400,
                    'body': json.dumps({
                        'success': False,
            ...
            self.logger.info(f"[LIST] PHASE 1: VALIDATING {len(nodegroups_config)} NODEGROUP CONFIGURATIONS")
            self.logger.info("─" * 60)
            ...
            self.logger.info(
                f"[OK] Validation complete: {len(valid_configs)}/{len(nodegroups_config)} configurations valid")

            # Process scaling operations
            self.logger.info(f"[START] PHASE 2: EXECUTING SCALING OPERATIONS")
            self.logger.info("─" * 60)
            ...
            self.logger.info("[STATS] SCALING OPERATION SUMMARY:")
...
def lambda_handler(event, context):
    ...
    """
    try:
        logger.info(f"[START] EKS Scaling Lambda function invoked")
        logger.info(f"[ROUNDPIN] Function ARN: {context.invoked_function_arn}")
        logger.info(f"[DATE] Template generated: {{CURRENT_DATETIME}}")
...
3. [START] MULTI-NODEGROUP SCALE-UP EVENT (Production):
{
    "action": "scale_up",
...
9. [ERROR] ERROR TEST EVENT (Invalid Configuration):
{
    "action": "scale_up",
...
12. [ALERT] EMERGENCY SCALE DOWN EVENT:
{
...
def main():
    ...
    print("=" * 80)
    print(f"[DATE] Template generated: {{CURRENT_DATETIME}}")
