...
class EKSClusterContinuationFromErrors:
    ...
    def setup_logging(self):
        ...
        log_file = os.path.join(
            log_dir, f"cluster_continuation_{self.execution_timestamp}.log"
        )
        ...
        try:
            ...
            if os.path.exists(config_file):
                with open(config_file, "r") as f:
                    config = json.load(f)
                self.logger.info(
                    f"Loaded AWS accounts configuration from {config_file}"
                )
                return config
            else:
                self.logger.warning(
                    f"AWS accounts config file not found: {config_file}"
                )
                return {}
    ...
    def find_error_files(self) -> List[Tuple[str, dict]]:
        ...
        try:
            ...
            if not file_paths:
                self.print_colored(
                    "YELLOW",
                    f"⚠️  No error files found matching pattern: {error_pattern}",
                )
                return []
            ...
            for file_path in file_paths:
                try:
                    ...
                    self.logger.error(
                        f"Failed to parse error file {file_path}: {str(e)}"
                    )
                    continue
    ...
    def group_clusters_by_date(
        self, error_files: List[Tuple[str, dict]]
    ) -> Dict[str, List[Dict]]:
    ...
    def select_date_and_clusters(
        self, clusters_by_date: Dict[str, List[Dict]]
    ) -> List[Dict]:
        ...
        while True:
            try:
                ...
                if 1 <= choice_num <= len(dates):
                    ...
                    self.print_colored(
                        "RED", f"❌ Please enter a number between 1 and {len(dates)}"
                    )
            except ValueError:
        ...
        self.print_colored(
            "GREEN",
            f"✅ Selected date: {selected_date} ({len(selected_clusters_pool)} clusters)",
        )
            ...
            if len(parts) >= 4:
                for part in parts:
                    if (
                        ...
                        or part.startswith("ap-")
                        or part.startswith("ca-")
        ...
        while True:
            ...
            try:
                selected_indices = self.parse_selection(
                    selection, len(selected_clusters_pool)
                )
                if selected_indices:
                    selected_clusters = [
                        selected_clusters_pool[i - 1] for i in selected_indices
                    ]
                    ...
                    confirm = (
                        input(
                            f"\nConfirm selection of {len(selected_clusters)} clusters? (Y/n): "
                        )
                        .strip()
                        .lower()
                    )
    ...
    def extract_cluster_details(self, cluster_info: Dict) -> Tuple[str, str]:
        ...
        if region_match:
            ...
            self.print_colored(
                "YELLOW",
                f"Extracted region '{region}' from cluster name '{cluster_name}'",
            )
        else:
    ...
    def get_credentials_for_cluster(
        ...
        if (
            "root-account" in cluster_name.lower()
            or "root_account" in cluster_name.lower()
        ):
            account_type = "root"
        ...
        override = (
            input(
                "Override account type? (r for Root, i for IAM, Enter to keep detected): "
            )
            .strip()
            .lower()
        )
        ...
        if account_type == "root":
            return self.get_root_credentials_from_cluster_legacy(
                cluster_name=cluster_name
            )
        else:
    ...
    def show_account_summary(self):
        ...
        try:
            ...
            total_users = sum(
                acc.get("users_per_account", 0) for acc in accounts.values()
            )
            ...
            if user_settings:
                ...
                self.print_colored(
                    Colors.WHITE, f"   • Default Password: {default_password}", 1
                )
                self.print_colored(
                    Colors.WHITE,
                    f"   • Allowed Instances: {len(allowed_instances)} types",
                    1,
                )
            ...
            for account_name, account_data in accounts.items():
                ...
                self.print_colored(
                    Colors.WHITE,
                    f"  ID: {account_id} | Email: {email} | Users: {users}",
                    1,
                )
            ...
            self.print_colored(
                Colors.RED, f"❌ Failed to show account summary: {str(e)}"
            )

    def get_iam_credentials(
        self, username: str = None, account_id: str = None, cluster_name: str = None
    ) -> Tuple[str, str, str]:
        """
        ...
        try:
            ...
            if cluster_name and not username:
                ...
                if not username:
                    raise ValueError(
                        f"Could not extract username from cluster name: {cluster_name}"
                    )
                self.print_colored(
                    Colors.GREEN, f"🎯 Extracted username from cluster: {username}"
                )
            ...
            if account_id and any(
                region_part in account_id
                for region_part in ["us-", "eu-", "ap-", "sa-", "ca-"]
            ):
                self.print_colored(
                    Colors.YELLOW,
                    f"⚠️ Account ID looks like a region ({account_id}), ignoring filter",
                )
                account_id = None
            ...
            if not credential_files:
                raise ValueError(
                    f"No IAM credential files found in {iam_dir} matching pattern: {pattern}"
                )
            ...
            return self._search_credentials_in_file(
                selected_file, username, account_id, cluster_name
            )

        except Exception as e:
            self.print_colored(
                Colors.RED, f"❌ Error loading IAM credentials: {str(e)}"
            )
        ...
        try:
            ...
            for i, (file_path, timestamp, timestamp_str) in enumerate(parsed_files, 1):
                ...
                if i == 1:
                    self.print_colored(
                        Colors.YELLOW,
                        f"{i}. {formatted_time} - {filename} [LATEST - DEFAULT]",
                    )
                else:
                    self.print_colored(
                        Colors.WHITE, f"{i}. {formatted_time} - {filename}"
                    )
            ...
            while True:
                try:
                    choice = input(
                        f"📁 Select credential file (1-{len(parsed_files)}, Enter for latest): "
                    ).strip()
                    ...
                    if 1 <= choice_num <= len(parsed_files):
                        ...
                        selected_timestamp = parsed_files[choice_num - 1][1].strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        ...
                        self.print_colored(
                            Colors.RED,
                            f"❌ Please enter a number between 1 and {len(parsed_files)}",
                        )

                except ValueError:
                    self.print_colored(
                        Colors.RED,
                        "❌ Please enter a valid number or press Enter for default",
                    )
    ...
    def _search_credentials_in_file(
        self,
        file_path: str,
        username: str,
        account_id: str = None,
        ...
        try:
            ...
            self.print_colored(
                Colors.BLUE, f"📁 Checking file: {os.path.basename(file_path)}"
            )
                ...
                for user in users:
                    ...
                    if user_username.lower() == username.lower():
                        ...
                        if not access_key or not secret_key:
                            raise ValueError(
                                f"Incomplete credentials for user {username} in account {account_name}"
                            )
                        ...
                        self.print_colored(
                            Colors.WHITE, f"   • Username: {user_username}", 1
                        )
                        self.print_colored(
                            Colors.WHITE,
                            f"   • Account: {account_name} ({current_account_id})",
                            1,
                        )
                        self.print_colored(Colors.WHITE, f"   • Region: {region}", 1)
                        self.print_colored(
                            Colors.WHITE, f"   • File: {os.path.basename(file_path)}", 1
                        )

                        if cluster_name:
                            self.print_colored(
                                Colors.WHITE, f"   • Cluster: {cluster_name}", 1
                            )

                        if real_user:
                            self.print_colored(
                                Colors.WHITE,
                                f"   • Real User: {real_user.get('full_name', 'N/A')}",
                                1,
                            )
                            self.print_colored(
                                Colors.WHITE,
                                f"   • Email: {real_user.get('email', 'N/A')}",
                                1,
                            )

                        self.print_colored(
                            Colors.WHITE, f"   • Access Key: {access_key[:8]}...", 1
                        )
                        self.print_colored(
                            Colors.WHITE, f"   • Secret Key: {secret_key[:8]}...", 1
                        )
                        self.print_colored(
                            Colors.CYAN,
                            f"   • Console URL: {user.get('console_url', 'N/A')}",
                            1,
                        )
            ...
            error_msg = (
                f"User '{username}' not found in file {os.path.basename(file_path)}"
            )
            if account_id:
            ...
            self.print_colored(
                Colors.RED, f"❌ Error reading {file_path}: Invalid JSON - {str(e)}"
            )
    ...
    def _show_available_users(self):
        ...
        try:
            ...
            if not credential_files:
                self.print_colored(
                    Colors.RED, f"No credential files found in {iam_dir}"
                )
                return
            ...
            self.print_colored(
                Colors.BLUE,
                f"📁 File: {os.path.basename(latest_file)} ({latest_timestamp})",
            )
        ...
        try:
            ...
            for i, (file_path, timestamp, timestamp_type) in enumerate(parsed_files, 1):
                ...
                if i == 1:
                    self.print_colored(
                        Colors.GREEN,
                        f"{formatted_time} - {filename} [LATEST] ({file_size} bytes)",
                    )
                else:
                    self.print_colored(
                        Colors.WHITE,
                        f"{formatted_time} - {filename} ({file_size} bytes)",
                    )
            ...
            self.print_colored(
                Colors.RED, f"❌ Error showing credential files: {str(e)}"
            )

    def get_iam_credentials_from_cluster(
        self, cluster_name: str, region: str = None
    ) -> Tuple[str, str, str]:
        """
        ...
        try:
            ...
            if not region:
                ...
                if region:
                    self.print_colored(
                        Colors.GREEN, f"🌍 Extracted region from cluster: {region}"
                    )

            # Get credentials using the extracted username (NO account_id filter)
            return self.get_iam_credentials(
                username=username, account_id=None, cluster_name=cluster_name
            )

        except Exception as e:
            self.print_colored(
                Colors.RED,
                f"❌ Error extracting credentials from cluster '{cluster_name}': {str(e)}",
            )
            raise ValueError(
                f"Error extracting credentials from cluster '{cluster_name}': {str(e)}"
            )
    ...
    def get_iam_credentials_legacy(
        self, username: str = None, account_id: str = None, cluster_name: str = None
    ) -> Tuple[str, str]:
        """
        ...
        """
        try:
            access_key, secret_key, account_id = self.get_iam_credentials(
                username, account_id, cluster_name
            )
            return access_key, secret_key
        except Exception as e:
            raise e

    def get_iam_credentials_from_cluster_legacy(
        self, cluster_name: str, region: str = None
    ) -> Tuple[str, str]:
        """
        Legacy method that returns only access_key and secret_key (for backward compatibility)
        ...
        """
        try:
            access_key, secret_key, account_id = self.get_iam_credentials_from_cluster(
                cluster_name, region
            )
            return access_key, secret_key
        except Exception as e:
    ...
    def get_root_credentials_from_cluster_legacy(
        self, cluster_name: str = None, region: str = None, account_id: str = None
    ) -> Tuple[str, str]:
        """
        Legacy method that returns only access_key and secret_key (for backward compatibility)
        ...
        """
        try:
            access_key, secret_key, _ = self.get_root_credentials(
    ...
    def get_root_credentials(
        ...
        try:
            ...
            if not self.aws_accounts_config or not self.aws_accounts_config.get(
                "accounts"
            ):
                raise ValueError(
                    "No AWS root accounts configuration found or invalid format"
                )
            ...
            self.print_colored(
                Colors.CYAN, f"🔍 Found {len(accounts)} root accounts in configuration"
            )
            ...
            if account_id:
                ...
                self.print_colored(
                    Colors.GREEN, f"✅ Found account by ID: {account_id}"
                )

            # Strategy 2: Try to extract account name from cluster name
            elif cluster_name:
                account_name_from_cluster = self._extract_account_from_cluster_name(
                    cluster_name
                )
                matching_accounts = []
                ...
                if account_name_from_cluster and account_name_from_cluster in accounts:
                    matching_accounts.append(
                        (account_name_from_cluster, accounts[account_name_from_cluster])
                    )
                    self.print_colored(
                        Colors.GREEN,
                    ...
                    self.print_colored(
                        Colors.YELLOW,
                        f"⚠️ Could not detect account from cluster name: {cluster_name}",
                    )
            ...
            # Strategy 3: Show all accounts
                ...
                matching_accounts = list(accounts.items())
                self.print_colored(
                    Colors.BLUE, "📋 No filters provided, showing all accounts"
                )
            ...
            if len(matching_accounts) == 1:
                ...
                self.print_colored(
                    Colors.GREEN,
                    f"✅ Auto-selected account: {account_name} (ID: {account_id})",
                )

            # Let user choose from multiple accounts
            else:
                account_name, account_data = self._interactive_account_selection(
                    matching_accounts
                )
            ...
            if not access_key or not secret_key or not account_id:
                raise ValueError(
                    f"Incomplete root credentials for account {account_name}"
                )

            if not isinstance(access_key, str) or not isinstance(secret_key, str):
                raise ValueError(
                    f"Invalid credential format for account {account_name}"
                )
            ...
            self.print_colored(
                Colors.RED, f"❌ Error loading root credentials: {str(e)}"
            )
    ...
    def _load_root_accounts_config(self) -> dict:
        ...
        try:
            ...
            if not credential_files:
                raise ValueError(
                    f"No root account files found. Searched patterns: {patterns}"
                )
            ...
            for file_path in credential_files:
                try:
                    self.print_colored(
                        Colors.BLUE, f"📁 Loading root accounts from: {file_path}"
                    )
                    ...
                    self.print_colored(
                        Colors.GREEN,
                        f"✅ Successfully loaded root accounts configuration",
                    )
                    return config

                except json.JSONDecodeError as e:
                    self.print_colored(
                        Colors.RED, f"❌ Invalid JSON in {file_path}: {str(e)}"
                    )
                    continue
                except Exception as e:
                    self.print_colored(
                        Colors.RED, f"❌ Error reading {file_path}: {str(e)}"
                    )
                    continue
            ...
            self.print_colored(
                Colors.RED, f"❌ Failed to load root accounts config: {str(e)}"
            )
            raise
        ...
        try:
            ...
            if cluster_name.startswith("eks-cluster-root-account"):
                ...
                if len(parts) >= 4:
                    ...
                    self.print_colored(
                        Colors.GREEN, f"✅ Detected root user: {username}"
                    )
                    return username
                else:
                    self.print_colored(
                        Colors.RED,
                        f"❌ Invalid root cluster name format: {cluster_name}",
                    )
                    return None
            ...
            if cluster_name.startswith("eks-cluster-"):
                ...
                if len(parts) >= 3:
                    ...
                    if "_" in username:
                        self.print_colored(
                            Colors.GREEN, f"✅ Detected IAM user: {username}"
                        )
                        return username
                    else:
                        self.print_colored(
            ...
            self.print_colored(
                Colors.RED, f"❌ Unrecognized cluster name format: {cluster_name}"
            )
            return None
        ...
        try:
            ...
            for i in range(len(parts) - 1):
                ...
                if i + 1 < len(parts):
                    ...
                    if any(
                        potential_region.startswith(prefix)
                        for prefix in ["us-", "eu-", "ap-", "sa-", "ca-"]
                    ):
                        ...
                        if i + 2 < len(parts) and parts[i + 2].isdigit():
                            ...
                            self.print_colored(
                                Colors.GREEN,
                                f"🌍 Extracted region (fallback): {region}",
                            )
                            return region

            self.print_colored(
                Colors.YELLOW,
                f"⚠️ Could not extract region from cluster name: {cluster_name}",
            )
            return None
    ...
    def _interactive_account_selection(
        self, matching_accounts: list
    ) -> Tuple[str, dict]:
        ...
        try:
            ...
            for i, (account_name, account_data) in enumerate(matching_accounts, 1):
                ...
                self.print_colored(
                    Colors.WHITE,
                    (
                        f"   • Access Key: {access_key[:8]}..."
                        if access_key
                        else "   • Access Key: Not available"
                    ),
                    1,
                )
                print()
            ...
            while True:
                try:
                    choice = input(
                        f"🔑 Select root account (1-{len(matching_accounts)}): "
                    ).strip()
                    choice_num = int(choice)
    ...
    def verify_cluster_exists(
        ...
        try:
            self.print_colored(
                "YELLOW", f"🔍 Verifying cluster {cluster_name} in {region}..."
            )
            ...
            if cluster["status"] != "ACTIVE":
                self.print_colored(
                    "RED", f"❌ Cluster is in {cluster['status']} state, not ACTIVE"
                )
                return False
    ...
    def analyze_existing_components(
        ...
        try:
            ...
            components["container_insights"] = (
                self.eks_manager._verify_container_insights(
                    cluster_name, region, access_key, secret_key
                )
            )

            components["cluster_autoscaler"] = (
                self.eks_manager._verify_cluster_autoscaler(
                    cluster_name, region, access_key, secret_key
                )
            )
    ...
    def display_cluster_status(self) -> None:
        ...
        if nodegroups:
            for ng in nodegroups:
                ...
                self.print_colored(
                    status_color,
                    f"  ✓ {ng['name']} ({ng['capacity_type']}) - {ng['status']}",
                )
                print(f"    Instance Types: {', '.join(ng['instance_types'])}")
                print(
                    f"    Scaling: {scaling.get('desiredSize', 0)}/{scaling.get('maxSize', 0)} nodes"
                )
        else:
        ...
        if addons:
            for addon in addons:
                ...
                self.print_colored(
                    status_color,
                    f"  ✓ {addon['name']} (v{addon['version']}) - {addon['status']}",
                )
        else:
        ...
        components_status = [
            (
                "Container Insights",
                self.existing_components.get("container_insights", False),
            ),
            (
                "Cluster Autoscaler",
                self.existing_components.get("cluster_autoscaler", False),
            ),
            (
                "Scheduled Scaling",
                self.existing_components.get("scheduled_scaling", False),
            ),
            (
                "CloudWatch Alarms",
                self.existing_components.get("cloudwatch_alarms", False),
            ),
    ...
    def continue_cluster_setup_from_errors(self) -> bool:
        ...
        try:
            ...
            for i, cluster_info in enumerate(selected_clusters, 1):
                ...
                print(f"\n{'=' * 80}")
                print(
                    f"🚀 CONTINUING CLUSTER {i}/{len(selected_clusters)}: {cluster_name}"
                )
                print(f"{'=' * 80}")
                ...
                try:
                    ...
                    admin_access_key, admin_secret_key = (
                        self.get_credentials_for_cluster(cluster_name, region)
                    )

                    # Verify cluster exists
                    if not self.verify_cluster_exists(
                        cluster_name, region, admin_access_key, admin_secret_key
                    ):
                        self.print_colored(
                            "RED", f"❌ Cluster {cluster_name} verification failed"
                        )
                        continue
                    ...
                    cluster_success = self.configure_single_cluster(
                        cluster_name, region, admin_access_key, admin_secret_key
                    )
                    ...
                    if cluster_success:
                        ...
                        self.print_colored(
                            "GREEN",
                            f"✅ Successfully configured cluster {cluster_name}",
                        )
                    else:
                        self.print_colored(
                            "YELLOW",
                            f"⚠️  Partial configuration for cluster {cluster_name}",
                        )

                except Exception as e:
                    self.logger.error(
                        f"Error configuring cluster {cluster_name}: {str(e)}"
                    )
                    self.print_colored(
                        "RED", f"❌ Error configuring cluster {cluster_name}: {str(e)}"
                    )
                    continue
            ...
            print(
                f"Failed/Partial: {len(selected_clusters) - successful_continuations}"
            )
        ...
        try:
            ...
            while True:
                ...
                if choice_num == 0:
                    ...
                    changed = self.configure_nodegroups(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 2:
                    changed = self.configure_addons(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 3:
                    changed = self.configure_container_insights(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 4:
                    changed = self.configure_cluster_autoscaler(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 5:
                    changed = self.configure_scheduled_scaling(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 6:
                    changed = self.configure_cloudwatch_monitoring(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 7:
                    changed = self.configure_cost_monitoring(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 8:
                    changed = self.generate_user_instructions(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 9:
                    changed = self.run_health_check(
                        cluster_name, region, access_key, secret_key
                    )
                elif choice_num == 10:
                    account_id = self._extract_account_from_cluster_name(cluster_name)
                    user_data = {
                        "username": self._extract_username_from_cluster_name(
                            cluster_name
                        ),
                        "email": "",
                    ...
                    protection_result = (
                        self.eks_manager.apply_no_delete_to_matching_nodegroups(
                            cluster_name, region, access_key, secret_key
                        )
                    )

                    self.eks_manager.protect_nodes_with_no_delete_label(
                        cluster_name, region, access_key, secret_key
                    )
                    ...
                    if protection_result.get("success"):
                        ...
                        monitoring_setup = (
                            self.eks_manager.setup_node_protection_monitoring(
                                cluster_name,
                                region,
                                access_key,
                                secret_key,
                    ...
                    if False:
                        from custom_cloudwatch_agent_deployer import (
                            CustomCloudWatchAgentDeployer,
                        )
                ...
                continue_choice = (
                    input("\nContinue configuring this cluster? (Y/n): ")
                    .strip()
                    .lower()
                )
    ...
    def configure_aws_auth_configmap_enhanced(
        ...
        try:
            ...
            if is_root_cluster:
                ...
                self.print_colored(
                    "CYAN", f"   👑 Root-created cluster - configuring root access only"
                )
            else:
            ...
            self.print_colored(
                "CYAN", f"   📋 Cluster authentication mode: {auth_mode}"
            )
            ...
            self.print_colored(
                "RED", f"   ❌ ConfigMap configuration failed: {error_msg}"
            )
            return False
    ...
    def apply_configmap_with_kubectl(
        ...
        try:
            ...
            if not kubectl_available:
                self.print_colored(
                    "YELLOW", f"   ⚠️  kubectl not found. ConfigMap setup skipped."
                )
                return True
            ...
            configmap_file = os.path.join(
                temp_dir, f"aws-auth-{cluster_name}-{timestamp}.yaml"
            )
            ...
            try:
                ...
                self.print_colored(
                    "RED", f"   ❌ Failed to create ConfigMap file: {str(e)}"
                )
                return False
            ...
            try:
                ...
                for i, strategy in enumerate(
                    apply_strategies[:2], 1
                ):  # Try first 2 strategies
                    self.print_colored(
                        "CYAN", f"   📋 Applying ConfigMap (strategy {i})..."
                    )

                    result = subprocess.run(
                        strategy, env=env, capture_output=True, text=True, timeout=300
                    )

                    if result.returncode == 0:
                        self.print_colored(
                            "GREEN",
                            f"   ✅ ConfigMap applied successfully (strategy {i})",
                        )
                        success = True
                        break
                    else:
                        self.print_colored(
                            "YELLOW", f"   ⚠️  Strategy {i} failed: {result.stderr}"
                        )

                # If standard strategies failed, try delete and recreate
                if not success:
                    self.print_colored(
                        "CYAN", f"   🔄 Trying delete and recreate strategy..."
                    )
                    ...
                    if create_result.returncode == 0:
                        self.print_colored(
                            "GREEN", f"   ✅ ConfigMap recreated successfully"
                        )
                        success = True
                    else:
                        self.print_colored(
                            "RED",
                            f"   ❌ All strategies failed: {create_result.stderr}",
                        )

                if success:
                    if is_root_cluster:
                        self.print_colored(
                            "GREEN", f"   ✅ Root user configured for cluster access"
                        )
                    else:
                        username = user_data.get("username", "unknown")
                        self.print_colored(
    ...
    def configure_nodegroups(
        ...
        if existing_nodegroups:
            ...
            choice = (
                input("\nDo you want to create additional nodegroups? (y/N): ")
                .strip()
                .lower()
            )
        ...
        try:
            ...
            eks_role_arn, node_role_arn = self.eks_manager.ensure_iam_roles(
                iam_client, account_id
            )

            # Get VPC resources
            subnet_ids, security_group_id = (
                self.eks_manager.get_or_create_vpc_resources(ec2_client, region)
            )
            ...
            if strategy == "on-demand":
                success = self.eks_manager.create_ondemand_nodegroup(
                    ...
                    node_role_arn,
                    selected_subnets,
                    nodegroup_config["ami_type"],
                    nodegroup_config["instance_selections"]["on-demand"],
                    nodegroup_config["min_size"],
                    nodegroup_config["desired_size"],
                    nodegroup_config["max_size"],
                    ec2_key_name,
                )
            elif strategy == "spot":
            ...
            if success:
                self.print_colored(
                    "GREEN",
                    f"✅ Successfully created nodegroup: {nodegroup_config['name']}",
                )
                return True
            else:
                self.print_colored(
                    "RED", f"❌ Failed to create nodegroup: {nodegroup_config['name']}"
                )
                return False
    ...
    def get_nodegroup_configuration(self, cluster_name: str) -> Optional[Dict]:
        ...
        if strategy == "mixed":
            ...
            try:
                on_demand_percentage = int(
                    input(
                        "Percentage of On-Demand capacity (0-100) [default: 30%]: "
                    ).strip()
                    or "30"
                )
        ...
        subnet_preference = {"1": "auto", "2": "public", "3": "private"}.get(
            subnet_choice, "auto"
        )

        # Generate nodegroup name
        nodegroup_name = self.eks_manager.generate_nodegroup_name(
            cluster_name, strategy
        )
    ...
    def configure_addons(
        ...
        existing_addons = [
            addon["name"] for addon in self.existing_components.get("addons", [])
        ]
        ...
        missing_addons = [
            addon for addon in essential_addons if addon not in existing_addons
        ]

        if not missing_addons:
            self.print_colored(
                "GREEN", "✅ All essential add-ons are already installed"
            )
            return True
    ...
    def configure_container_insights(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> bool:
        ...
        try:
            ...
            if success:
                self.print_colored(
                    "GREEN", "✅ Container Insights configured successfully"
                )
            else:
                self.print_colored(
                    "YELLOW", "⚠️  Container Insights configuration had issues"
                )
            ...
            self.print_colored(
                "RED", f"❌ Error configuring Container Insights: {str(e)}"
            )
            return False
    ...
    def configure_cluster_autoscaler(
        ...
        try:
            ...
            if success:
                self.print_colored(
                    "GREEN", "✅ Cluster Autoscaler configured successfully"
                )
            else:
                self.print_colored(
                    "YELLOW", "⚠️  Cluster Autoscaler configuration had issues"
                )
            ...
            self.print_colored(
                "RED", f"❌ Error configuring Cluster Autoscaler: {str(e)}"
            )
            return False
    ...
    def configure_scheduled_scaling(
        ...
        try:
            # Get nodegroup names
            nodegroup_names = [
                ng["name"] for ng in self.existing_components.get("nodegroups", [])
            ]

            if not nodegroup_names:
                self.print_colored(
                    "YELLOW", "⚠️  No nodegroups found for scheduled scaling"
                )
                return False
            ...
            if success:
                self.print_colored(
                    "GREEN", "✅ Scheduled Scaling configured successfully"
                )
            else:
                self.print_colored(
                    "YELLOW", "⚠️  Scheduled Scaling configuration had issues"
                )
            ...
            self.print_colored(
                "RED", f"❌ Error configuring Scheduled Scaling: {str(e)}"
            )
            return False
    ...
    def configure_cloudwatch_monitoring(
        ...
        try:
            ...
            if success:
                self.print_colored(
                    "GREEN", "✅ CloudWatch Monitoring configured successfully"
                )
            else:
                self.print_colored(
                    "YELLOW", "⚠️  CloudWatch Monitoring configuration had issues"
                )
            ...
            self.print_colored(
                "RED", f"❌ Error configuring CloudWatch Monitoring: {str(e)}"
            )
            return False

    def configure_cost_monitoring(
        self, cluster_name: str, region: str, access_key: str, secret_key: str
    ) -> bool:
        ...
        try:
            ...
            if success:
                self.print_colored(
                    "GREEN", "✅ Cost Monitoring configured successfully"
                )
            else:
                self.print_colored(
                    "YELLOW", "⚠️  Cost Monitoring configuration had issues"
                )
    ...
    def generate_user_instructions(
        ...
        try:
            ...
            self.generate_mini_instructions(
                credential_info, cluster_name, region, credential_info.username
            )
            ...
            self.print_colored(
                "RED", f"❌ Error generating user instructions: {str(e)}"
            )
            return False
    ...
    def reconfigure_cluster(self, cluster_names: List[str]) -> bool:
        ...
        try:
            ...
            self.print_colored(
                Colors.BLUE, f"📋 Found {len(cluster_names)} clusters to reconfigure"
            )
            ...
            for i, cluster_name in enumerate(cluster_names, 1):
                ...
                self.print_colored(
                    Colors.CYAN,
                    f"🚀 PROCESSING CLUSTER {i}/{len(cluster_names)}: {cluster_name}",
                )
                self.print_colored(Colors.CYAN, "=" * 80)
                ...
                try:
                    ...
                    if not region:
                        ...
                        if not region:
                            self.print_colored(
                                Colors.RED,
                                f"❌ No region provided, skipping cluster {cluster_name}",
                            )
                            continue
                    ...
                    self.print_colored(
                        Colors.BLUE,
                        f"🔐 Retrieving credentials for cluster {cluster_name}...",
                    )

                    try:
                        access_key, secret_key, account_id = (
                            self.get_iam_credentials_from_cluster(cluster_name, region)
                        )
                        self.print_colored(
                            Colors.GREEN,
                            f"✅ Found IAM credentials for cluster {cluster_name}",
                        )
                    except Exception as e1:
                        self.print_colored(
                            Colors.YELLOW, f"⚠️ Could not get IAM credentials: {str(e1)}"
                        )
                        self.print_colored(
                            Colors.YELLOW, f"⚠️ Attempting to use root credentials..."
                        )
                        try:
                            access_key, secret_key, account_id = (
                                self.get_root_credentials(cluster_name, region)
                            )
                            self.print_colored(
                                Colors.GREEN,
                                f"✅ Found root credentials for cluster {cluster_name}",
                            )
                    ...
                    if not self.verify_cluster_exists(
                        ...
                        self.print_colored(
                            Colors.RED,
                            f"❌ Could not access cluster {cluster_name}, skipping",
                        )
                        continue
                    self.print_colored(
                        Colors.GREEN, f"✅ Cluster {cluster_name} is accessible"
                    )
                    ...
                    success = self.configure_single_cluster(
                        cluster_name, region, access_key, secret_key
                    )
                    ...
                    if success:
                        ...
                        self.print_colored(
                            Colors.GREEN,
                            f"✅ Successfully reconfigured cluster {cluster_name}",
                        )
                    else:
                        self.print_colored(
                            Colors.YELLOW,
                    ...
                    self.logger.error(
                        f"Error reconfiguring cluster {cluster_name}: {str(e)}"
                    )
                    self.print_colored(
                        Colors.RED,
                        f"❌ Error reconfiguring cluster {cluster_name}: {str(e)}",
    ...
    def select_clusters_from_eks_accounts(self, base_dir="aws/eks"):
        ...
        accounts = [
            ...
            if os.path.isdir(os.path.join(base_dir, d)) and d not in unused_accounts
        ]
        if not accounts:
        ...
        for acc in selected_accounts:
            ...
            print(
                "Enter cluster numbers (comma, range, or 'all') for this account: ",
                end="",
            )
