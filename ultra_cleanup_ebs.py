...
class UltraCleanupEBSManager:
    ...
    def setup_detailed_logging(self):
        ...
        try:
            ...
            self.log_filename = (
                f"{self.ebs_dir}/ultra_ebs_cleanup_log_{self.execution_timestamp}.log"
            )
            ...
            self.operation_logger.info(
                "🚨 ULTRA ELASTIC BEANSTALK CLEANUP SESSION STARTED 🚨"
            )
            self.operation_logger.info("=" * 100)
    ...
    def create_client(self, service, access_key, secret_key, region):
        ...
        try:
            ...
            self.log_operation(
                "ERROR", f"Failed to create {service} client for {region}: {e}"
            )
            raise
    ...
    def get_all_beanstalk_apps(self, ebs_client, region, account_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🔍 Scanning for Elastic Beanstalk applications in {region} ({account_name})",
            )
            print(
                f"   🔍 Scanning for Elastic Beanstalk applications in {region} ({account_name})..."
            )
            ...
            if not apps:
                self.log_operation(
                    "INFO",
                    f"No Elastic Beanstalk applications found in {region} ({account_name})",
                )
            ...
            for app in apps:
                ...
                environments = self.get_environments_for_app(
                    ebs_client, app_name, region, account_info
                )
            ...
            self.log_operation(
                "INFO",
                f"📦 Found {len(applications)} Elastic Beanstalk applications in {region} ({account_name})",
            )
            print(
                f"   📦 Found {len(applications)} Elastic Beanstalk applications in {region} ({account_name})"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Error getting Elastic Beanstalk applications in {region} ({account_name}): {e}",
            )
    ...
    def get_environments_for_app(self, ebs_client, app_name, region, account_info):
        ...
        try:
            ...
            response = ebs_client.describe_environments(
                ApplicationName=app_name, IncludeDeleted=False
            )
            ...
            for env in envs:
                ...
                platform_name = (
                    platform_parts[-2] if len(platform_parts) > 1 else platform
                )
                platform_version = (
                ...
                resources = self.get_environment_resources(
                    ebs_client, env_name, region, account_info
                )
            ...
            self.log_operation(
                "ERROR",
                f"Error getting environments for app {app_name} in {region}: {e}",
            )
            return []
    ...
    def get_environment_resources(self, ebs_client, env_name, region, account_info):
        ...
        try:
            ...
            self.log_operation(
                "ERROR",
                f"Error getting resources for environment {env_name} in {region}: {e}",
            )
            return {}

    def terminate_environment(
        self, ebs_client, env_info, app_name, region, account_info
    ):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🗑️  Terminating environment {env_name} ({env_id}) in app {app_name} ({region}, {account_name})",
            )
            ...
            if env_info["status"] in ["Terminating", "Terminated"]:
                self.log_operation(
                    "INFO", f"Environment {env_name} is already {env_info['status']}"
                )
            ...
            ebs_client.terminate_environment(
                EnvironmentName=env_name, TerminateResources=True
            )

            # Wait for environment termination to complete
            print(
                f"      ⏳ Waiting for environment {env_name} termination to complete..."
            )
            self.log_operation(
                "INFO",
                f"⏳ Waiting for environment {env_name} termination to complete...",
            ...
            while waiter and retry_count < max_retries:
                try:
                    ...
                    if not response["Environments"]:
                        ...
                        waiter = False
                        self.log_operation(
                            "INFO", f"✅ Environment {env_name} terminated successfully"
                        )
                        print(
                            f"      ✅ Environment {env_name} terminated successfully"
                        )
                        break
                    ...
                    if status == "Terminated":
                        ...
                        if retry_count % 6 == 0:  # Log every 3 minutes
                            self.log_operation(
                                "INFO",
                                f"Environment {env_name} status: {status} (Waiting...)",
                            )
                            print(
                                f"      ⌛ Environment {env_name} status: {status} (Still terminating...)"
                            )
                        time.sleep(30)  # Check every 30 seconds
                        retry_count += 1
                    else:
                        self.log_operation(
                            "WARNING", f"Unexpected environment status: {status}"
                        )
                        time.sleep(30)
                        retry_count += 1
                    ...
                    if "No Environment found" in str(e):
                        ...
                        self.log_operation(
                            "ERROR", f"Error checking environment status: {e}"
                        )
                        raise
                except Exception as e:
                    self.log_operation(
                        "ERROR", f"Unexpected error checking environment status: {e}"
                    )
                    raise
            ...
            if retry_count >= max_retries:
                self.log_operation(
                    "WARNING",
                    f"Timed out waiting for environment {env_name} termination",
                )
                print(
                    f"      ⚠️ Timed out waiting for environment {env_name} termination"
                )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to terminate environment {env_info['environment_name']}: {e}",
            )
            print(
                f"      ❌ Failed to terminate environment {env_info['environment_name']}: {e}"
            )
    ...
    def delete_application(self, ebs_client, app_info):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🗑️  Deleting Elastic Beanstalk application {app_name} in {region} ({account_name})",
            )
            print(
            ...
            if environments:
                ...
                self.log_operation(
                    "INFO",
                    f"Found {env_count} environments to terminate in application {app_name}",
                )
                print(
                    f"      Found {env_count} environments to terminate in application {app_name}"
                )
                ...
                for env in environments:
                    self.terminate_environment(
                        ebs_client, env, app_name, region, app_info["account_info"]
                    )
            else:
                self.log_operation(
                    "INFO", f"No environments found in application {app_name}"
                )
            ...
            try:
                ...
                if versions:
                    self.log_operation(
                        "INFO", f"Found {len(versions)} application versions to delete"
                    )
                    ...
                    for version in versions:
                        ...
                        try:
                            ...
                            self.log_operation(
                                "INFO", f"Deleted application version {version_label}"
                            )
                            print(
                                f"      ✅ Deleted application version {version_label}"
                            )
                        except Exception as version_error:
                            self.log_operation(
                                "WARNING",
                                f"Could not delete version {version_label}: {str(version_error)}",
                            )
                            print(
                                f"      ⚠️ Could not delete version {version_label}: {str(version_error)}"
                            )
                else:
                    self.log_operation(
                        "INFO", f"No application versions found for {app_name}"
                    )
                    print(f"      No application versions found for {app_name}")
            except Exception as versions_error:
                self.log_operation(
                    "WARNING",
                    f"Error deleting application versions: {str(versions_error)}",
                )
                print(
                    f"      ⚠️ Error deleting application versions: {str(versions_error)}"
                )
            ...
            self.log_operation(
                "INFO", f"✅ Successfully deleted application {app_name}"
            )
            ...
            self.log_operation(
                "ERROR",
                f"Failed to delete application {app_info['application_name']}: {e}",
            )
            print(
                f"   ❌ Failed to delete application {app_info['application_name']}: {e}"
            )
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation(
                "INFO",
                f"🧹 Starting EBS cleanup for {account_key} ({account_id}) in {region}",
            )
            print(
                f"\n🧹 Starting EBS cleanup for {account_key} ({account_id}) in {region}"
            )

            # Create Elastic Beanstalk client
            try:
                ebs_client = self.create_client(
                    "elasticbeanstalk", access_key, secret_key, region
                )
            except Exception as client_error:
                self.log_operation(
                    "ERROR",
                    f"Could not create Elastic Beanstalk client for {region}: {client_error}",
                )
                print(
                    f"   ❌ Could not create Elastic Beanstalk client for {region}: {client_error}"
                )
            ...
            if not applications:
                self.log_operation(
                    "INFO",
                    f"No Elastic Beanstalk applications found in {account_key} ({region})",
                )
                print(
                    f"   ✓ No Elastic Beanstalk applications found in {account_key} ({region})"
                )
            ...
            region_summary = {
                ...
                "environments_found": sum(
                    len(app.get("environments", [])) for app in applications
                ),
            }
            self.cleanup_results["regions_processed"].append(region_summary)

            self.log_operation(
                "INFO", f"📊 {account_key} ({region}) EBS resources summary:"
            )
            self.log_operation("INFO", f"   🌐 Applications: {len(applications)}")
            self.log_operation(
                "INFO", f"   🌱 Environments: {region_summary['environments_found']}"
            )

            print(
                f"   📊 EBS resources found: {len(applications)} applications, {region_summary['environments_found']} environments"
            )
            ...
            for i, app in enumerate(applications, 1):
                ...
                print(
                    f"   [{i}/{len(applications)}] Processing application {app_name}..."
                )
                ...
                try:
                    ...
                    failed_count += 1
                    self.log_operation(
                        "ERROR", f"Error deleting application {app_name}: {e}"
                    )
                    print(f"   ❌ Error deleting application {app_name}: {e}")

            print(
                f"   ✅ Deleted {deleted_count} applications, ❌ Failed: {failed_count}"
            )

            self.log_operation(
                "INFO", f"✅ EBS cleanup completed for {account_key} ({region})"
            )
            print(f"\n   ✅ EBS cleanup completed for {account_key} ({region})")
            return True
            ...
            self.log_operation(
                "ERROR",
                f"Error cleaning up EBS resources in {account_key} ({region}): {e}",
            )
            print(
                f"   ❌ Error cleaning up EBS resources in {account_key} ({region}): {e}"
            )
        ...
        try:
            ...
            report_data = {
                ...
                "detailed_results": {
                    ...
                    "deleted_applications": self.cleanup_results[
                        "deleted_applications"
                    ],
                    "deleted_environments": self.cleanup_results[
                        "deleted_environments"
                    ],
            ...
            self.log_operation(
                "INFO", f"✅ Ultra EBS cleanup report saved to: {report_filename}"
            )
            return report_filename

        except Exception as e:
            self.log_operation(
                "ERROR", f"❌ Failed to save ultra EBS cleanup report: {e}"
            )
            return None
    ...
    def run(self):
        ...
        try:
            self.log_operation(
                "INFO", "🚨 STARTING ULTRA ELASTIC BEANSTALK CLEANUP SESSION 🚨"
            )

            self.print_colored(Colors.YELLOW, "🚨" * 30)
            self.print_colored(
                Colors.RED, "💥 ULTRA ELASTIC BEANSTALK CLEANUP - SEQUENTIAL 💥"
            )
            self.print_colored(Colors.YELLOW, "🚨" * 30)
            self.print_colored(
            ...
            self.print_colored(
                Colors.YELLOW,
                "\n🔑 Select Root AWS Accounts for Elastic Beanstalk Cleanup:",
            )
            ...
            self.print_colored(
                Colors.YELLOW, f"\n🎯 ELASTIC BEANSTALK CLEANUP CONFIGURATION"
            )
            self.print_colored(Colors.YELLOW, "=" * 80)
            ...
            self.print_colored(
                Colors.RED,
                f"\n⚠️  WARNING: This will delete ALL of the following Elastic Beanstalk resources:",
            )
            ...
            self.print_colored(
                Colors.WHITE, f"    • Associated AWS resources (EC2, ALB, ASG, etc.)"
            )
            self.print_colored(
                Colors.WHITE,
            ...
            confirm1 = (
                input(f"\nContinue with Elastic Beanstalk cleanup? (y/n): ")
                .strip()
                .lower()
            )
            ...
            if confirm2 != "yes":
                self.log_operation(
                    "INFO", "Ultra EBS cleanup cancelled at final confirmation"
                )
                self.print_colored(Colors.RED, "❌ Cleanup cancelled")
                return
            ...
            self.print_colored(
                Colors.RED, f"\n💥 STARTING ELASTIC BEANSTALK CLEANUP..."
            )
            self.log_operation(
                "INFO",
                f"🚨 EBS CLEANUP INITIATED - {len(selected_accounts)} accounts, {len(selected_regions)} regions",
            )
            ...
            self.print_colored(
                Colors.YELLOW,
                f"\n💥" + "=" * 25 + " ELASTIC BEANSTALK CLEANUP COMPLETE " + "=" * 25,
            )
            self.print_colored(
            ...
            self.print_colored(
                Colors.WHITE,
                f"🌐 Applications deleted: {len(self.cleanup_results['deleted_applications'])}",
            )
            self.print_colored(
                Colors.WHITE,
                f"🌱 Environments deleted: {len(self.cleanup_results['deleted_environments'])}",
            )
            self.print_colored(
                Colors.WHITE,
            ...
            if (
                self.cleanup_results["deleted_applications"]
                or self.cleanup_results["deleted_environments"]
            ):
            ...
            self.print_colored(
                Colors.CYAN, f"\n📄 Saving Elastic Beanstalk cleanup report..."
            )
            report_file = self.save_cleanup_report()
            if report_file:
                self.print_colored(
                    Colors.GREEN,
                    f"✅ Elastic Beanstalk cleanup report saved to: {report_file}",
                )
            ...
            self.log_operation(
                "ERROR", f"FATAL ERROR in EBS cleanup execution: {str(e)}"
            )
            self.print_colored(Colors.RED, f"\n❌ FATAL ERROR: {e}")
            import traceback
...
def main():
    ...
    try:
        ...
        print("\n\n❌ Elastic Beanstalk cleanup interrupted by user")
        exit(1)
    except Exception as e:
