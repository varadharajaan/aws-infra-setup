...
class UltraCleanupEBSManager:
    ...
    def cleanup_account_region(self, account_info, region):
        ...
        try:
            ...
            self.log_operation('INFO', f"🧹 Starting EBS cleanup for {account_key} ({account_id}) in {region}")
            ...
            self.log_operation('INFO', f"📊 {account_key} ({region}) EBS resources summary:")
            ...
            print(f"   📊 EBS resources found: {len(applications)} applications, {region_summary['environments_found']} environments")
            ...
            print(f"   ✅ Deleted {deleted_count} applications, ❌ Failed: {failed_count}")

            self.log_operation('INFO', f"✅ EBS cleanup completed for {account_key} ({region})")
            ...
            print(f"   ❌ Error cleaning up EBS resources in {account_key} ({region}): {e}")
    ...
    def save_cleanup_report(self):
        ...
        try:
            ...
            self.log_operation('INFO', f"✅ Ultra EBS cleanup report saved to: {report_filename}")
            return report_filename

        except Exception as e:
            self.log_operation('ERROR', f"❌ Failed to save ultra EBS cleanup report: {e}")
            return None
    ...
    def run(self):
        ...
        try:
            ...
            self.print_colored(Colors.RED, "💥 ULTRA ELASTIC BEANSTALK CLEANUP - SEQUENTIAL 💥")
            self.print_colored(Colors.YELLOW, "🚨" * 30)
            ...
            self.log_operation('INFO', f"🚨 EBS CLEANUP INITIATED - {len(selected_accounts)} accounts, {len(selected_regions)} regions")
            ...
            self.print_colored(Colors.WHITE, f"🌐 Applications deleted: {len(self.cleanup_results['deleted_applications'])}")
            ...
            if self.cleanup_results['deleted_applications'] or self.cleanup_results['deleted_environments']:
                ...
                for account, summary in account_summary.items():
                    ...
                    self.print_colored(Colors.WHITE, f"      🌐 Applications: {summary['applications']}")
            ...
            if report_file:
                self.print_colored(Colors.GREEN, f"✅ Elastic Beanstalk cleanup report saved to: {report_file}")
